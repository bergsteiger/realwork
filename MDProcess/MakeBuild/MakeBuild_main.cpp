////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Модуль: "w:/MDProcess/MakeBuild/MakeBuild_main.cpp"
// генератор файлов реализации C++ (.cpp)
// Generated from UML model, root element: <<ExeTarget::Category>> MDProcess::MakeBuild
//
// Создает на сервере MDС  объекты Сборки на основе событий сборочного скрипта (вызывается из него).
//
//
...
void print_usage (const Core::Root::LibHomeManager& lhm) {
	std::cout << "MakeBuild - Создает на сервере MDС  объекты Сборки на основе событий\n";
	std::cout << "сборочного скрипта (вызывается из него).\n";
	std::cout << std::endl;
	lhm.print_usage ();
	std::cout << "\nВсе права принадлежат ООО НПП \"Гарант-Сервис\"\n";
}
...
public:
	static int execute (int argc, char** argv) {
		...
		try {
			Core::ParamManagerInitData init = {"MakeBuild", "Core::RegistryHelper::KEY_LOCAL_MACHINE\\Software\\Garant\\MDProcess\\MakeBuild"};
		...
		lhm.registrate_lib_home (&BuildSupportLib::LibHomeFactory::get ());
