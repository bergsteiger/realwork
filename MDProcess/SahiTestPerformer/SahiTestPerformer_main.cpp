////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Модуль: "w:/MDProcess/SahiTestPerformer/SahiTestPerformer_main.cpp"
// генератор файлов реализации C++ (.cpp)
// Generated from UML model, root element: <<ExeTarget::Category>> MDProcess::SahiTestPerformer
//
// экзешник, контролирующая выполнение тестов sahi передающий результаты тестирования серверу
// запуска тестов
//
//
...
void print_usage (const Core::Root::LibHomeManager& lhm) {
	std::cout << "SahiTestPerformer - экзешник, контролирующая выполнение тестов sahi\n";
	std::cout << "передающий результаты тестирования серверу запуска тестов\n";
	std::cout << std::endl;
	lhm.print_usage ();
	std::cout << "\nВсе права принадлежат ООО НПП \"Гарант-Сервис\"\n";
}
...
public:
	static int execute (int argc, char** argv) {
		...
		try {
			Core::ParamManagerInitData init = {"SahiTestPerformer", "Core::RegistryHelper::KEY_LOCAL_MACHINE\\Software\\Garant\\MDProcess\\SahiTestPerformer"};
		...
		lhm.registrate_lib_home (&FilePublishLib::LibHomeFactory::get ());
		lhm.registrate_lib_home (&SahiTestLib::LibHomeFactory::get ());
