...
public class UmlCreationWizard extends Wizard implements INewWizard {
	...
	public void init(IWorkbench workbench, IStructuredSelection selection) {
		...
		setDefaultPageImageDescriptor(UmlDiagramEditorPlugin
				.getBundledImageDescriptor("icons/wizban/NewEumlWizard.gif")); //$NON-NLS-1$
	...
	public void addPages() {
		diagramModelFilePage = new UmlCreationWizardPage(
				"DiagramModelFile", getSelection(), "euml_diagram"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		domainModelFilePage = new UmlCreationWizardPage(
				"DomainModelFile", getSelection(), "euml") { //$NON-NLS-1$ //$NON-NLS-2$

			public void setVisible(boolean visible) {
				if (visible) {
					String fileName = diagramModelFilePage.getFileName();
					fileName = fileName.substring(0, fileName.length()
							- ".euml_diagram".length()); //$NON-NLS-1$
					setFileName(UmlDiagramEditorUtil.getUniqueFileName(
							getContainerFullPath(), fileName, "euml")); //$NON-NLS-1$
				}
