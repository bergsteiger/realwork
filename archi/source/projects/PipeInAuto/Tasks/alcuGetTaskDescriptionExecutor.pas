...
var
 ...
 begin
  Result := False;
  l_Reply.RemoteFolder := IncludeTrailingPathDelimiter(anItem.DeliverySourceFolder);
  l_Reply.LocalFolder := IncludeTrailingPathDelimiter(anItem.DeliveryTargetFolder);
  anItem.SetProgress(0, '');
  anItem.RequestDelivering;
  l3System.Msg2Log('Передача пользователю результатов экспорта');
  l3System.Msg2Log('Задача от пользователя: %d', [anItem.UserID]);
