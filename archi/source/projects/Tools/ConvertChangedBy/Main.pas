   ...
   try
    ...
    try
     ...
     begin
      ...
      MessageDlg(Format('Во время исправления произошла ошибка "%s".'#10'Подробности в лог-файле', [E.Message]), mtError, [mbOK], 0);
     end;
    end
    else
     MessageDlg('Не найдено документов для исправления', mtWarning, [mbOK], 0);
   finally
    FreeAndNil(l_Filter);
   end;
  finally
