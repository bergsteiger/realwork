	...
	:main.begin
		...
		pushd "%~dp0"
			...
			pushd "W:\"
				...
				ren "MDProcess" "MDProcess-UNKNOWN-X.XXX" 1>nul 2>&1& ren "MDProcess-%PROJECT%" "MDProcess" 1>nul 2>&1
			popd
			...
			net use "N:" "\\10.0.1.155\%BUILD_SHARE%" 1>nul 2>&1
				pushd "N:\deployment"
					...
					rmdir "!daily-installer\MDProcess" /S/Q 1nul 2&1
			...
			if exist "%LOGFILE%" (del /f /q "%LOGFILE%" 1>nul 2>&1)
				...
				call "..\..\confbuild.bat" "start" "MDP" "%CVS_TAG%" "%VERSION_MINOR%" "%VERSION_MAJOR%" "%BUILD_PATH%"
			...
			if "%ANT_SUCCESS%" == "0" (
				...
				call "..\..\confbuild.bat" "failure" "MDP" "%CVS_TAG%" "%VERSION_MINOR%" "%VERSION_MAJOR%" "%BUILD_PATH%" "%LOGFILE%"
			) else (
				call "..\..\buildmail.bat" "BUILD SUCCESS [%PROJECT_SUFFIX%: %SUBINFO%], CVS TAG [%CVS_TAG%]" "%LOGFILE%"
				call "..\..\confbuild.bat" "done" "MDP" "%CVS_TAG%" "%VERSION_MINOR%" "%VERSION_MAJOR%" "%BUILD_PATH%"
			)
			...
			pushd "W:\"
				...
				ren "MDProcess" "MDProcess-%PROJECT%" 1>nul 2>&1& ren "MDProcess-UNKNOWN-X.XXX" "MDProcess" 1>nul 2>&1
			popd
		popd
