...
// ћодуль: "w:\common\components\gui\Garant\Everest\evParaListCursorDeletable.pas"
...
var
 ...
 begin//_ModifyCursor1
  ...
  begin
   (* - курсор стоит на удал€емом параграфе - надо что-то делать,
   ...
   if ev_dpfInternal in aFlags then
    ...
    begin
     if not PLC^.Move(nil, ev_ocNextParaTopLeft, anOpPack) then
      // - не смогли сместитьс€ на следующий параграф
      if not PLC^.Move(nil, ev_ocPrevParaBottomRight, anOpPack) then
       // - не смогли сместитьс€ на предыдущий параграф
       PLC^.SetEntryPoint(ev_cpNull, anOpPack As IevCursorContext);
       // - надо что то делать !!!
    end;//Internal
