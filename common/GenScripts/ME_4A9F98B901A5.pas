unit NOT_COMPLETED_BaseSearchInterfaces;

// Модуль: "w:\garant6x\implementation\Garant\GbaNemesis\Search\NOT_COMPLETED_BaseSearchInterfaces.pas"
// Стереотип: "Interfaces"
// Элемент модели: "BaseSearchInterfaces" MUID: (4A9F98B901A5)
 ...
 *)
 ...
 TnsBaseSearchOpenKind = (
  ...
  ns_bsokLocal
   {* Локальный поиск контекста (Ctrl+F) }
  , ns_bsokSpecify
   {* Уточнить }
  , ns_bsokGlobal
   {* Глобальный поиск (F4) }
...
Вообще говоря там надо ещё обрабатывать ситуацию - локальный поиск или глобальный, т.е. смениться прецедент после поиска или нет }
 ...
 TbsBaseSearchResult = (
  bsrNone
   {* в случае если параметры поиска не заданы }
  , bsrContinueSearch
   {* продолжить поиск }
  , bsrSearchFinished
   {* поиск в текте завершен }
  , bsrSearchFinishedNotFound
   {* в тексте ничего не найдено }
  , bsrSearchFinishedAllInHidden
   {* тексте нечто найдено, но все в скрытом тексте }
 );//TbsBaseSearchResult
 ...
 *)
 ...
 TnsBaseSearchWindowState = (
  ...
  , ns_wsOpened
   {* Окно открыто }
  , ns_wsForce
   {* Окно было открыто пользователем }
 );//TnsBaseSearchWindowState
 ...
 InsBaseSearcherState = interface
...
initialization
{$If NOT Defined(NoScripts)}
 TtfwTypeRegistrator.RegisterType(TypeInfo(ETryToFindEmptyContext));
 {* Регистрация типа ETryToFindEmptyContext }
{$IfEnd} // NOT Defined(NoScripts)
{$IfEnd} // NOT Defined(Admin)
