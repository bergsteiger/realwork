...
//#UC START# *4ED7594E0218_4ED7576E02F7_var*
 ...
 {* Текст для изменяющего документа - ссылка или название. }
 const
 ...
 begin
  ...
  try
   ...
   // Если номер документа не нулевой - делаем ссылку
   ...
   // Номера документа нет - выводим строку с сервера
 ...
 {* Возвращает строку с номерами и датами изменяющих документов и ссылками на них. }
 const
  ...
  begin
   ...
   try
    ...
    (* end//l3IsNil(l_EditionIntervals)
      ...
      l_EditionWarning := str_wgBadRedation.AsCStr;*)
     // Выводим текст о "недействии" редакции
...
var
 ...
 begin//lp_TimeMachine
  ...
  begin
   if (Result <> cEmptyWarning) then
    // Абзац-разделитель
    AddBlankPara(aGen);
   // Выводим текст предупреждения для Машины времени
 ...
 begin//lp_NotShure
  if l_NotShure then
  begin
   // Предупреждение об интервале неуверенности со ссылкой на документ
...
(*      if TdmStdRes.IsCurEditionActual(aDoc) then
     ...
     begin
      // первое предупреждение (Стандартное)
      lp_Standart(aDoc);
      // третье предупреждение (Машина времени)
      if not (aUserType in [WarnJuror, WarnPreActive]) then
       lp_TimeMachine;
      if not (aUserType in cTimeMachine) then
      // четвертое предупреждение (Предупреждение о редакциях)
       lp_Redaction(aDoc);
      // пятое предупреждение (Интервал неуверенности)
      lp_NotShure;
      // ссылки
      lp_Links;
     end;//else
