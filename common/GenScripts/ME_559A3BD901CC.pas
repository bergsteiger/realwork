 ...
 begin
   ...
   Case TdaJournalOperation(l_OperationField.AsInteger) of
    ...
    da_oobSessionBegin:
     begin
      ...
      CurS_Node:=CurStatTree.InsertNode(TmpNode,
                                         MakeNode(Format('Рабочая сессия (%d/%d/%d %.2d:%.2d:%.2d)',
    ...
    da_oobOpenDoc    :
     begin
      Inc(FullOpenDocs);
      TmpNode:=CurStatTree.InsertNode(CurS_Node,MakeNode(Format('Открыт документ N %d',[l_ExtIDField.AsLargeInt])));
      (TmpNode as Il3HandleNode).Handle:=l_ExtIDField.AsLargeInt;
     end;
    da_oobCreateDoc :
     begin
      Inc(FullOpenDocs);
      TmpNode:=CurStatTree.InsertNode(CurS_Node,MakeNode(Format('Создан документ N %d',[l_ExtIDField.AsLargeInt])));
      (TmpNode as Il3HandleNode).Handle:=l_ExtIDField.AsLargeInt;
     end;
    da_oobDeleteDoc : CurStatTree.InsertNode(CurS_Node,MakeNode(Format('Удален документ N %d',[l_ExtIDField.AsLargeInt])));
    da_oobEditDoc:
     begin
       ...
       then
        begin
         TmpNode:=CurStatTree.InsertNode(CurS_Node,
                                          MakeNode(Format('Отредактирован документ N %d',[l_ExtIDField.AsLargeInt])));
         Inc(FullModifDocs);
        end
       else
      ...
      Case PdaDocEditRec(@l_Additional).EditType of
       da_detText      : CurStatTree.InsertNode(TmpNode,MakeNode('Изменен Текст'));
       da_detAttribute : CurStatTree.InsertNode(TmpNode,MakeNode('Изменены Аттрибуты'));
       da_detHyperLink : CurStatTree.InsertNode(TmpNode,MakeNode('Изменены Гиперлинки'));
       da_detSub       : CurStatTree.InsertNode(TmpNode,MakeNode('Изменены Subы'));
       da_detDiction:
        Case PdaDocEditRec(@l_Additional).Details.DictType of
         da_dlSources    : CurStatTree.InsertNode(TmpNode,MakeNode('Изменены Органы'));
         da_dlTypes      : CurStatTree.InsertNode(TmpNode,MakeNode('Изменены Типы'));
         da_dlClasses    : CurStatTree.InsertNode(TmpNode,MakeNode('Изменены Классы'));
         da_dlKeyWords   : CurStatTree.InsertNode(TmpNode,MakeNode('Изменены KW'));
         da_dlBases      : CurStatTree.InsertNode(TmpNode,MakeNode('Изменены Группы'));
         da_dlDateNums   : CurStatTree.InsertNode(TmpNode,MakeNode('Изменены Дата или Номер'));
         da_dlWarnings   : CurStatTree.InsertNode(TmpNode,MakeNode('Изменены Предупреждения'));
         da_dlCorrects   : CurStatTree.InsertNode(TmpNode,MakeNode('Внесена вычитка'));
        end;
      end;
     end;
    da_oobPrintDoc : CurStatTree.InsertNode(CurS_Node,MakeNode(Format('Документ N %d напечатан',[l_ExtIDField.AsLargeInt])));
    da_oobSaveDoc  : CurStatTree.InsertNode(CurS_Node,MakeNode(Format('Документ N %d сохранен на диск',[l_ExtIDField.AsLargeInt])));
    da_oobEditDictionary :
     begin
      ...
      Case PdaDictEditRec(@l_Additional).DictType of
       da_dlSources    : CurStatTree.InsertNode(CurS_Node,MakeNode(Format('Изменен словарь Органы %s',[TmpStr])));
    ...
    da_oobExport : CurStatTree.InsertNode(CurS_Node,MakeNode(Format('Проведен Экспорт %d документов',
                                                                                [l_AdditionalInfoField.AsInteger])));
    da_oobImport : CurStatTree.InsertNode(CurS_Node,MakeNode('Проведен Импорт'));
    da_oobSearch_Deprecated : ;
    da_oobAutoClass: CurStatTree.InsertNode(CurS_Node,MakeNode(Format('Проведена автоклассификация %d документов',
                                                                                [l_AdditionalInfoField.AsInteger])));
   else
    CurStatTree.InsertNode(CurS_Node,MakeNode(Format('Нераспознанная операция (%s)', [GetEnumName(TypeInfo(TdaJournalOperation), l_OperationField.AsInteger)])));
   end;
...
procedure TdaJournal.LogEvent(aOperation: TdaJournalOperation;
 aFamilyID: TdaFamilyID;
 aExtID: LongInt;
 aData: LongInt);
//#UC START# *573C3BC601D1_559A3BD901CC_var*
//#UC END# *573C3BC601D1_559A3BD901CC_var*
begin
//#UC START# *573C3BC601D1_559A3BD901CC_impl*
 if not UserIsService(f_CurUser) then
  DoLogEvent(aOperation, aFamilyID, aExtID, aData);
//#UC END# *573C3BC601D1_559A3BD901CC_impl*
end;//TdaJournal.LogEvent
...
procedure TdaJournal.AlienCheckUser(anUserID: TdaUserID);
//#UC START# *573C4C3B031D_559A3BD901CC_var*
//#UC END# *573C4C3B031D_559A3BD901CC_var*
begin
//#UC START# *573C4C3B031D_559A3BD901CC_impl*
 CheckUser(anUserID);
//#UC END# *573C4C3B031D_559A3BD901CC_impl*
end;//TdaJournal.AlienCheckUser

procedure TdaJournal.AlienUserChanged(anUserID: TdaUserID);
//#UC START# *573C4C4903B7_559A3BD901CC_var*
//#UC END# *573C4C4903B7_559A3BD901CC_var*
begin
//#UC START# *573C4C4903B7_559A3BD901CC_impl*
 UserChanged(anUserID);
//#UC END# *573C4C4903B7_559A3BD901CC_impl*
end;//TdaJournal.AlienUserChanged
