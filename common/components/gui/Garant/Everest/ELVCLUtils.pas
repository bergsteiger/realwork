...
// $Id: ELVCLUtils.pas,v 1.1 2013/11/29 19:17:56 kostitsin Exp $

// $Log: ELVCLUtils.pas,v $
// Revision 1.1  2013/11/29 19:17:56  kostitsin
// [$120723448]
//
// Revision 1.6  2013/05/24 15:59:52  lulin
// - пытаемся портироваться под XE4.
//
// Revision 1.5  2013/04/04 11:33:17  lulin
// - портируем.
//
// Revision 1.4  2012/11/01 09:42:59  lulin
// - забыл точку с запятой.
//
// Revision 1.3  2012/11/01 07:45:13  lulin
// - делаем возможность логирования процесса загрузки модулей.
//
// Revision 1.2  2011/06/15 19:48:34  lulin
// {RequestLink:253664876}.
//
// Revision 1.1  2008/02/07 11:59:53  lulin
// - распилили наследие ElPack'а по двум библиотекам.
//
// Revision 1.9  2007/08/20 10:14:44  lulin
// - bug fix: была желтая подсветка кнопок (<K>-983101).
//
// Revision 1.8  2007/08/14 17:14:25  lulin
// - оптимизируем очистку памяти.
//
// Revision 1.7  2007/08/14 14:30:01  lulin
// - оптимизируем перемещение блоков памяти.
//
// Revision 1.6  2007/04/09 06:05:21  lulin
// - используем родные битмапы.
//
// Revision 1.5  2007/03/27 09:46:03  lulin
// - cleanup.
//
// Revision 1.4  2007/03/27 09:28:44  lulin
// - cleanup.
//
// Revision 1.3  2007/03/23 13:35:20  lulin
...
begin
  ...
  begin
    Canvas.Brush.Bitmap := Tl3Bitmap.Create;
    try
...
begin
  ...
  l3ZeroMemory(@Info.bmiHeader, sizeof(Info.bmiHeader));
  ...
  l3Free(TmpBitmap);
  l3Free(DstBitmap);
  l3System.FreeLocalMem(dBits);
  l3System.FreeLocalMem(pBits);
end;
...
begin
  ...
  l3System.GetLocalMem(dib, i * sizeof(Integer));
  try
...
begin
  ...
  begin
    ...
    begin
      strCopyUsed := true;
      l3System.GetLocalMem(Line, (len + 1) * sizeof(WideChar));
      WideStrLCopy(Line, lpString, len);
      lpString := Line;
    end;//not IsWinNTUp
    statBufUsed := false;
    STATIC_BUFFER_LEN := len;
    l3System.GetLocalMem(Line, (len + 4) * sizeof(WideChar));
    l3System.GetLocalMem(swapStr, (len + 4) * sizeof(WideChar));
  end;//len < MAX_STATIC_BUFFER - 4
  try
    tabStop := 8;
    count := nCount;
    ...
    if strCopyUsed then
      l3System.FreeLocalMem(lpString);
    if not statBufUsed then
    begin
      l3System.FreeLocalMem(Line);
      l3System.FreeLocalMem(swapStr);
    end;
  end
end;
...
{$IFDEF ELPACK_UNICODE}
  ...
  l3Move(PWideChar(SelText)^, ptr^, SelLen);
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\Garant\L3\ElClone\ELVCLUtils.pas initialization enter'); {$EndIf}
...
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\Garant\L3\ElClone\ELVCLUtils.pas initialization leave'); {$EndIf}
finalization
  if @GradientFillProc <> nil then
    GradientFillProc := nil;
  if MSImg32Lib <> 0 then
