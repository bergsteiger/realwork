...
//
// Revision 1.2  2015/02/10 13:44:35  morozov
// {RequestLink: 587164624}
//
// Revision 1.1  2013/08/14 14:11:55  kostitsin
// [$377169452] - перенес ElPopBtn
//
// Revision 1.17  2012/11/01 09:42:31  lulin
// - забыл точку с запятой.
//
// Revision 1.16  2012/11/01 07:44:21  lulin
// - делаем возможность логирования процесса загрузки модулей.
//
// Revision 1.15  2009/10/15 14:43:37  lulin
// {RequestLink:166856141}.
//
// Revision 1.14  2008/05/13 17:06:49  lulin
// - изменения в рамках <K>: 90441490.
//
// Revision 1.13  2008/05/13 15:24:58  lulin
// - чистка в рамках <K>: 90441490.
//
// Revision 1.12  2008/05/13 13:15:41  lulin
// - чистка в рамках <K>: 90441490.
//
// Revision 1.11  2008/05/13 12:16:45  lulin
// - чистка в рамках <K>: 90441490.
//
// Revision 1.10  2008/02/07 11:59:51  lulin
// - распилили наследие ElPack'а по двум библиотекам.
//
// Revision 1.9  2007/08/31 11:26:03  mmorozov
// - hit fix;
//
// Revision 1.8  2007/08/23 08:50:09  oman
// - fix: Для служебных окон не вызывалось DefWndProc (cq25585)
//
// Revision 1.7  2007/08/14 14:44:15  lulin
// - оптимизируем перемещение блоков памяти.
//
// Revision 1.6  2007/08/14 14:30:01  lulin
// - оптимизируем перемещение блоков памяти.
//
// Revision 1.5  2007/04/09 06:05:21  lulin
// - используем родные битмапы.
//
// Revision 1.4  2007/03/27 13:08:01  lulin
// - используем родной список.
//
// Revision 1.3  2007/03/21 12:16:28  lulin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if false or ((FMouseInControl or Focused) and
      ...
      begin
        ...
        begin
          if ((not FFlat) or ((FMouseInControl or FMouseInArrow) or
            ...
            DrawButtonFrameEx(Canvas.Handle, ArrRect, (FMouseInArrow or false) and (not FThinFrame) and (not FDown), (FState in [ebsDown, ebsArrDown]), AColor, ThinFrame);
        end;
      end;
  ...
  FTextRect := FGlyph.Draw(Canvas, PaintRect, Offset, Caption, FLayout, FMargin,
    ...
    FShowText, Aw, TextDrawType, Color, {$ifdef MSWINDOWS}ParentFont and (Theme <> 0), Theme, BP_PUSHBUTTON, sid{$else}false,0,0,0{$endif}, true, ImageIsAlphaBlended, FChangeDisabledText);
  ...
  begin
    ...
    {$ifdef MSWINDOWS}
    InflateRect(R, -1, -1);
    // Для маленьких кнопочек не должно налезать на текст.
...
procedure TCustomElPopupButton.WMKillFocus(var Message: TWMKillFocus);
begin
 inherited;
 Invalidate;
...
procedure TCustomElPopupButton.WMSetFocus(var Message: TWMSetFocus);
begin
 inherited;
 Invalidate;
 // - http://mdp.garant.ru/pages/viewpage.action?pageId=587164624
...
begin
  ...
  begin
    FThinFrame := newValue;
    if false or ((FMouseInControl or Focused) and (not (FState in [ebsDown, ebsExclusive]))) then Invalidate;
  end;  { if }
end;  { SetThinFrame }
...
{$ENDIF}
  ...
  FDownBackground := Tl3Bitmap.Create;
...
procedure TCustomElPopupButton.Cleanup;
begin
  UseArrow := false;
...
begin
  if Enabled then
  begin
    if (not false) or IsThemeApplied then
      b := true;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
             DrawButtonFrameEx(Canvas.Handle, PaintRect, false or ((FMouseInControl or Focused)) and (not ThinFrame),
                             (FDown and FIsSwitch) or (FState in [ebsDown, ebsExclusive]), AColor, ThinFrame);
        end;
      end;
    end
    ...
    begin
      ...
      begin
        ...
        begin
          if ((Background.Empty) or (BackgroundDrawBorder)) and ShowBorder then
             DrawButtonFrameEx(Canvas.Handle, PaintRect, false, //false or ((FMouseInControl or Focused)) and (not ThinFrame),
                             (FDown and FIsSwitch) or (FState in [ebsDown, ebsExclusive]), AColor, true{ThinFrame});
        end;
      end;
    end;
      ...
      begin
        ...
        begin
          if ((not FFlat) or ((FMouseInControl or FMouseInArrow or Focused) or
              (FState in [ebsArrDown, ebsExclusive])
               or (csDesigning in ComponentState))) then
            DrawButtonFrameEx(Canvas.Handle, ArrRect, ({FMouseInArrow or }false) and (not FThinFrame) and (not FDown), (FState in [ebsDown, ebsArrDown]), AColor, ThinFrame or Flat);
        end;
      end;
      DrawArrow(Canvas, eadDown, ArrRect, Font.Color, FState <> ebsDisabled);
    end;
  ...
  FTextRect := FGlyph.Draw(Canvas, PaintRect, Offset, Caption, FLayout, FMargin,
    ...
    ShortcutsEnabled, ImageIsAlphaBlended, FChangeDisabledText);
//  Canvas.Stop;
...
begin
  ...
  begin
    FThinFrame := newValue;
    if false or (FMouseInControl and (not (FState in [ebsDown, ebsExclusive]))) then Invalidate;
  end;  { if }
end;  { SetThinFrame }
...
var
 l_Result: Longint;
begin
 // Хитрый хак чтоб понять, что сообщение не обработано
 // м передать ее DefWndProc. Иначе были зацикливания
 // с установкой фокуса в это окно и невозможностью
 // потом вернуть фокус в приложение (cq25585)
 l_Result := Message.Result;
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\gui\Garant\VT\elClone\Source\ElPopBtn.pas initialization enter'); {$EndIf}

  MenuCancelMsg := RegisterWindowMessage('El - Cancel pulldown menu');
(*  FArrow := Tl3Bitmap.Create;
  FArrow.LoadFromResourceName(HInstance, 'ELPOPUPBUTTONDOWNARROW');*) // <trash> // LAW

{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\gui\Garant\VT\elClone\Source\ElPopBtn.pas initialization leave'); {$EndIf}
finalization

(*  FreeAndNil(FArrow);*) // <trash> // LAW
