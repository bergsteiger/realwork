...
(*   k2_tiObject:
   begin
    // - дерева пока нету - можно здесь дернуть CallBack и спросить у пользователя
    //   где же его взять.
 ...
 (*       if Result then
         ...
         // - чтобы вместо пустой строки подставлялось значение по-умолчанию*)
       end;//l_Def.IsValid
      end;//evInPara(Self, k2_idReqRow)
     end;//not Result
  ...
  k2_tiText, k2_tiSegments:
   ...
   begin
    ...
    begin
     Data := nil;
     Result := false;
     // - это ОБЯЗАТЕЛЬНО, иначе в _k2Tag_.cAtom "залипнет" null
