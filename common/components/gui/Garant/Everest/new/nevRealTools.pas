...
// Библиотека "Everest"
// Автор: Люлин А.В.
// Модуль: "w:/common/components/gui/Garant/Everest/new/nevRealTools.pas"
// Начат: 31.03.2009 16:12
// Родные Delphi интерфейсы (.pas)
// Generated from UML model, root element: <<Interfaces::Category>> Shared Delphi::Everest::nevRealTools
//
// Реальные инструменты для работы с EVD документами (в отличии от nevTools - куда свалено всё
// подряд). Инструмент получается методом QT, и ТОЛЬКО им
//
//
...
type
 InevBaseTool = nevTools.InevBase;
  {* Базовый инструмент }

  TnevShapeArea = {$IfDef XE4}record{$Else}object{$EndIf}
   {* Область вывода }
  public
     rView : HnevView; // Область вывода
     rCanvas : Hl3Canvas; // Канва для рисования
     rNeedBottom : Boolean; // Нужно ли вычислять rBottom
  end;//TnevShapeArea

  TnevShapeInfo = record
   {* Информация об отрисованном объекте. }
    rMap : InevMap; // Карта форматирования.
    rBottom : InevBasePoint; // Точка указывающая за последний отрисованный элемент объекта.
    rCalculatedHeight : Integer; // Посчтианная высота оставшейся части таблицы, для использования на следующих страницах.
    rCellBottom : InevBasePoint; // Продолжение объедиенной ячейки (если есть).
  end;//TnevShapeInfo

  TnevShapePoints = record
   {* Данные для отображения. }
    rTop : InevBasePoint; // Точка начала отображения.
    rCaret : InevBasePoint; // Точка, в которой надо рисовать каретку.
    rSelection : InevRange; // Выделенные участки.
    rPrevHeight : Integer; // Высота оставшейся части таблицы, вычисленная на при печати предыдущих страниц.
    rRealTop : InevBasePoint; // Точка, которая реально отрисовывается в этом месте (начало объедиения).
    rFake : Boolean; // Фейковая точка.
    rCheckCaret : InevBasePoint; // Каретка для проверки в дочерних
    rCheckSelection : InevRange; // Выделение для проверки дочерними.
  end;//TnevShapePoints
 ...
 InevPaintersHolder = interface(InevBaseTool)
  ...
  TnevCheckType = (
   {* вид проверки отрисованности }
    nev_None // не проверять
  , nev_CheckOnly // только проверка
  , nev_NeedAdd // проверить и добавить новое значение
  );//TnevCheckType
