...
// Библиотека "Everest"
// Автор: Люлин А.В.
// Модуль: "w:/common/components/gui/Garant/Everest/new/nevTools.pas"
// Начат: 31.03.2005 13:13
...
*)
 ...
 InevPara = interface;
 { - предварительное описание InevPara. }

 InevRange = interface;
 { - предварительное описание InevRange. }

 InevPoint = interface;
 { - предварительное описание InevPoint. }

 InevBasePoint = interface;
 { - предварительное описание InevBasePoint. }

 InevAnchor = interface;
 { - предварительное описание InevAnchor. }

 InevView = interface;
 { - предварительное описание InevView. }

 InevObjectList = interface;
 { - предварительное описание InevObjectList. }

 InevObjectHolder = interface;
 { - предварительное описание InevObjectHolder. }

 InevBaseLine4Anchor = interface;
 { - предварительное описание InevBaseLine4Anchor. }

 InevBaseLine4Print = interface;
 { - предварительное описание InevBaseLine4Print. }
 ...
 InevParaInternal = interface;
  { - предварительное описание InevParaInternal. }


 InevDocumentContainer = interface;
 { - предварительное описание InevDocumentContainer. }

 InevParaList = interface;
 { - предварительное описание InevParaList. }

 InevSubList = interface;
 { - предварительное описание InevSubList. }
 ...
 IevMarker = interface;
 { - предварительное описание IevMarker. }
 ...
 InevDataFormatting = interface(InevBase)
   ...
   , misfDraw
   , misfSetText
   , misfForSelection // Операция применяется изначально не к курсору, а к выделению http://mdp.garant.ru/pages/viewpage.action?pageId=220596558
   );//TnevInsertStringFlag
...
*)
 ...
 InevRangeFactory = interface;
  { - предварительное описание InevRangeFactory. }
   ...
   , ev_cmKeepOne
   , ev_cmEmpty
   , ev_cmForceKeepOne // ВСЕГДА оставлять один параграф. http://mdp.garant.ru/pages/viewpage.action?pageId=228693150
   , ev_cmVertMergeCells
   , ev_cmClearFirstCell
 ...
 InevConfirm = interface;
  { - предварительное описание InevConfirm. }
...
*)
 ...
 { - предварительное описание InevDataObjectPrim2. }
...
*)
  ...
  TevParaMarkerStyle = (
    ev_pmsNone
  , ev_pmsFirstIndent // отступ первой строки
  , ev_pmsLeftIndent // левый отступ
  , ev_pmsWrapColumn // правая граница текста
  , ev_pmsColumnBorder // граница колонки таблицы
  , ev_pmsParaIndent // левый отступ списка параграфов
  , ev_pmsParaSize // размер списка параграфов
  , ev_pmsPageLeftIndent // левый отступ страницы
  , ev_pmsPageRightIndent // правый отступ страницы
  , ev_pmsPageSize // размер страницы
  , ev_pmsTabIndent // отступ параграфа по табуляции
  );//TevParaMarkerStyle
 ...
 InevMap = interface;
 { - предварительное описание InevMap. }
...
*)
 ...
 { - предварительное описание InevControlView. }
 ...
 InevLinkedPoint = interface;
  { - предварительное описание InevLinkedPoint. }
...
*)
 ...
 { - предварительное описание InevActiveElement. }
...
*)
 ...
 InevView = interface(InevSimpleView)
  ...
  TnevChangePlace = (
   {* Что меняется }
    nev_cpView // параметры области отображения изменились
  , nev_cpPara // состав параграфов изменился
  , nev_cpText // текст параграфа изменился
  );//TnevChangePlace
...
а если true - то жёстко сверху }
 ...
 { - предварительное описание InevInputView. }
 ...
 IevSub = interface;
 { - предварительное описание IevSub. }
...
*)
 ...
 { - предварительное описание InevTextSource. }

 IevDocument = interface;
 { - предварительное описание IevDocument. }
...
*)
 ...
 { - предварительное описание IevDocumentPart. }
 ...
 IevControl = interface;
  { - предварительное описание IevControl. }
 ...
 TnevPointByPtMode = (
  {* Режим получения точки из списка отрисованных. }
   nev_ppmNone // Обычные параметры.
 , nev_ppmNeedAnchor // Инициализация якоря (для строк таблицы инициализируется базовая линия).
 , nev_ppmCheckHidden // Проверка невидимых, но отрисованных шейпов - для поиска начальных частей объединенной ячейки.
 , nev_ppmSetCursor // Получение точки при установке курсора. Если найдена ячейка-продолжения объединенной, то пытаемся получить Head-ячейку.
 , nev_ppmNeedAnchorWithSimpleBaseLine // Получение якоря с не полностью инициализированной базовой линией.
 );//TnevPointByPtMode
