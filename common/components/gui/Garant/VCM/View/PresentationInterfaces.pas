...
type
 ...
 TnsSaveDialogKind = (
  {* Типы диалогов сохранения }
   ns_sdkSimple // Обычный диалог
 , ns_sdkDocument // Для документа (дополнительный чекбокс "Сохранять выделение"
 , ns_sdkList // Для списка - Радиогруппа что сохранять (названия, тексты) и пр. см ЦК25445
 , ns_sdkListInternal // Для списка - Радиогруппа что сохранять (названия, тексты, номера топиков) и пр. см ЦК25445
 );//TnsSaveDialogKind

 TnsSaveDialogListTarget = (
  {* Что сохранять для списка }
   ns_sdlkTitles // Названия документов
 , ns_sdlkContents // Тексты документов
 , ns_sdlkIdentifiers // номера топиков
 );//TnsSaveDialogListTarget
