...
// Generated from UML model, root element: <<UtilityPack::Class>> Shared Delphi::VCM$Visual::Visual$ChromeLike::vcmTabbedContainerFormDispatcher
//
//
...
type
 ...
 {$Include ..\ChromeLike\vcmKeyDataListener.imp.pas}
...
// start class TvcmChromeLikeMainFormProvider
...
type
 ...
 // public methods
   ...
   class function Make(const aRealContainerMaker: IvcmContainerMaker): IvcmContainerMaker; reintroduce;
     {* Сигнатура фабрики TvcmContainerWithTabMaker.Make }
 end;//TvcmContainerWithTabMaker

// start class TvcmContainerWithTabMaker
...
end;//TvcmContainedFormDescr.C

// start class TvcmContainedFormDescr
...
// start class TvcmFormToContainerMap
...
// start class TvcmTabbedContainerFormDispatcher
...
//#UC START# *53A4169700BE_537AEC5E03DD_impl*
 ...
 case aOpenKind of
  vcm_okInCurrentTab: // В текущей вкладке
   begin
    l_NewContainer := l_PrevContainer;
    Result := aPrevContainer;
   end;
  vcm_okInNewTab: // В новой вкладке, в том же окне
   if NeedUseTabs then // до включения вкладок - так, потом нужно убрать !
   begin
  ...
  vcm_okInNewWindow: // В новом окне
   begin
    if NeedUseTabs then // до включения вкладок - так, потом нужно убрать !
...
//#UC START# *53FADEAF02F3_537AEC5E03DD_impl*
 ...
 // - Если закрываемая форма была максимизирована - сбросим границы для размещения
 // новой, потому что иначе у пользователя нажатие кнопки "восстановить" не будет
 // приводить к изменению размеров кнопки, что плохо:
...
initialization
{$If not defined(NoTabs) AND not defined(NoVCM) AND not defined(NoVGScene)}
// Регистрация TvcmTabbedContainerFormDispatcher
