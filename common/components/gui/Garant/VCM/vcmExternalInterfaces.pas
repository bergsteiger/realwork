...
// Начат: 27.06.2003 18:27
// Родные Delphi интерфейсы (.pas)
// Generated from UML model, root element: <<Interfaces::Category>> Shared Delphi::VCM::vcmExternalInterfaces
//
...
type
 ...
 IvcmNode = l3TreeInterfaces.Il3SimpleNode;

 TvcmStateType = (
  {* Определяет тип хранимой для формы информации в элементе истории. }
   vcm_stContent // данные формы.
 , vcm_stPosition // меняется только позиция в данных формы.
 );//TvcmStateType

 IvcmCString = afwInterfaces.IafwCString;
  {* Атомарная строка с поддержкой различных кодировок }

 TvcmOperationType = (
  {* Тип представления операции. }
   vcm_otButton // Операция стандартного вида. Представляется как                        строка в меню и кнопка на панели инструментов.
 , vcm_otShortCut
 , vcm_otTextButton
 , vcm_otButtonCombo
 , vcm_otButtonPopup
 , vcm_otCheck // Операция с возможностью пометки. Отображается как                        строка с пометкой в меню.
 , vcm_otRadio
 , vcm_otEdit
 , vcm_otTyper
 , vcm_otCombo // Выпадающий список строк. В меню отображается как                        подменю, на панели инструментов - как выпадающий                        список строк.
 , vcm_otMenuButtonCombo
 , vcm_otEditCombo
 , vcm_otFontName // Выпадающий список имен шрифтов. В меню                        отображается как подменю, на панели инструментов -                        как выпадающий список строк.
 , vcm_otFontSize // Выпадающий список размеров шрифтов. В меню                        отображается как подменю, на панели инструментов -                        как выпадающий список строк.
 , vcm_otDate
 , vcm_otInternal // Внутренняя операции сущности. Не отображается ни в меню, ни на панели инструментов. Кандидат на удаление
 , vcm_otHidden
 );//TvcmOperationType
 ...
 TvcmDoneStatus = (
  {* Статус выполнения операции. }
   vcm_dsNotDone // не выполнено
 , vcm_dsDisabled // операция запрещена (Disabled)
 , vcm_dsInvalidParams // неверный набор параметров
 , vcm_dsSkippedInTest // разрежен OnTest - сейчас такого нету
 , vcm_dsExcludedInUserType // запрещена в данном UserType'у
 , vcm_dsNoAggregate // нет агрегации
 , vcm_dsNoEntity // нет такой сущности
 , vcm_dsFormInClose // форма закрывается
 , vcm_dsNullContainer // операция пришла псевдо-контейнеру
 , vcm_dsDone // операция выполнена
 );//TvcmDoneStatus

 IvcmStrings = afwInterfaces.IafwStrings;
  {* Список строк. }

 TvcmOPID = record
  {* Идентификатор операции сущности }
   rEnID : TvcmControlID; // Сущность.
   rOpID : TvcmControlID; // Операция.
 end;//TvcmOPID

 TvcmMOPID = record
  {* Идентификатор операции модуля. }
   rMoID : TvcmControlID; // Модуль.
   rOpID : TvcmControlID; // Операция
 end;//TvcmMOPID
 ...
 IvcmTree = l3TreeInterfaces.Il3SimpleTree;
  {* Дерево. }

 IvcmFont = afwInterfaces.IafwFont;
  {* Шрифт. }
 ...
 IvcmSettings = afwInterfaces.IafwSettings;

 IvcmResources = afwInterfaces.IafwResources;

 IvcmSettingsSource = afwInterfaces.IafwSettingsSource;

 TvcmSettingId = afwInterfaces.TafwSettingId;
 ...
 TvcmControlGetStateEvent = procedure (var State: TvcmOperationStateIndex) of object;
   {* Обработчик события проверки состояния операции }
