...
type
  ...
  private
    function GetPageControl : TElCustomPageControl;
  public
    {$ifdef VCL_5_USED}
    {$ifndef VCL_6_USED}
...
var PageControl : TElCustomPageControl;
...
function TElPageControlEditor.GetPageControl : TElCustomPageControl;
begin
  if (Component is TElCustomPageControl) then
    result := TElCustomPageControl(Component)
  else
  if (Component is TElTabSheet) then
...
var
  I: Integer;
begin
  if GetComponent(0) is TElCustomPageControl then
  begin
    for I := 0 to TElCustomPageControl(GetComponent(0)).PageCount - 1 do
      Proc(TElCustomPageControl(GetComponent(0)).Pages[i].Name);
  end;
end;
