...
uses SysUtils, Dialogs, {$IFDEF WIN32} CommCtrl, {$ELSE} Str16, {$ENDIF}
  VCLUtils, l3MinMax, Consts, {$IFDEF RX_D5}RxAppUtils{$ELSE}AppUtils{$ENDIF} {$IFDEF RX_D4}, ImgList,  // Polaris
  ActnList {$ENDIF}
...
(*{$IFDEF WIN32}
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\gui\Garant\VT\rx\RxCtrls.pas initialization enter'); {$EndIf}
  FCheckBitmap := nil;
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\gui\Garant\VT\rx\RxCtrls.pas initialization leave'); {$EndIf}
finalization
  DestroyLocals;
{$ELSE}
initialization
