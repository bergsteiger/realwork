...
{$ELSE}
    ...
    function GetValue: AnsiString; virtual; abstract;
    procedure SetValue(const Value: AnsiString); virtual; abstract;
{$ENDIF}
  ...
  public
    ...
    function GetPopupText: AnsiString; virtual;
    procedure Hide;
...
//    procedure SetShowCaret; // Polaris
    ...
    procedure SetButtonHint(const Value: AnsiString);
...
{$ELSE}
    function AcceptPopup(var Value: AnsiString): Boolean; virtual;
    procedure AcceptValue(const Value: AnsiString); virtual;
    procedure SetPopupValue(const Value: AnsiString); virtual;
    function GetPopupValue: AnsiString; virtual;
{$ENDIF}
...
type
  TExecOpenDialogEvent = procedure(Sender: TObject; var Name: AnsiString;
...
{$IFDEF WIN32}
    function GetLongName: AnsiString; virtual; abstract;
    function GetShortName: AnsiString; virtual; abstract;
{$ENDIF}
    procedure DoAfterDialog(var FileName: AnsiString; var Action: Boolean); dynamic;
    procedure DoBeforeDialog(var FileName: AnsiString; var Action: Boolean); dynamic;
    procedure ReceptFileDir(const AFileName: AnsiString); virtual; abstract;
  ...
  public
    constructor Create(AOwner: TComponent); override;
{$IFDEF WIN32}
    property LongName: AnsiString read GetLongName;
...
{ TFilenameEdit }
  ...
  private
    ...
    function GetDialogTitle: AnsiString;
    ...
    procedure SetInitialDir(const Value: AnsiString);
    procedure SetHistoryList(Value: TStrings);
    procedure SetOptions(Value: TOpenOptions);
    procedure SetDialogTitle(const Value: AnsiString);
    function IsCustomTitle: Boolean;
    function IsCustomFilter: Boolean;
  protected
    procedure ButtonClick; override;
    procedure ReceptFileDir(const AFileName: AnsiString); override;
    procedure ClearFileList; override;
{$IFDEF WIN32}
    function GetLongName: AnsiString; override;
...
//Polaris
    ...
    property FileEditStyle: TFileEditStyle read GetFileEditStyle write SetFileEditStyle
      default fsEdit;
    property FileName: AnsiString read GetFileName write SetFileName stored False;
    property Filter: AnsiString read GetFilter write SetFilter stored IsCustomFilter;
    property FilterIndex: Integer read GetFilterIndex write SetFilterIndex default 1;
    property InitialDir: AnsiString read GetInitialDir write SetInitialDir;
    ...
    property DialogTitle: AnsiString read GetDialogTitle write SetDialogTitle
...
{$ENDIF}
  ...
  private
    ...
    FInitialDir: AnsiString;
{$IFDEF WIN32}
    FDialogText: AnsiString;
...
{$IFDEF WIN32}
    property DialogKind: TDirDialogKind read FDialogKind write FDialogKind
      default dkVCL;
    property DialogText: AnsiString read FDialogText write FDialogText;
{$ENDIF}
    property DialogOptions: TSelectDirOpts read FOptions write FOptions default [];
    property InitialDir: AnsiString read FInitialDir write FInitialDir;
...
//    procedure SetDate(Value: TDateTime);
    ...
    procedure SetBlanksChar(Value: AnsiChar);
...
{$ELSE}
    function AcceptPopup(var Value: AnsiString): Boolean; override;
{$ENDIF}
    ...
    procedure ButtonClick; override;
    property BlanksChar: AnsiChar read FBlanksChar write SetBlanksChar default ' ';
...
// Polaris
    ...
    function GetDateMask: AnsiString;
...
function PaintComboEdit(Editor: TCustomComboEdit; const AText: AnsiString;
...
uses ShellAPI, Consts, {$IFDEF RX_D3} ExtDlgs, {$ENDIF} RXCConst, VCLUtils,
  l3FileUtils, {PickDate,} l3MinMax,
  {$IFDEF RX_D6} Variants, RxStrUtils, RTLConsts, {$ELSE} StrUtils, {$ENDIF}
  rxDialogs
  ; // Polaris
...
{$ENDIF RX_D4}
    ...
    try
      ...
      with ACanvas do begin
        ...
        S := AText;
        AWidth := TextWidth(S);
        l_Margins := EditorTextMargins(Editor);
        if PopupVisible then ALeft := l_Margins.X
        else begin
          ...
          case AAlignment of
            taLeftJustify:
              ALeft := l_Margins.X;
            taRightJustify:
              ALeft := ClientWidth - ButtonWidth - AWidth - l_Margins.X{Polaris - 2};
            else
...
{$ENDIF}
        TextRect(R, ALeft, l_Margins.Y, S);
      end;
    finally
      ACanvas.Handle := 0;
...
function TPopupWindow.GetPopupText: AnsiString;
begin
  Result := '';
end;
...
{$ELSE}
function TCustomComboEdit.GetPopupValue: AnsiString;
{$ENDIF}
begin
...
{$ELSE}
procedure TCustomComboEdit.SetPopupValue(const Value: AnsiString);
{$ENDIF}
begin
...
{$ELSE}
procedure TCustomComboEdit.AcceptValue(const Value: AnsiString);
begin
...
{$ELSE}
function TCustomComboEdit.AcceptPopup(var Value: AnsiString): Boolean;
{$ENDIF}
begin
  Result := True;
end;
...
//Polaris  if (Key = AnsiChar(VK_RETURN)) or (Key = AnsiChar(VK_ESCAPE)) or ((KEY=#10) and PopupVisible) then
  if (Key = AnsiChar(VK_RETURN)) or (Key = AnsiChar(VK_ESCAPE)) or ((KEY=#10) and PopupVisible) then
  begin
    if PopupVisible then begin
//Polaris      PopupCloseUp(FPopup, Key = AnsiChar(VK_RETURN));
      PopupCloseUp(FPopup, Key <> AnsiChar(VK_ESCAPE));
      Key := #0;
    end
    else begin
      { must catch and remove this, since is actually multi-line }
      GetParentForm(Self).Perform(CM_DIALOGKEY, Byte(Key), 0);
      if Key = AnsiChar(VK_RETURN) then begin
...
function TCustomComboEdit.GetButtonHint: AnsiString;
begin
  Result := FButton.Hint;
end;

procedure TCustomComboEdit.SetButtonHint(const Value: AnsiString);
begin
  FButton.Hint := Value;
end;
...
begin
  case FGlyphKind of
    ...
    gkDropDown:
      begin
        FButton.Glyph.Handle := LoadBitmapA(0, PAnsiChar(32738));
        NumGlyphs := 1;
      end;
    gkEllipsis:
      begin
...
procedure TFileDirEdit.DoBeforeDialog(var FileName: AnsiString;
  var Action: Boolean);
begin
  if Assigned(FOnBeforeDialog) then FOnBeforeDialog(Self, FileName, Action);
end;

procedure TFileDirEdit.DoAfterDialog(var FileName: AnsiString;
  var Action: Boolean);
begin
...
var
  AFileName: array[0..255] of AnsiChar;
  I, Num: Cardinal;
begin
  Msg.Result := 0;
  try
...
{$ENDIF}
    if Num > 0 then begin
      ...
      for I := 0 to Num - 1 do begin
        DragQueryFileA(Msg.Drop, I, PAnsiChar(@AFileName), Pred(SizeOf(AFileName)));
...
function ClipFilename(const FileName: AnsiString): AnsiString;
var
  Params: AnsiString;
begin
...
function ExtFilename(const FileName: AnsiString): AnsiString;
begin
...
function TFilenameEdit.GetFileName: AnsiString;
begin
  Result := ClipFilename(inherited Text);
end;

procedure TFilenameEdit.SetFileName(const Value: AnsiString);
begin
...
function TFilenameEdit.GetLongName: AnsiString;
begin
  Result := ShortToLongFileName(FileName);
end;

function TFilenameEdit.GetShortName: AnsiString;
begin
...
procedure TFilenameEdit.ReceptFileDir(const AFileName: AnsiString);
begin
...
function TFilenameEdit.GetFilter: AnsiString;
begin
...
function TFilenameEdit.GetInitialDir: AnsiString;
begin
...
function TFilenameEdit.GetDialogTitle: AnsiString;
begin
...
procedure TFilenameEdit.SetFilter(const Value: AnsiString);
begin
...
procedure TFilenameEdit.SetInitialDir(const Value: AnsiString);
begin
...
procedure TFilenameEdit.SetDialogTitle(const Value: AnsiString);
begin
  FDialog.Title := Value;
end;
...
{$IFDEF WIN32}
    ...
    begin
     {$IfDef XE}
     l_S := Temp;
     Action := SelectDirectory(l_S, FOptions, Self.HelpContext);
     Temp := l_S;
     {$Else  XE}
...
procedure TDirectoryEdit.ReceptFileDir(const AFileName: AnsiString);
var
  Temp: AnsiString;
begin
...
function TDirectoryEdit.GetLongName: AnsiString;
var
  Temp: AnsiString;
  Pos: Integer;
begin
...
function TDirectoryEdit.GetShortName: AnsiString;
var
  Temp: AnsiString;
  Pos: Integer;
begin
...
function TCustomDateEdit.GetDateFormat: AnsiString;
begin
  Result := FDateFormat;
end;
...
procedure TCustomDateEdit.SetBlanksChar(Value: AnsiChar);
begin
...
function TCustomDateEdit.GetDateMask: AnsiString;
begin
...
function TCustomDateEdit.GetDialogTitle: AnsiString;
begin
  Result := FTitle^;
end;

procedure TCustomDateEdit.SetDialogTitle(const Value: AnsiString);
begin
...
procedure TCustomDateEdit.KeyPress(var Key: AnsiChar);
begin
...
{$ELSE}
function TCustomDateEdit.AcceptPopup(var Value: AnsiString): Boolean;
{$ENDIF}
var
  D: TDateTime;
begin
...
{$IFDEF WIN32}
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\gui\Garant\VT\rx\ToolEdit.pas initialization enter'); {$EndIf}
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\gui\Garant\VT\rx\ToolEdit.pas initialization leave'); {$EndIf}
finalization
  DestroyLocals;
{$ELSE}
initialization
