...
function MakeBitmap(ResID: PAnsiChar): TBitmap;
function MakeBitmapID(ResID: Word): TBitmap;
function MakeModuleBitmap(Module: THandle; ResID: PAnsiChar): TBitmap;
...
function MakeIcon(ResID: PAnsiChar): TIcon;
function MakeIconID(ResID: Word): TIcon;
function MakeModuleIcon(Module: THandle; ResID: PAnsiChar): TIcon;
...
procedure ResourceNotFound(ResID: PAnsiChar);
...
{$ELSE}
function FindPrevInstance(const MainFormClass, ATitle: ansistring): HWnd;
...
function LoadDLL(const LibName: ansistring): THandle;
...
function GetEnvVar(const VarName: ansistring): ansistring;
function AnsiUpperFirstChar(const S: ansistring): ansistring;
function StringToPAnsiChar(var S: string): PAnsiChar;
function StrPAlloc(const S: string): PAnsiChar;
...
procedure StartWait;
procedure StopWait;
function DefineCursor(Instance: THandle; ResID: PAnsiChar): TCursor;
{$IFDEF WIN32}
function LoadAniCursor(Instance: THandle; ResID: PAnsiChar): HCursor;
{$ENDIF}
...
{ TResourceStream }
  ...
  private
    ...
    FEndPos: LongInt;
  protected
    constructor CreateFromPAnsiChar(Instance: THandle; ResName, ResType: PAnsiChar);
  public
    constructor Create(Instance: THandle; const ResName: string; ResType: PAnsiChar);
    constructor CreateFromID(Instance: THandle; ResID: Integer; ResType: PAnsiChar);
    destructor Destroy; override;
...
Uses SysUtils, Messages, l3MinMax, l3String, Consts, RxConst, {$IFDEF RX_V110} SysConst, {$ENDIF}
...
{$IFDEF WIN32}
    if Module <> 0 then begin
      ...
      Result.Handle := LoadBitmapA(Module, ResID);
...
function MakeBitmapID(ResID: Word): TBitmap;
begin
  Result := MakeModuleBitmap(hInstance, MakeIntResourceA(ResID));
end;
...
function MakeIconID(ResID: Word): TIcon;
begin
  Result := MakeModuleIcon(hInstance, MakeIntResourceA(ResID));
end;

function MakeModuleIcon(Module: THandle; ResID: PAnsiChar): TIcon;
begin
  Result := TIcon.Create;
  Result.Handle := LoadIconA(Module, ResID);
...
{$IFDEF WIN32}
  Result := LoadLibraryA(PAnsiChar(LibName));
{$ELSE}
...
begin
  ...
  try
    Result := FindWindowA(BufClass, BufTitle);
  finally
    l3StrDispose(BufTitle);
    l3StrDispose(BufClass);
  end;
end;
...
begin
{$IFDEF WIN32}
  if GetEnvironmentVariableA(PAnsiChar(VarName), S, SizeOf(S) - 1) > 0 then
    Result := StrPas(S)
  else Result := '';
{$ELSE}
  L := Length(VarName);
  ...
  while P^ <> #0 do begin
    if (StrLIComp(P, {$IFDEF WIN32} PAnsiChar(VarName) {$ELSE} S {$ENDIF}, L) = 0) and
      (P[L] = '=') then
    begin
      Result := StrPas(P + L + 1);
      Exit;
...
function ParamStrFromCommandLine(CmdLine: PAnsiChar; Index: Integer): ansistring;
var
  P: PAnsiChar;
begin
  P := CmdLine;
...
function StrPAlloc(const S: string): PAnsiChar;
begin
  Result := StrPCopy(l3StrAlloc(Length(S) + 1), S);
end;
...
begin
  Result := 0;
  Rsrc := FindResourceA(Instance, ResID, {$IfDef XE}MakeIntResourceA(21){$Else}RT_ANICURSOR{$EndIf});
...
{$ELSE}
  {$IFDEF WIN32}
    ExtTextOut(ACanvas.Handle, Left, ARect.Top + DY, ETO_OPAQUE or
      ETO_CLIPPED, @ARect, PAnsiChar(Text), Length(Text), nil);
  {$ELSE}
    ExtTextOut(ACanvas.Handle, Left, ARect.Top + DY, ETO_OPAQUE or
...
{$IFDEF RX_D4}
        if (ACanvas.CanvasOrientation = coRightToLeft) then
          ChangeBiDiModeAlignment(Alignment);
        DrawText(Handle, PAnsiChar(Text), Length(Text), R, AlignFlags[Alignment]
...
begin
  CreateFromPAnsiChar(Instance, StrPCopy(ResID, ResName), ResType);
end;

constructor TResourceStream.CreateFromID(Instance: THandle; ResID: Integer;
  ResType: PAnsiChar);
begin
  CreateFromPAnsiChar(Instance, MakeIntResource(ResID), ResType);
end;

constructor TResourceStream.CreateFromPAnsiChar(Instance: THandle; ResName,
  ResType: PAnsiChar);
var
  ResInfo: THandle;
  Handle: Integer;
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\gui\Garant\VT\rx\VCLUtils.pas initialization enter'); {$EndIf}
{$IFDEF WIN32}
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\gui\Garant\VT\rx\VCLUtils.pas initialization leave'); {$EndIf}
finalization
  ReleaseBitmap;
{$ELSE}
