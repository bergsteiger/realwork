...
// Элемент модели: "TvgRemindersLine" MUID: (532322D901A3)
...
type
 TvgReminder = class(Tl3InterfacedComponent{$If NOT Defined(NoVCM)}
 , IvcmDatasourceChangeListener
 {$IfEnd} // NOT Defined(NoVCM)
 , IvgReminder)
  private
   f_FlashTimer: TTimer;
 ...
 TvgRemindersLine = class(Tl3InterfacedComponent, Il3WndProcListener, IafwTabSheetControl, IafwControlDock, Il3WndProcRetListener, IvgRemindersLine{$If NOT Defined(NoVCM)}
 , IvcmComponentContainer
 {$IfEnd} // NOT Defined(NoVCM)
 {$If NOT Defined(NoVCM)}
 , IvcmResourceHolder
 {$IfEnd} // NOT Defined(NoVCM)
...
//#UC START# *5368D3FD03CD_532322D901A3_var*
 ...
 procedure lp_SetZOrder(ASubjectWnd: HWND; AInsertAfter: HWND);
 begin
  SetWindowPos(ASubjectWnd, AInsertAfter, 0, 0, 0, 0,
   SWP_NOACTIVATE or SWP_NOMOVE or SWP_NOSIZE or SWP_NOOWNERZORDER or SWP_NOSENDCHANGING);
 end;
...
//#UC START# *5368D3FD03CD_532322D901A3_impl*
 ...
 begin
  ...
  if (not (IsWindow(l_NextWindow) and IsWindowVisible(l_NextWindow))) then
  begin
   lp_SetZOrder(f_Form.Handle, HWND_TOPMOST);
   lp_SetZOrder(f_Form.Handle, HWND_NOTOPMOST);
   Exit;
  end;
  lp_SetZOrder(f_Form.Handle, l_NextWindow);
 end;
