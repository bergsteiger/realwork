...
// Generated from UML model, root element: <<GuiControl::Class>> Shared Delphi::VT::Outliner::TvtLister
//
...
type
 ...
 TGetItemImage = l3ControlsTypes.Tl3GetItemImage;
...
type
 ...
 TvtCustomListerPrim = class(TvtItemStorageViewer {$If not defined(DesignTimeLibrary)}, IafwStyleTableSpy{$IfEnd} //not DesignTimeLibrary
 , Il3Window)
 private
 // private fields
 ...
 // private methods
   ...
   procedure MakeSureCaretIsCreated;
     {* Сигнатура метода MakeSureCaretIsCreated }
   procedure DoNeedRecalcConstraints;
   procedure InvalidateNonclientArea;
     {* Сигнатура метода InvalidateNonclientArea }
 ...
 // property methods
   ...
   procedure pm_SetSelected(Index: LongInt; aValue: Boolean); virtual;
   function pm_GetSelectState(Index: LongInt): Integer;
   procedure pm_SetSelectState(Index: LongInt; aValue: Integer);
 ...
 // protected methods
   ...
   procedure DoVScrollVisibleChanged;
     {* Сигнатура метода DoVScrollVisibleChanged }
   procedure DoHScrollVisibleChanged;
     {* Сигнатура метода DoHScrollVisibleChanged }
   ...
   procedure FreeHintWindow;
     {* Сигнатура метода FreeHintWindow }
   ...
   procedure UpdateInterRowIndent;
     {* Сигнатура метода UpdateInterRowIndent }
...
умолчанию отступ делается на ширину иконки, если таковые имеются. }
   ...
   procedure DoFooterClick;
     {* Сигнатура метода DoFooterClick }
   procedure DropFooterSelection;
     {* Сигнатура метода DropFooterSelection }
   ...
   procedure DropDrawPoints;
     {* Сигнатура метода DropDrawPoints }
   ...
   procedure ResetSelectArray;
     {* Сигнатура метода ResetSelectArray }
   ...
   procedure VlbSetVScrollPos;
     {* Сигнатура метода vlbSetVScrollPos }
 ...
 // public methods
   ...
   function CalcFullHeight: Integer;
     {* Функция для получения общей высоты всех элементов }
   ...
   procedure DrawLock;
     {* Сигнатура метода DrawLock }
   procedure DrawUnLock;
     {* Сигнатура метода DrawUnLock }
   procedure FullRedraw;
     {* Сигнатура метода FullRedraw }
   function GetDrawString(aIndex: Integer): Il3CString;
     {* получаем строку для рисования }
   procedure LockChangeSelected;
     {* Сигнатура метода LockChangeSelected }
   procedure UnLockChangeSelected;
     {* Сигнатура метода UnLockChangeSelected }
   procedure LockMakeCurrentVisible;
     {* Сигнатура метода LockMakeCurrentVisible }
   procedure UnLockMakeCurrentVisible;
     {* Сигнатура метода UnLockMakeCurrentVisible }
...
var
   g_InSetTop : Boolean = false;

const
   { vtLister Internal Const }
...
// start class TvtCustomListerPrim
...
procedure TvtCustomListerPrim.pm_SetSelected(Index: LongInt; aValue: Boolean);
...
procedure TvtCustomListerPrim.pm_SetSelectState(Index: LongInt; aValue: Integer);
...
// start class TvtCustomDataListerPrim
...
// start class TvtCustomIStrListListerPrim
...
// start class TvtCustomDStringLister
