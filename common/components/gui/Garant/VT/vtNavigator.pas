...
// Generated from UML model, root element: <<GuiControl::Class>> Shared Delphi::VT$WithVCM::vtNavigator::TvtNavigator
//
...
type
 ...
 // private methods
   ...
   procedure AutoUpdateSize;
     {* Сигнатура метода AutoUpdateSize }
   ...
   procedure CreateSurface;
     {* Сигнатура метода CreateSurface }
   procedure CreateHeader;
     {* Сигнатура метода CreateHeader }
   procedure CreatePageControl;
     {* Сигнатура метода CreatePageControl }
   procedure InitMainForm(aOwner: TComponent);
   procedure RecalcSize;
     {* Сигнатура метода RecalcSize }
   ...
   procedure PlaceSurface;
     {* Сигнатура метода PlaceSurface }
   procedure AttachSurface;
     {* Сигнатура метода AttachSurface }
   procedure DetachSurface;
     {* Сигнатура метода DetachSurface }
   procedure SetSizeMini;
     {* Сигнатура метода SetSizeMini }
   ...
   procedure AutoHideUpdate;
     {* Сигнатура метода AutoHideUpdate }
   procedure CloseDelay;
     {* Сигнатура метода CloseDelay }
   ...
   procedure Minimized;
     {* Сигнатура метода Minimized }
   procedure DoSaveSize;
     {* Сигнатура метода DoSaveSize }
   procedure DrawSplit(const aRect: TRect);
   procedure InitDockDrop;
     {* Сигнатура метода InitDockDrop }
   procedure ModeAutoUpdate;
     {* Сигнатура метода ModeAutoUpdate }
   procedure ModeHide;
     {* Сигнатура метода ModeHide }
   procedure ModeNormalUpdate;
     {* Сигнатура метода ModeNormalUpdate }
   ...
   procedure DoSplit;
     {* Сигнатура метода DoSplit }
   procedure SetUndockSize;
     {* Сигнатура метода SetUndockSize }
   ...
   procedure NormalUpdateSize;
     {* Сигнатура метода NormalUpdateSize }
   procedure ModeShow;
     {* Сигнатура метода ModeShow }
 ...
 // private methods
   procedure HideCloseButton;
     {* Сигнатура метода HideCloseButton }
   ...
   procedure UpdateImages;
     {* Сигнатура метода UpdateImages }
   procedure UpdateSize;
     {* Сигнатура метода UpdateSize }
   procedure UpdatePositions;
     {* Сигнатура метода UpdatePositions }
   procedure UpdateHideImage;
     {* Сигнатура метода UpdateHideImage }
   procedure UpdateCloseImage;
     {* Сигнатура метода UpdateCloseImage }
   procedure UpdateAutoHideImage;
     {* Сигнатура метода UpdateAutoHideImage }
 ...
 // public methods
   ...
   procedure DefineSplitterAlign;
     {* Сигнатура метода DefineSplitterAlign }
 public
 // public properties
   property PageControl: TnpPageControl
     read f_PageControl
 ...
 // private methods
   ...
   procedure UpdateFloatWindowsBounds;
     {* Сигнатура метода UpdateFloatWindowsBounds }
    {$IfEnd} //Nemesis AND not NoVCM
...
type
  TnpDoActionWithFormControl = (
   {* Действие, которое надо выполнить с компонентом на форме, который никуда не привязан }
    acRedraw // компонент нужно перерисовать. Используется после отпускания и перерисовки splitter-а навигатора, иначе на компонентах с Align = alNone, оставалась полоса от сплиттера, т.к. их никто не перерисовывал (cq: 00015856).
  , acUpInZOrder // поднять по оси, чтобы после пристыковки навигатора, на заслонить лежащий в этом месте компонент
  );//TnpDoActionWithFormControl

// start class TnpCustomPanel
...
{$IfEnd} //not NoVCL
// start class TnpContainerSurface
