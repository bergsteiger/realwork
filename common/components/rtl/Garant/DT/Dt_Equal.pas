...
Type
 ...
 TEqualItemsTbl = Class(TPrometTbl)
  ...
  public
   ...
   procedure AddEqualsTo(aIDs: Tl3LongintList); // добавляет в aIDs эквивалентные
...
var
 ...
 procedure MarkEqualsFor(aID: Integer); // рекурсивная
 var
  I: Integer;
 begin
  for I := 0 to l_Arr.Count - 1 do
   if PEqualRec(l_Arr[I])^.FirstID = aID then     // Опа, нашли
   begin
...
begin
 ...
 try
  ...
  // Собственно поиск эквивалентных.
  // Все найденные значения (а это будут только SecondId) никуда не перекладываем,
  ...
  // выкладываем найденные (помеченные) значения в aIds
  // Внимание! Исходные значения не удаляем (никакого aIds.Clear не делаем)
