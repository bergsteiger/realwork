...
// Начат: 17.12.2009 20:41
// Родные Delphi интерфейсы (.pas)
// Generated from UML model, root element: <<UtilityPack::Class>> Shared Delphi Testing Framework::DUnitTuning::Source::KTestRunner
//
//
...
type
 TKFileOperation = (
  {* Операция с файлом в К }
   foGet // Прочитать файл
 , foSet // Записать файл
 , foGetLabel // Получить метки
 , foSetLabel // Установить метки
 , foAttach
 );//TKFileOperation
...
{$Include w:\common\components\rtl\Garant\L3\l3RecordWithEQList.imp.pas}
 ...
 // private methods
   ...
   procedure PutTimesToK;
    ...
    {$If defined(nsTest) AND not defined(NoKPageTool) AND not defined(NotTunedDUnit)}
   function GetLabelsFileName: AnsiString;
     {* Возвращает имя файла с метками страницы тестов }
    {$IfEnd} //nsTest AND not NoKPageTool AND not NotTunedDUnit
...
var
   f_File : TextFile;
var
   f_IsFakeK : Boolean = false;
var
   f_FakeKValid : Boolean = false;
var
   f_FakeCVSValid : Boolean = false;
var
   f_IsFakeCVS : Boolean = false;

// start class TKTestListener
...
var
 l_Name : TTimeInfoName; // Имя текущего замера
 l_i : Integer;
 l_j : Integer;
 l_C : Integer;
 l_SeriesCount : Integer;
