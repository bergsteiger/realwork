...
type
 TseModalExecute = (
  {* se_meUsual - это обычное поведение модальных диалогов. 
 А остальные - заточка для диалогов поиска и проверки орфографии. Они не MODAL, а StayOnTop. И вызываются в цикле. Для каждого такого вызова используется флаг: se_meInLoop. Плюс после окончания проверки может быть вызов диалога, когда он вызывается с надписью, что все закончилось. Чтобы этот вызов отличить от вызова в цикле и добавлен флаг se_meAfterLoop. 
 Разница есть на уровне скриптов:
[code]
PROCEDURE SpellNext BOOLEAN IN aLast

;  
[code]
aLast - как раз и показывает, что диалог уже финальный.
Примеры использования: [Requestlink:622826341] }
  se_meUsual
   {* Флаг обычного режима вызова MODAL. }
  , se_meInLoop
   {* Флаг циклического вызова MODAL }
  , se_meAfterLoop
   {* Флаг вызова после выхода из цикла (перед закрытием окна поиска). }
 );//TseModalExecute
