...
type
  TMergeNameFunc = function (MergeNum : Integer) : AnsiString;
...
{$ENDIF}
  ...
  TStSorter = class(Tl3ProtoObject)
  {.Z+}
  protected
    {property instance variables}
    FCount     : Int64;                  {Number of records put to sort}
    FRecLen    : Cardinal;               {Size of each record}
    FCompare   : Im3SorterComparer;      {Compare function}
...
{$IFDEF WIN32}
 ...
 {$ENDIF}
    ...
    {$IfDef m3StoreNewInThread}
    f_CurrentBuffer : Pointer;
    f_WriteBuffer   : Pointer;
    f_Flusher       : Tl3ThreadContainer;
    {$EndIf m3StoreNewInThread}
{$ELSE}
...
{$ENDIF}
    ...
    procedure sorSetCompare(const Comp : Im3SorterComparer);
  ...
  public
    ...
    property Compare : Im3SorterComparer
  ...
  Tm3Sorter = class(TStSorter, Im3Sorter)
    private
    // internal fields
      f_Remaining : Int64;
    public
    // public methods
      constructor Create(aItemSize             : LongInt;
                         const aSorterComparer : Im3SorterComparer);
        {-}                 
      class function MakeInterface(AItemSize             : LongInt;
                                   const ASorterComparer : Im3SorterComparer): Im3Sorter;
        {-}                           
...
function MinimumHeapToUse(RecLen : Cardinal) : LongInt;
  {-Returns the _absolute minimum heap that allows MergeSort to succeed}
...
function DefaultMergeName(MergeNum : Integer) : AnsiString;
...
begin
  ...
  l3FillChar(Result, SizeOf(TMergeInfo), 0);
...
begin
  ...
  repeat
    ...
    {$ELSE}
    {$IfDef m3StoreNewInThread}
    sorBuffer := l3System.GlobalAllocPtr(AllocRecs * LongInt(FRecLen) * 2);
    f_CurrentBuffer := sorBuffer;
    f_WriteBuffer := PAnsiChar(sorBuffer) + AllocRecs * LongInt(FRecLen);
    {$Else  m3StoreNewInThread}
    sorBuffer := l3System.GlobalAllocPtr(AllocRecs * LongInt(FRecLen));
    {$EndIf m3StoreNewInThread}
    {$ENDIF}
  ...
  {$IfDef m3StoreNewInThread}
  sorMergeBufSize := LongInt(FRecLen)*(AllocRecs div (MergeOrder+1))*2;
  {$Else m3StoreNewInThread}
...
begin
 {$IfDef m3StoreNewInThread}
  Result := PAnsiChar(f_CurrentBuffer)+Index*LongInt(FRecLen);
 {$Else m3StoreNewInThread}
...
(*
begin
  Result := PAnsiChar(sorBuffer[Index shr Byte(sorRecsShr)])+
...
begin
  ...
  {$ELSE}
  l3System.GlobalFreePtr(sorBuffer);
  {$ENDIF}
end;
{$ELSE}
var
...
begin
  ...
  for M := 2 to sorMergeOpenCount do
    if FCompare.Compare(sorMergePtrs[M], MinElPtr, FRecLen) < 0 then begin
      Result := M;
  ...
  repeat
    ...
    repeat
      ...
      if PartitionLen >= MedianThreshold then begin
        ...
        if FCompare.Compare(sorElementPtr(Pm), sorElementPtr(Pl), FRecLen) < 0 then
          sorSwapElements(Pm, Pl);
        if FCompare.Compare(sorElementPtr(Pr), sorElementPtr(Pl), FRecLen) < 0 then
          sorSwapElements(Pr, Pl);
        if FCompare.Compare(sorElementPtr(Pr), sorElementPtr(Pm), FRecLen) < 0 then
      ...
      repeat
        while FCompare.Compare(sorElementPtr(Pl), sorPivotPtr, FRecLen) < 0 do
          Inc(Pl);
        while FCompare.Compare(sorPivotPtr, sorElementPtr(Pr), FRecLen) < 0 do
          Dec(Pr);
...
procedure TStSorter.sorSetCompare(const Comp : Im3SorterComparer);
...
{$IfDef m3StoreNewInThread}
type
  Tm3Flusher = class(Tl3ThreadContainer)
    private
    // internal fields
      f_Sorter       : TStSorter;
      f_BytesToWrite : LongInt;
    protected
    // internal methods
      procedure Execute;
        override;
    ...
    public
    // public methods
      constructor Create(aSorter       : TStSorter;
                         aBytesToWrite : LongInt);
        reintroduce;
        {-}
  end;//Tm3Flusher

constructor Tm3Flusher.Create(aSorter       : TStSorter;
                              aBytesToWrite : LongInt);
  //reintroduce;
  {-}
begin
 f_BytesToWrite := aBytesToWrite;
 f_Sorter := aSorter;
 inherited Create;
end;

procedure Tm3Flusher.Execute;
  //override;
  {-}
var
  BytesWritten : Integer;
begin
 with f_Sorter do begin
  sorCreateNewMergeFile(sorOutFile);
  try
    BytesWritten := FileWrite(sorOutFile, f_WriteBuffer^, f_BytesToWrite);
    if (BytesWritten <> f_BytesToWrite) then
      RaiseError(stscFileWrite);
  finally
    {Close merge file}
    FileClose(sorOutFile);
...
procedure TStSorter.FlushNew;
  {-}
begin
 {$IfDef m3StoreNewInThread}
 l3Swap(f_WriteBuffer, f_CurrentBuffer);
 // - здесь надо позаботиться о синхронизации
 if (f_Flusher = nil) then
  f_Flusher := Tm3Flusher.Create(Self, sorRunCount*LongInt(FRecLen))
 else
  Tm3Flusher(f_Flusher).f_BytesToWrite := sorRunCount*LongInt(FRecLen);
 f_Flusher.Suspended := false; 
 {$Else  m3StoreNewInThread}
 sorStoreNewMergeFile;
 {$EndIf m3StoreNewInThread}
end;
...
var
  ...
  begin
    LPtr := ElementPtr(L);
    RPtr := ElementPtr(R);
    l3Move(LPtr^, SwapPtr^, RecLen);
    l3Move(RPtr^, LPtr^, RecLen);
    l3Move(SwapPtr^, RPtr^, RecLen);
  end;
    ...
    try
      ...
      repeat
        ...
        repeat
          ...
          {Save the pivot element}
          l3Move(ElementPtr(Pm)^, PivotPtr^, RecLen);
...
// start class Tm3Sorter

constructor Tm3Sorter.Create(aItemSize             : LongInt;
                             const aSorterComparer : Im3SorterComparer);
  {-}
var
 l_Total : Int64;  
begin
 l_Total := m2MEMPhusAvail;
 if (l_Total > High(LongInt)) then
  l_Total := High(LongInt);
 inherited Create(l_Total * 2 div 3, aItemSize);
 Compare := aSorterComparer;
end;

class function Tm3Sorter.MakeInterface(AItemSize             : LongInt;
                                       const ASorterComparer : Im3SorterComparer): Im3Sorter;
  {-}
var
 l_Sorter : Tm3Sorter;
begin
 l_Sorter := Tm3Sorter.Create(aItemSize, aSorterComparer);
 try
  Result := l_Sorter;
 finally
  l3Free(l_Sorter);
...
function Tm3Sorter.GetCount: Int64;
  {-}
begin
 Result := Count;
end;

function Tm3Sorter.GetItemSize: LongInt;
  {-}
begin
 Result := FRecLen;
end;
