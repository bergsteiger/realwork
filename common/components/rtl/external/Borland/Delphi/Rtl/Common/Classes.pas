...
{$Define ReadPropErrorToLog}
{$IfDef ReadPropErrorToLog} {V}
type
 TDefReaderError = procedure(Reader: TReader; const Message: string; var Handled: Boolean);
const
 gDefReaderErrorHandler : TDefReaderError = nil;
{$EndIf ReadPropErrorToLog}
...
{$IFDEF MSWINDOWS}
uses RTLConsts, SysConst, Types, StrUtils;
{$ENDIF}
{$IFDEF LINUX}
...
begin
  inherited Destroy;
  FreeAndNil(FClassList); {V}
  FreeAndNil(FAliasList); {V}
  FreeAndNil(FGroupClasses); {V}
end;
...
begin
  ...
  {$IfDef ReadPropErrorToLog}  {V}
   ...
   if Assigned(gDefReaderErrorHandler) then
    gDefReaderErrorHandler(Self, Message, Result)
  {$EndIf ReadPropErrorToLog}
  ;
end;
...
var
  ...
  ThreadCount: Integer;
// FIX - Защита MakeObjectInstance/FreeObjectInstance
  InstanceLock: TRTLCriticalSection;
// FIX END
...
begin
  Value.FAction := Self;
  Assert(FClients.IndexOf(Value) < 0); {V}
  FClients.Add(Value);
end;
...
begin
// FIX - Защита MakeObjectInstance/FreeObjectInstance
 EnterCriticalSection(InstanceLock);
 try
// FIX END
...
// FIX - Защита MakeObjectInstance/FreeObjectInstance
  ...
  LeaveCriticalSection(InstanceLock);
 end;
// FIX END
end;
...
// FIX - Нельзя было вызывать DeallocateHWnd в потоке отличном от того где оно создавалось
//       Не отрабатывала DestroyWindow => оставалось окно с методом убитого объекта
//       в качестве WndProc
{procedure DeallocateHWnd(Wnd: HWND);
var
  Instance: Pointer;
...
function l_DestroyWindowProc(hWnd: HWND; Msg: UINT; wParam: WPARAM; lParam: LPARAM): LRESULT; stdcall;
begin
...
begin
  Instance := Pointer(GetWindowLong(Wnd, GWL_WNDPROC));
  if GetCurrentThreadID = GetWindowThreadProcessId(Wnd, nil) then
  begin
   DestroyWindow(Wnd);
   if Instance <> @DefWindowProc then FreeObjectInstance(Instance);
  end
  ...
  begin
// Дополнительная функция потребовалась т.к. DefWindowProc в ответ на WM_CLOSE
// не делает DestroyWindow, хотя по MSDN должна. Т.е. устраняем утечку окон   //
   SetWindowLong(Wnd, GWL_WNDPROC, Longint(@l_DestroyWindowProc));
   if Instance <> @DefWindowProc then FreeObjectInstance(Instance);
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Rtl\Common\Classes.pas initialization enter'); {$EndIf}
// FIX - Защита MakeObjectInstance/FreeObjectInstance
...
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Rtl\Common\Classes.pas initialization leave'); {$EndIf}
finalization
