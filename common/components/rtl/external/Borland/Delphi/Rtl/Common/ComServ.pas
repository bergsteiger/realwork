...
begin
  ...
  begin
    ...
    //
    if (Result = '') then // migel: In Windows7 function may return for path like
     Result := FileName;  // "Documents and Settings" empty Result, don't use second
                          //  part "for Windows95", it works not fine for hard links.
  end
  else
  begin
    Result := '';
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Rtl\Common\ComServ.pas initialization enter'); {$EndIf}
begin
...
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Rtl\Common\ComServ.pas initialization leave'); {$EndIf}
finalization
begin
  ComServer.Free;
  ComServer := nil;
