...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Rtl\Sys\system.pas initialization enter'); {$EndIf}
{$IFDEF LINUX}
  _setlocale(LC_ALL, '');
{$ENDIF}
...
{$IFDEF MSWINDOWS}
  ...
  begin
    ...
    DefaultUserCodePage := LCIDToCodePage(GetThreadLocale);
  { BEGIN Garant Морозов. М.А. "Если по умолчанию CodePage <> Russian =>
                                при чтении из dfm unicode не правильно
                                происходит перекодировка". }
   if DefaultUserCodePage <> 1251 then
    DefaultUserCodePage := 1251;
  { END Garant Морозов. М.А. }
{$ENDIF}
  MainThreadID := GetCurrentThreadID;

{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Rtl\Sys\system.pas initialization leave'); {$EndIf}
finalization
  Close(Input);
  Close(Output);
  Close(ErrOutput);
