...
(*   for I := 0 to FClients.Count - 1 do
   ...
   // - не надо это всё, там другая проблема - с медалями
  end;//Value <> FVisible
end;
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\actnlist.pas initialization enter'); {$EndIf}
  GroupDescendentsWith(TCustomActionList, TControl);
  GroupDescendentsWith(TContainedAction, TControl);
  
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\actnlist.pas initialization leave'); {$EndIf}
end.
