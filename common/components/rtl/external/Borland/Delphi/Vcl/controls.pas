...
{$ENDIF}
  ...
  protected
    procedure ActionRefChanged;
    function IsActionLinked: Boolean; dynamic;
    ...
    function HintChanged(const aHint: string):boolean ; virtual;
...
{ TWinControl }
  ...
  protected
    ...
    procedure SelectNext(CurControl: TWinControl;
      GoForward, CheckTabStop: Boolean); virtual;
  ...
  public
    ...
    {$IfNDef DesignTimeLibrary}
    procedure ForceWMSize(AWidth, AHeight: Integer); virtual;
...
begin
  ...
  if (Msg <> CM_APPKEYDOWN) and (Application.Handle <> 0) then {V}
  begin
   l_Message.Msg := Msg;
...
begin
  ...
  FreeAndNil(FFont); {V} // - эти клоуны в своем репертуаре
  StrDispose(FText);
  FText := nil; {V}
  inherited Destroy;
end;
...
begin
  ...
  If GetKeyState(VK_MENU) < 0 then exit; {V}
...
begin
  if HintChanged(Value) then
    FHint := Value;
end;

function TControl.HintChanged(const aHint: string): boolean;
begin
  Result := true;
end;

procedure TControl.ActionRefChanged;
begin
 if IsActionLinked then
  Include(FControlStyle, csActionClient)
 else
  Exclude(FControlStyle, csActionClient)
end;

function TControl.IsActionLinked: Boolean;
begin
...
{$IfNDef DesignTimeLibrary}
 {$IfDef Nemesis}
 function NeedSkip235864089 : Boolean;
 // http://mdp.garant.ru/pages/viewpage.action?pageId=235864089
 {$WriteableConst On}
 const
  l_Inited : Boolean = false;
  l_Skip : Boolean = false;
 begin//NeedSkip235864089
  if not l_Inited then
  begin
   l_Inited := true;
   l_Skip := FindCmdLineSwitch('FixTC6bug', ['-', '/', '\'], False);
  end;//not l_Inited
  Result := l_Skip;
 end;//NeedSkip235864089
 {$EndIf Nemesis}
{$EndIf DesignTimeLibrary}
...
begin
 ...
 {$ENDIF nsTest}
    try
      {$IfNDef DesignTimeLibrary}
      {$IfDef Nemesis}
      if (Message.Msg = 80) AND NeedSkip235864089 then
       Exit;
      {$EndIf Nemesis}
      {$EndIf DesignTimeLibrary}
      WindowProc(Message);
...
begin
  with Message do
    ...
    begin
      ...
      // Перенесено в DoDragMsg - при ручном докинге вызывать перенос фокуса
      // совсем необязательно - а иногда совсем ненужно
      //Form := GetParentForm(Self);
      //if (Form <> nil) and Form.Visible then Form.BringToFront; {V}
      Result := 1;
    end;
end;
...
begin
  ...
  begin
   // - эти клавиши в одиночку не могут быть ShortCut'ами - значит нефига их пытаться транслировать в
   //   ShortCut.
   Result := false;
   Exit;
...
{$IfNDef DesignTimeLibrary}
procedure TWinControl.ForceWMSize(AWidth, AHeight: Integer); //virtual;
begin
 Perform(WM_Size, 0, LParam(LoWord(aWidth) + (LoWord(AHeight) shl 16)));
end;
{$EndIf  DesignTimeLibrary}
  ...
  begin
    ...
    begin
     ...
     l_NeedsHack := {false}
                    ...
                    // на самом деле закомментировал хакерство в TvtCustomPanel.WMSize
                    // а тут - оставил
                    (*l3NeedsHackFor64System;*)
                    false; // - отключаем в связи с использованием багфикса от
                           // Embarcadero (ControlResizeBugFix.pas)
                           // http://mdp.garant.ru/pages/viewpage.action?pageId=494505726
     f_WasWMSize := not ((AWidth <> FWidth) or (AHeight <> FHeight));
     {$EndIf}
      SetWindowPos(FHandle, 0, ALeft, ATop, AWidth, AHeight,
        SWP_NOZORDER + SWP_NOACTIVATE);
     {$IfNDef DesignTimeLibrary}
     if l_NeedsHack then
     // http://mdp.garant.ru/pages/viewpage.action?pageId=229214269
      if not f_WasWMSize then
      begin
       f_WasWMSize := true;
       ForceWMSize(aWidth, aHeight);
       f_WasWMSize := true;
      end;//not f_WasWMSize
     {$EndIf}
    end
...
function TWinControl.IsAcceptableControlForTabNavigation(aControl: TWinControl): Boolean;
begin
 Result := true;
end;
    ...
    begin
      ...
      repeat
        ...
        if CurControl.CanFocus and
          ...
          (not CheckParent or (CurControl.Parent = Self)) AND
          IsAcceptableControlForTabNavigation(CurControl) then
          Result := CurControl;
...
begin
  ...
  try
    ...
    SetWindowPos(Handle, HWND_TOPMOST, Rect.Left, Rect.Top, Width, Height,
      SWP_NOACTIVATE);
    Caption := AHint; {V} //MSBUG (r) workarround: SetWindowPos убивает Caption начинающийся с 'я'
    ...
    //ParentWindow := Application.Handle; {V}
    // данная строчка появилась только в D7 RTL - hint'ы вытаскивают
    // окно application'a на передний план, что вредно сказывается
    // на popup'ах. За что и убита.
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\controls.pas initialization enter'); {$EndIf}
...
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\controls.pas initialization leave'); {$EndIf}
finalization
