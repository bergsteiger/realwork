...
{ THeader
  ...
  TIsHandledMessage = function (var Msg: TWMKeyDown): Boolean; {naa_}
...
var
 g_IsHandledShortcut : TIsHandledMessage; {naa_}
...
type
  ...
  protected
    procedure WMKeyDown(var Msg: TWMKeyDown); message WM_KEYDOWN; {naa_}
    procedure WMSysKeyDown(var Msg: TWMSysKeyDown); message WM_SYSKEYDOWN; {naa_}
    procedure KeyDown(var Key: Word; Shift: TShiftState); override;
    procedure KeyPress(var Key: Char); override;
  public
...
procedure TGroupButton.WMKeyDown(Var Msg: TWMKeyDown); {naa_}
begin
 if Assigned(g_IsHandledShortcut) then
  if g_IsHandledShortcut(Msg) then
   Exit;
 inherited;
end;

procedure TGroupButton.WMSysKeyDown(var Msg: TWMSysKeyDown); {naa_}
begin
 if Assigned(g_IsHandledShortcut) then
  if g_IsHandledShortcut(Msg) then
   Exit;
...
begin
  ...
  begin
    case Align of
      //alLeft: FControl.Width := FNewSize; {V}
      alLeft: {V}
        begin
          Parent.DisableAlign;
          try
      ...
      //alTop: FControl.Height := FNewSize; {V}
      alTop: {V}
        begin
          Parent.DisableAlign;
          try
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\extctrls.pas initialization enter'); {$EndIf}
  StartClassGroup(TControl);
  GroupDescendentsWith(TTimer, Controls.TControl);

{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\extctrls.pas initialization leave'); {$EndIf}
finalization
