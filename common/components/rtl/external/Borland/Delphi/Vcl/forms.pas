...
{ TCustomForm }
  ...
  private
    ...
    procedure SetWindowStatePrim(Value: TWindowState); {V}
    ...
    procedure WMSize(var Message: TWMSize); message WM_SIZE; {V}
...
protected
  ...
  public
    ...
    function IsRealInstance: Boolean; {V}
      virtual;
      {-}
    function NeedAutoScroll: Boolean; {V}
      virtual;
      {-}
    procedure Close;
    ...
    procedure Show;
    function NeedCancelModal: Boolean; virtual;
...
{ TApplication }
  ...
  public
    procedure DoProcessMessage(var Msg: TMsg); {v}
  private
  ...
  public
    ...
    procedure HandleMessage; overload;  {V}
    procedure HandleMessage(var Msg: TMsg); overload;  {V}
    function CallProcessMessage(var Msg: TMsg): Boolean; {V}
    procedure CallActionIdle; {V}
...
begin
  ...
  Result := AnsiSameText(Char(VK), GetHotKey(Str)); {V}
end;
...
begin
  ...
  if (Kind = sbVertical) and (FControl.ClientHeight <= 0) then
    ScrollInfo.nMax := 0;    //http://mdp.garant.ru/pages/viewpage.action?pageId=297699119
...
begin
  ...
  try
    ...
    begin
      ...
      If (BorderStyle in [bsSizeable, bsSizeToolWin])        {V}
         and (FPosition in [poDefault, poDefaultSizeOnly])   {V}
         and IsRealInstance                                  {V}
       then RecreateWnd;                                     {V}
...
begin
  ...
  try
    ...
    if (FPixelsPerInch <> 0) and (FTextHeight > 0)
       and IsRealInstance then {V}
    begin
...
begin
  ...
  begin
    if HandleAllocated and (BorderStyle <> bsDialog) then
      SendMessage(Handle, WM_SETICON, 1, LPARAM(GetIconHandle)); {V}
  end else
    ...
    case Msg of
      ...
      begin
       for I := 0 to Pred(MDIChildCount) do
        MDIChildren[I].SetWindowStatePrim(wsNormal);
      end;
...
begin
// FIX (cq26839) - refactoring
  Result := Screen.FindMonitor(MonitorFromWindow(Handle, MONITOR_DEFAULTTONEAREST));
(*
  Result := nil;
...
procedure TCustomForm.SetWindowStatePrim(Value: TWindowState); {V}
const
  ShowCommands: array[TWindowState] of Integer =
    (SW_SHOWNORMAL, SW_MINIMIZE, SW_SHOWMAXIMIZED);
var
 lNeedRealign : boolean;
begin
 if FWindowState <> Value then
 begin
  lNeedRealign := (FormStyle = fsMDIChild) and (FWindowState = wsMaximized) and (Value = wsNormal);
  fWindowState := Value;
  if not (csDesigning in ComponentState) and Showing then
   ShowWindow(Handle, ShowCommands[Value]);
  if lNeedRealign then
   Realign;
 end;
end;
...
begin
 if (FormStyle = fsMDIChild) and (FWindowState = wsMaximized) and (Value = wsNormal) then
  SendMessage(Application.MainForm.ClientHandle, WM_MDIRESTORE, Handle, 0)
 else
  SetWindowStatePrim(Value);
end;
  ...
  begin
    ...
    begin
      ...
      case CreateStyle of
        ...
        bsSizeable, bsSizeToolWin:
          begin
            ...
            if (FPosition in [poDefault, poDefaultSizeOnly])
               and not (fsCreating in FormState) AND
               IsRealInstance then         {V}
            begin
      ...
      begin
        ...
        //if (FormStyle = fsMDIChild) then      {V}
        // Style := Style and not WS_MINIMIZEBOX; {V}
...
begin
  ...
  if NewStyleControls then
    if BorderStyle <> bsDialog then
      SendMessage(Handle, WM_SETICON, 1, LPARAM(GetIconHandle)) else {V}
...
begin
  ...
  if FormStyle = fsMDIForm then     {V}
  begin
    ActForm:=GetActiveMDIChild;
    If Assigned(ActForm) then ActForm.FActive := False;
  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if NeedAutoScroll then {V}
        begin
         TempControl := Control.Parent;
         while TempControl <> nil do
         begin
           if TempControl is TScrollingWinControl then
            if not (TempControl Is TCustomForm) OR TCustomForm(TempControl).NeedAutoScroll then {V}
...
begin
  ...
  begin
    ...
    begin
     ...
     if (Message.Result = MNC_IGNORE) then
      // - <K>: 135598608
      Message.Result := MNC_CLOSE shl 16;
    end;//Message.Result = MNC_IGNORE
  end
  else
    inherited;
end;
...
begin
  // C учетом вот этого http://qc.embarcadero.com/wc/qcmain.aspx?d=71595
  // и вот этого http://mdp.garant.ru/pages/viewpage.action?pageId=570121096
  // код ниже взят из VCL.Forms.pas Delphi XE4
  if not (csReading in ComponentState) then
  begin
    LMinMaxInfo := Message.MinMaxInfo;
    with LMinMaxInfo^, Constraints do
    begin
...
begin
  if not (csDesigning in ComponentState) and (fsShowing in FFormState) then
   Exit; //raise EInvalidOperation.Create(SVisibleChanged);
...
function TCustomForm.IsRealInstance: Boolean; {V}
  //virtual;
  {-}
begin
 Result := true;
...
function TCustomForm.NeedAutoScroll: Boolean; {V}
  //virtual;
  {-}
begin
 Result := true;
...
function TCustomForm.NeedCancelModal: Boolean;
begin
 Result := False;
end;
...
{$ENDIF}
    ...
    if NewStyleControls then
    begin
      SendMessage(FHandle, WM_SETICON, 1, LPARAM(GetIconHandle)); {V}
...
begin
  ...
  with Msg do
    ...
    begin
      ...
      begin
       if (LongWord(GetWindowLong(Wnd, GWL_HINSTANCE)) = HInstance)
       {$IfDef Nemesis}
          or (LongWord(GetWindowLong(GetParent(Wnd), GWL_HINSTANCE)) = HInstance)
       {$EndIf Nemesis} then
          // - Такое тоже может быть http://mdp.garant.ru/pages/viewpage.action?pageId=591134966
         if SendMessage(Wnd, CN_BASE + Message, WParam, LParam) <> 0 then
           Result := True;
      end;
    end;
...
procedure TApplication.DoProcessMessage(var Msg: TMsg); {v}
var
  Handled: Boolean;
begin
...
procedure TApplication.HandleMessage(var Msg: TMsg);  {V}
begin
  if not ProcessMessage(Msg) then Idle(Msg);
end;

function TApplication.CallProcessMessage(var Msg: TMsg): Boolean; {V}
begin
 Result := ProcessMessage(Msg);
end;

procedure TApplication.CallActionIdle; {V}
begin
 DoActionIdle;
end;
...
// {V} {
  function GetOwnerWindow(ActiveWindow: HWnd): HWnd;
  begin
    if (GetWindowLong(Handle, GWL_EXSTYLE) and not(WS_EX_TOOLWINDOW) <> 0) then
      Result := ActiveWindow
    else
      Result := Handle;
  end;
...
begin
  ...
  try
    Result := Windows.MessageBox({Handle} GetOwnerWindow(ActiveWindow), Text, Caption, Flags); {V}
  finally
    if MBMonitor <> AppMonitor then
      SetWindowPos(Handle, 0,
  ...
  for I := 0 to Screen.CustomFormCount - 1 do
    with Screen.CustomForms[I] do
      if HandleAllocated and IsWindowVisible(Handle) and
        ...
        UpdateActions;
  {$IfDef Nemesis}
  Sleep(1); // http://mdp.garant.ru/pages/viewpage.action?pageId=523305273
  {$EndIf}
end;
...
begin
  ...
  if FShowHint and (FHintControl <> nil) and ForegroundTask and
    (FHintControl = GetHintControl(FindDragTarget(CursorPos, True)))
    and (FHintControl <> nil) {V - это не бред, в GetHintControl могут FHintControl занулить} then
  begin
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\forms.pas initialization enter'); {$EndIf}
  InitProcs;
  RM_TaskBarCreated := RegisterWindowMessage('TaskbarCreated');
  Classes.RegisterFindGlobalComponentProc(FindGlobalComponent);

{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\forms.pas initialization leave'); {$EndIf}
finalization
