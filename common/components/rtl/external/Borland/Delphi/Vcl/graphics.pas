...
function SignColor(aColor: TColor): TColor;
  {-}
{$EndIf nsSign1}
...
procedure OutStat;
const
 N : array [0..2] of String = ('R', 'G', 'B');
var
 F    : Text;
 i, j : Integer;
begin
 AssignFile(F, ChangeFileExt(ParamStr(0), '.stat'));
 Rewrite(F);
 for i := 0 to 2 do
  for j := 0 to 2 do
   WriteLn(F, N[i], ' ', j, ' ', g_Stat[i, j]);
 CloseFile(F);
end;
*)
...
begin
 ...
 begin
  C[i] := (aColor shr (i * 8)) AND $FF;
  dC[i] := (g_uid shr (i * 3)) AND 7;
 end;//for i
 for i := Low(C) to High(C) do
 begin
  k := 0;
  ...
  for j := 0 to 7 do
  begin
   if (C[i] < x + 32) OR (j = 7) then
   begin
    if (C[i] >= High(Byte) - cDelta) then
    begin
     if (k = 1) then
      k := 2;
     if Test(dC[i] shr k) then
      C[i] := High(Byte)
     else
      C[i] := High(Byte) - cDelta;
    end
    else
    begin
     if Test(dC[i] shr k) then
      ...
      C[i] := x + cDelta;
    end;//C[i] <= High(Byte) - cDelta
    if (g_Stat[i, k] < High(g_Stat[i, k])) then
     Inc(g_Stat[i, k]);
    break;
   end;//C[i] < 1 shl j
   Inc(x, 32);
...
begin
  ...
  {$IfDef nsSign1}
  if {(Pen.Style = bsSolid) AND }not f_SignPen then
  begin
   f_SignPen := true;
   try
    Pen.Color := SignColor(Pen.Color);
   finally
    f_SignPen := false;
   end;//try..finally
  end;//Brush.Style = bsSolid
  {$EndIf nsSign1}
end;
...
begin
  ...
  {$IfDef nsSign1}
  if (Brush.Style = bsSolid) AND not f_SignBrush then
  begin
   f_SignBrush := true;
   try
    Brush.Color := SignColor(Brush.Color);
   finally
    f_SignBrush := false;
   end;//try..finally
  end;//Brush.Style = bsSolid
...
begin
  ...
  l_GotHeader := l3IsValidMetafileHeader(WMF);
  if l_GotHeader then
   Dec(Length, SizeOf(WMF))
  else
   Stream.Seek(-SizeOf(WMF), soFromCurrent);
  ...
  try
    ...
    if l_GotHeader then
    begin
     FWidth := MulDiv(WMF.Box.Right - WMF.Box.Left, HundredthMMPerInch, FImage.FInch);
     FHeight := MulDiv(WMF.Box.Bottom - WMF.Box.Top, HundredthMMPerInch, FImage.FInch);
    end;
    ...
    GetEnhMetaFileHeader(FHandle, Sizeof(EMFHeader), @EMFHeader);
    if not l_GotHeader then
    begin
     FWidth := MulDiv(EMFHeader.rclFrame.Right-EMFHeader.rclFrame.Left, HundredthMMPerInch, FImage.FInch*100);
     FHeight := MulDiv(EMFHeader.rclFrame.Bottom-EMFHeader.rclFrame.Top, HundredthMMPerInch, FImage.FInch*100);
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      CheckSum := l3ComputeAldusChecksum(WMF);
    end;
    RefDC := GetDC(0);
    try
      ...
      begin
        ...
        try
          ...
          TransparentStretchBlt(ACanvas.FHandle, Left, Top, Right - Left,
            ...
            FDIB.dsbm.bmHeight, MaskDC, 0, 0, ACanvas.CopyMode); {V}
        finally
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\graphics.pas initialization enter'); {$EndIf}
  InitScreenLogPixels;
...
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\graphics.pas initialization leave'); {$EndIf}
finalization
  {$IfDef nsSign1}
(*
  OutStat;
