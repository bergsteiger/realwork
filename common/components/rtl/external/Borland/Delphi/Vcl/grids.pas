...
type
  ...
  TGridOption = (goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine,
    ...
    goAllRowSizing, goAllColSizing, goInnerSizing,
    // goAllRowSizing, goAllColSizing - возможность ресайзить не только заголовок
    // goInnerResizing - ресайз происходит только между двумя колонками (строками).
    // Суммарный их размер при этом не меняется
...
procedure TCustomGrid.CalcSizingState(X, Y: Integer; var State: TGridState;
  ...
  procedure CalcAxisState(const AxisInfo: TGridAxisDrawInfo; Pos: Integer;
    NewState: TGridState; IsBorderSpan: boolean = false);
  var
    I, Line, Back, Range, BorderSpan: Integer;
  begin
    if IsBorderSpan then BorderSpan := 1
    else BorderSpan := 0;
    ...
    begin
      ...
      for I := FirstGridCell to GridCellCount - 1 - BorderSpan do
      begin
...
begin
  ...
  if [goColSizing, goRowSizing, goAllColSizing, goAllRowSizing] * EffectiveOptions <> [] then
    with FixedInfo do
    begin
      Vert.GridExtent := ClientHeight;
      Horz.GridExtent := ClientWidth;
      if (goAllColSizing in EffectiveOptions) or (goAllRowSizing in EffectiveOptions) then
      begin
        if (goAllColSizing in EffectiveOptions) and (Y >= Vert.GridBoundary) then Exit;
        if (goAllRowSizing in EffectiveOptions) and (X >= Horz.GridBoundary) then Exit;
        if (goAllColSizing in EffectiveOptions) then
          CalcAxisState(Horz, X, gsColSizing, goInnerSizing in EffectiveOptions);
        if (State <> gsColSizing) and (goAllRowSizing in EffectiveOptions) then
          CalcAxisState(Vert, Y, gsRowSizing, goInnerSizing in EffectiveOptions);
      end
      else
      begin
...
var
  ...
  begin
    ...
    begin
      Result := 0;
      if (goInnerSizing in Options) and (FSizingIndex = AxisInfo.GridCellCount - 1) then Exit;
      Result := FixedBoundary;
      for I := FirstGridCell to FSizingIndex - 1 do
        Inc(Result, GetExtent(I) + EffectiveLineWidth);
      Result := FSizingPos - Result;
      if (goInnerSizing in Options) and
         (Result > GetExtent(FSizingIndex) + GetExtent(FSizingIndex + 1) - 2*EffectiveLineWidth) then
        Result := 0;
    end;
  end;
      ...
      gsRowSizing, gsColSizing:
        begin
          ...
          begin
            ...
            if NewSize > 1 then
            begin
              if goInnerSizing in Options then
                ColWidths[FSizingIndex + 1] := ColWidths[FSizingIndex + 1] - NewSize + ColWidths[FSizingIndex];
          ...
          begin
            ...
            begin
              if goInnerSizing in Options then
                RowHeights[FSizingIndex + 1] := RowHeights[FSizingIndex + 1] - NewSize + RowHeights[FSizingIndex];
