...
{$IFDEF LINUX}
uses WinUtils, Windows, SysUtils, Classes, Contnrs, Messages, Graphics, ImgList, ActnList, Consts;
{$ENDIF}
{$IFDEF MSWINDOWS}
uses Windows, SysUtils, Classes, Contnrs, Messages, Graphics, ImgList, ActnList, Consts;
{$ENDIF}
...
{ TMenuItem }
  ...
  public
    ...
    property DrawGraphicChecks: boolean read FDrawGraphicChecks write FDrawGraphicChecks stored false;
  ...
  public
    ...
    procedure ForceRebuildHandle; {V}
  published
...
type
  TMenuKeyCap = (mkcBkSp, mkcTab, mkcEsc, mkcEnter, mkcSpace, mkcPgUp,
    ...
    mkcDel, mkcShift, mkcCtrl, mkcAlt, mkcApps {migel: VK_APPS support});

var
  MenuKeyCaps: array[TMenuKeyCap] of string = (
    SmkcBkSp, SmkcTab, SmkcEsc, SmkcEnter, SmkcSpace, SmkcPgUp,
    SmkcPgDn, SmkcEnd, SmkcHome, SmkcLeft, SmkcUp, SmkcRight,
    SmkcDown, SmkcIns, SmkcDel, SmkcShift, SmkcCtrl, SmkcAlt, SmkcApps {migel: VK_APPS support});
...
// Костицын: перенес в interface для kwHardKey
//type
//  TMenuKeyCap = (mkcBkSp, mkcTab, mkcEsc, mkcEnter, mkcSpace, mkcPgUp,
//    mkcPgDn, mkcEnd, mkcHome, mkcLeft, mkcUp, mkcRight, mkcDown, mkcIns,
//    mkcDel, mkcShift, mkcCtrl, mkcAlt, mkcApps {migel: VK_APPS support});
...
begin
  case WordRec(ShortCut).Lo of
    ...
    $5D: Name := MenuKeyCaps[mkcApps]; {migel: VK_APPS support}
    $60..$69: Name := 'NumPad' + Chr(WordRec(ShortCut).Lo - $60 + Ord('0'));  {V}
  ...
  begin
    Result := '';
    // V. М. Морозов. Изменен порядок следования модификаторов.
...
function TranslateMenuItemInfo(AMenuItemInfo: TMenuItemInfo): TMenuItemInfoW;
begin
 with AMenuItemInfo do
 begin
  ...
  Result.wID := wID;
  Result.hSubMenu := hSubMenu;
  Result.hbmpChecked := hbmpChecked;
  Result.hbmpUnchecked := hbmpUnchecked;
  Result.dwItemData := dwItemData;
  Result.dwTypeData := PWideChar(AnsiToWide(dwTypeData));
  Result.cch := cch;
  Result.hbmpItem := hbmpItem;
 end;
end;
{$EndIf}
...
var
  ...
  IsOwnerDraw: Boolean;
  ParentMenu: TMenu;
  l_MenuItemInfoW: TMenuItemInfoW;
begin
  if FVisible then
  begin
    Caption := FCaption;
...
//      IsOwnerDraw := Assigned(ParentMenu) and ParentMenu.IsOwnerDraw or
      ...
      if GetCount > 0 then
        MenuItemInfo.hSubMenu := GetHandle;
      {$IfDef Nemesis}
      l_MenuItemInfoW := TranslateMenuItemInfo(MenuItemInfo);
      InsertMenuItemW(Menu, DWORD(-1), True, l_MenuItemInfoW);
      {$Else Nemesis}
    ...
    begin
      ...
      if GetCount > 0 then
        {$IfDef Nemesis}
        InsertMenuW(Menu, DWORD(-1), MF_POPUP or NewFlags, GetHandle,
          PWideChar(AnsiToWide(FCaption)))
        {$Else Nemesis}
        ...
        {$IfDef Nemesis}
        InsertMenuW(Menu, DWORD(-1), NewFlags, Command, PWideChar(AnsiToWide(Caption)));
        {$Else Nemesis}
...
begin
  ...
  begin
    ...
    begin
      ...
      while I > 0 do
      begin
        //if GetMenuState(Handle, I - 1, MF_BYPOSITION) and MF_SYSMENU{MF_BITMAP} {V} = 0 then {V}
        begin
...
const
 clBtnFaceHack = {$IfDef Nemesis}$00F9F8FA{$Else}clBtnFace{$EndIf};

{$IfDef Nemesis}
procedure TMenuItem.HandleChanged;
  // virtual;
var
 l_MenuInfo : TMenuInfo;
begin
 if f_BackgroundBrush = nil then
 begin
  f_BackgroundBrush := TBrush.Create;
  f_BackgroundBrush.Color := clBtnFaceHack;
 end;
 l_MenuInfo.cbSize := SizeOf(TMenuInfo);
 l_MenuInfo.fMask := MIM_BACKGROUND or MIM_APPLYTOSUBMENUS;
 l_MenuInfo.hbrBack := f_BackgroundBrush.Handle;
 SetMenuInfo(Self.Handle, l_MenuInfo);
end;
{$EndIf Nemesis}
...
begin
  ...
  with ACanvas do
  begin
    Font.Charset := RUSSIAN_CHARSET; // GARANT
...
{$IfDef NemesisMenuStyle}
  cMenuBackground = $00E1B293;
  cBorderColor = $00FF8888;
{$EndIf NemesisMenuStyle}
...
{$EndIf NemesisMenuStyle}
  ...
  procedure DrawBrush(const ABrushRect: TRect);
  begin
    with ACanvas do
    begin
      ...
      begin
        OldBrushColor := Brush.Color;
        Brush.Bitmap := AllocPatternBitmap(clBtnFaceHack, clBtnHighlight);
        FillRect(ABrushRect);
      end
      else
      begin
        Brush.Color := clBtnFaceHack;
        FillRect(ABrushRect);
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          {$Else}
          Brush.Color := $00F9F8FA;
(*          if UseThemes and IsAppThemed then
           Brush.Color := clMenuBar
          else
           Brush.Color := clMenu;*)
          {$EndIf Nemesis}
      ...
      begin
        ...
        begin
          ...
          begin
            l_Menu := GetParentMenu;
            if (l_Menu <> nil) and l_Menu.DrawGraphicChecks and WinXP then
            begin
              if Checked and (ImageIndex > -1) and (ImageIndex < ImageList.Count) then
              begin
                l_Rect := GlyphRect;
                l_Rect.Right := l_Rect.Right+2;
                l_Rect.Bottom := l_Rect.Bottom+2;
                DrawFrameControl(ACanvas.Handle, l_Rect, DFC_BUTTON, DFCS_PUSHED or DFCS_BUTTONPUSH {or DFCS_FLAT});
            ...
            else if Checked {and not WinXP} then
          ...
          begin
           {$IfDef NemesisMenuStyle}
            if Checked then
            begin
             Brush.Color := cMenuBackground;
             Pen.Color := cBorderColor;
             l_Side := ARect.Bottom - ARect.Top;
             l_Rect := Rect(ARect.Left, ARect.Top, ARect.Left + l_Side, ARect.Top + l_Side);
             FillRect(l_Rect);
             Rectangle(l_Rect);
            end;
           {$EndIf NemesisMenuStyle}
...
begin
  ...
  {V - Borland не принимает во внимание, что элемент меню связан с Action. }
  if not Assigned(Result) and (ImageIndex >= 0) and
     Assigned(ActionLink) and
     (ActionLink.Action is TContainedAction) and
     (TContainedAction(ActionLink.Action).ActionList <> nil) then
   Result := TContainedAction(ActionLink.Action).ActionList.Images;
  {V}
end;
...
begin
  ...
  if (ParentMenu <> nil) and ParentMenu.DrawGraphicChecks and
...
begin
  //if Item.FParent <> nil then Error(@SMenuReinserted);  {V}
...
begin
 ...
 // - шаманство с I, т.к. в процессе InitiateAction может меняться состав меню
 I := 0;
  while (I < Count) do
  begin
...
{$IfNDef DesignTimeLibrary}
procedure TMenuItem.CallInitiateActions;
begin
...
begin
  //FItems.Free;
  FreeAndNil(FItems);
  FreeAndNil(FImageChangeLink){.Free};
  inherited Destroy;
end;
...
begin
  ...
  begin
    ...
    if Items.InternalRethinkLines(False) then //LAW
     Items.RebuildHandle;
  end;
end;
...
var
  ...
  function FindEnabledItemByShortCut(Value: Integer): TMenuItem; {V}
  var
   FoundItem: TMenuItem;
...
{$ENDIF}
    ...
    try
      repeat
        ...
        ShortCutItem := FindEnabledItemByShortCut(ShortCut); {V}
        //ShortCutItem := FindItem(ShortCut, fkShortCut);
        if ShortCutItem <> nil then
        begin
...
procedure TMainMenu.ForceRebuildHandle; {V}
 begin
  If FItems <> nil then FItems.RebuildHandle;
 end;
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\menus.pas initialization enter'); {$EndIf}
...
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\menus.pas initialization leave'); {$EndIf}
finalization
  FreeAndNil(ShortCutItems){.Free};
  FreeAndNil(PopupList){.Free};
  FreeAndNil(CommandPool){.Free};
