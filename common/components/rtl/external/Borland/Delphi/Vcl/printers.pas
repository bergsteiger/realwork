...
begin
  ...
  begin
    ...
    if Output <> '' then
      DocInfo.lpszOutput := PChar(Output);
  end;
...
begin
  ...
  try
   ...
   on EOverflow do;
   // http://mdp.garant.ru/pages/viewpage.action?pageId=96484293&focusedCommentId=108232728#comment-108232728
  end; 
  FPrinting := False;
  FAborted := False;
  FPageNumber := 0;
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\printers.pas initialization enter'); {$EndIf}

{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\external\Borland\Delphi\Vcl\printers.pas initialization leave'); {$EndIf}
finalization
  FreeAndNil(FPrinter);
end.
