...
type
  ...
  protected
    ...
    function GetSelLength: Integer; virtual; {V}
    function GetSelStart: Integer;  virtual; {V}
...
procedure TCustomEdit.SetSelLength(Value: Integer); {V} // - не работает на NT 4.0
var
...
begin
  ...
  {if Longint(Result) = CB_ERR then
    Error(SListIndexError, Index); } {V} //Нехера юзерские данные интерпретировать
end;
  ...
  case Ord(Key) of
    ...
    VK_TAB:
      if FAutoDropDown and DroppedDown then
        DroppedDown := False;
    VK_BACK:
     if (Style in [csDropDown, csSimple]) then {V}
      begin
        if HasSelectedText(StartPos, EndPos) then
          DeleteSelectedText
        else
          if {(Style in [csDropDown, csSimple]) and} {V} (Length(Text) > 0) then
          begin
           If (StartPos > 0) then   {V}
           begin
            SaveText := Text;
            LastByte := StartPos;
