...
uses Use32, Objects, Drivers, Memory, HistList, Views, Menus, Dialogs;
...
begin
  GetExtent(R);
  New(Background, Init(R, ldDesktopBackground));
end;
...
begin
  NumTileable := 0;
  ForEach(LocalAddr(@DoCountTileable));
...
{ TProgram }
type
  TFNHandlerRoutine = function(CtrlType: Longint): boolean; stdcall;

function ShutDownHandler(CtrlType: Longint): boolean; stdcall;
var
  QuitEvent: TEvent;
begin
  Result := false;
  if (CtrlType = CTRL_CLOSE_EVENT) or (CtrlType = CTRL_SHUTDOWN_EVENT) then
  begin
    with QuitEvent do
    begin
      What := evCommand;
...
function SetConsoleCtrlHandler(HandlerRoutine: TFNHandlerRoutine; Add: boolean): boolean; stdcall; external 'kernel32.dll' name 'SetConsoleCtrlHandler';
...
begin
  ...
  if StatusLine <> nil then
    if (Event.What and evKeyDown <> 0) or
      ...
      (FirstThat(LocalAddr(@ContainsMouse)) = PView(StatusLine)) then
...
begin
  ...
  DoneDosMem;
  WriteShellMsg;
  // Force update of cursor prior to shell
  SysGetCurPos(x,y);
  SysTVSetCurPos(x,y);
//  Exec(GetEnv('COMSPEC'), '');
  InitDosMem;
