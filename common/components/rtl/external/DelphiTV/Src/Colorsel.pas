...
uses Use32, Objects, Drivers, Views, Dialogs;
...
const
  ...
  cBackgroundBlink: Boolean = False;

{ True: 8 available/selectable background colors}
{ False:16 available/selectable colors}
...
begin
  ...
  begin
    if I < 4 then
      ...
      begin
        C := I * 4 + J;
        MoveChar(B[ J*3 ], ldBlockFull, C, 3);
...
begin
  ...
  case Event.What of
    ...
    evKeyDown:
      begin
        if (SelType = csBackground) And cBackgroundBlink then
          MaxCol := 7 else
          MaxCol := 15;
...
begin
  ...
  If cBackgroundBlink Then Dec(R.B.Y,2);
  ...
  If cBackgroundBlink Then Begin Dec(R.A.Y,2); Dec(R.B.Y,2); End;
...
initialization
finalization
  if assigned(ColorIndexes) then
    FreeMem(ColorIndexes);
end.
