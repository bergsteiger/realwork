...
uses Use32, Objects, Drivers, Views, Validate;
...
{ TDialog object }
  ...
  TDialog = object(TWindow)
    ...
    procedure SizeLimits(var Min, Max: TPoint); virtual; // Added for VPPM
  end;
...
procedure TDialog.SizeLimits(var Min, Max: TPoint);
begin
...
begin
  ...
  if CanScroll(1) then MoveChar(B[Size.X - 1], ldRightScroll, GetColor(4), 1);
  if State and sfFocused <> 0 then
  begin
    if CanScroll(-1) then MoveChar(B[0], ldLeftScroll, GetColor(4), 1);
    L := SelStart - FirstPos;
    R := SelEnd - FirstPos;
    if L < 0 then L := 0;
...
begin
  if (Validator = nil) or
    ...
    begin
      DSize := DataSize;
      Move(Rec, Data^[0], DSize);
      if Length(Data^) > DSize then Data^[0] := Chr(DSize);
    end;
  SelectAll(True);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if Y = 0 then
          WordRec(B[S]).Lo := Byte(ldBlockBottom) else
          WordRec(B[S]).Lo := Byte(ldBlockFull);
        Ch := ldBlockTop;
      end;
      I := 1;
    end;
...
var
  B: TDrawBuffer;
begin
  MoveCStr(B, ldHistoryDropDown, GetColor($0102));
  WriteLine(0, 0, Size.X, Size.Y, B);
end;
