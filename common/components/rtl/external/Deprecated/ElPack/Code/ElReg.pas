...
{$ifndef CLX_USED}
  ...
  {$R 'Design\TElSysHotKey.dcr'}
  {$R 'Design\TElTopMost.dcr'}
{$endif}
...
{$ifndef CLX_USED}
  ...
  // ?? duplicated {$R 'Design\ElTreeGrids.dcr'}
  {$R 'Design\ElURLLabel.dcr'}
  {$R 'Design\ElVerInfo.dcr'}
  {$R 'Design\ElAppBar.dcr'}
{$endif}
...
{$IFDEF ELPACK_COMPLETE}
{$ifdef ELPACK_USE_STYLEMANAGER}
  , ElStyleMan, ElStyleManProp
{$endif}
{$ifndef CLX_USED}
  , ElTreeAdvEdit, ElTreeBtnEdit, ElTreeCheckBoxEdit, ElTreeComboBox,
  ElTreeCurrEdit, ElTreeDTPickEdit, ElTreeMaskEdit, ElTreeColorCombo,
...
procedure Register;
begin
  RegisterComponents('ElPack', [{$ifdef ELPACK_USE_STYLEMANAGER}TElStyleManager,{$endif} TElHeader, TElTree, TElTreeStringGrid]);
...
{$IFDEF ELPACK_COMPLETE}
  ...
  TElTreeInplaceMemoEdit, TElTreeInplaceSpinEdit, TElTreeInplaceColorCombo,
  TElTreeInplaceFloatSpinEdit, TElTreeInplaceTreeComboEdit]);
{$ENDIF}
{$endif}

{$IFDEF ELPACK_COMPLETE}
  RegisterComponents('ElPack Edit', [TElDateTimePicker
    {$ifndef CLX_USED}
    , TElAdvancedEdit, TElAdvancedMemo, TElEdit, TElMemo, TElButtonEdit,
    TElSpinEdit, TElFloatSpinEdit, TElMaskEdit, TElCurrencyEdit, TElIPEdit, 
    TElFileNameEdit, TElFolderNameEdit, TElHotKey, TElCalculatorEdit
     {$endif}]);
  
  {$ifndef CLX_USED}
  RegisterComponents('ElPack List && Combo', [TElColorCombo,
  ...
  RegisterComponents('ElPack', [TElPageControl, TElScrollBar, TElPanel, TElCalendar, TElGroupBox,
    // TElCheckBox, <trash> // LAW
    ...
    {$ifndef CLX_USED}
    , TElTrackBar, TElSplitter, TElURLLabel, TElDragDrop,
    TElSpeedButton,  TElClock,
    // TElLabel, <trash> // LAW
    TElScrollBox, TElExplorerBar
    {$endif}
  ]);

  RegisterComponents('ElPack Tools', [TElIniFile, TElMRU, TElStringPool
    {$ifndef CLX_USED}
    , TElOneInstance, TElSysHotKey, TElTrayIcon, TElFormCaption, TElFormPersist,
    TElImageList, TElVersionInfo, TElSoundMap,
    TElMouseHint,
    TElDropTarget, TElHook, TElTimerPool,
    TElImageForm, TElTrayInfo,
    TElClipboardMonitor, {$ifdef ELPACK_USE_STYLEMANAGER}TElStyleController,{$endif} TElFlatController, TElFlatMultiController,
    TElPowerManager, TElTopmost, TElAppBar
    {$endif}
    ]);

  {$ifndef CLX_USED}
  RegisterComponents('ElPack Dialogs', [TElPromptDialog, TElFolderDialog,
    TElDailyTipDialog, TElCalendarDialog, TElInputDialog, TElCalculatorDialog]);
  {$endif}

  RegisterComponents('ElPack HTML', [TElHTMLHint
    {$ifndef CLX_USED}
...
{$endif}
  ...
  RegisterComponents('ElPack', [TElMainMenu, TElPopupMenu]);
