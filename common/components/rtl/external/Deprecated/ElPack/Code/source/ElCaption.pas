...
04/16/2003
  ...
  Ampersand (&) is now drawn correctly in caption text

12/15/2002

  Fixed design-time problem when Access Violation happened

11/22/2002

  Fixed the bug that happened in design-time when Active = true and Texts.Count = 0

10/22/2002

  When the text was longer than the space for it, it was overriding the buttons. Fixed.
...
begin
  {
  if (FFormCaption <> nil) then
    if OwnerStyle then
    begin
      with FFormCaption do
      begin
        if FSystemFont then
...
begin
  ...
  begin
    ...
    begin
      ...
      ElVCLUtils.DrawTextW(Canvas.Handle, PWideChar(Self.FCaption), Length(Self.FCaption), R, DT_SINGLELINE or DT_CALCRECT or DT_NOPREFIX);
      {$else}
      DrawText(Canvas.Handle, PChar(Self.FCaption), Length(Self.FCaption), R,  DT_SINGLELINE or DT_CALCRECT or DT_NOPREFIX);
      {$endif}
    ...
    WM_GETTEXT:
      begin
        if UseXPThemes and IsThemeApplied then
          AllocThemes;
        if not (IsThemeApplied and QueryThemeData) then
        begin
          if (not FPreventUpdate) and (FTexts.Count > 0) then
          begin
            S := '';
    ...
    WM_SETTEXT:
      begin
        if UseXPThemes and ThemesAvailable then
          AllocThemes;
        if IsThemeApplied then
          Msg.lParam := Integer(EmptyCaption);
      end;
