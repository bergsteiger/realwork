...
06/27/2003
  ...
  Improved positioning of the sort mark when filter mark is set

02/25/2003

  Added elhsImageIndex header style

11/14/2002

  Fixed painting of the section being dragged, when it uses ImageForm

11/05/2002

  When the header section is dragged, it is now drawn in pressed state
  (the image that is on original place, not drag image)

10/09/2002

  OnSectionResize event is now fired after the width is changed in code.
  To track down this case you can handle OnSectionResizing(trsEnd).
  If OnSectionResize comes without prior invokation of OnSectionResizing, then
  the width was changed by code.

10/08/2002

  Added BeginUpdate/EndUpdate to TElHeaderSections (previously available in TElHeader)
  Fixed an AV that happens when setting SectionOrder to a string that contains invalid index

09/18/2002

  Added ShowLookup function for manual invokation of lookup operation

08/21/2002 (c) Akzhan Abdulin

  DefaultHeaderSectionWidth constant added (way to use it as default on TElTree).
...
{$ENDIF}
    ...
    procedure RedrawSection(Canvas : TCanvas; Section : TElHeaderSection; R : TRect; xHOffs : integer; Dithered : Boolean);
...
{$endif}
    ...
    property AlphaForImages : TImageList read FAlphaImages write SetAlphaImages; { Published }
...
begin
  ...
  while i >= 0 do
  begin
    if (TElHeaderSection(FSections.FList[i]).Visible)
{$ifndef LITE}
    and ((FLockedSection <> TElHeaderSection(FSections.FList[i])) or
       (FSections.Count = 1))
{$endif}
    then
    begin
...
begin
  ...
  begin
    ...
    if (TS = FTrackSection) or ((TS = FDropSrc) and Dithered) then
      ...
      sid := HIS_NORMAL;
    DrawThemeBackground(Theme, Canvas.Handle, 0, 0, R, @R);
...
{$ENDIF}
    begin
      ...
      if IntersectRect(R1, R, Canvas.ClipRect) and (Sections[i].Visible) then
         RedrawSection(Canvas, Sections[i], R, 0, false);
    end;
  end;
{$IFNDEF LITE}
  ...
  begin
    ...
    if IntersectRect(R1, R, Canvas.ClipRect) and (FLockedSection.Visible) then
       RedrawSection(Canvas, FLockedSection, R, 0, false);
  end;
{$ENDIF}
  R := ClientRect;
...
procedure TCustomElHeader.SetAlphaImages(newValue : TImageList);
...
{$ifdef ELPACK_UNICODE}
procedure TCustomElHeader.SetHint(Value: TElFString);
var S : String;
    i,
    l : integer;
    T : WideChar;
