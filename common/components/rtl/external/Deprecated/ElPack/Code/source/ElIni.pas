...
04/14/2003
  ...
  Added ReadMultiString/WriteMultiString methods that accept WideStrings as parameter

12/25/2002

  Added TrimSpaces property
...
{$ENDIF}
  ...
  public
    ...
    function ReadMultiString(Key : string; ValueName : string; Strings :
        TStrings): Boolean; {$ifdef VCL_4_USED}overload;{$endif}
    {$ifdef VCL_4_USED}
    {$ifdef ELPACK_UNICODE}
    function ReadMultiString(Key : string; ValueName : string; Strings :
        TElFStrings): Boolean; overload;
    {$endif}
    {$endif}
  ...
  begin
    ...
    for i := 0 to FCurEntry.FChildren.Count - 1 do
      if (TElIniEntry(FCurEntry.FChildren[i]).FIsKey) then
        Strings.Add(TElIniEntry(FCurEntry.FChildren[i]).FValueName);
    result := true;
  end;
  OpenKey(S, false);
end;
          ...
          case PropList[i]^.PropType^.Kind of
            ...
            tkWString:
              begin
                ...
                ReadWideString('', PropName, WSDef, WS);
...
begin
  ...
  begin
    ...
    if Result then
      ConvertUTF8toUTF16(S, Value, strictConversion, false)
    else
      Value := DefValue;
  end
  else
...
var
  S: string;
begin
  ConvertUTF16toUTF8(Value, S, strictConversion, false);
  result := WriteString(Key, ValueName, S);
end;
{$endif}
    ...
    case vt of
      ...
      evtBinary :
        begin
          ...
          begin
            GetMem(P, i);
            if ReadBinary('', ValueName, p, i) then
              Result := WriteBinary('', ValueName, p, i);
            FreeMem(p);
          end;
        end;
    end;
