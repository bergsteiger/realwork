...
{todo: Zero in FUpdateCount when menu is hidden.}
...
06/12/2003
  ...
  Due to a typo mistake hints were not shown for popup menu. Fixed

12/28/2002

  Images in selected top-level items are now drawn correctly in WindowsXP mode
  Rectangle of top-level items without children is not drawn correctly in OfficeXP mode

12/20/2002

  And again hints were fixed. This time correctly :)

12/15/2002

  Application.Hint was not set properly. Fixed

11/25/2002

  Menu font is now not updated when WM_SETTINGCHANGE message is
  received and SystemFont = false

11/18/2002

  Fixed separator drawing in OfficeXP mode

11/07/2002

  Added BeginUpdate and EndUpdate for TElMainMenu for support of group changes
    Example:
      ElMainMenu1.BeginUpdate;
         ... changes very many of menu items
      ElMainMenu1.EndUpdate;

  Added procedure LoadItemsFromFiles

10/27/2002

  Added double-buffering and reduced flicker when painting items

10/15/2002

  Fixed the color of the menu items (especially separator in Windows XP style)

09/20/2002

  Fixed caption setting via action

08/28/2002

  Fixed some separator issues

08/22/2002

  Fixed some drawing problems with OfficeXP style
...
type
  ...
  published
    ...
    property AlphaForSubMenuImages: TCustomImageList read FAlphaSubMenuImages write SetAlphaSubMenuImages;
...
//    property HelpContext: THelpContext read FHelpContext write FHelpContext {$ifdef VCL_4_USED}stored IsHelpContextStored{$endif} default 0;
  ...
  public
    ...
    {$ifdef VCL_4_USED}
    procedure LoadItemsFromFiles(const ItemsFileName:array of String);
    {$endif}
...
//    FAImageChangeLink: TChangeLink;
  ...
  published
    ...
    {$endif}
    property RightToLeft: Boolean read FPopupRightToLeft write SetPopupRightToLeft default false; //GTP1
    property SystemFont: Boolean read FSystemFont write SetSystemFont default true;
    {$ifdef HAS_HTML_RENDER}
    property IsHTML : Boolean read FIsHTML write SetIsHTML default false;
    property OnImageNeeded: TElHTMLImageNeededEvent read FOnImageNeeded write
...
//vAd:
{$ifdef VCL_4_USED}
procedure LoadItemsFromFiles(Menu:TMenu; const ItemsFileName:array of String);
{$endif}
...
const WM_REDRAWMENUBAR = WM_USER + 7236;
...
begin
  ...
  begin
    hTestTheme := OpenThemeData(0, 'MENU');
    if hTestTheme <> 0 then
    begin
      result := true;
      CloseThemeData(hTestTheme);
...
begin
  ...
  begin
    ...
    DestItem.AlphaForSubMenuImages := SourceItem.AlphaForSubMenuImages;
...
begin
  try
    ...
    On E : Exception do
    begin
      MessageBox(0, PChar('Removal from parent failed: ' + E.Message), nil, 0);
    end;
  end;
  try
  while Count > 0 do Items[0].Free;
    ...
    On E : Exception do
    begin
      MessageBox(0, PChar('Children removal failed: ' + E.Message), nil, 0);
    end;
  end;
  try
  if FHandle <> 0 then
  begin
    ...
    On E : Exception do
    begin
      MessageBox(0, PChar('Merge & DestroyMenu failed: ' + E.Message), nil, 0);
    end;
  end;
  try
  FItems.Free;
    ...
    On E : Exception do
    begin
      MessageBox(0, PChar('Freeing items failed: ' + E.Message), nil, 0);
    end;
  end;
  try
  {$ifdef VCL_4_USED}
  ...
  {$endif}
    ...
    On E : Exception do
    begin
      MessageBox(0, PChar('ActionLink disposal failed: ' + E.Message), nil, 0);
    end;
  end;
  try
  ...
  FImageChangeLink := nil;
    ...
    On E : Exception do
    begin
      MessageBox(0, PChar('ImageLink disposal failed: ' + E.Message), nil, 0);
    end;
  end;
  try
  FAImageChangeLink.Free;
  FAImageChangeLink := nil;
  except
    On E : Exception do
    begin
      MessageBox(0, PChar('AlphaImageLink disposal failed: ' + E.Message), nil, 0);
    end;
  end;
  ...
  if Assigned(FDisBitMap) then FDisBitMap.Free;
    ...
    On E : Exception do
    begin
      MessageBox(0, PChar('Bitmap disposal failed: ' + E.Message), nil, 0);
    end;
  end;
  try
  inherited Destroy;
    ...
    On E : Exception do
    begin
      MessageBox(0, PChar('inherited failed: ' + E.Message), nil, 0);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not Enabled then
      begin
        if ((not Selected) or (GetParentComponent is TElMainMenu)) and (DrawStyle = tdsNormal) and ((not IsWinXPUp) or (not QueryThemeData)) then
        begin
          OffsetRect(Rect, 1, 1);
...
var
  TopLevel: Boolean;
  ImageList: {$ifdef VCL_4_USED}TCustomImageList{$else}TCustomImageList{$endif};
  AlphaImageList: {$ifdef VCL_4_USED}TCustomImageList{$else}TcustomImageList{$endif};
  ParentMenu: TMenu;
...
//  MenuItemInfo : TMenuItemInfo; //GTP1
...
//GTP1
    if TMenu(ParentMenu) is TElPopupMenu then
    begin
      RightToLeftMenuItem := TElPopupMenu(ParentMenu).RightToLeft;
    end
    else
    if TMenu(ParentMenu) is TElMainMenu then
    begin
      RightToLeftMenuItem := TElMainMenu(ParentMenu).RightToLeft;
    end
    else
    begin
      RightToLeftMenuItem := false;
    end;
...
{    FillChar(MenuItemInfo, sizeof(MenuItemInfo), 0);
    ...
    RightToLeftMenuItem := (MenuItemInfo.fType and MFT_RIGHTORDER) <> 0;} //GTP1
//GTP
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          AlphaImageList := TElPopupMenu(ParentMenu).AlphaForImages;
        end else
        begin
          ImageList := TElMainMenu(ParentMenu).Images;
          AlphaImageList := TElMainMenu(ParentMenu).AlphaForImages;
        end;

        if FParent.SubMenuImages <> nil then
        begin
          ImageList := FParent.SubMenuImages;
          AlphaImageList := FParent.AlphaForSubMenuImages;
        end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                BarWidth := TImageList(ImageList).Height + 7;
              end
              else
              begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              {$else}
              if Count > 0 then
                ACanvas.Polyline([Point(Arect.Left, ARect.Bottom), Point(Arect.Left, ARect.Top), Point(Arect.Right, ARect.Top), Point(Arect.Right - 1, ARect.Top), Point(ARect.Right - 1, ARect.Bottom)])
              else
                ACanvas.Polygon([Point(Arect.Left, ARect.Bottom - 1), Point(Arect.Left, ARect.Top), Point(Arect.Right, ARect.Top), Point(Arect.Right - 1, ARect.Top), Point(ARect.Right - 1, ARect.Bottom - 1), Point(Arect.Left, ARect.Bottom - 1)]);
//              Rectangle(ARect);
              {$endif}
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  GlyphRect.Right := GlyphRect.Left + TImageList(ImageList).Width;
                  GlyphRect.Bottom := GlyphRect.Top + TImageList(ImageList).Height;
                end
                else
                begin
                  if ImageList <> nil then
                  begin
                    GlyphRect.Right := GlyphRect.Left + Max(TImageList(ImageList).Width, GetSystemMetrics(SM_CXMENUCHECK));
                    GlyphRect.Bottom := GlyphRect.Top + Max(TImageList(ImageList).Height, GetSystemMetrics(SM_CYMENUCHECK));
                  end
                  else
                  begin
            ...
            begin
              ...
              if DStyle <> tdsOfficeXP then
              begin
                if (not Selected) or (not Enabled) then
                begin
                  OldBrushColor := Brush.Color;
                  if DStyle = tdsWindowsXP then
                    Brush.Color := clWindow
            ...
            begin
              ...
              begin
                DrawAImage := Assigned(AlphaImageList) and (ImageIndex < AlphaImageList.Count);
                if (DStyle = tdsOfficeXP) and (Selected) and (Enabled) and (not Checked) then
                begin
                  FillRect(Rect(GlyphRect.Left, GlyphRect.Top + 1, GlyphRect.Right, GlyphRect.Bottom));
                end;
                ...
                begin
                  ...
                  begin
                    ...
                    amBMP.Width := TImageList(AlphaImageList).Width;
                    amBMP.Height := TImageList(AlphaImageList).Height;
                    ImageList_DrawEx(AlphaImageList.Handle, ImageIndex, amBMP.Canvas.Handle, 0, 0, 0, 0,
                      clNone, clNone, ILD_Normal);

                     AlphaCopyRect2(ACanvas.Handle, GlyphRect, FDisBitmap.Canvas.Handle,
                       rect(0, 0, FDisBitmap.Width, FDisBitmap.Height), amBMP, 255);
                     amBMP.Free;
                  end
                  else
                ...
                begin
                  ImgRect := GlyphRect;
                  OffsetRect(ImgRect, 0, (GlyphRect.Bottom - GlyphRect.Top - TImageList(ImageList).Height) div 2);
                  ar2 := ImgRect;
                  ...
                  begin
                    ...
                    begin
                      ...
                      ShadowBitmap(Bmp);
                      ElVCLUtils.DrawTransparentBitmapEx(ACanvas.Handle, Bmp, ImgRect.Left + 1,
                        ImgRect.Top + 1, Rect(0, 0, Bmp.Width, Bmp.Height), $FeFeFe);
                      if DrawAImage then
                        OffsetRect(ar2, -1, -1)
                      else
                        ImageList.Draw(ACanvas, ImgRect.Left - 1, ImgRect.Top - 1, ImageIndex{$ifdef VCL_4_USED}, true{$endif});
                    end
                    else
                      if not DrawAImage then
                        ImageList.Draw(ACanvas, ImgRect.Left, ImgRect.Top, ImageIndex{$ifdef VCL_4_USED}, true{$endif});
                  end
                  else
                  if not DrawAImage then
                    ImageList.Draw(ACanvas, ImgRect.Left, ImgRect.Top, ImageIndex{$ifdef VCL_4_USED}, true{$endif});
                  ...
                  begin
                    aBMP := TBitmap.Create;
                    aBMP.Width := TImageList(ImageList).Width;
                    aBMP.Height := TImageList(ImageList).Height;
                    ImageList_DrawEx(ImageList.Handle, ImageIndex, aBMP.Canvas.Handle, 0, 0, 0, 0,
                      clNone, clNone, ILD_Normal);
                    ...
                    ImageList_DrawEx(AlphaImageList.Handle, ImageIndex, amBMP.Canvas.Handle, 0, 0, 0, 0,
                     ...
                     AlphaCopyRect2(ACanvas.Handle, ar2, aBMP.Canvas.Handle,
                       rect(0, 0, aBMP.Width, aBMP.Height), amBMP, 255);
              ...
              begin
                ...
                try
                  ...
                  if (Selected) {and ((IsWinXPUp) or (DStyle = tdsWindowsXP))} and Enabled then
                  begin
                  ...
                  begin
                    ...
                    Glyph.Height := 13;
                    Glyph.Width := 13;
                    if (DStyle = tdsWindowsXP) and (Selected) and Enabled then
                    begin
...
//            end;
          ...
          begin
            ...
            begin
              ...
              begin
                if ImageList <> nil then
                  GlyphRect.Right := GlyphRect.Left + Max(GetSystemMetrics(SM_CXMENUCHECK), TImageList(ImageList).Width) + 1
                else
        ...
        begin
          ...
          if DStyle = tdsOfficeXP then
          begin
            if (RightToLeftMenuItem) and (not TopLevel) then
              dec(SaveRect.Right, BarWidth)
            else
              inc(SaveRect.Left, BarWidth);
          end;
          DoDrawText(ACanvas, Caption, SaveRect, Selected, DrawStyle);
        end;
...
begin
  ...
  *)
  ...
  if FParent.SubMenuImages <> nil then
    ImageList := TImageList(FParent.SubMenuImages);
  ...
  if (ShortCut <> 0) and (Caption <> cLineCaption) and (not TopLevel) then
...
begin
  ...
  begin
    ...
    begin
      if (not Assigned(FDisBitmap)) and Assigned(ImageList) then
        GrayBitmap(ImageList, ImageIndex, FDisBitmap);
    end;
  end;
end;
...
begin
  ...
  if (AComponent = AlphaForImages) and (Operation = opRemove) then AlphaForImages := nil;
end;
...
procedure TElMainMenu.SetAlphaImages(Value: TImageList);
begin
...
begin
  ShortCut := Byte(Message.CharCode);
    ...
    {$ifdef USE_DBUGINTF}
    SendDebug(Format('Key: %d', [ShortCut]));
    {$endif}
...
{$ifdef VCL_4_USED}
procedure TElMainMenu.LoadItemsFromFiles(const ItemsFileName:array of String);
begin
  ElMenus.LoadItemsFromFiles(Self, ItemsFileName);
end;
{$endif}
...
begin
    ...
    begin
      S := TFileStream.Create(ItemsFileName[i], fmOpenRead);
      try
...
begin
  ...
  begin
    case Msg of
      ...
      WM_ACTIVATE:
        begin
          if (IsWinXPUp and QueryThemeData) then
          begin
            WindowActive := Message.wParam in [wa_Active, wa_ClickActive];
            //Handled := false;
            if (ComponentState * [csLoading, csReading, csDestroying] = []) then
              PostMessage(WindowHandle, WM_REDRAWMENUBAR, 0, 0);
          end;
        end;
      WM_HELP: //GTP2
        begin
      ...
      WM_MENUSELECT:
        begin
          MenuItem := nil;
          if (TWMMenuSelect(Message).MenuFlag <> $FFFF) or (TWMMenuSelect(Message).IDItem <> 0) then
          begin
            FindKind := fkCommand;
        ...
        with PDrawItemStruct(LParam)^ do
          ...
          begin
            ...
            begin
              ...
              Bmp := TBitmap.Create;
              try
                Bmp.Handle := CreateCompatibleBitmap(hDC, rcItem.Right - rcItem.Left, rcItem.Bottom - rcItem.Top);
                R := rcItem;
                ...
                begin
                    // Handle := hDC;
                    if Assigned(Self.Font) and (not SystemFont) then
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            ...
                            if MenuItem.Enabled then
                              DrawButtonFrameEx3(Canvas.Handle, R, false, true, clBtnFace, true, AllBorderSides);
                            Brush.Style := bsClear;
                            // if {WindowActive} ((itemState and ODS_INACTIVE) = ODS_INACTIVE) or ((not IsWinXPUp) or (not IsAppThemed)) then
                            if WindowActive or ((not IsWinXPUp) or (not IsAppThemed)) then
                        ...
                        begin
                          ...
                          begin
                            ...
                            Canvas.FillRect(R);
                            if MenuItem.Enabled then
                              DrawButtonFrameEx3(Canvas.Handle, R{rcItem}, false, false, clBtnFace, true, AllBorderSides); //GTP3c
                            Brush.Style := bsClear;
                          end
                          else
                          begin
                    ...
                    tdsOfficeXP:
                      begin
                        if (itemState and ODS_SELECTED <> 0) and (MenuItem.Enabled) then
                        begin
                          if TopLevel then
                          begin
                    ...
                    TElMenuItem(MenuItem).DrawItem(Canvas, R, Selected);
                end;
                bitblt(hDC, rcItem.Left, rcItem.Top, rcItem.Right - rcItem.left, rcItem.Bottom - rcItem.Top, Bmp.Canvas.Handle, 0, 0, SRCCOPY);
              finally
                Bmp.Free;
              end;
              Handled := true;
...
begin
  if Menu <> nil
  then FUnicodeItems.MergeWith(Menu.FUnicodeItems)
  else FUnicodeItems.MergeWith(nil);
end;
...
{$endif}
procedure TElPopupMenu.SetAlphaImages(Value: TImageList);
begin
...
//vAd:
{$ifdef VCL_4_USED}
procedure TElPopupMenu.LoadItemsFromFiles(const ItemsFileName:array of String);
begin
  ElMenus.LoadItemsFromFiles(Self, ItemsFileName);
end;
{$endif}
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                Item := IDItem;
              MenuItem := FindItem(Item, FindKind);
              if MenuItem <> nil then
                break;
            end;
          end;
            ...
            begin
              ...
              try
                ...
                begin
                  ...
                  tdsOfficeXP:
                  begin
                    if (itemState and ODS_SELECTED <> 0) and TElMenuItem(MenuItem).Enabled then
                    begin
                      Brush.Color := BrightColor(clHighlight, 70);
                      Font.Color := clMenuText;
                      Pen.Color := clHighlight;
...
initialization
  ...
  //vAd:
  Classes.RegisterClasses([TElMenuItem, TElMainMenu, TElPopupMenu]);
