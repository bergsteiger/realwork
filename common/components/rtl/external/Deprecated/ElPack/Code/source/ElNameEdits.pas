...
05/12/2003
  ...
  Published several Button* and AltButton* properties

10/11/2002

  Now the edit width Save dialog type has a corresponding icon
...
begin
  ...
  begin
    ...
    if Self.FDialogType = fdtSave then
      THackElEditBtn(FButton).Glyph.LoadFromResourceName(HInstance, 'ELFILESAVEEDITBUTTON')
    else
      THackElEditBtn(FButton).Glyph.LoadFromResourceName(HInstance, 'ELFILENAMEEDITBUTTON');
  end
end;
...
procedure TElFolderNameEdit.SetPrompt(const Value: string);
begin
