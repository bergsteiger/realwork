...
02/12/2003
  ...
  Resizable and Autosize are exclusive now.

12/22/2002

  Fixed autosize calculation when left/top grab handles are visible
...
begin
  ...
  if not ( (FButtonType = ebtSeparator)and(not (csDesigning in ComponentState)) ) then
    inherited;
  if (ButtonType = ebtDivider) and (not IsThemeApplied) then
  begin
    {$ifdef ELPACK_USE_IMAGEFORM}
...
begin
  ...
  begin
    ...
    begin
      ...
      Caption := '';
      if (Parent is TElToolBar) then
        Color := TElToolBar(Parent).ButtonColor;
      if (Color = TElToolBar(Parent).Color) then
        ParentColor := True;
    end;
    if Parent is TElToolBar then
      with TElToolBar(Parent) do
      begin
        UpdateButtons;
        AlignButtons;
      end; // with
...
begin
  if Parent is TElToolbar then with TElToolbar(Parent) do
  begin
    case FButtonType of
      ebtButton:
...
function TElToolBar.GetCustomToolButton(index : integer) : TCustomElToolButton;
begin
  if (FButtons = nil) then
    result := nil
  else
...
procedure TElToolBar.SetAlphaImages(newValue : TImageList);
var
  i : integer;
  //b,
  b1 : boolean;
...
procedure TElToolBar.SetAlphaHotImages(newValue : TImageList);
var
  I : Integer;
begin
...
procedure TElToolBar.SetAlphaDisabledImages(newValue : TImageList);
var
  I : Integer;
begin
...
procedure TElToolBar.ReadImageIsAlphaBlended(Reader : TReader);
begin
  Reader.ReadBoolean;
end;

procedure TElToolBar.DefineProperties(Filer: TFiler);
begin
  inherited;
...
begin
  ...
  begin
    ...
    begin
      with TElGraphicButton(Controls[i]) do
...
procedure TElToolBar.SetShowBorder(Value: Boolean);
begin
...
procedure TElToolBar.SetDownImages(newValue : TImageList);
var
  I : Integer;
begin
  ...
  begin
    ...
    if csLoading in ComponentState then exit;
    for i := 0 to FButtons.Count - 1 do // Iterate
      TCustomElToolButton(FButtons[i]).DisabledImages := FDownImages;
    if FDownImages <> nil then FDownImages.FreeNotification(Self);
  end; { if }
end; { SetDisabledImages }

procedure TElToolBar.SetAlphaDownImages(newValue : TImageList);
var
  I : Integer;
begin
...
procedure TElToolBar.UpdateButton(Button : TCustomElToolButton);
begin
...
procedure TelToolbar.SetResizable(newValue : Boolean);
begin
  inherited;
  if newValue and (ComponentState * [csReading, csLoading] = []) then
    AutoSize :=false;
