...
07/16/2003
  ...
  Disabled cell controls could be clicked. Fixed. 
...
07/01/2003
  ...
  Changing SelectColumn or ItemFocused didn't reset coordinates of cell to edit. Fixed

05/21/2003

  Fixed the problem with not redrawing the tree when HideSelection is false

05/16/2003

  Click on disabled items caused unnecessary mouse capturing. Fixed.
  Fixed some keyboard operations (Down and Right keys handling when focused item is invisible)

05/05/2003

  Switching between vlTextAndStyles and vlAllProps virtuality levels caused an AV. Fixed.

04/26/2003

  Fixed a problem with incremental search, which could hang in certain situations

04/19/2003

  When MouseFrameSelect action is done, first item is always selected now.

04/16/2003

  Reworked IncrementalSearch to start from focused item, if present.

03/23/2003

  Fixed Delphi 3-specific problem when Selected state was incorrectly determined

03/14/2003

  Added IncrementalSearchScope property
  Fixed the problem with word-wrap items in additional columns text

03/13/2003

  ScrollTracking property has been split to VertScrollTracking and HorzScrollTracking

03/05/2003

  Renamed RightAlignedText to RightAlignedView

02/26/2003

  Added WordWrap to multiline items

02/25/2003

  Added support for cell images when corresponding header section has elhsImageIndex style

02/11/2003

  Access to MainStyle in certain cases could cause an AV in virtual modes. Fixed.

01/26/2003

  Added IsFirstChild and IsLastChild methods to tree item

01/04/2003

  Added SortSectionToCompare property in ElTree. When OnCompareItems is fired, this
  property contains a section index to use for comparision

01/02/2003

  Fixed incorrect length calculation in TryStartHint that caused the hint not
  to be displayed in certain cases.
  Fixed cell width calculation so the vertical line doesn't overlap the text now.

12/27/2002

  SaveTo*StringList/LoadFrom*StringList now save/load whatever text the item has,
  even when ShowColumns = false
  Fixed the problem when change in Accepted value was ignored
  after OnAfterOperation event handler was called

12/22/2002

  ParentStyle renamed to ParentFontStyle
  ParentFontStyle and ParentColors now affect the style and color of the whole row,
  not just main text as before

12/21/2002

  Fixed an idiotic bug when MultiSelect := false didn't remove selection from selected items.

12/17/2002

  VirtualityLevel = vlAllProps added. Cool speed-up in tree operations.
  Incremental search didn't work with space (#32). Fixed.

12/15/2002

! HideSelection property behaviour has been remade. Now HideSelection *hides* selection of the items
  when the tree is inactive

12/12/2002

  Added HighlightAlphaLevel property, which defines whether selection and
  DragTarget are drawn with alpha-blending.

11/28/2002

  Fixed FocusRect drawing bug when scrolling text horizontally and RowSelect = false

11/12/2002

  Fixed calculation of number of multiline items.

11/10/2002

  Fixed painting issues (Font property was not taken into account completely) in cell controls
  Fixed text alignment in cell buttons

11/01/2002

  Added default for LinkStyle property

10/16/2002

  Fixed calculation of hint showing in LineHintMode = shmLong

10/15/2002

  Adjusted vertical alignment of text

10/10/2002

  ParentFont was forced to false when the form was opened. Fixed.

10/07/2002

  DoubleBufferedOnResize property added. It controls whether the resize operation
  flickers (if DoubleBufferedOnResize is false) or is slowed down (DoubleBufferedOnResize = true)

10/01/2002

  OnAfterSelectionChange event was not called after incremental search changed selection

09/18/2002

  The list of parameters in SelectRange2 and SelectRangeEx2 was changed to
  include possibility to skip children of collapsed items

09/04/2002

  Changed the way the item focus rectangle is drawn when RowSelect is false

  Added ElTreeItem.RecalcHeight method that is used to refresh item height when
  using virtual mode and style is updated and this update can affect item height
  (font change, image assignment etc.)

08/25/2002

  Checkboxes are drawn as disabled in disabled tree
  Focus and select is correctly hidden when the parent form becomes inactive
  MouseFrameVisible property added
...
04/09/2002
  ...
  Fixed MoveToItem to insert the item above the target when the item belonged to the same parent
...
todo:
// 7. Move texts and styles to DataEx
5. Проверить двойной вызов ClearSubChild
8. сделать функцию InvalidateItem
...
type
  ...
  TVirtualityLevel= (vlNone, vlTextAndStyles, vlAllProps);
...
{$endif}
    ...
    property Caption: TElFString read GetText write SetText stored false;
...
{$ifndef ELPACK_USE_INPUTPROCESSOR}
...
{$ENDIF}
    ...
    // Painting routines
    procedure RedrawTree(ACanvas : TCanvas; RealLeftPos : integer; ItemsList : TElList); virtual;
...
{$ENDIF}
    ...
    procedure SetAlphaImages2(newValue: TImageList);
    ...
    procedure SetAlphaHeaderImages(newValue: TImageList);
...
{$endif}
    ...
    procedure TriggerHeaderLookupDoneEvent(Section: TElHeaderSection; Text: string;
...
{$endif}
    ...
    function GetIncSearchColumnText(Item : TElTreeItem): TElFString; virtual;
    ...
    property AlphaForImages2: TImageList read FAlphaImages2 write SetAlphaImages2;
...
{$endif}
    ...
    property OwnerDrawMask: TElFString read FODMask write FODMask stored IsOwnerDrawMaskStored;
    ...
    property AlphaForHeaderImages: TImageList read GetAlphaHeaderImages write SetAlphaHeaderImages; { Protected }
      ...
      begin
        if ((not FTree.FBarStyle) and (FOwner.FOwner.BorderStyle = ibsNone)) and
           (((FTree.FSelectColumn = FTree.FMainTreeCol) and (FTree.FShowHeader)) or
             FTree.FRowSelect) {and (FTree.HighlightAlphaLevel = 255)} then
        begin
          if FTree.FView.FHasFocus then
            Canvas.Font.Color := FTree.FFocusedSelectTextColor
          else
          if not FTree.HideSelection then
...
begin
  ...
  begin
    ...
    begin
      if (TextAlignment = taRightJustify) xor FTree.FRightAlignedView then
      begin
  ...
  begin
    ...
    begin
      if not FTree.FRightAlignedView then
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      with FOwner do
        ExtDrawBkgnd(FTmpBmp.Canvas.Handle, BgRect1, BgRect, BgRect, BgRect2, false, BkColor, BkColor, Background, backgroUndtype);
    end;
  end;
end;
...
var
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (FAlphaImages<>nil)and InRange(0,FAlphaImages.Count-1,StImIndex)then
            begin
              BMP := TBitmap.Create;
  ...
  {$ENDIF}
          begin
            ...
            {$endif}
          {$ifdef ELPACK_UNICODE}
            ElVCLUtils.DrawTextW(Surface.Canvas.Handle, PWideChar(SText), Length(SText), R3, DT_NOPREFIX or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF}DT_CALCRECT);
          {$else}
            DrawText(Surface.Canvas.Handle, PChar(SText), Length(SText), R3, DT_NOPREFIX or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF}DT_CALCRECT);
          {$endif}
          end;
          if RightAlignedView then
          begin
  ...
  {$ENDIF}
          ...
          begin
            ...
            begin
              {$ifdef ELPACK_UNICODE}
              ElVCLUtils.DrawTextW(Surface.Canvas.Handle, PWideChar(SText), Length(SText), R1, DT_NOPREFIX or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF}DT_VCENTER or DT_RIGHT or DT_RTLREADING);
              {$else}
              DrawText(Surface.Canvas.Handle, PChar(SText), Length(SText), R1, DT_NOPREFIX or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF}DT_VCENTER or DT_RIGHT or DT_RTLREADING);
              {$endif}
            end else
            begin
              {$ifdef ELPACK_UNICODE}
              ElVCLUtils.DrawTextW(Surface.Canvas.Handle, PWideChar(SText), Length(SText), R1, DT_NOPREFIX or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF}DT_VCENTER);
              {$else}
              DrawText(Surface.Canvas.Handle, PChar(SText), Length(SText), R1, DT_NOPREFIX or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF}DT_VCENTER);
              {$endif}
            end;
          end;
        end;
...
var
  ...
  begin
    ...
    begin
      ...
      if (FATImages <> nil) and InRange(0, FATImages.Count - 1, StImIndex) then
      begin
        amBMP := TBitmap.Create;
...
{$endif}
    ...
    if Item.Selected and (FHasFocus or (not HideSelection)) and ((not FBarStyle) and (Item.BorderStyle = ibsNone)) and
       (((FSelectColumn = FMainTreeCol) and (FShowHeader)) or FRowSelect) and
...
{$ENDIF}
    ...
    if (Item = FDropTrg) and (FDragTrgDrawMode in [ColorRect, SelColorRect])
       and (((FSelectColumn = FMainTreeCol) and (FShowHeader)) or FRowSelect) and
       (FHighlightAlphaLevel = 255) then
    begin
...
{$ENDIF}
    if (InSizeMove) {$ifndef LITE}and (BackgroundType = bgtColorFill){$endif}{$ifdef ELPACK_COMPLETE}and ((csDesigning in ComponentState) {$ifdef ELPACK_USE_IMAGEFORM}or (ImageForm = nil){$endif}){$endif} then
    begin
      if not OwnBk then
...
{$ENDIF}
        begin
          ...
          {$endif}
        {$ifdef ELPACK_UNICODE}
          ElVCLUtils.DrawTextW(ACanvas.Handle, PWideChar(SText), Length(SText), R3, DT_NOPREFIX or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF} DT_LEFT or DT_CALCRECT);
        {$else}
          DrawText(ACanvas.Handle, PChar(SText), Length(SText), R3, DT_NOPREFIX or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF} DT_LEFT or DT_CALCRECT);
        {$endif}
        //InflateRect(R, 2, 0);
        end;
...
{$ENDIF}
          begin
            ...
            {$ifdef ELPACK_UNICODE}
              ElVCLUtils.DrawTextW(ACanvas.Handle, PWideChar(SText), Length(SText), R3, VAL or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF} DT_NOPREFIX or AL or DT_EXTERNALLEADING);
            {$else}
              DrawText(ACanvas.Handle, PChar(SText), Length(SText), R3, VAL or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF}DT_NOPREFIX or AL or DT_EXTERNALLEADING);
            {$endif}
          end;
          if RightAlignedView then
            inc(R1.Right)
...
{$ifdef ELTREE_USE_STYLES}
              ...
              if (Item.UseStyles and (CurStyle.FStyle = elhsText)) or (not ((Item.FStaticDataEx.FBoolData1 and ibfUseStyles) = ibfUseStyles)) then
{$endif}
              begin
                if (not Item.UseStyles) and (HS.Style = elhsImageIndex) then
                begin
                  R := Rect(HS.Left - FHPos - xOffs, ItemRect.Top, HS.Right - (FHPos + flv + 1) - xOffs, ItemRect.Bottom);
...
{$ifdef ELTREE_USE_STYLES}
  ...
  {$ENDIF}
                  begin
                    ...
                    {$ifdef ELPACK_UNICODE}
                    ElVCLUtils.DrawTextW(ACanvas.Handle, PWideChar(SText), Length(SText), R3, DT_NOPREFIX or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF} DT_CALCRECT);
                    {$else}
                    DrawText(ACanvas.Handle, PChar(SText), Length(SText), R3, DT_NOPREFIX or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF} DT_CALCRECT);
                    {$endif}
                  end;
  ...
  {$IFDEF HAS_HTML_RENDER}
                  ...
                  begin
                    ...
                    if (VAL and DT_VCENTER) = DT_VCENTER then
                    begin
                      OffsetRect(R3, 0, R.Top + (((R.Bottom - R.Top) - (R3.Bottom - R3.Top)) shr 1));
                    end else
                    begin
  ...
  {$ENDIF}
                  begin
                    ...
                    {$ifdef ELPACK_UNICODE}
                      ElVCLUtils.DrawTextW(ACanvas.Handle, PWideChar(SText), Length(SText), R3, DT_NOPREFIX or VAL or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF} AL or DT_EXTERNALLEADING);
                    {$else}
                      DrawText(ACanvas.Handle, PChar(SText), Length(SText), R3, DT_NOPREFIX or VAL or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or WordWrapFlags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF} AL or DT_EXTERNALLEADING);
                    {$endif}
                  end;
                  ACanvas.Brush.Style := bsSolid;
                end;
...
{$endif}
          ...
          if FVLines and (((not FBarStyle) and (Item.BorderStyle = ibsNone)) or FBSVLines or FNotRaiseForBarStyle) then
          begin
            xOffs := 0;
{$IFNDEF LITE}
            if HS.Locked then
...
{$ENDIF LITE}
            ...
            begin
              ...
              begin
                if (Item.FStaticData.FStyles = nil) or (j >= Item.FStaticData.FStyles.Count) or (HS.UseMainStyle) then
                   ...
                   CurStyle := Item.FStaticData.FStyles[j];
              end
              else
              begin
            ...
            if (not Item.UseStyles and (Item.BkColor = FOwner.VertDivLinesColor)) or
               (Item.UseStyles and (CurStyle.CellBkColor = FOwner.VertDivLinesColor)) then
  ...
  {$ENDIF LITE}
    ...
    begin
      if FHasFocus then
        ...
        ACanvas.Brush.Color := FHideSelectColor;
      AlphaFillRect(ACanvas, R1, ACanvas.Brush.color, FHighlightAlphaLevel);
    end;
        ...
        ColorRect:
          begin
            if FDropAcc then
              ...
              ACanvas.Brush.Color := FOwner.FDragRectDenyColor;
            AlphaFillRect(ACanvas, R1, ACanvas.Brush.color, FHighlightAlphaLevel shr 1);
          end;
        SelColorRect:
          begin
            ACanvas.Brush.Color := clHighlight;
...
{$endif}
    ...
    Inc(R2.Right);
    Inc(R2.Bottom);
    if IntersectRect(R4, R2, ACanvas.ClipRect) {$ifndef LITE}and (BackgroundType = bgtColorFill){$endif} {$ifdef ELPACK_COMPLETE}and ((csDesigning in ComponentState) {$ifdef ELPACK_USE_IMAGEFORM} or (ImageForm = nil){$endif}){$endif} then
    begin
    ...
    begin
      if FHLines and ((not BarStyle) or (FNotRaiseForBarStyle)) then
      begin
        y := 0;
        if FShowHeader then
          ...
          begin
            if TElTreeItem(ItemsList[i]).DrawHLine then
              ...
              begin
                ...
                {$else}
                if TElTreeItem(ItemsList[i]).BkColor = FOwner.HorzDivLinesColor then
                {$endif}
  ...
  begin
    ...
    begin
      ...
      if TSI.FHTMLDataArray<>nil then
        htmldata := TElHTMLData(TSI.FHTMLDataArray[HCol])
      else
        htmldata := nil;
    end;
    ...
    begin
      ...
      begin
        if Item.FHTMLDataArray <> nil then
          htmldata := TElHTMLData(Item.FHTMLDataArray[HCol-1])
        else
          htmldata := nil;
      end;
      if htmldata <> nil then
...
{$ifdef ELTREE_USE_INPLACE_EDITORS}
      if (FOwner.FCanEdit) and (FHeader.Sections[HCol].Editable) and
         Item.AllowEdit and (Item.Enabled or (FOwner.IgnoreEnabled)) and
         not (csDesigning in FOwner.ComponentState) then
         begin
...
begin
  ...
  begin
    ...
    with FOwner do
    begin
      if (Item.FTextRect.Left < FHPos) or (Item.FTextRect.Left > FHPos + Self.Width) then
         SetHPosition(Item.FTextRect.Left);
      R.Left := Item.FTextRect.Left - FHPos;
      if Section <> nil then
...
begin
  FOwner.DoEndEditItem(FEditingItem, FOwner.HeaderSections[FInpEdit.FSectionIndex], true);
  FInpEdit := nil;
end;
  ...
  begin
    ...
    begin
      ...
      while FEditingItem.FStaticData.FColText.Count <= si do
        FEditingItem.FStaticData.FColText.Add('');
      FEditingItem.FStaticData.FColText[si] := FInpEdit.ValueAsText;
    end;
  end;
  FOwner.DoEndEditItem(FEditingItem, FOwner.HeaderSections[FInpEdit.FSectionIndex], false);
  FInpEdit := nil;
end;
...
begin
  ...
  begin
    ...
    begin
      ExtDrawBkgnd(DC,  BgRect1, BgRect, BgRect, AClipRect, false, Canvas.Brush.Color, Canvas.Brush.Color, FTmpBmp, bgtCenterBitmap);
    end
    else
    begin
      ExtDrawBkgnd(DC, BgRect1, BgRect, BgRect, AClipRect, false, Canvas.Brush.Color, Canvas.Brush.Color, FOwner.Background, FOwner.BackgroundType);
    end;
  end
  else
{$ENDIF}
...
procedure TElTreeView.DoPerformSearch(SearchText : TElFString);
var
  liSearchTextLength,
  liIndex,
  siIndex  : Integer ;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        {$ifdef ELPACK_UNICODE}
        if WideSameText(SearchText, WideCopy(ItemText, 1, liSearchTextLength)) then
        {$else}
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      FBoolData1    := PInfo.FBoolData1 and not (ibfMultiline or ibfHidden or ibfOwnerHeight or ibfSelected or ibfFocused or ibfEnabled or ibfIsHTML or ibfWordWrap);
    end;

    FTag      := PInfo.FTag;
    FBoolData := PInfo.FBoolData1 and (ibfMultiline or ibfHidden or ibfOwnerHeight or ibfEnabled or ibfIsHTML or ibfWordWrap);
...
{$ELSE}
  if ((FBoolData and ibfOwnerHeight) <> ibfOwnerHeight) or {$IFDEF HAS_HTMl_RENDER}IsHTML or {$ENDIF} (Multiline and FOwner.adjUstmultilineheight) then
  begin
...
var
  ...
  begin
    ...
    // check for (not VisibleOnly) removed - it was a bug, think of it and you will see :)
...
begin
  {$ifdef MSWINDOWS}
  if ((Flat or FUseCustomBars) and (BorderStyle = bsSingle)) {$ifdef MSWINDOWS}or IsThemeApplied{$endif} then
    DrawFlatBorder(false, false);
  {$endif}
...
{$endif}
    ...
    begin
{$ifdef ELTREE_USE_STYLES}
      if (((CurStaticDataEx.FBoolData1 and ibfUseStyles) = ibfUseStyles) and (CurStyle.FStyle = elhsText)) or
         ((CurStaticDataEx.FBoolData1 and ibfUseStyles) <> ibfUseStyles) then
{$endif}
      begin
        if ((CurStaticDataEx.FBoolData1 and ibfUseStyles) <> ibfUseStyles) and
  ...
  {$endif}
          ...
          AL := DT_NOPREFIX or {$IFNDEF LITE}MultiLineFlags[Item.Multiline] or Wordwrapflags[Item.Multiline, Item.WordWrap] or {$ELSE} DT_SINGLELINE or {$ENDIF}DT_VCENTER or DT_LEFT or DT_CALCRECT;
          if RightAlignedText then
...
{$endif}
  ...
  {$ifdef ELTREE_USE_STYLES}
    ...
    if (((CurStaticDataEx.FBoolData1 and ibfUseStyles) = ibfUseStyles) and (CurStyle.FStyle = elhsText)) or ((CurStaticDataEx.FBoolData1 and ibfUseStyles) <> ibfUseStyles) then
  {$endif}
    begin
      SaveFontStyle := Canvas.Font.Style;
      SaveFontSize := Canvas.Font.Height;
...
begin
  inherited;
  if (Flat or FUseCustomBars) and (not IsThemeApplied) then DrawFlatBorder(false, false);
end;  { WMEnable }
...
function TCustomElTree.GetAlphaHeaderImages: TImageList;
{ Returns the value of data member FHeaderImages. }
begin
...
procedure TCustomElTree.SetAlphaHeaderImages(newValue: TImageList);
{ Sets data member FHeaderImages to newValue. }
begin
...
procedure TCustomElTree.SetAlphaImages2(newValue: TImageList);
{var
  i: integer;}
begin
...
procedure TCustomElTree.WMIsThemedControl(var Message: TMessage);
begin
...
begin
  ...
  begin
    ...
    {$endif}
    {$ifdef LITE}
    Filer.DefineProperty('AdjustMultilineHeight', FakeBoolean, nil, false);
    Filer.DefineProperty('Background', FakeValue, nil, false);
    Filer.DefineProperty('BackgroundType', FakeValue, nil, false);
    Filer.DefineProperty('GradientStartColor', FakeValue, nil, false);
    Filer.DefineProperty('GradientEndColor', FakeValue, nil, false);
    Filer.DefineProperty('GradientSteps', FakeValue, nil, false);
    Filer.DefineProperty('HeaderWrapCaptions', FakeBoolean, nil, false);
    {$endif}
    Filer.DefineProperty('NoBlendSelected', FakeBoolean, nil, false);
    Filer.DefineProperty('ScrollBackground', FakeBoolean, nil, false);
    {$IFNDEF ELPACK_COMPLETE}
    // Filer.DefineProperty('Storage', FakeValue, nil, false);
    // Filer.DefineProperty('StoragePath', FakeValue, nil, false);
    {$ENDIF}
    {$IFNDEF HAS_HTML_RENDER}
    Filer.DefineProperty('OnHTMLImageNeeded', FakeValue, nil, false);
    Filer.DefineProperty('OnLinkClick', FakeValue, nil, false);
    Filer.DefineProperty('LinkCursor', FakeValue, nil, false);
    Filer.DefineProperty('LinkColor', FakeValue, nil, false);
    Filer.DefineProperty('LinkStyle', FakeValue, nil, false);
    {$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      Item.FStaticDataEx := FDataCache[0];
...
function TCustomElTree.GetIncSearchColumnText(Item : TElTreeItem): TElFString;
var cc    : integer;
{$IFDEF HAS_HTML_RENDER}
    FData : TElHTMLData;
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if cc <> -1 then
            FData := TElHTMLData(Item.FHTMLDataArray[cc])
          else
            FData := Item.FHTMLData;
        end
        else
...
(*
{$ifdef ELPACK_USE_STYLEMANAGER}
procedure TCustomElTree.SetStyleManager(Value: TElStyleManager);
begin
  inherited;
  if not (csDestroying in ComponentState) then
  begin
    FHScrollBar.StyleManager := Value;
    FVScrollBar.StyleManager := Value;
  end;
end;

procedure TCustomElTree.SetStyleName(const Value: string);
begin
  inherited;
  FHScrollBar.StyleName := Value;
  FVScrollBar.StyleName := Value;
end;
{$endif}
*)
