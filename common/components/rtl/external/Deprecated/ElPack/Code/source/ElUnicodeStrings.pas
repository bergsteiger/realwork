...
{$endif}
 ...
 {$warnings off}
  ...
  protected
    ...
    procedure Put(Index: Integer; const S: WideString); override;
    procedure PutObject(Index: Integer; AObject: TObject); override;
    procedure PutStringEntry(Index: Integer; const S : WideString; AObject: TObject); virtual;
    procedure SetUpdateState(Updating: Boolean); override;
  public
    procedure Clear; override;

    function Add(const S: WideString): Integer; override;
    function AddStringEntry(const S: WideString; AObject: TObject): Integer; virtual;
...
begin
  ...
  try
    ...
    if SaveUnicode or (Order = U_LSB_FIRST) or (Order = U_MSB_FIRST) then
    begin
      if SaveUnicode then
      begin
      {  BytesRead := }Stream.Read(Order, 2);
      ...
      while (Length (WS) > 0) and //gtp1 ensure no unwanted Unicode identifier chars
            ((WS [1] = U_LSB_FIRST) or
             (WS [1] = U_MSB_FIRST)) do
      begin
...
begin
  ...
  begin
    ...
    try
      ...
      {vAd: ??? need memory optimize.}      Delete(CurIndex);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          C[i] := #0;
          S := StrPas(C);
          ConvertUTF8ToUTF16(S, WS, strictConversion, false);
          FreeMem(C);
        Add(WS);
      end
      else
...
function TElWideStringList.Find(const S: WideString; var Index: Integer):Boolean;
var
  L, H, I, C: Integer;
begin
  Result := False;
...
procedure TElWideStringList.QuickSort(L, R: Integer; SCompare: TElWideStringListSortCompare);
var
  I, J, P: Integer;
begin
  repeat
...
procedure TElWideStringArray.OnItemDelete(Sender : TObject; Item : TxListItem);
...
begin
  ...
  begin
    ...
    begin
      O := GetObject(i);
      Stream.WriteBuffer(Integer(O), sizeof(Integer));
    end;
  end;    // for
end;
