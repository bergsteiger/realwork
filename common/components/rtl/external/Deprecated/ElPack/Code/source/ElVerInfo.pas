...
{                                                    }
{   Copyright (c) 1998-2003, EldoS Corporation, Alex Ionov       }
{                                                    }
{====================================================}
{$include elpack2.inc}
...
Type
  ...
  private
    ...
    procedure SetDummyEx(Index: integer; const Value: string);
...
begin
    SetLength(Result, 64);
    SetLength(Result, VerLanguageName(StrToIntDef('$' + Copy(FLanguage, 1, 4), $0409), PChar(Result), 64));
end;
...
function TElVersionInfo.GetValue(const AName: string): string;
var
  Size: DWORD;
  Value: PChar;
begin
//.net:
//function VerQueryValue(const pBlock: TBytes; lpSubBlock: string;
//  var lplpBuffer: IntPtr; var puLen: UINT): BOOL;
//  B := AnsiEncoding.GetBytes(Data);
  if Length(FBuffer) > 0 then
  begin
      Value := nil;
      VerQueryValue(PChar(FBuffer), PChar('\StringFileInfo\' + FLanguage + '\' + AName), Pointer(Value), Size);
      Result := Value;
  end
  else
    Result := '';
...
procedure TElVersionInfo.SetDummyEx(Index: integer; const Value: string);
begin
end;
