...
uses
  ...
  ElVCLUtils, ElFontCombo, ElEdits, ElBtnEdit, ElCombos, ElCLabel;
...
type
  TElTreeDemoMainForm = class(TForm)
    ...
    InplaceEditorsTypeCombo: TElComboBox;
...
begin
  P := TElCellProgressBar(TreeCellCtl.Items[TElTimerPoolItem(Sender).Index].Styles[2].Control);
  P.Value := P.Value + 5;
  if P.Value >= 100 then
  begin
    TElTimerPoolItem(Sender).Enabled := false;
    TElCellButton(TreeCellCtl.Items[TElTimerPoolItem(Sender).Index].Styles[1].Control).Caption := 'Start';
  end;
end;
