...
var
  s : string;
begin
  if TemplateEdit.Text = '' then
    s := ExtractFilePath(ExePath)
  else
    s := ExtractFilePath(TemplateEdit.Text);
  OpenTemplateDialog.InitialDir := s;
...
begin
  if TemplateEdit.Text = '' then
    ...
    s := ExtractFilePath(OutputFilesEdit.Text);
...
begin
  DemoMlGen.OutputFileName := OutputFilesEdit.Text;
  DemoMlGen.Multipage := MultipageCheckBox.Checked;
  DemoMlGen.LinesOnPage := LinesOnPageEdit.Value;
  DemoMlGen.UseHeader := HeaderCheckBox.Checked;
  DemoMlGen.UseFooter := FooterCheckBox.Checked;
  DemoMlGen.Execute;
...
begin
  ...
  TranslationTableNamesListBox.ItemIndex := DemoMlGen.TranslationTables.DefaultForMacro;
