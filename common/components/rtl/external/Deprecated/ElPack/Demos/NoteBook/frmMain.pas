...
uses ElCalendar, ElCalendDlg, ElBaseComp, ElTray, ElCaption,
  ...
  ElXPThemedControl, ElGroupBox, ElEdits, ElBtnEdit;
{
...
begin
  (*
  edEventStart:= TElDateTimePicker.Create(Self);
    edEventStart.Parent := GroupBox3;

    edEventStart.Left := 112;
    edEventStart.Top := 24;
    edEventStart.Width := 137;
    edEventStart.Height := 21;

  RemindTime:= TElDateTimePicker.Create(Self);
    RemindTime.Parent := ElPanel6;
    RemindTime.Left := 74;
    RemindTime.Top := 13;
    RemindTime.Width := 130;
    RemindTime.Height := 21;
  *)

  cbPeriod.Items.Add('Seconds');
  cbPeriod.Items.Add('Minutes');
  cbPeriod.Items.Add('Hours');
...
begin
  if (NoteTree.ItemFocused<>nil) and
     (NoteTree.ItemFocused.Text <> cPerson) and
     (NoteTree.ItemFocused.Text <> cEvent) and
     (NoteTree.ItemFocused.Text <> cRemind) then
  begin
    NoteTree.Items.DeleteItem(NoteTree.ItemFocused);
  end;
end;
