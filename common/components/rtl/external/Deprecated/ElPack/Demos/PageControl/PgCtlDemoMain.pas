...
uses
  ...
  ElEdits, ElCLabel, ElLabel, ElBtnEdit, ElCombos;
...
type
  TPgCtlDemoMainForm = class(TForm)
    ...
    ElLabel1: TElLabel;
    CloseCombo: TElAdvancedComboBox;
    ...
    procedure CloseComboChange(Sender: TObject);
    procedure PageCtlClosePageQuery(Sender: TObject; Page: TElTabSheet;
...
begin
  PageCtl.DrawFocusRect := DrawFocusCB.Checked;
end;
...
procedure TPgCtlDemoMainForm.CloseComboChange(Sender: TObject);
begin
  PageCtl.CloseButtonMode := TElCloseBtnMode(CloseCombo.ItemIndex);
end;

procedure TPgCtlDemoMainForm.PageCtlClosePageQuery(Sender: TObject;
  Page: TElTabSheet; var CanClose: Boolean);
begin
  CanClose := false;
  Page.Free;
  PagesSpin.Value := PagesSpin.Value - 1; 
end;
