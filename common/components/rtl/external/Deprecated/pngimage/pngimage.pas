...
type
  ...
  private
    ...
    function GetTransparentColor: TColor;
                                       procedure SetAlpha(X, Y: Integer; const Value: Byte);
  ...
  public
    ...
    procedure SaveToStream(Stream: TStream); {$IFDEF UseDelphi}override;{$ENDIF}
                                       procedure GrayDown(DecreaseAlpha: Byte = 0);
...
function TPngObject.GetAlpha(X, Y: Integer): Byte;
begin
 if TransparencyMode = ptmPartial then
  Result := AlphaScanline[Y][X]
 else
  Result := 255; 
end;
...
procedure TPngObject.GrayDown(DecreaseAlpha: Byte = 0);
var
 lClr: TColor;
 lRowBytes: Cardinal;
 lExtraIncrease: Integer;
 MaxVal: Byte;
 R, G, B: Byte;
 Pixel: TColor;
 I, J, N: Integer;
 lData, lAlpha: Pointer;
begin
 with Header do
 begin
  if ColorType in [COLOR_GRAYSCALE, COLOR_PALETTE] then
  begin
   for I := 0 to Height-1 do
    ...
    begin
     Pixel := GetByteArrayPixel(Self, J, I);
     ...
     begin
      ...
      SetByteArrayPixel(Self, J, I, lClr);
      //SetByteArrayPixel(Self, J, I, RGB(MaxVal, MaxVal, MaxVal));
     end;
    end;
  end
  else
  begin
   N := Width*Height;
   lExtraIncrease := BytesPerRow mod 3;
   lRowBytes := 0;
   lData := ImageData;
   lAlpha := ImageAlpha;
   for I := 1 to N do
   begin
    MaxVal := Max(TRGBTriple(lData^).rgbtBlue, TRGBTriple(lData^).rgbtGreen);
    MaxVal := Max(MaxVal, TRGBTriple(lData^).rgbtRed);
    TRGBTriple(lData^).rgbtBlue := MaxVal;
    TRGBTriple(lData^).rgbtGreen := MaxVal;
    TRGBTriple(lData^).rgbtRed := MaxVal;
    if DecreaseAlpha > 0 then
     Byte(lAlpha^) := Max(0, Byte(lAlpha^) - DecreaseAlpha);
    Inc(Longint(lData), SizeOf(TRGBTriple));
    Inc(LongInt(lAlpha), 1);
    Inc(lRowBytes);
    if lRowBytes = Width then
    begin
     lRowBytes := 0;
     Inc(Longint(lData), lExtraIncrease);
    end;
   end;
  end;
 end;
...
procedure TPngObject.SetAlpha(X, Y: Integer; const Value: Byte);
var
 ScanLine: pByteArray;
begin
 if TransparencyMode = ptmPartial then
 begin
  Longint(ScanLine) := LongInt(Header.ImageAlpha)+Y*Width;
  ScanLine[X] := Value;
 end;
end;
