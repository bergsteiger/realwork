...
{ TMergeManager }
  ...
  public
    ...
    procedure Merge(AForm: TCustomForm);
    procedure GotoForm(AForm: TCustomForm);
    procedure GotoFormClass(AFormClass: TFormClass);
    procedure GoBack;
    procedure GoForward;
...
procedure TMergeManager.Merge(AForm: TCustomForm);
begin
  MergeForm(MergeFrame, TForm(AForm), alClient, True);
  GotoForm(AForm);
end;

procedure TMergeManager.GotoForm(AForm: TCustomForm);
var
  I: Integer;
...
procedure TMergeManager.GotoFormClass(AFormClass: TFormClass);
var
  I: Integer;
begin
  if MergeFrame = nil then Exit;
  for I := 0 to MergeFrame.ControlCount - 1 do begin
    if MergeFrame.Controls[I] is AFormClass then begin
      GotoForm(MergeFrame.Controls[I] as TCustomForm);
      Exit;
    end;
  end;
end;
