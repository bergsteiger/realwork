...
{$R dxBarExtDBItemsStrs.res}

uses
  {$IFDEF DELPHI6}Variants,{$ENDIF} dxBarCommon;

const
  DXBAR_LOOKUPDIALOGCAPTION = 39460;
  DXBAR_LOOKUPDIALOGOK = 39461;
  DXBAR_LOOKUPDIALOGCANCEL = 39462;
...
begin
  ...
  with CurItemLink.ItemRect do
    if Right - Left > FPopupWidth then
...
begin
  ...
  begin
    ...
    Caption := LoadStr(DXBAR_LOOKUPDIALOGCAPTION);
    ...
    with ButtonOk do
    begin
      Caption := LoadStr(DXBAR_LOOKUPDIALOGOK);
      Default := True;
      ModalResult := mrOk;
      Parent := FForm;
    end;
    ...
    begin
      Caption := LoadStr(DXBAR_LOOKUPDIALOGCANCEL);
      Cancel := True;
