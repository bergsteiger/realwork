...
type
  ...
  public
    ...
    property OnLocationUpdated: TBindingLocationUpdatedEvent read FOnLocationUpdated write FOnLocationUpdated;
  ...
  private
    ...
    procedure EvaluateFormatCallback(ACol, ARow: Integer; AEditorScope: IScope);
...
type
  ...
  private
    ...
    function GetTrack: Boolean;
    procedure SetTrack(const Value: Boolean);
    function GetBindComponent: TActivatedContainedBindComponent;
  ...
  public
    ...
    property Active: Boolean read GetActive write SetActive;
    property Track: Boolean read GetTrack write SetTrack;
    property BindComponent: TActivatedContainedBindComponent read GetBindComponent;
  end;
...
procedure TCustomBindGridLink.EvaluateFormatCallback(ACol, ARow: Integer; AEditorScope: IScope);
var
  LExpressions: TExpressions;
  ...
  begin
    ...
    begin
      ...
      LVirtualEditor.SetOnCallbackEvaluateFormat(
        ...
        begin
          EvaluateFormatCallback(ACol, ARow, AEditorScope);
        end);
    end;
  end;
  Result := AEditor <> nil;
...
begin
  FBinding := TInternalLinkControlToFieldBinding.Create(Self);
  inherited;
end;
...
var
  LNeedNew: Boolean;
  LMember: string;
  LSource, LControl: TComponent;
  LTrack: Boolean;
  LActive: Boolean;
  LBindTrack: IBindTrack;
  LParse, LClear, LFormat: TExpressions;
  LAssigningEvent: TBindCompAssigningValueEvent;
  LAssignedEvent: TBindCompAssignedValueEvent;
  LEvalEvent: TBindCompEvalErrorEvent;
  LLocationUpdatedEvent: TBindingLocationUpdatedEvent;
  LActivatingEvent, LActivatedEvent: TNotifyEvent;
begin
  ...
  begin
    ...
    if LNeedNew and (not Loading) then
    begin
      LParse := TExpressions.Create(Self, TExpressionItem);
      LClear := TExpressions.Create(Self, TExpressionItem);
      LFormat := TExpressions.Create(Self, TExpressionItem);
      try
        LSource := FBinding.SourceComponent;
        LMember := FBinding.SourceMemberName;
        LControl := FBinding.ControlComponent;
        if Supports(FBinding, IBindTrack, LBindTrack) then
          LTrack := LBindTrack.Track
        else
          LTrack := False;
        LParse.Assign(FBinding.ParseExpressions);
        LFormat.Assign(FBinding.FormatExpressions);
        LClear.Assign(FBinding.ClearExpressions);
        LAssigningEvent := FBinding.OnAssigningValue;
        LAssignedEvent := FBinding.OnAssignedValue;
        LEvalEvent := FBinding.OnEvalError;
        LLocationUpdatedEvent := FBinding.OnLocationUpdated;
        LActivatingEvent := FBinding.OnActivating;
        LActivatedEvent := FBinding.OnActivated;
        LActive := FBinding.Active;
        FBinding.Free;
        FDirection := Value;
        FBinding := TInternalLinkControlToFieldBinding.Create(Self);
        FBinding.SourceComponent := LSource;
        FBinding.SourceMemberName := LMember;
        FBinding.ControlComponent := LControl;
        if Supports(FBinding, IBindTrack, LBindTrack) then
          LBindTrack.Track := LTrack;
        FBinding.SetParseExpressions(LParse);
        FBinding.SetFormatExpressions(LFormat);
        FBinding.SetClearExpressions(LClear);
        FBinding.OnAssigningValue := LAssigningEvent;
        FBinding.OnAssignedValue := LAssignedEvent;
        FBinding.OnEvalError := LEvalEvent;
        FBinding.OnLocationUpdated := LLocationUpdatedEvent;
        FBinding.OnActivating := LActivatingEvent;
        FBinding.OnActivated := LActivatedEvent;
        FBinding.Active := LActive;
      finally
        LClear.Free;
        LFormat.Free;
        LParse.Free;
...
begin
  ...
  begin
    ...
    Result := CheckActiveScope(FillDataSource)
  else
  begin
    Result := ((DataSource = nil) or CheckActiveScope(DataSource)) and ((FillDataSource = nil) or CheckActiveScope(FillDataSource))
  end;
end;
...
begin
  if Sender = BindControlValue then
    ...
    Result := CheckActiveScope(FillDataSource) or (FillDataSource = nil)
...
function TInternalLinkControlToFieldBinding.GetBindComponent: TActivatedContainedBindComponent;
begin
