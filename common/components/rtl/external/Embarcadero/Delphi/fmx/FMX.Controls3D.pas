...
uses
  ...
  System.UIConsts, FMX.Types, System.Generics.Defaults, FMX.Messages, FMX.PixelFormats, FMX.Types3D;
...
begin
  ...
  Filer.DefineProperty('Quanternion', ReadQuaternion, WriteQuaternion, 
    (FQuaternion.ImagPart.X <> IdentityQuaternion.ImagPart.X) or
    (FQuaternion.ImagPart.Y <> IdentityQuaternion.ImagPart.Y) or 
    (FQuaternion.ImagPart.Z <> IdentityQuaternion.ImagPart.Z) or 
    (FQuaternion.RealPart <> IdentityQuaternion.RealPart));
end;
...
begin
  ...
  begin
    ...
    if RayCastIntersect(AbsoluteToLocalVector(rPos), (AbsoluteToLocalVector(rDir)).GetNormalize, IP) then
    begin
...
begin
  ...
  begin
    ...
    if CheckHitTest(HitTest) and RayCastIntersect(AbsoluteToLocalVector(rPos),
      (AbsoluteToLocalVector(rDir)).GetNormalize, IP)
...
begin
  ...
  begin
    ...
    begin
      ...
      AContext.SetRenderToMatrix(Matrix3DMultiply(S, T));
    end
    else
    begin
      S := IdentityMatrix3D;
...
{      T.m41 := (Frac(R.Left) + ((R.Width - AContext.Width) / 2) + Offset.X + ((FViewport.GetContext.Width / 2) - ((R.Left + R.Right) / 2))) / AContext.Width * 2;
      ...
      AContext.SetRenderToMatrix(Matrix3DMultiply(Matrix3DMultiply(S, T), FViewport.Context.CurrentScreenMatrix));
//      AContext.FRenderToMatrix := Matrix3DMultiply(S, FViewport.GetContext.GetScreenMatrix);
//      AContext.FRenderToMatrix := FViewport.GetContext.GetScreenMatrix;
  ...
  try
    ...
    try
      ...
      AContext.SetRenderToMatrix(IdentityMatrix3D);
      FInRenderTo := False;
      AContext.EndScene;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        BitmapContext.SetRenderToMatrix(Matrix3DMultiply(S, t));
      end;
...
begin
  ...
  try
    ...
    begin
      ...
      TempContext := BitmapContext;
      try
        Context.SetRenderToMatrix(Matrix3DMultiply(S, T));
        Context.SetStateFromContext(FViewport.Context);
        // render
        if Context.BeginScene then
        try
...
begin
  ...
  begin
    ...
    Context.DrawLine(Vector3D(0, 0,   0), Vector3D(0, 0, 1).Scale(1000), 1, $FF9C60A7);
  end;
end;
...
begin
  ...
  begin
    Result := MatrixLookAtDirRH(AbsolutePosition,
      AbsolutePosition - Target.AbsolutePosition,
      AbsoluteUp.Scale(-1));
  end
  else
    Result := MatrixLookAtDirRH(AbsolutePosition,
      AbsoluteDirection.Scale(-1),
      AbsoluteUp.Scale(-1));
end;
...
begin
  ...
  begin
    if not Enabled then
    begin
      Context.FillCube(NullVector3D, Vector3D(Width, Height, Depth).Scale(0.8), 1, TAlphaColors.Gray);
      Context.DrawCube(NullVector3D, Vector3D(Width, Height, Depth).Scale(0.8), 1, TAlphaColors.Navy);
    end
    else
    begin
      Context.FillCube(NullVector3D, Vector3D(Width, Height, Depth).Scale(0.8), 1, TAlphaColors.Yellow);
      Context.DrawCube(NullVector3D, Vector3D(Width, Height, Depth).Scale(0.8), 1, TAlphaColors.Navy);
      case LightType of
        TLightType.ltDirectional:
          begin
            Context.DrawLine(NullVector3D, Vector3D(0, 0, 1).Scale(Width * 5), 1, TAlphaColors.Navy);
          end;
        TLightType.ltPoint:
          begin
            for i := 1 to 18 do
              Context.DrawLine(NullVector3D,
                Vector3D(cos(DegToRad(i * 20)), sin(DegToRad(i * 20)), 0).Scale(Width * 2), 1, TAlphaColors.Navy);
            for i := 1 to 18 do
              Context.DrawLine(NullVector3D, Vector3D(cos(DegToRad(i * 20)), 0,
                sin(DegToRad(i * 20))).Scale(Width * 2), 1, TAlphaColors.Navy);
          end;
        TLightType.ltSpot:
          begin
            Context.DrawLine(NullVector3D, Vector3D( 0.2, -0.2, 1).GetNormalize.Scale(Width * 5), 1, TAlphaColors.Navy);
            Context.DrawLine(NullVector3D, Vector3D( 0.2,  0.2, 1).GetNormalize.Scale(Width * 5), 1, TAlphaColors.Navy);
            Context.DrawLine(NullVector3D, Vector3D(-0.2,  0.2, 1).GetNormalize.Scale(Width * 5), 1, TAlphaColors.Navy);
            Context.DrawLine(NullVector3D, Vector3D(-0.2, -0.2, 1).GetNormalize.Scale(Width * 5), 1, TAlphaColors.Navy);
          end;
...
begin
  Result := TLightDescription.Create(Enabled, Color, LightType, SpotCutOff, SpotExponent, AbsolutePosition, AbsoluteDirection);
end;
...
begin
  ...
  begin
    if (Left.Opacity < 1.0) and (Right.Opacity = 1.0) then
      ...
      if (Left.Opacity = 1.0) and (Right.Opacity < 1.0) then
        Result := -1;
{    if (Result = 0) and Assigned(Left.Context) and Assigned(Left.Context.FCamera) then
    begin
      V1 := Left.AbsolutePosition - Left.Context.FCamera.AbsolutePosition;
      V2 := Right.AbsolutePosition - Left.Context.FCamera.AbsolutePosition;
