...
uses
  System.Classes, System.Types, System.Rtti, System.UITypes, FMX.Types, FMX.Types3D,
...
begin
  ...
  begin
    ...
    Rec.Texture.SetSize(Width, Height);
  ...
  begin
    ...
    begin
      case FValues[i].ValueType of
        ...
        TFilterValueType.vtPoint:
          TFilterManager.FilterContext.SetShaderVariable(FValues[i].Name, [Vector3D(FValues[i].Value.AsType<TPointF>.x / FInput.Width, FValues[i].Value.AsType<TPointF>.y / FInput.Height, 0, 0)]);
        TFilterValueType.vtColor:
          begin
...
begin
  ...
  try
    ...
    if (TTextureStyle.tsRenderTarget in Texture.Style) and (TContextStyle.RenderTargetFlipped in Context.Style) then
    begin
      Ver.TexCoord0[0] := PointF((DstPos.X + ARect.Left) / Texture.Width, 1 - (DstPos.Y + ARect.Top) / Texture.Height);
      Ver.TexCoord0[1] := PointF((DstPos.X + ARect.Right) / Texture.Width, 1 - (DstPos.Y + ARect.Top) / Texture.Height);
      Ver.TexCoord0[2] := PointF((DstPos.X + ARect.Right) / Texture.Width, 1 - (DstPos.Y + ARect.Bottom) / Texture.Height);
      Ver.TexCoord0[3] := PointF((DstPos.X + ARect.Left) / Texture.Width, 1 - (DstPos.Y + ARect.Bottom) / Texture.Height);
    end else begin
...
begin
  ...
  try
    ...
    begin
      ...
      try
        TFilterManager.FilterContext.SetMatrix(IdentityMatrix3D);
