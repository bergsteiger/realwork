...
procedure GetFixupReferenceNames(const Root: TComponent; Names: TStrings);
...
function InternalReadComponentRes(const ResName: string; HInst: THandle; var Instance: TComponent{$IFDEF ARM_FIXME}; unitname: string = ''{$ENDIF ARM_FIXME}): Boolean; overload;
var
  HRsrc: THandle;
...
//  LTotalCount := Read(Buffer, 0, Count);
//
...
////      (Count - LTotalCount));
//    LReadCount := Read(Buffer, LTotalCount, (Count - LTotalCount));
//
...
procedure TStream.WriteBuffer(Buffer: TBytes; Offset, Count: Longint);
var
  Buf: TBytes;
  I: LongInt;
begin
  ...
  begin
    SetLength(Buf, Length(Buffer) - Offset);
    for I := 0 to Length(Buffer) - Offset - 1 do
...
begin
  ...
  begin
    ...
    begin
      ...
      MoveData(Buffer, Offset, FMemory, FPosition, Count);
      FPosition := Pos;
      Result := Count;
      Exit;
    end;
...
begin
  ...
  begin
    ...
    MoveData( FBuffer, FBufPos, Buffer, Offset, LShouldRead);
    Inc(FBufPos, LShouldRead);
...
begin
  if MoveBuffer and (Keeping > 0) then
    //System.Array.Copy(FBuffer, Length(FBuffer) - Keeping, FBuffer, 0, Keeping);
    MoveData(FBuffer, Length(FBuffer) - Keeping, FBuffer, 0, Keeping);
...
begin
  ...
  begin
    ...
    if C > Count then
      C := Count;
    //System.Array.Copy(System.Array(Buffer), P, System.Array(FBuffer), FBufPos, C);
    MoveData(Buffer, P, FBuffer, FBufPos, C);
    Inc(P, C);
    Inc(FBufPos, C);
    Dec(Count, C);
  end;
