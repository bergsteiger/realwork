...
begin
  ...
  OffsetCount := pcre_exec(FPattern, FHints, M.AsUtf8(TEncoding.UTF8.GetString(FSubject)).ToPointer, FStop, 0, Opts, @Offsets[0], High(Offsets));
  Result := OffsetCount > 0;
...
begin
  ...
  if FStart >= FStop then
    ...
    OffsetCount := pcre_exec(FPattern, FHints, M.AsUtf8(TEncoding.UTF8.GetString(FSubject)).ToPointer, FStop, FStart, Opts, @Offsets[0], High(Offsets));
  Result := OffsetCount > 0;
  // Convert offsets into TBytes indices
  if Result then
  begin
...
begin
  ARegEx.Subject := UTF8ToString(FSubject);
  ARegEx.Start := FStart;
end;
