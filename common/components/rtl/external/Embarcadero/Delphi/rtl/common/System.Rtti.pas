...
function Invoke(CodeAddress: Pointer; const Args: TArray<TValue>;
  ...
  begin
    ...
    FreeVFP := 8; // Number of VFP(FP) registers. D0-D7
    for i := 0 to Length(Args) - 1 do
      if PassByRef(Args[i].TypeInfo, CallingConvention) then
      begin
        if FreeCR > 0 then
...
begin
  ...
  freeCRegs := regCRAll;
  if CallingConvention in [ccPascal, ccStdCall, ccCdecl] then
    freeFPRegs := regNone
  else
    freeFPRegs := regFPRAll;

  if IsStatic and UseResultPointer(AResultType, IsConstructor) then
  begin
    TValue.Make(nil, AResultType, Result);
    PutRefArg(Result.GetReferenceToRawData);
  end;
  ...
  if not IsStatic and UseResultPointer(AResultType, IsConstructor) then
  begin
    TValue.Make(nil, AResultType, Result);
    PutRefArg(Result.GetReferenceToRawData);
  end;
  ...
  if AResultType = nil then
    ...
    // do nothing
  else if not(CallingConvention in [ccPascal, ccStdCall, ccCdecl]) and
...
begin
  ...
  if FCallConv in [ccPascal, ccStdCall, ccCdecl] then
    freeFPRegs := regNone
  else
    freeFPRegs := regFPRAll;
...
{$ELSEIF defined(CPUARM) and defined(iOS)}
