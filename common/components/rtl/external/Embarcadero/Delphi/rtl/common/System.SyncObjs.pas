...
begin
  if dispatch_semaphore_wait(FEvent, dispatch_time(0, NSEC_PER_MSEC * Timeout)) = 0 then
    Result := wrSignaled
  else
    Result := wrTimeout;
end;
...
{$IFDEF MACOS}
begin
  if dispatch_semaphore_wait(FSem, dispatch_time(0, NSEC_PER_MSEC * Timeout)) = 0 then
    Result := wrSignaled
  else
    Result := wrTimeout;
end;
