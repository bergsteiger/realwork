...
{$IFDEF ANDROID}
begin
  raise ENotImplemented.Create('TPath.GetDocumentsPath');
  Result := '';
end;
{$ENDIF ANDROID}
{$ENDIF POSIX}
...
begin
  Result := UTF8ToString(
              TNSFileManager.Wrap(TNSFileManager.OCClass.defaultManager
                                 ).URLForDirectory(NSLibraryDirectory, NSUserDomainMask, nil, true, nil
                                                   ).path.UTF8String);
end;
{$ENDIF MACOS}
{$IFDEF ANDROID}
begin
  raise ENotImplemented.Create('TPath.GetLibraryPath');
  Result := '';
end;
{$ENDIF ANDROID}
{$ENDIF POSIX}
