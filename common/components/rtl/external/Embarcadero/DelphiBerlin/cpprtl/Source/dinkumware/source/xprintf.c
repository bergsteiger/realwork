...
int _Printf(void *(*pfn)(void *, const char *, size_t),
	...
	for (; ; )
		...
		for (; ; )
		 ...
		 {	/* process an argument position */
		const char *fmt_sav = fmt;

		for (x.argno = 0;
			x.argno < 1000 && isdigit((unsigned char)*fmt); ++fmt)
			x.argno = x.argno * 10 + (unsigned char)*fmt - '0';
		if (*fmt++ != '$' || x.argno <= 0 || 1000 <= x.argno)
			{	/* ill formed position, roll back */
			x.argno = 0;
			fmt = fmt_sav;
			}
		 }
 ...
 #define FBIT_SIZE	(sizeof (fbit) - 1) / sizeof (fbit[0])
		...
		if (0 < x.argno)
			{	/* positional argument, don't advance argument pointer */
			va_list ap2;

			--x.argno;
			_Vacopy(&ap2, ap);
			if (_Putfld(&x, &ap2, *fmt++, ac))
				return (EOF);
			}
		else if (_Putfld(&x, &ap, *fmt++, ac))
			return (EOF);
		if (_Puttxt(&x, ac) < 0)
			return (EOF);
		 }
