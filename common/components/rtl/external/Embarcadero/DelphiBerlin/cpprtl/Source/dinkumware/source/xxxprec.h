...
FTYPE FNAME(Xp_getw)(FTYPE *p, int n)
	{	/* get total value */
	if (n == 0)
		return (FLIT(0.0));
	else if (n == 1 || p[0] == FLIT(0.0) || 1 < n && p[1] == FLIT(0.0))
		return (p[0]);
	else if (n == 2)
		return (p[0] + p[1]);
	else
...
FTYPE *FNAME(Xp_mulh)(FTYPE *p, int n, FTYPE x0)
	...
	for (j = 1, k = 0; k < n; ++k, --j)
		...
		if (buf[0] == FLIT(0.0))
			...
			FNAME(Dint)(&y0, BITS_WORD);	/* clear how half bits */
...
FTYPE *FNAME(Xp_movx)(FTYPE *p, int n, FTYPE *q)
	{	/* copy an extended precision value */
	memcpy(p, q, n * sizeof (FTYPE));
	return (p);
	}

FTYPE *FNAME(Xp_addx)(FTYPE *p, int n, FTYPE *q, int m)
...
FTYPE *FNAME(Xp_subx)(FTYPE *p, int n, FTYPE *q, int m)
...
FTYPE *FNAME(Xp_mulx)(FTYPE *p, int n, FTYPE *q, int m, FTYPE *ptemp2)
