...
#define vtParent(x)     *(((void **)(x)) - 12)  // Delphi Parent class offset
#define vtClassName(x)  *(((char **)(x)) - 14)  // Delphi ClassName offset
#endif
...
// Here we really only need to know that Delphi objects are TObject derived and since
// all Delphi objects are by reference, have a virtual destructor and Delphi can actually
// throw any derived TObjects (not just Exception &), we really only care about the base class.
// Also, the compiler has intrinsic knowledge of this type and we really don't care about any
// of the other methods on this class, we can just declare a stub here and the rest of this code
// is nice and happy.
namespace System
{
  class __declspec(delphiclass, package) TObject { };
}
...
int     *               __pCPPdebugHook;
#endif
extern  "C" void    __cdecl __raiseDebuggerException(int                    kind,  /*XXdebuggerNotification */
						 int                    argCount,
						 ...);
...
// Declare some actual function pointer types so that the casting is a little cleaner
typedef void * (_RTLENTRY * GetExceptionObjectProc)(_EXCEPTION_RECORD *, tpidPtr *);
typedef void (__fastcall * RaiseExceptObjProc)(_EXCEPTION_RECORD *);

// This function is called to set some call backs that will eventually wind their way back
// to the Delphi RTL (System.pas) in order to make sure the threadvar RaiseList is properly
// updated when handling or throwing Delphi based exceptions. Some Delphi code out there directly
// accesses the exception object that is in play by calling Delphi RTL functions which peek at
// the current RaiseList.
extern void _RTLENTRY _EXPFUNC setRaiseListFuncAddr(void * setAddr, void * clearAddr);
// getExceptionObjectProc
//   Instead of hard-coding references to Delphi OS exception objects, just let the Delphi RTL
//   do this work for us. Just like the above callback, this function is called to setup a callback
//   that winds its way to the Delphi RTL via the ExceptObjProc function pointer in System.pas.
// raiseExceptObjectProc
//   When a Delphi exception is raised from the C++ side, this callback is invoked which gives
//   the Delphi RTL a chance to grab the inner exception object and attach it to the new exception
//   being raised. This allows for automatic tracking of nested exceptions.
//   This is a new Delphi 2009 feature.
extern void _RTLENTRY _EXPFUNC setExceptionFuncAddr(GetExceptionObjectProc getExceptionObjectProc, RaiseExceptObjProc *raiseExceptObjProc);
// This makes sure that we can get access to the *real* exception thread local information
extern exceptVarPtr _RTLENTRY _EXPFUNC __getExceptVarRec(void);

PEXCEPTION_RECORD (* _RTLENTRY pVCL_add_EH)(PEXCEPTION_RECORD, void *);
void (* _RTLENTRY pVCL_clear_EH)();
GetExceptionObjectProc getExceptionObject;
...
static  void            freeExceptObjectMem(excDscPtr xdp);

static  excDscPtr       getExceptObject(EXCEPTION_RECORD *P, unsigned long osEsp, unsigned long osERR, PCONTEXT ctx)
{
	excDscPtr       xdp;
	tpidPtr         typeID;
	tpidPtr         baseID;
	...
	// Previously, the following code did all the same mapping of OS exceptions to Delphi
	// exceptions in the same manner that SysUtils.pas did. This caused a hard-coded reference
	// to the Delphi RTL which in-turn made this code hard to maintain since two teams
	// independently handled each RTL. Now we just let the Delphi side do all this work so there
	// is one, and only one, place this will be done. If Delphi makes enhancements to how these
	// mappings are being done, C++ just inherits that behavior automatically.
	if (getExceptionObject)
		excObj = (TObject *)getExceptionObject(P, &typeID);

	if (excObj == NULL || typeID == NULL)
	{
		return 0;
	}
	...
	xdp = allocExceptMem(sizeof(*xdp) + sizeof(TObject *));
	...
	// These exception descriptors have always had a nice polymorphic way of doing cleanup
	// but it was never really used. So now it is being used so that any C++ exception
	// descriptor that is really holding onto a Delphi class instance, can now properly
	// ensure things are nicely cleaned up. Someone was looking ahead, hooray for them!
	xdp->xdFreeFunc   = freeExceptObjectMem;
	...
	memcpy(xdp->xdValue, &excObj, sizeof(TObject *));
...
extern  "C"     void    __cdecl __cleanupTObject(void);
...
{
	pVCL_clear_EH();
	_EAX = (unsigned long)*(void **)xdp->xdValue;
	__cleanupTObject();
	freeExceptMem(xdp);
}
				...
				)
{
	...
	unsigned long   argTable[3];
        ...
        /* if we're throwing, we can't be unwinding any more
            - otherwise we go infinite on a throw from a finally
             that's caught within the finally (see bugs\241390a) */
...
//      printf("Debugger info signature value: 0x%08X\n", __DebuggerInfoPtr->DTsignature);
	...
	*/
	...
	/*
		...
		**** New for Delphi/C++Builder 2009 ****
		This is going to need some explination so bear with me. Previously the common idiom for
		throwing a Delphi exception was to "throw by value" (aka. throw Exception();), however
		this was inconsistent with the rest of the C++ handling of Delphi class instances which
		never hid the fact that a Delphi class instance was really a pointer reference. Behind
		the scenes the compiler still would call the object constructor which also would allocate
		the instance on the heap. This code detects that what is being thrown is, in fact, a Delphi
		exception object and so the above comment is valid.

		What didn't work, and is what the inconsistency was, is that if the Delphi exception was
		being thrown by reference (or pointer) (aka. throw new Exception();), it would pass by this
		code and throw the exception as a C++ pointer. The problem was that while you *could*
		still catch this exception, it wouldn't be properly destroyed. Also, the handler search
		logic later on was, in fact, able to deal with both cases, it was simply the throw side
		of things that buggered things up. So, now we do a little more exhaustive search through
		the type descriptors and see if we're throwing a Delphi exception by reference or pointer.
	*/
	if      ((typeID && (((typeID->tpMask & TM_IS_CLASS) && (typeID->tpClass.tpcFlags & CF_DELPHICLASS)) ||
			 ((typeID->tpMask & (TM_IS_PTR|TM_IS_REF)) &&
			 (typeID->tpPtr.tppBaseType->tpMask & TM_IS_CLASS) &&
			 (typeID->tpPtr.tppBaseType->tpClass.tpcFlags & CF_DELPHICLASS)))) ||
			 (flags & XDF_ISDELPHIEXCEPTION))
	{
		/*
			Once we find that we are in fact throwing this exception by pointer, we have to check again
			within this code block and dereference the throwAddr value since it became a double
			indirection.
		*/
		if (!reThrow && typeID && (typeID->tpMask & (TM_IS_PTR|TM_IS_REF)) &&
		   (typeID->tpPtr.tppBaseType->tpMask & TM_IS_CLASS) &&
		   (typeID->tpPtr.tppBaseType->tpClass.tpcFlags & CF_DELPHICLASS))
		   throwAddr = *(void **)throwAddr;
		baseID = typeID;
		/*
			Next we grab the type descriptor of the actual Delphi class (which this
			code was always trying to do, however I've yet to find a case where throwing by value
			would cause this code to execute. Now this code seems to actually do what is was
		...
		*/
		...
		asm     {
			...
			/*
				The following code was added in Delphi/C++Builder 2009 to support
				nesting of exceptions where the outer exception would acquire the
				inner exception.
			*/
			mov     eax,reThrow      /* if this is a reThrow, don't do any notification */
			test    eax,eax
			jnz     @@2
			mov     eax,pVCL_add_EH
			mov		edx,raiseExceptObject
			test    edx,edx
			jz      @@2
			mov     edx,[edx]
			test    edx,edx
		...
		@@1:
			...
			mov     eax,esp
			call    edx
			add     esp,13 * 4       /* clean up 13 dwords from the stack */
				  ...
				  unsigned char *       throwFile)
{
	...
	/* The following code does NOT work correctly. Specifically when
	   ...
	   AB - Some more comments on the above comment:

	   The real reason that this code was disabled is because of a fundemental difference
	   between the way C++ processes and handles exceptions and how Delphi handles them.
	   During exception processing (both Delphi & C++ use the OS SEH system), Delphi never
	   resets the stack pointer to return to the current frame. It only set the frame pointer
	   (EBP) before a handler is executed. The handler still has access to that frame's local
	   variables, *and* all the existing exception state is still on the stack. So for Delphi,
	   the act of doing a re-raise merely tells the OS unwinder to "keep looking" (note the
	   mov eax, 1 just before the ret below).

	   In contrast, C++ does reset the stack pointer (thus the reason for the original comment
	   above. When an exception handler is entered, the entire stack state is reset to the
	   current frame. So ESP and EBP are reset. While this has the effect of cleaning up the
	   stack sooner, there is some critical bits of information that are now lost. The most
	   critical of these is that the original exception state is now gone so there is no way to
	   have some foreign exception code percolate through a series of C++ handlers unchanged.
	   The Delphi model ensures that all this state is preserved so that the exception isn't
	   inadvertently lost when traveling back through the thread of execution. For instance,
	   suppose a call was made to a foreign library from which a callback was invoked. During
	   the processing of this callback, a foreign (non-C++) exception was thrown (think Access
	   Violation, or Control C). If there were any intervening "catch (...)" blocks that did
	   a simple rethrow (aka. "throw;"), that exception will suddenly become a Delphi exception
	   because it is rethrown using the Delphi exception code (0x0EEDFADE). If this foreign
	   library which may have filters in place to actually catch these "non-C++" exception codes,
	   this exception would pass right by thus possibly violating any implied "contract" for
	   the use of this foreign library.

	   Unfortunately, this is not something we can fix for this release as it would require a
	   more fundemental change to how we handle exceptions in general. The recommendation would
	   be for us to revisit this in the next cycle (post Delphi/C++Builder 2009) and look at
	   changing the model to better match Delphi. This would keep more information on the stack
	   and at the same time allow much better handling an processing of foreign exceptions.

	   For now this foreign exception will be rethrown as a Delphi exception.
...
#pragma message Take this out before we ship
	...
	*/
	...
	if      (xdp->xdFlags & XDF_ISDELPHIEXCEPTION)
	{
		...
		/*
			We need to "move the exception along" from this exception descriptor because it will
			get attached to a new descriptor later on. Also, since this field is set to NULL,
			it is safe to still attempt to free it.
		*/
		*(void __far **)xdp->xdValue = NULL;
	}
#ifdef DELPHI_EXCEPTIONS
	/*
		In accordance with the long comment above, we need to prepare the value contained in
		this exception descriptor to have an extra indirection. We also "move it along" by
		clearing the value out of the current exception descriptor so the Delphi object
		referenced doesn't get freed. Later, in tossAnException, it will appear to be a
		throw by pointer (aka. throw new Exception()), so it will be able to undo this extra
		indirection and get the actual object reference.

		The above comment now rendered incorrect - tossAnException now tests 'reThrow' and
		skips the extra indirection (it had been doubly applying it for rethrow cases). As
		a result, we don't need to build the extra reference here.

	*/
	else if (xdp->xdFlags & XDF_OSEXCEPTION)
	{
                value = *(void __far **)xdp->xdValue;
/*
		holder = *(void __far **)xdp->xdValue;
		*(void __far **)xdp->xdValue = NULL;
		value = &holder;
*/
	}
	...
	// For C++ exceptions we need to rethrow the original exception
	// object, calling the copy-constructor isn't allowed. So below
	// we substitute a surrogate descriptor for the use of the
	// unwind code but prevent it from destroying the exception
	// object.

	typeID =  xdp->xdTypeID;

	// Set the global flags to indicate we're processing this
	// 'new' exception.
	...
	// Construct and initialize a surrogate exception descriptor, this
	// is for the consumption of the forthcoming catch-unwind. But we
	// pretend there is no exception object to delete - we need that
	// one to stay where it is.

	size = typeID->tpSize;
	surrogate = allocExceptMem(sizeof(*xdp) + size);
	memcpy(surrogate, xdp, sizeof(*xdp) + size);
	surrogate->xdArgBuff = 0;
	__CPPexceptionList = surrogate;

	// Now reset the relevant fields in the original descriptor to
	// make it seem like a new exception, only with the original
	// exception object.
...
#ifdef  PRONTO
			/*
				Don't destroy it if it's a delphi class since that will be handled
				in the xdFreeFunc()
			*/
			if      (!(xdp->xdFlags & XDF_ISDELPHIEXCEPTION))
#endif
					   ...
					   unsigned             cctx)
{
	...
	/*
		This is that nice polymorphic mechanism to allow different kinds of cleanup on a per-
		exception descriptor basis. Since this a Delphi object exception descriptor make sure
		we're going to properly free the object when appropriate. This helps ensure we don't
		ever leak a Delphi object.
	*/
	xdp->xdFreeFunc   = freeExceptObjectMem;
...
static  void    __near  local_unwind(PEXCEPTION_RECORD excPtr, PREGREC_BC errPtr, unsigned target)
{
	EXCTABPTR       xctPtr;
...
DPRINTF(("Unwinding '__finally' block: handler at %08lX\n", (long)finBody));
		...
		case    XB_CATCH:               /* catch              (C++) */
			...
			*xdrLPP = xdrPtr->xdPrevious;
			...
			/* Free the memory taken up by the exception descriptor */
#ifdef PRONTO
                        /* Except - if a Delphi exception object is being
                           rethrown from inside the catch, we don't want to double
                           delete the object.
                        */
			if (excPtr && (excPtr->ExceptionCode == PAS_EXCEPT_CODE))
			{
			    void * pObj;
			    pObj = (void *)excPtr->ExceptionInformation[1];
			    if (pObj == *(void **)xdrPtr->xdValue)
				break;
			}
#endif
			xdrPtr->xdFreeFunc(xdrPtr);
...
{
	local_unwind(0, (PREGREC_BC)errPtr, target);
}
...
{
	local_unwind(0, (PREGREC_BC)errPtr, 0);

	_Exception_list = ((PREGREC_BC)errPtr)->ERRcNext;
}

void _RTLENTRY setRaiseListFuncAddr(void *setAddr, void *clearAddr)
{
	pVCL_add_EH = (PEXCEPTION_RECORD (*)(_EXCEPTION_RECORD *, void *))setAddr;
	pVCL_clear_EH = (void (*)())clearAddr;
}

void _RTLENTRY setExceptionFuncAddr(GetExceptionObjectProc getExceptionObjectProc, RaiseExceptObjProc *raiseExceptObjProc)
{
	getExceptionObject = getExceptionObjectProc;
	raiseExceptObject = raiseExceptObjProc;
}
							  ...
							  PEXCEPTION_RECORD     excPtr)
{
	...
	for     (hdtPtr = htdPtr->HTDtable; hdtPtr->HDhndPtr; hdtPtr++)
	{
		...
		*/
		...
		/*
			If the catch handler is for a Delphi class, strip off
			namespace qualifiers.  Thrown Delphi class objects
			don't have namespace qualifiers in their vtable names.
		*/
		baseTpid = tpid->tpPtr.tppBaseType;
		if (baseTpid && IS_CLASS(baseTpid->tpMask))
		{
		    if ((baseTpid->tpClass.tpcFlags & CF_DELPHICLASS) != 0)
		    {
			while   (*tmp)
			{
			    if      (*tmp == ':')
						...
						unsigned long     flags)
{
	...
	// Keep track of if the unexpected handler is called
	static bool unexpected_called = FALSE;
...
#endif
        /* We want to check the throw specification if the unexpected
	   handler has been called */

 if      (excPtr->ExceptionFlags & UNWIND_IN_PROGRESS && !unexpected_called)
	{
...
DPRINTF(("Back from local_unwind [ESP = 0x%08X]\n", esp));
 ...
 // Seperate the checking of the throw specification in the event that
 // the unexpected handler is called from the regular unwinding.  In
 // this case, we don't need to perform local unwind, and we can also
 // remove the checking for "std::bad_exception" from the standard
 // check.
	...
	if (unexpected_called)
	{
		...
		if      (excPtr->ExceptionCode == CPP_EXCEPT_CODE)
		{
			...
			if      (throwList)
			{
			    ...
			    {
				DPRINTF(("Throw list type '%s'.\n", __typeIDname(*throwList)));
				if (strcmp((char *)(*throwList)+(*throwList)->tpName,
					   "std::bad_exception") == 0)
				    found_Bad_Except = TRUE;
				if      (typeIDcaught(dscPtr->xdTypeID,
						      dscPtr->xdFriendList,
						      *throwList,
						      dscPtr->xdFlags,
						      1))
				    /* Thrown type from unexpected handler
				       matches throw specification, so
				       break out from here. */
			    ...
			    /* We have not found a matching
			       specification, so we either
			       throw bad_exception (if that is
			       in the throw specification) or
			       call_terminiate.*/

			    if (found_Bad_Except)
			    {
				exceptionHandled(dscPtr);
				unexpected_called = FALSE;
		...
		case    XB_TRYCPP:              /* try                (C++) */
			...
			*/
			if      (isOSException(excPtr) && (flags & XPF_CATCHOS))
			{
				...
				asm     mov     osERR, eax
				/*
					Instead of having hard-coded processing of this OS exception,
					just defer this out to the Delphi RTL which already knows how
					to deal with this.
				*/
				dscPtr = getExceptObject(excPtr, OSEsp, osERR, ctxPtr);
				if      (!dscPtr)
					break;
			}
			else
...
//                              DPRINTF(("Matching catch handlers against Delphi type '%s'\n", char *));
			...
			if      (!hdtPtr)
			{
				...
				*/
				...
				{
					// dscPtr->xdPrevious always NULL.
					//__CPPexceptionList = dscPtr->xdPrevious;
					//delete(*(TObject  **)dscPtr->xdValue);
...
//                      if      (*__pCPPdebugHook && (excPtr->ExceptionCode < 0xeedface || excPtr->ExceptionCode > 0xeefface))
		...
		JUMP_HAND:
			...
			local_unwind(excPtr, errPtr, tgtctx);
...
#ifdef  PRONTO
			if ((kind == XB_TRYCPP) && (excPtr->ExceptionCode == PAS_EXCEPT_CODE))
			{
			    pVCL_add_EH(excPtr, (void *)excPtr->ExceptionInformation[1]);
			}
#endif
...
#ifdef  PRONTO
				if      (excPtr->ExceptionCode == PAS_EXCEPT_CODE || dscPtr->xdFlags & XDF_OSEXCEPTION)
				{
					void *  pVTable;
					void *	pObj;
				 ...
				 PREGREC_BC     errPtr)
{
	...
	try
	{
		try
		{
			assert((vbFlag && (errPtr->ERRcInitDtc >= varType->tpClass.tpcDtorCount)) ||
			       (!vbFlag && (errPtr->ERRcInitDtc >= varType->tpClass.tpcNVdtCount)) ||
			       flags);
			/*
...
// virtBasesInited
//   Given an ERR, determine if we have virtual bases that have been inited
//   in _this_ frame.

static bool virtBasesInited(PREGREC_BC errPtr)
{
    EXCTABPTR       xctPtr;
    unsigned        newBP;
    ...
    if (xctPtr->xtVirtCondOffs == 0)
	return false; // not inited and not present, for that matter

    /* Compute the BP value needed for this function */

    newBP = (unsigned)(char *)errPtr - xctPtr->xtBPoffs;

    /* And check the compiler magic parameter to see if this ctor
	was supposed to initialize virtual bases. */
    return (*((unsigned *)(newBP + xctPtr->xtVirtCondOffs))) == 1;
}
...
//              errPtr->ERRcInitDtc += dtorCnt;
	...
	for     (dtvtPtr = dttPtr; 1; dtvtPtr++)
	{
		...
		DPRINTF(("Cleanup variable type mask = %04X\n", varType->tpMask));
...
//                              printf("Polyptr points   to 0x%08lX\n", (long)varAddr);
		...
		DPRINTF(("Cleanup variable type flags = %04X\n", elemType->tpClass.tpcFlags));
...
DO_DST:
	...
	do
	{
		...
		DPRINTF(("Variable flags   = 0x%04lX\n", varFlags));
		...
		if      (!freePtr)
		{
			...
			if      (varType->tpMask & TM_IS_ARRAY)
				...
				destroyVariable(varAddr, varType, flags, dtorCnt, !virtBasesInited(errPtr), newBP, errPtr);
				     ...
				     (DTCVF_PTRVAL|DTCVF_DELPTR))
		{
			...
			DPRINTF(("Deleting new'ed pointer, type mask  = 0x%04lX\n", varType->tpMask));
			...
			else
			{
				 ...
				 * Check tpMask to make sure we are dealing
				 * with a class; tpcDlOAddr is invalid otherwise
				 */

			        if ((varType->tpMask & TM_IS_CLASS) &&
...
exceptVarPtr    _RTLENTRY __getExceptVarRec(void)
{
    return &__xxInfo;
}
