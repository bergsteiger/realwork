...
#define	VT_TYPE_INFO_OFFS	(-72)

	// string analysis routines for strings without null-termination
static int psCountChars (const char* s, int strSize, char c)
{
	int result = 0;
...
static int psFindFirstChar (const char* s, int strSize, char c)
{
	for (int i = 0; i < strSize; ++i)
		if (s[i] == c)
			return i;
...
char * __cdecl type_info::name    () const
{
	...
	if	(!h->tpp)
	{
		...
		int	fullNameLength;
		char* unitScope;
		int dotCount = 0, scopeLen;
		char* pos = 0;
		bool fixCapitalization, capitalizeNext;

		className = *(char **)((char *)h->vmt + VT_TYPE_INFO_OFFS)
			+ 1; // skip TTypeData.Kind
		unitScope = className
			+ className[0] + 1 // skip TTypeData.Name
			+ sizeof (void*) // skip TTypeInfo.ClassType
			+ sizeof (void*) // skip TTypeInfo.ParentInfo
			+ sizeof (short); // skip TTypeInfo.PropCount
		if ((unsigned char) unitScope[0] > 0 && unitScope[1] == '@') // class declared in C++
		{
			scopeLen = psFindFirstChar (unitScope + 2, (unsigned char) unitScope[0] - 1, ':');
			unitScope += 2; // skip length byte and '@'
			fixCapitalization = false; // C++ is case sensitive
			assert (scopeLen >= 0);
		}
		else // class declared in Delphi
		{
			scopeLen = (unsigned char) unitScope[0];
			fixCapitalization = true;
			++unitScope; // skip length byte
		}
		dotCount = psCountChars (unitScope, scopeLen, '.');

		fullNameLength = (unsigned char) className[0] // class name length
					   + scopeLen + dotCount // scope length
					   + (scopeLen > 0 ? 2 : 0); // another 2 characters for the scope/class separator
		h->name = new char[fullNameLength + 1];

		pos = h->name;
		capitalizeNext = true;
		for (int i = 0; i < scopeLen; ++i) // copy the namespace
		{
			char c = unitScope[i];
			if (c == '.')
			{
			...
			else
			{
				if (fixCapitalization)
					c = capitalizeNext ? toupper (c) : tolower (c);
				*(pos++) = c;
				capitalizeNext = false;
			}
		}
		if (scopeLen > 0)
			*(pos++) = ':', *(pos++) = ':'; // separate the namespace and the class name
...
// Changed from -36 to -48 (three more members)
#define	VT_PARENT_OFFS	(-48)
