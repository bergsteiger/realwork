 ...
 *
 * function(s)
 *        _fstat      - gets open file information
 *--------------------------------------------------------------------------*/
...
Name            _fstat - gets open file information

Usage           #include <sys/stat.h>
                int _fstat(int handle, struct stat *buff)
...
int _RTLENTRY _EXPFUNC _fstat (int fd, struct _stat *bufP)
{
    int         rc;
    HANDLE      h;
