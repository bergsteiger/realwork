...
Usage           static unsigned CheckOpenType (const _TCHAR *type,
                                               unsigned *oflagsP,
                                               unsigned *modeP,
                                               unsigned flags)
...
static unsigned
CheckOpenType (register const _TCHAR *type, unsigned *oflagsP, unsigned *modeP, unsigned shflag)
{
    unsigned        mode = 0;
    unsigned        oflags;
    unsigned        flags;
    ...
    /* Accept 'u' as first character follwed by either 'a' or 'w' for secure mode */
    if (*type == _TEXT('u') && shflag & _O_SECURE)
    {
	type++;
	if (!(*type == _T('w') || *type == _T('a')))
	    return 0;
    }
    if ((c = *type++) == _TEXT('r'))
    {
		      ...
		      const _TCHAR *type, int shflag)
{
    ...
    if (((fp->flags = CheckOpenType (type, &oflag, &mode, shflag)) == 0) ||
        ((fp->fd < 0) &&
...
FILE * __tfopen_helper(const _TCHAR *filename, const _TCHAR *type, int sharemode)
{
    register FILE   *fp;
    ...
    if ((fp = __getfp()) != NULL)
        fp = __topenfp (fp, filename, type, sharemode);
...
{
    return __tfopen_helper(filename, type, 0);
}
