...
ssSwitch:
    ...
    switch (curClass)
    {
        ...
        case (_lo) :
                        if ((flags & isLong) || (flags & isLongDouble))
                            flags = (flags | isLongDouble) & ~isLong;
                        else
                            flags |= isLong;
        ...
        case (_ld) :    flags = (flags | isLongDouble) & ~isLong;
                        b = *(formP++);
                        goto  ssSwitch;
...
ssPTR:
    lRes = __scanttoint64 (__Gett, __UnGett, srceP, 16, 8, &charCt, &status, 1);
    if (status <= 0)
        goto ssEND;
    if ((flags & isSuppressed) == 0)
    {
	p.l = va_arg(varPP,long *);
#if defined(_SECURE)
	if (!p.l)
	    goto constraint_error;
#endif
