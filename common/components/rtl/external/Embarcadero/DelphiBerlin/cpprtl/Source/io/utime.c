...
BOOL unixtofile(time_t tim, FILETIME *ftp)
{
     ...
     */
    ret = SystemTimeToFileTime(&st,ftp);
    /* We need to make a copy since the in and out paramaters to
       LocalFileTimeToFileTime are documented as required to be different */
    memcpy(&lftp, ftp, sizeof(FILETIME));
    return (ret && LocalFileTimeToFileTime(&lftp, ftp));
}
...
int _RTLENTRY _EXPFUNC _futime(int fd, const struct utimbuf *times)
{
     ...
     */
    if ((handle = (HANDLE)_get_osfhandle(fd)) == (HANDLE)-1)
        return -1;
