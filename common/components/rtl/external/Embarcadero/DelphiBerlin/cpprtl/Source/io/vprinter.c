 ...
 *
 * function(s)
 *        HexPtr     - converts pointer to hex digits
...
int _RTLENTRY _EXPFUNC _wctomb_quiet( char *s, wchar_t wc );
/****************************************************************************/
...
/* FIXME: may overflow during wide<->narrow conversions RS-48283 */
...
Name            HexPtr - converts point to hex digits

Usage           static void HexPtr(void *p, _TCHAR *buf)

Description     Convert void * parameter to hex digits at buf.

*------------------------------------------------------------------------*/

static void HexPtr (void *p, _TCHAR *buf)
{
    int i, c;
...
/*        `   a   b   c   d   e   f   g   h   i   j   k   l   m   n   o  */
        _dc,_dc,_dc,_ch,_de,_fl,_fl,_fl,_sh,_de,_sz,_dc,_lo,_dc,_ns,_oc,

/*        p   q   r   s   t   u   v   w   x   y   z   {   |   }   ~ DEL  */
        _pt,_dc,_dc,_st,_dc,_un,_dc,_dc,_he,_dc,_sz,_dc,_dc,_dc,_dc,_dc,
};
...
*/
    ...
    put.pnsize = ((__use_nsize & use_nsize) ? (&__nsize) : NULL);
...
case_nu:
            ...
            case (_lo):                     /* 'l' was seen (long)          */
                if ((flagSet & isLongBit) || (flagSet & LongDoubleBit))
                {
                    /* Another 'ell' was seen which means I64 int */
...
CopyLen:                                /* comes from %c or %s section  */
		...
		/* Handle special cases for alternate forms unless zero (7.19.6.1.6 N1124) */
