...
int _getLocaleEra(char* buf, int bufsize, const struct tm *t )
{
    ...
    _lock_locale();
    handle = __locale->handle;
    _unlock_locale();

    if ((result = GetDateFormat(handle, DATE_USE_ALT_CALENDAR, &st, "gg", &buf[0], 0)) > bufsize)
        return result;
    else
    {
        GetDateFormat(handle, DATE_USE_ALT_CALENDAR, &st, "gg", &buf[0], bufsize);
        return 0;
    }
}
