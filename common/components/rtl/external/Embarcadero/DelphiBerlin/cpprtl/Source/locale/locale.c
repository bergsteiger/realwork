 ...
 *      _lockLocale     - lock globale locale pointer
 *      _unlockLocale   - unlock globale locale pointer
 *
 * internal
...
/*
  ...
  in setLocale32x
*/
static LCID _getLocale(LPWIN32LOCALE _locale, LPSTR LocaleName);

/*
  frees the memory for the used categories
*/
static void _cleanCategories(LPWIN32LOCALE _locale);

/*
  Allocates memory for new categories and initialize them
*/
static int _setNewCategories(LPWIN32LOCALE _locale, SHORT Category);

/*
  Adds the category name to the list used by the locale
*/
static int _addCategoryName(LPWIN32LOCALE _locale, LPSTR category);
...
*/
static LPSTR _setLocaleFromString(LPWIN32LOCALE _locale, LPCSTR LocaleName);
...
static int _addCategoryName(LPWIN32LOCALE _locale, LPSTR category)
{
    ...
    strcat(LocaleName, ".");
    _ltoa(_locale->codepage, &LocaleName[strlen(LocaleName)], 10);
