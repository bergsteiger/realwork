...
int _RTLENTRY _EXPFUNC mbtowc_cp(wchar_t *pwc, const char *s, size_t n, const unsigned _codepage )

{
	...
	{
		if ( (int)n < MB_MAX_CHARLEN ||
			MultiByteToWideChar( _codepage, MB_PRECOMPOSED|MB_ERR_INVALID_CHARS,
			s, MB_MAX_CHARLEN, pwc, (pwc) ? 1: 0) == 0 )
		{
			if ( (int)n < MB_MAX_CHARLEN || (*(s+1) == 0 ))
				return -1;
...
/* Converts a wide char to MBCS char with '?' substitution */
int _RTLENTRY _EXPFUNC _wctomb_quiet( char *s, wchar_t wc)
{
	...
	else
	{
		...
		size = WideCharToMultiByte ( __locale->codepage, WC_COMPOSITECHECK|WC_SEPCHARS,
				&wc, 1, s, MB_MAX_CHARLEN, NULL, NULL);
		if (size == 0)
			return -1;
		return size;
	}
...
/* Converts a wide char to MBCS char with codepage		*/

int _RTLENTRY _EXPFUNC wctomb_cp(char *s, wchar_t wc, const unsigned _codepage )
{
	...
	else
	{
		...
		size = WideCharToMultiByte ( _codepage, WC_COMPOSITECHECK|WC_SEPCHARS,
				&wc, 1, s, MB_MAX_CHARLEN, NULL, &UsedDefault);
		if ( (size == 0) || (UsedDefault) )
			return -1;
		return size;
