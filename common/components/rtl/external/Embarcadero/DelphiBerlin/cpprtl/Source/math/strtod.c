...
double _RTLENTRY _EXPFUNC _tcstod (const _TCHAR *strP, _TCHAR **suffixPP)
{
        ...
        */
#if defined(_WIN64) /* double and long double are 8 bytes */
    return _tcstold(strP, suffixPP);
#else
        return __ldtrunc(DBL, _tcstold(strP, suffixPP), HUGE_VAL);
#endif
}

float _RTLENTRY _EXPFUNC _tcstof (const _TCHAR *strP, _TCHAR **suffixPP)
{
    /*
      __ldtrunc sets 'errno' to ERANGE if the result
      is to become 0 or HUGE_VAL.
    */
#if defined(_WIN64)
    // FIXME: need to implement real converter
    return _tcstold(strP, suffixPP);
#else
    return __ldtrunc(FLT, _tcstold(strP, suffixPP), HUGE_VAL);
#endif
}
