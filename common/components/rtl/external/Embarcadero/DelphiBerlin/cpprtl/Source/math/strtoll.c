 ...
 *        strtol - convert a string to a long long integer
...
Name            strtoll - convert a string to a long integer

Usage           long strtoll(const char *strP, char **suffixPP, int radix);

Prototype in    stdlib.h

Description     Convert  a string  to a long long integer.  The syntax  of the
                string must be:
...
Return value    If the  radix is invalid or  no number could be  found then
                ...
                If  the  number  overflows,  LLONG_MAX  or  LLONG_MIN will be
                returned and errno will be set to ERANGE.

*---------------------------------------------------------------------------*/

long long _RTLENTRY _EXPFUNC _tcstoll(const _TCHAR *strP, _TCHAR **suffixPP, int radix)
{
        int     charCt = 0;
        int     status;
        int     is_signed;
        ...
        {
                errno = ERANGE;
                result = is_signed ? LLONG_MIN : LLONG_MAX;
        }
