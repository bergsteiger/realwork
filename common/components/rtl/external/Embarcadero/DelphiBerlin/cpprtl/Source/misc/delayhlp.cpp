...
__delayLoadHelper2(
    ...
    if (0 == (idd.grAttrs & dlattrRva)) {
        ...
        RaiseException(
            BCPPException(ERROR_SEVERITY_ERROR, ERROR_INVALID_PARAMETER),
            0,
            1,
            PULONG_PTR(rgpdli)
            );
...
__FUnloadDelayLoadedDLL2(LPCSTR szDll) {
	...
	PUnloadInfo puiNext;
        size_t      cbDll = szDll ? __strlen(szDll): 0;
        
	for (pui = __puiHead; pui; pui = puiNext) {
		...
		//
		puiNext = pui->puiNext;
		if (szDll && ((cbName != cbDll) || (__memcmp(szDll, szName, cbName) != 0)))
			continue;
...
extern "C"
//WINAPI
void ShutdownDelayHelp2(void)
{
#ifdef __BORLANDC__
#pragma exit    ShutdownDelayHelp2 0 /* Finalizes the DLL delay load helper routines */
#endif
