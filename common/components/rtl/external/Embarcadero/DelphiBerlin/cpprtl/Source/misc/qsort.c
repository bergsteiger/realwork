...
typedef int  _USERENTRY comparF_s (const void *, const void *, void *);

static  _RTLTHREAD unsigned    qWidth;

#if defined(__SECURE)
static  _RTLTHREAD comparF_s  *Fcmp;
static  _RTLTHREAD void       *context;
#else
...
tailRecursion:
    if (nElem <= 2)
    {
        if (nElem == 2)
	{
            if (Fcmp (pivotP, rightP = qWidth + pivotP
#if defined(__SECURE)
		      ,context
#endif
		     ) > 0)
...
/*  sort the pivot, left, and right elements for "median of 3" */
    ...
    if (Fcmp (leftP, rightP
#if defined(__SECURE)
	      ,context
#endif
	     ) > 0)
        Exchange (leftP, rightP);
    if (Fcmp (leftP, pivotP
#if defined(__SECURE)
	      ,context
#endif
	     ) > 0)
        Exchange (leftP, pivotP);
    else if (Fcmp (pivotP, rightP
#if defined(__SECURE)
		   ,context
#endif
		  ) > 0)
...
/*  now for the classic Hoare algorithm */
    ...
    do
    {
        while ((retval = Fcmp(leftP, pivotP
#if defined(__SECURE)
			      ,context
#endif
	      )) <= 0)
...
#if defined(__SECURE)
errno_t _RTLENTRYF _EXPFUNC qsort_s(void *baseP, rsize_t nElem, rsize_t width,
				 comparF_s *compar, void *ctxt)
#else
...
#if defined(__SECURE)
    /* Constraints */
    if (nElem > RSIZE_MAX || width > RSIZE_MAX || (nElem && !(baseP && compar)))
    {
