 ...
 *
 * handle_backslashes - special case series of backslashes
 *
 * Follow the Microsoft rules for dealing with backslashes: 
 * 
 * If a series of backslashes are followed by a quote (") then they
 * are treated as escapes.  An odd number will escape the quote and it
 * will be inserted as a literal.  All pairs will be placed by single
 * backslashes.
 *
 * If a series of backslashes aren't followed by a quote then they
 * are treated as literal backslashes.
 */

static void handle_backslashes(_TCHAR **dst, _TCHAR **src)
{
    _TCHAR *p = *src;
    unsigned count = 0;
    unsigned literal;
