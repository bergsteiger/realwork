...
#if defined(_WIN64)
    /* if no handler set on thread then continue */
    if (!__Handler)
        return (XCPT_CONTINUE_SEARCH);
	
    sc._pexecptionrecord = ExceptionInfo->ExceptionRecord;
    sc._pcontext = ExceptionInfo->ContextRecord;
...
#if defined(_WIN64)
static void SetVectoredExceptionHandler()
{
    __Handler = AddVectoredExceptionHandler(1, VectoredHandler);
}

static void UnsetVectoredExceptionHandler()
{
    if (__Handler)
	RemoveVectoredExceptionHandler(__Handler);
}
#pragma exit UnsetVectoredExceptionHandler 0
#endif
