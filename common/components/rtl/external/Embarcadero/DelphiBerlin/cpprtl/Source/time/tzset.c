...
static _TCHAR _tDfltZone[ TZstrlen+1 ], _tDfltLight[ TZstrlen+1 ], _tDfltLight2[ TZstrlen+1 ];
// FIXME: having const below causes CLANG to generate bad object
_TCHAR * _RTLENTRY _EXPDATA _ttzname[3] = {&_tDfltZone[0], &_tDfltLight[0],&_tDfltLight2[0]};
...
*---------------------------------------------------------------------*/
void _RTLENTRY _EXPFUNC _ttzsetx(void)
{
#ifndef _UNICODE
...
#if defined(_UNICODE)
#define __ttzset_hook __set_wtzset_hook
#define ttzset_hook wtzset_hook
#else
#define __ttzset_hook ___set_tzset_hook
#define ttzset_hook tzset_hook
#endif

typedef void (*tzset_t)(void);
static tzset_t ttzset_hook = 0;

void _RTLENTRY _EXPFUNC __ttzset_hook(tzset_t tzfunc)
{
    ttzset_hook = tzfunc;
}
