...
#pragma option  -xX -xXd- -xv -Vo- -p- -O- -G-
...
#if !defined(_WIN32_WINNT_WS08) /* Can't use NTDDI_VERSION < 0x06000100 since headers have it wrong */
...
extern          void    _RTLENTRY _EXPFUNC  _NO_RETURN_DECL terminate (void);

#if defined(_DINKUMWARE)
extern          PFV     _RTLENTRY _EXPFUNC  set_unexpected(PFV) throw();
extern          PFV     _RTLENTRY _EXPFUNC  set_terminate (PFV) throw();
#else
...
// If you change EXCTAB, you must update definitions in the compiler sources:
//    indep\type.h - look for XCT_FIRST_OFF
typedef struct
{
        tpidPtr TPDST * xtThrowLst;             // throw-list address or NULL
	unsigned	xtVirtCondOffs;		// offset to virtual base class conditional init var
