...
uses
  System.RTLConsts, Data.DBConsts, System.Generics.Defaults
{$IFDEF MACOS}
...
begin
  ...
  begin
    ...
    begin
      ...
      if FDataSet.FieldList <> nil then
        FDataSet.FieldList.Updated := False;
      F.DisposeOf;
      lChanged := True;
    end;
...
begin
  ...
  if Result then
    case DataType of
      ...
      ftLongWord:
        Value := TDBBitConverter.UnsafeInto<LongWord>(FIOBuffer);
      else
...
begin
  ...
  if FIOBuffer <> nil then
    TDBBitConverter.UnsafeFrom<LongInt>(Value, FIOBuffer);
  SetData(FIOBuffer);
end;
...
begin
  ...
  begin
    ...
    if (Value < 0) or (Value > High(LongWord)) then
      RangeError(Value, FMinRange, FMaxRange);
    CheckRange(Value, FMinRange, FMaxRange);
  end;
  if FIOBuffer <> nil then
    TDBBitConverter.UnsafeFrom<LongWord>(Value, FIOBuffer);
  SetData(FIOBuffer);
end;
...
procedure TSingleField.SetAsInteger(Value: Longint);
begin
  SetAsSingle(Value);
end;
...
begin
  Result := LongWord(Round(GetAsExtended));
end;
...
function TMemoField.GetAsInteger: Longint;
begin
  Result := StrToInt(GetAsString);
end;
...
function TWideMemoField.GetAsInteger: Longint;
begin
  Result := StrToInt(GetAsString);
end;
...
procedure TWideMemoField.SetAsInteger(Value: Longint);
begin
  SetAsString(IntToStr(Value));
end;
...
begin
  if Source is TADTField then
    ...
    begin
      SourceField := TADTField(Source).Fields.FieldByName(Fields[I].FieldName);
...
{$ENDIF !NEXTGEN}
    ...
    ftLongWord: TDBBitConverter.UnsafeFrom<LongWord>(GetAsLongWord, Buffer);
...
{$ENDIF !NEXTGEN}
    ...
    ftLongWord: AsLongWord := TDBBitConverter.UnsafeInto<LongWord>(Buffer);
  ...
  begin
    ...
    begin
      ...
      if (FieldDef.DataType <> ftUnknown) and
        ...
        ((FieldOptions.AutoCreateMode <> acCombineAlways) or (Fields.FindField(FieldDef.Name) = nil)) then
      begin
  ...
  begin
    ...
    begin
      ...
      if (FieldDef.DataType <> ftUnknown) and not (FieldDef.DataType in ObjectFieldTypes) and
        not ((faHiddenCol in FieldDef.Attributes) and not FieldDefs.HiddenFields) and
        ((FieldOptions.AutoCreateMode <> acCombineAlways) or (FieldList.Find(FieldDefList.Strings[I]) = nil)) then
      begin
...
var
  Count: Integer;
begin
  if not IsUniDirectional and Active then
  begin
...
begin
  ...
  begin
    ...
    if CurIndex < NewIndex then
      Move(FBuffers[CurIndex + 1], FBuffers[CurIndex], (NewIndex - CurIndex) * SizeOf(NativeInt))
    else
      Move(FBuffers[NewIndex], FBuffers[NewIndex + 1], (CurIndex - NewIndex) * SizeOf(NativeInt));
