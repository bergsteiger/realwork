...
type
  ...
  public
    ...
    function BuildAuthorizationString(const StringToSign, DateISO, Region, SignedStrHeaders: string): string; reintroduce;
  end;
  ...
  TAmazonRegion = (amzrNotSpecified, amzrEUWest1, amzrEUCentral1, amzrUSEast1, amzrUSWest1, amzrUSWest2,
                   amzrAPSoutheast1, amzrAPSoutheast2, amzrAPNortheast1, amzrSAEast1, amzrUSClassic, amzrEU);
...
function TAmazonAWS4Authentication.BuildAuthorizationString(const StringToSign, DateISO, Region, SignedStrHeaders: string): string;
...
var
  ...
  SigningKey : TBytes;
begin
  SigningKey := GetSignatureKey(DateISO, Region, 's3');
  Credentials   := 'Credential='+FConnectionInfo.AccountName + '/'+ DateISO + '/'+Region+ '/s3' + '/aws4_request'+',';
...
begin
  ...
  if Endpoint.Contains('s3-sa-east-1.amazonaws.com') then
    ...
    Exit('us-east-1');
    //raise Exception.Create('Endpoint: '+Endpoint+' not found, please review compatible endpoints');
end;
...
begin
  if AnsiSameText(Region, 'EU') then
    Exit(amzrEUWest1);
  if AnsiSameText(Region, 'eu-west-1') then
    Exit(amzrEUWest1);
  if AnsiSameText(Region, 'eu-central-1') then
    Exit(amzrEUCentral1);
  if AnsiSameText(Region, 'us-east-1') then
    Exit(amzrUSClassic);
  if AnsiSameText(Region, 'us-west-1') then
    Exit(amzrUSWest1);
  if AnsiSameText(Region, 'us-west-2') then
    Exit(amzrUSWest2);
  if AnsiSameText(Region, 'ap-southeast-1') then
    Exit(amzrAPSoutheast1);
  if AnsiSameText(Region, 'ap-southeast-2') then
    Exit(amzrAPSoutheast2);
  if AnsiSameText(Region, 'ap-northeast-1') then
    Exit(amzrAPNortheast1);
  if AnsiSameText(Region, 'sa-east-1') then
    Exit(amzrSAEast1)
  else
    Exit(amzrNotSpecified); //Exit(amzrUSClassic);
end;
...
begin
  case Region of
    amzrEU: Result := 's3-eu-west-1.amazonaws.com';
    ...
    amzrNotSpecified: Result := EmptyStr;
    amzrUSClassic: Result := 's3.amazonaws.com';
  end;
end;
...
begin
  case Region of
    ...
    amzrUSClassic: Result := 'us-east-1';
    amzrNotSpecified: Result := EmptyStr;
  end;
end;
...
begin
    if location = amzrUSClassic then
...
begin
  ...
  begin
    if GetRegionFromEndpoint(GetConnectionInfo.FStorageEndpoint) <> GetRegionString(BucketRegion) then
      virtualhost:= GetVirtualHostFromRegion(BucketName,amzrUSClassic)
    else
...
begin
  ...
  try
    ...
    if GetRegionFromEndpoint(GetConnectionInfo.GetStorageEndpoint) <> GetRegionString(location) then
    begin
      virtualhost:= GetVirtualHostFromRegion(BucketName,location);
      url := GetConnectionInfo.GetServiceURL(virtualhost)+ '/';
      Headers.Values['host'] := virtualhost;
    end
    else
...
begin
  ...
  try
    ...
    if GetRegionFromEndpoint(GetConnectionInfo.FStorageEndpoint) <> GetRegionString(location) then
    begin
      virtualhost:= GetVirtualHostFromRegion(BucketName,location);
      url := GetConnectionInfo.GetServiceURL(virtualhost)+ '/';
      Headers.Values['host'] :=  virtualhost;
