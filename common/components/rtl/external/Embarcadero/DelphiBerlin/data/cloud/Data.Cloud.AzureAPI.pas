...
type
  ...
  private
    ...
    function HandleContainerLease(const ContainerName: string;
                                  LeaseAction: TAzureContainerLeaseActionMode;
                                  var LeaseId: string; const ProposedLeaseId: string;
                                  LeaseDuration: Integer;
                                  ResponseInfo: TCloudResponseInfo): Boolean;
    function HandleBlobLease(ContainerName, BlobName: string; const LeaseAction, LeaseId, ProposedLeaseId: string;
                             SuccessCode: Integer;
                             out Success: Boolean; ResponseInfo: TCloudResponseInfo; LeaseDuration: Integer): string;
  ...
  public
    ...
    ///          Or passing the single SignedIdentifier's Id and AccessPolicy.
    /// </remarks>
    /// <param name="AContainerName">The name of the container to set the signed identifier for</param>
    /// <param name="ASignedIdentifierId">The unique ID of the single signed identifier to add/update</param>
    /// <param name="AAccessPolicy">The SignedIdentifier's access policy</param>
    ...
    function SetContainerACL(ContainerName: string;
                             ...
                             PublicAccess: TBlobPublicAccess = bpaContainer;
                             ResponseInfo: TCloudResponseInfo = nil;
                             const AClientRequestID: string = ''): Boolean; overload;
    ...
    /// <param name="LeaseDuration">Specifies the duration of the lease, in seconds, or negative one (-1) for a lease that never expires. A non-infinite lease can be between 15 and 60 seconds.</param>
    /// <param name="ResponseInfo">The optional class for storing response info into.</param>
    /// <returns>True if the lease was acquired, false otherwise.</returns>
    function AcquireContainerLease(const ContainerName: string; var LeaseId: string; LeaseDuration: Integer;
      ResponseInfo: TCloudResponseInfo): Boolean;
    ...
    function RenewContainerLease(const ContainerName: string; var LeaseId: string;
      ResponseInfo: TCloudResponseInfo): Boolean;
    ...
    function ReleaseContainerLease(const ContainerName: string; var LeaseId: string;
      ResponseInfo: TCloudResponseInfo = nil): Boolean;
    ...
    /// <param name="ResponseInfo">The optional class for storing response info into.</param>
    /// <returns>True if the call executes successfully, false otherwise</returns>
    function BreakContainerLease(const ContainerName: string; out LeaseTimeRemaining: Integer;
      ResponseInfo: TCloudResponseInfo): Boolean;
    /// <summary>Changes the ID of an existing lease.</summary>
    /// <param name="ContainerName">The name of the container.</param>
    /// <param name="LeaseID">The actual lease ID.</param>
    /// <param name="ProposedLeaseID">The new lease ID.</param>
    /// <param name="ResponseInfo">The optional class for storing response info into.</param>
    /// <returns>True if the call executes successfully, false otherwise</returns>
    function ChangeContainerLease(const ContainerName: string; var LeaseID: String; const ProposedLeaseID: string;
      ResponseInfo: TCloudResponseInfo): Boolean;
    ...
    /// <param name="ResponseInfo">The optional class for storing response info into</param>
    /// <param name="LeaseDuration">Specifies the duration of the lease, in seconds, or negative one (-1) for a lease that never expires. A non-infinite lease can be between 15 and 60 seconds</param>
    /// <param name="ProposedLeaseID">Allow the option for users to propose the lease-id</param>
    /// <returns>True if the lease was acquired, false otherwise</returns>
    ...
    /// <summary>Changes the ID of an existing lease</summary>
    /// <param name="ContainerName">The name of the container the blob is in</param>
    /// <param name="BlobName">The name of the blob to change the lease ID</param>
    /// <param name="LeaseID">The actual lease ID</param>
    /// <param name="ProposedLeaseID">The new lease ID</param>
    /// <param name="ResponseInfo">The optional class for storing response info into</param>
    /// <returns>True if the call executes successfully, false otherwise</returns>
    function ChangeBlobLease(const ContainerName, BlobName: string;
                             const LeaseID, ProposedLeaseID: string;
                             ResponseInfo: TCloudResponseInfo): Boolean;
...
begin
  ...
  begin
    ...
    try
      ...
      URL := Format('%s/%s', [GetConnectionInfo.BlobURL, Name]);
...
begin
  ...
  try
    ...
    Response := IssuePutRequest(url, Headers, LQueryParams, QueryPrefix, ResponseInfo, contentStream);
    Result := (Response <> nil) and (Response.ResponseCode = 201);
  finally
    if Assigned(Response) then
      FreeAndNil(Response);
...
begin
  ...
  try
    LHeaders := BuildMetadataHeaderList(AMetaData);
    PopulateDateHeader(LHeaders, False);
    LHeaders.Values['x-ms-blob-type'] := 'AppendBlob';

    if AOptionalHeaders <> nil then
      LHeaders.AddStrings(AOptionalHeaders);
...
begin
  ...
  try
    ...
    Result := SetContainerACL(ContainerName, SignedIdentifierId, acp, PublicAccess, '', ResponseInfo);
  finally
    acp.Free;
  end;
end;
...
function TAzureBlobService.SetContainerACL(const AContainerName: string; const ASignedIdentifierId: string;
  ...
  const AResponseInfo: TCloudResponseInfo): Boolean;
var
  LSignedIdentifiers: TList<TSignedIdentifier>;
  LSignedIdentifier: TSignedIdentifier;
begin
  LSignedIdentifier := nil;
  LSignedIdentifiers := TList<TSignedIdentifier>.Create;
  try
    //create a signed identifier list with the single signed identifier,
    //then call the function that takes a list.
    LSignedIdentifier := TSignedIdentifier.Create(AContainerName, AAccessPolicy, ASignedIdentifierId);
    LSignedIdentifiers.Add(LSignedIdentifier);
    Result := SetContainerACL(AContainerName, LSignedIdentifiers, APublicAccess, AResponseInfo);
  finally
    LSignedIdentifier.Free;
    LSignedIdentifiers.Free;
  end;
end;
...
begin
  ...
  if (SourceContainerName = EmptyStr) or (SourceContainerName = ROOT_CONTAINER) then
    CopySourceVal := Format('https://%s.blob.core.windows.net/%s', [GetConnectionInfo.AccountName, SourceBlobName])
  else
    CopySourceVal := Format('https://%s.blob.core.windows.net/%s/%s', [GetConnectionInfo.AccountName, SourceContainerName, SourceBlobName]);
    ...
    begin
      try
        ...
        begin
          ...
          begin
            Policy := TBlobPolicy.Create;
            try
              SignedIdentifier := TSignedIdentifier.Create(ContainerName, Policy, ChildNode.Text);
...
function TAzureBlobService.HandleContainerLease(const ContainerName: string;
  LeaseAction: TAzureContainerLeaseActionMode; var LeaseId: string; const ProposedLeaseId: string;
  LeaseDuration: Integer; ResponseInfo: TCloudResponseInfo): Boolean;
var
  url: string;
  Headers: TStringList;
  QueryPrefix: string;
...
begin
  ...
  FreeResponseInfo := False;
  try
    url := GetConnectionInfo.BlobURL + '/' + ContainerName.ToLower;
    ...
    case LeaseAction of
      clamAcquire:
        begin
          Headers.Values['x-ms-lease-action'] := 'acquire';
          if ProposedLeaseId <> '' then
            Headers.Values['x-ms-proposed-lease-id'] := ProposedLeaseId;
          Headers.Values['x-ms-lease-duration'] := LeaseDuration.ToString;
          SuccessCode := 201;
        end;
      clamRenew:
        begin
          Headers.Values['x-ms-lease-action'] := 'renew';
          Headers.Values['x-ms-lease-id'] := LeaseId;
          SuccessCode := 200;
        end;
      clamChange:
        begin
          Headers.Values['x-ms-lease-action'] := 'change';
          Headers.Values['x-ms-lease-id'] := LeaseId;
          Headers.Values['x-ms-proposed-lease-id'] := ProposedLeaseId;
          SuccessCode := 200;
        end;
      clamRelease:
        begin
          Headers.Values['x-ms-lease-action'] := 'release';
          Headers.Values['x-ms-lease-id'] := LeaseId;
          SuccessCode := 200;
        end;
      clamBreak:
        begin
          Headers.Values['x-ms-lease-action'] := 'break';
          SuccessCode := 202;
        end;
      else
        SuccessCode := -1;
    ...
    QueryPrefix := GetBlobContainerQueryPrefix(Headers, ContainerName.ToLower, '');
    ...
    Response := IssuePutRequest(url, Headers, QueryParams, QueryPrefix, ResponseInfo);
    Result := Response.ResponseCode = SuccessCode;
  finally
    Response.Free;
    if FreeResponseInfo then
      ResponseInfo.Free;
...
function TAzureBlobService.HandleBlobLease(ContainerName, BlobName: string; const LeaseAction,
                                           LeaseId, ProposedLeaseId: string; SuccessCode: Integer; out Success: Boolean;
                                           ResponseInfo: TCloudResponseInfo; LeaseDuration: Integer): string;
var
  url: string;
  Headers: TStringList;
  QueryPrefix: string;
...
function TAzureBlobService.AcquireBlobLease(ContainerName, BlobName: string; out LeaseId: string;
                                           ...
                                           const ProposedLeaseID: string): Boolean;
begin
  LeaseId := HandleBlobLease(ContainerName, BlobName, 'acquire', '', ProposedLeaseID, 201, Result, ResponseInfo, LeaseDuration);
end;

function TAzureBlobService.AcquireContainerLease(const ContainerName: string; var LeaseId: string;
  LeaseDuration: Integer; ResponseInfo: TCloudResponseInfo): Boolean;
var
  FreeResponseInfo: Boolean;
begin
  ...
  try
    ...
    Result := HandleContainerLease(ContainerName, clamAcquire, LeaseId, LeaseId, LeaseDuration, ResponseInfo);
    if Result then
      LeaseID := ResponseInfo.Headers.Values['x-ms-lease-id'];
  finally
    if FreeResponseInfo then
      ResponseInfo.Free;
  end;
...
function TAzureBlobService.RenewBlobLease(ContainerName, BlobName: string; const LeaseId: string;
                                          ResponseInfo: TCloudResponseInfo): Boolean;
begin
  HandleBlobLease(ContainerName, BlobName, 'renew', LeaseId, '', 200, Result, ResponseInfo, -1);
end;

function TAzureBlobService.RenewContainerLease(const ContainerName: string; var LeaseId: string;
  ResponseInfo: TCloudResponseInfo): Boolean;
begin
  Result := HandleContainerLease(ContainerName, clamRenew, LeaseId, '', -1, ResponseInfo);
end;

function TAzureBlobService.ReleaseBlobLease(ContainerName, BlobName: string; const LeaseId: string;
                                            ResponseInfo: TCloudResponseInfo): Boolean;
begin
  HandleBlobLease(ContainerName, BlobName, 'release', LeaseId, '', 200, Result, ResponseInfo, -1);
end;

function TAzureBlobService.ReleaseContainerLease(const ContainerName: string; var LeaseId: string;
  ResponseInfo: TCloudResponseInfo): Boolean;
begin
  Result := HandleContainerLease(ContainerName, clamRelease, LeaseId, '', -1, ResponseInfo);
end;
...
begin
  ...
  try
    HandleBlobLease(ContainerName, BlobName, 'break', '', '', 202, Result, ResponseInfo, -1);
    if Result then
    begin
      TimeRemaining := ResponseInfo.Headers.Values['x-ms-lease-time'];
...
function TAzureBlobService.BreakContainerLease(const ContainerName: string; out LeaseTimeRemaining: Integer;
  ResponseInfo: TCloudResponseInfo): Boolean;
var
  FreeResponseInfo: Boolean;
  LeaseID: string;
begin
  ...
  try
    ...
    Result := HandleContainerLease(ContainerName, clamBreak, LeaseID, '', -1, ResponseInfo);
    if Result then
      LeaseTimeRemaining := StrToIntDef(ResponseInfo.Headers.Values['x-ms-lease-time'], -1);
  finally
    if FreeResponseInfo then
      ResponseInfo.Free;
  end;
...
function TAzureBlobService.ChangeBlobLease(const ContainerName, BlobName,
  LeaseID, ProposedLeaseID: string; ResponseInfo: TCloudResponseInfo): Boolean;
begin
  HandleBlobLease(ContainerName, BlobName, 'change', LeaseId, ProposedLeaseID, 200, Result, ResponseInfo, -1);
end;

function TAzureBlobService.ChangeContainerLease(const ContainerName: string; var LeaseID: String;
  const ProposedLeaseID: string; ResponseInfo: TCloudResponseInfo): Boolean;
begin
  Result := HandleContainerLease(ContainerName, clamChange, LeaseId, ProposedLeaseID, -1, ResponseInfo);
end;
      ...
      begin
        try
          ...
          begin
            ...
            BlobURL := rootnode.attributes['ServiceEndpoint'] + rootnode.attributes['ContainerName'] + '/' + blobName;
