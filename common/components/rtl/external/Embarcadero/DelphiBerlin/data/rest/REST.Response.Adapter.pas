...
begin
  ...
  begin
    ...
    begin
      if APath <> '' then
      begin
        AList.Add(TAdaptJSONField.Create(APath, APath, TAdaptJSONFieldType.JSONObject));
        LDepth := ADepth - 1;
      end
      else
        LDepth := ADepth;
      ...
      begin
        AddJSONFieldNames(LPair.JsonValue, LPath + LPair.JsonString.Value, AList, LDepth);
      end;
    end
    else if AParent is TJSONArray then
    begin
      if APath <> '' then
        AList.Add(TAdaptJSONField.Create(APath, APath, TAdaptJSONFieldType.JSONArray));
    end
    else
    begin
      if APath <> '' then
        AList.Add(TAdaptJSONField.Create(APath, APath, TAdaptJSONFieldType.JSONValue));
    end;
...
begin
  try
    if Assigned(FAdapter) then
      FAdapter.UpdateDataSet(GetJSONValue);
  except
