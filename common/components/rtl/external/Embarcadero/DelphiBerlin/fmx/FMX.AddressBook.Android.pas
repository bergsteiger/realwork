...
uses
  System.Generics.Collections, System.Rtti, Androidapi.JNI.GraphicsContentViewText, Androidapi.JNI.JavaTypes,
  Androidapi.JNI.Provider, Androidapi.JNIBridge, Androidapi.JNI.Embarcadero, Androidapi.JNI.Net, FMX.Graphics,
  FMX.AddressBook, FMX.AddressBook.Types, FMX.Surfaces;
...
uses
  System.SysUtils, System.Classes, System.Math, AndroidApi.Helpers, Androidapi.JNI,  Androidapi.JNI.Accounts,
  FMX.Helpers.Android, FMX.Platform, FMX.Consts, FMX.Types;
...
begin
  ...
  try
    try
      ...
      raise EAddressBookExecute.CreateFmt(SCannotCheckExisitingDataRecord, [E.Message]);
    end;
  finally
    QueryParams.Free;
    Projection.Free;
...
procedure TAndroidAddressBook.RequestPermission;
var
  Message: string;
  AccessGranted: Boolean;
  ReadAuthorizationStatus: TAuthorizationStatus;
  WriteAuthorizationStatus: TAuthorizationStatus;
begin
  ReadAuthorizationStatus := AuthorizationStatus(TAddressBookAccessType.Read);
  WriteAuthorizationStatus := AuthorizationStatus(TAddressBookAccessType.Write);

                                                                                          
  if (ReadAuthorizationStatus = TAuthorizationStatus.Authorized) and (WriteAuthorizationStatus = TAuthorizationStatus.Authorized) then
    AccessGranted := True
  else if (ReadAuthorizationStatus = TAuthorizationStatus.Authorized) and (WriteAuthorizationStatus <> TAuthorizationStatus.Authorized) then
  begin
...
const
  ...
  ReadContactPermission = 'android.permission.READ_CONTACTS';
var
  PackageName: JString;

  function CheckPermission(const APermissionName: string): TAuthorizationStatus;
  begin
    if TAndroidHelper.Context.getPackageManager.checkPermission(StringToJString(APermissionName), PackageName) =
...
begin
  ...
  case AAccessType of
    TAddressBookAccessType.Read:
      Result := CheckPermission(ReadContactPermission);
    TAddressBookAccessType.Write:
      Result := CheckPermission(WriteContactsPermission);
  else
    Result := TAuthorizationStatus.Denied;
  end;
end;
...
function TAndroidAddressBook.CheckPermission(const APermissionName: string): Boolean;
var
  PackageName: JString;
begin
  PackageName := TAndroidHelper.Context.getPackageName;
...
constructor TAndroidAddressBook.Create;
begin
  inherited;
...
begin
  ...
  else if not CheckPermission('android.permission.GET_ACCOUNTS') then
    raise EAddressBookAccess.Create('Cannot read sources, because your application doesn''t have [GET_ACCOUNTS] permission');
