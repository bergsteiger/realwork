...
{ TControl }
  ...
  public
    ...
    property Visible: Boolean read FVisible write SetVisible stored VisibleStored nodefault;
...
{ TPathAnimation }
  ...
  strict private
    [Weak] FInflatable: IInflatableContent<T>;
    FBusy: Boolean;
...
begin
  ...
  try
    ...
    begin
      ...
      TControl(AObject).RecalcEnabled;
...
begin
  ...
  try
    ...
    begin
      ...
      NeedUpdate := (LParent <> nil) and (not TCommonCustomForm(LParent).Released);
    end;
    ...
    begin
      ...
      if NeedUpdate and (TControl(AObject).Align <> TAlignLayout.None) then
        Realign;
    end;
    if AObject is TEffect then
    begin
...
var
  I: Integer;
begin
  if (AScene = nil) and FIsFocused and not (csDestroying in ComponentState) then
    Exit;
  FScene := AScene;
  if FControls <> nil then
    for I := 0 to FControls.Count - 1 do
...
begin
  ...
  begin
    ...
    FreeStyle;
    if (csDesigning in ComponentState) or ((Scene <> nil) and (Scene.StyleBook <> nil)) then
      SaveResourceLink.Release
    else
...
begin
  ...
  if (OldScene <> AScene) and (FScene <> nil) and (FScene.StyleBook <> nil) and (not IsUpdating) then
    NeedStyleLookup;
end;
