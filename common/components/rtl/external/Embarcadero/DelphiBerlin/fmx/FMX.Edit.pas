...
uses
  ...
  {$IFDEF MSWINDOWS}, FMX.Edit.Win{$ENDIF}{$IFDEF IOS}, FMX.Edit.iOS{$ENDIF}, FMX.BehaviorManager;
...
begin
  ...
  if FindWordBound(Text, GetOriginCaretPosition, SelBeg, SelEnd) then
  begin
...
begin
  ...
  begin
    OldText := FText;
    FText := DoValidating(TruncateText(FilterText(Value, FilterChar), MaxLength));
    if FText <> OldText then
    begin
      DoChangeTracking;
      SendMessage<string>(MM_EDIT_TEXT_CHANGED, FText);
