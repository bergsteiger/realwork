...
begin
  inherited;
  if not Enabled and (Model <> nil) then
    Model.HideEditor(False);
end;
...
var
  LDate: TDateTime;
begin
  if not ValueIsEmpty(Value) and Value.TryAsType<TDateTime>(LDate) then
  begin
    if Format.IsEmpty then
      Result := DateToStr(LDate)
    else
      Result := FormatDateTime(Format, LDate);
  end
  else
...
begin
  ...
  begin
    ...
    OldRow := FRow;
    try
      if AObservable and (Owner is TPresentedControl) then
...
begin
  Result := TGridOption.Editing in Options;
end;
