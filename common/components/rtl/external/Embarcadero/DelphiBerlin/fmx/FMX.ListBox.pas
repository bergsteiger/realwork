...
{ TCustomListBox }
  ...
  const
    lsVertical = TListStyle.Vertical deprecated 'Use TListStyle.Vertical';
    lsHorizontal = TListStyle.Horizontal deprecated 'Use TListStyle.Horizontal';
  end;
  TListGroupingKind = (Plain, Grouped);

  TListGroupingKindHelper = record helper for TListGroupingKind
  const
    gsPlain = TListGroupingKind.Plain deprecated 'Use TListGroupingKind.Plain';
    gsGrouped = TListGroupingKind.Grouped deprecated 'Use TListGroupingKind.Grouped';
  end;
  ...
  public type
    ...
    const
      tsoAdded = TStringsChangeOp.Added deprecated 'Use TStringsChangeOp.Added';
      tsoDeleted = TStringsChangeOp.Deleted deprecated 'Use TStringsChangeOp.Deleted';
      tsoClear = TStringsChangeOp.Clear deprecated 'Use TStringsChangeOp.Clear';
    end;
  ...
  strict private
    ...
    procedure InitSelectionObjects;
    procedure FreeSelectionObjects;
...
begin
  if (FIcon <> nil) and FIcon.Visible then
    ImagesChanged;
end;
...
begin
  ...
  begin
    ...
    try
      PaintStage := TPaintStage.Background;
      inherited;
      PaintStage := TPaintStage.Text;
      inherited;
    finally
...
begin
  ...
  begin
    ...
    begin
      ...
      if SameValue(JoinedRect.Height, R.Height, TEpsilon.Position) then
...
procedure TCustomListBox.InitSelectionObjects;
begin
  if FSelections = nil then
    FSelections := TControlList.Create;
end;

procedure TCustomListBox.FreeSelectionObjects;
var
  I: Integer;
  Sel: TFmxObject;
begin
...
begin
  if (Selection = nil) or (FUpdating > 0) then
    Exit;
  ...
  try
    ...
    // create selections
    for I := 0 to SelRects.Count - 1 do
    begin
      if Self.IsFocused and (FFocusedSelection <> nil) then
        CloneObj := FFocusedSelection.Clone(Self)
      else
        CloneObj := Selection.Clone(Self);
      Clone := TControl(CloneObj);
{$IFDEF AUTOREFCOUNT}
      CloneObj := nil;
{$ENDIF}
    ...
    if SelRects.Count < FSelections.Count then
      for I := SelRects.Count to FSelections.Count - 1 do
    ...
    if (MultiSelectStyle = TMultiSelectStyle.Extended) and (ItemIndex >= 0) and (ItemIndex < Count) then
    begin
      ...
      R := TRectF.Create(P.X, P.Y, P.X + Item.Width, P.Y + Item.Height);
...
begin
  ...
  begin
    ...
    begin
      ...
      R.TopLeft := ContentLayout.AbsoluteToLocal(R.TopLeft);
      R.BottomRight := ContentLayout.AbsoluteToLocal(R.BottomRight);
      if R.Bottom < 0 then
        First := Mid + 1
      else if R.Top >= ContentLayout.Height then
        Last := Mid
      else
        Break;
    end;
...
begin
  ...
  if Observers.IsObserving(TObserverMapping.ControlValueID) then
    TLinkObservers.ControlValueUpdate(Observers);
  if (FListPicker <> nil) and (TDropDownKind.Native = DropDownKind) and
...
begin
  ...
  inherited;
  if (Count > 0) and
     ((Key in [vkEscape, vkHome, vkEnd]) or ([ssAlt, ssCtrl, ssShift, ssCommand] * Shift = []) or
...
var
  ...
  LItem: TListBoxItem;
begin
  if (not (csLoading in ComponentState)) and (FImageLink <> nil) and (FImageLink.Images <> FImages) then
  begin
    FImages := Images;
...
begin
  AddEnumElementAliases(TypeInfo(TListStyle), ['lsVertical', 'lsHorizontal']);
  AddEnumElementAliases(TypeInfo(TListGroupingKind), ['gsPlain', 'gsGrouped']);
  AddEnumElementAliases(TypeInfo(TCustomListBox.TStringsChangeOp), ['tsoAdded', 'tsoDeleted', 'tsoClear']);
end;

procedure UnregisterAliases;
begin
  RemoveEnumElementAliases(TypeInfo(TListStyle));
  RemoveEnumElementAliases(TypeInfo(TListGroupingKind));
  RemoveEnumElementAliases(TypeInfo(TCustomListBox.TStringsChangeOp));
end;
