...
begin
  ...
  if Succeeded(CoCreateInstance(CLSID_FilterGraph, nil, CLSCTX_INPROC_SERVER, IID_IGraphBuilder, FGraphBuilder)) and
    Succeeded(FGraphBuilder.QueryInterface(IID_IMediaControl, FMediaControl)) then
  begin
    {VMR-9 filter creation}
    if Succeeded(CoCreateInstance(CLSID_VideoMixingRenderer9, nil, CLSCTX_INPROC, IID_IBaseFilter, FVMR)) and
      ...
      Succeeded(FVMRFilterConfig9.SetRenderingMode(VMR9Mode_Windowless)) then
    begin
      ...
      begin
        ...
        {VMR-7 filter creation when VMR-9 is not availiable}
        if Succeeded(CoCreateInstance(CLSID_VideoMixingRenderer, nil, CLSCTX_INPROC, IID_IBaseFilter, FVMR)) and
          ...
          Succeeded(FVMRFilterConfig7.SetRenderingMode(VMRMode_Windowless)) then
        begin
      ...
      // Create GraphTree
      if Succeeded(FGraphBuilder.RenderFile(PChar(AFileName), nil)) then
      begin
        if FMediaControl <> nil then
          FMediaControl.StopWhenReady;
        // Get IMediaSeeking
