...
{ TStyledMemo }
  ...
  protected type
    ...
    private
      ...
      function GetLayoutShift: Single;
...
begin
  if Model.Lines.Count > 0 then
  begin
    Point := TPointF.Create(FLineObjects.GetLayoutShift - ViewportPosition.X, Model.Caret.Pos.Y - ViewportPosition.Y +
      GetLineHeight / 2);
...
begin
  ...
  begin
    ...
    Result := TPointF.Create(FLineObjects.GetLayoutShift, 0);
end;
...
begin
  ...
  FLineObjects.FDefaultMetrics := TSizeF.Create(-1, -1);
...
begin
  ...
  begin
    ...
    begin
      Line := FLineObjects[I];
      Line.Rect.Location := Point + TPointF.Create(FLineObjects.GetLayoutShift, 0);
      if Line.Layout <> nil then
...
begin
  FCursorFill.Free;
  FCursorFill := TBrush.Create(TBrushKind.Solid, Model.TextSettingsInfo.ResultingTextSettings.FontColor);
  FLineObjects.FDefaultMetrics := InvalidSize;
...
begin
  ...
  begin
    ...
    begin
      ...
      {$ELSE}
      if LinesToText(Model.Lines) <> FTextService.CombinedText then
      begin
        TextChanged := True;
...
var
  Layout: TTextLayout;
begin
  if (FDefaultMetrics.Width < 0) or (FDefaultMetrics.Height < 0) then
  begin
    Layout := CreateLayout('Sample Text');
    try
      FDefaultMetrics := TSizeF.Create(Layout.TextRect.Left, Layout.Height);
    finally
      Layout.Free;
    end;
  end;
...
function TStyledMemo.TLines.GetLayoutShift: Single;
begin
  CalculateDefaultLineMetrics;
  Result := FDefaultMetrics.Width;
end;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          Layout.TopLeft := FLines[I].Rect.TopLeft - TPointF.Create(GetLayoutShift, 0);
        end;
...
begin
  ...
  begin
    OldLine.Layout.TopLeft := NewLine.Rect.Location - TPointF.Create(GetLayoutShift, 0);
    NewLine.Layout.TopLeft := TopLeft - TPointF.Create(GetLayoutShift, 0);
  end
  else
  begin
    OldLine.FreeLayout;
...
begin
  ...
  Line.Rect := TRectF.Create(GetLayoutShift - FMemo.ViewportPosition.X, 0, GetLayoutShift + Max(0, Line.Size.Width), Line.Size.Height);
  if (Index > 0) then
  ...
  if Line.Layout <> nil then
    Line.Layout.TopLeft := Line.Rect.TopLeft - TPointF.Create(GetLayoutShift, 0);
  if IsWordWrap then
  ...
  begin
    ...
    begin
      ...
      if FLines[I].Layout <> nil then
        FLines[I].Layout.TopLeft := FLines[I].Rect.TopLeft - TPointF.Create(GetLayoutShift, 0);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Line.Rect := TRectF.Create(GetLayoutShift - ViewPosition.X, 0, GetLayoutShift - ViewPosition.X + Line.Size.Width, Line.Size.Height);
        if I > 0 then
          Line.Rect.Offset(0, FLines[I - 1].Rect.Bottom)
        else
          Line.Rect.Offset(0, -ViewPosition.Y);
        if RectsIntersect(Content, FLines[I].Rect) then
          Layout.TopLeft := Line.Rect.TopLeft - TPointF.Create(GetLayoutShift, 0)
        else
          Line.FreeLayout;
        if IsWordWrap then
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if FLines[J].Layout <> nil then
                FLines[J].Layout.TopLeft := FLines[J].Rect.TopLeft - TPointF.Create(GetLayoutShift, 0);
            end;
          end;
        end
      end
...
begin
  ...
  begin
    ...
    Line.FreeLayout;
  Line.Rect := TRectF.Create(GetLayoutShift - FMemo.ViewportPosition.X, 0, GetLayoutShift + Max(0, Line.Size.Width),
    Line.Size.Height);
  if Index > 0 then
    Line.Rect.Offset(0, FLines[Index - 1].Rect.Bottom)
  else
...
begin
  if (FMemo.Model.Caret <> nil) and (FMemo.Model.Caret.Flasher <> nil) then
    ...
    FlasherWidth := 0;
  ContentBounds.Width := ContentBounds.Width + GetLayoutShift + Max(1, FlasherWidth);
