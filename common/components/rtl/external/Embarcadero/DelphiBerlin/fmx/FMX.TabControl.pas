...
begin
  Result := (TabControl <> nil) and TabControl.HasTouchScreen and not (csDesigning in ComponentState) and
    not Released;
end;
...
begin
  if not TabControl.Released and (([csLoading, csUpdating] * TabControl.ComponentState) = []) then
  begin
...
begin
  ...
  begin
    ...
    begin
      TTabItem(AObject).ShowAsDot := EffectiveTabPosition
        in [TTabPosition.Dots, TTabPosition.None];
      RealignTabs;
    ...
    begin
      ...
      if IndexOfLastTab < FStoredTabSize.Count then
        Tabs[IndexOfLastTab].Content.BoundsRect :=
          TRectF.Create(Tabs[IndexOfLastTab].Content.Position.Point,
          FStoredTabSize[IndexOfLastTab].cx, FStoredTabSize[IndexOfLastTab].cy);
    end;
  end
  else if (AObject is TEffect) or (AObject is TAnimation) or
    AObject.Equals(FContent) or AObject.Equals(FNoItemsContent) or
    AObject.Equals(ResourceLink) then
    inherited DoAddObject(AObject)
...
begin
  inherited;
  if ([csLoading, csUpdating] * ComponentState = []) and not Released then
