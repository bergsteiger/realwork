...
{$ENDIF DYNAMIC_LIB}
    function InternalNamedGroup(const Name: TBytes): Integer; inline;
  protected
    procedure CleanUp;
...
function UnicodeIndexToUTF8(const S: string; AIndex: Integer): Integer; deprecated;
function UTF8IndexToUnicode(const S: TBytes; AIndex: Integer): Integer; deprecated;
...
function CopyBytes(const S: TBytes; Index, Count: Integer; LastCharZero: Boolean = False): TBytes;
var
  Len: Integer;
begin
  SetLength(Result, 0);
...
procedure InsertBytes(const Source: TBytes; var S: TBytes; Index: Integer); overload;
var
  Len: Integer;
  Len1, Len2: Integer;
begin
...
procedure InsertBytes(Source: PByte; var S: TBytes; Index: Integer; ByteCount: Integer); overload;
var
  LTail: Integer;
begin
  if ByteCount > 0 then
  begin
    if Index < 0 then
      Index := 0;
    LTail := Length(S);
    SetLength(S, LTail + ByteCount);
    if Index < LTail then
      Move(S[Index], S[Index + ByteCount], LTail - Index);
    Move(Source^, S[Index], ByteCount);
  end;
end;
...
begin
  Len := Length(S);
  if (Index >= 0) and (Index < Len) and (Count > 0) then
  begin
    TailLen := Len - Index ;
    if Count > TailLen then
      Count := TailLen;
    if TailLen <> Count then
          ...
          'A'..'Z', 'a'..'z', '_':
            begin
              ...
              if (J < Length(S)) and (S[J] = Ord('}')) then
                Number := InternalNamedGroup(CopyBytes(S, I+2, J-I-2, True));
            end;
        end;
      ...
      case Char(S[J]) of
        ...
        '`':
          begin
            ...
            DeleteBytes(S, I, J+1-I);
            // Insert SubjectLeft
            InsertBytes(@FSubject[0], S, I, Offsets[0]);
            I := I + Offsets[0];
            Exit;
          end;
        '''':
          begin
            ...
            DeleteBytes(S, I, J+1-I);
            // Insert SubjectRight
            InsertBytes(@FSubject[Offsets[1]], S, I, Length(FSubject) - Offsets[1]);
            ...
            'g':
              begin
                ...
                begin
                  ...
                  begin
                    N := InternalNamedGroup(CopyBytes(S, I+3, J-I-3, True));
                    Inc(J);
                    Mode := Byte(#0);
                    if N > 0 then
...
function TPerlRegEx.InternalNamedGroup(const Name: TBytes): Integer;
begin
  Result := pcre_get_stringnumber(FPattern, @Name[0]);
end;
...
begin
  ...
  if (Index >= 0) and (Index <= GroupCount) then
    Result := - UTF8IndexToUnicode(Offsets[Index*2]) + UTF8IndexToUnicode(Offsets[Index*2+1])
  else
...
begin
  ...
  if Index > GroupCount then
    ...
    Result := TEncoding.UTF8.GetString(FSubject, Offsets[Index*2], Offsets[Index*2+1]-Offsets[Index*2]);
end;
...
begin
  Result := TEncoding.UTF8.GetString(FSubject, 0, Offsets[0]);
end;

function TPerlRegEx.GetSubjectRight: string;
begin
  Result := TEncoding.UTF8.GetString(FSubject, Offsets[1], Length(FSubject) - Offsets[1]);
end;
...
begin
  SetFSubject(TEncoding.UTF8.GetBytes(Value));
end;
...
begin
  ...
  begin
    ...
    Count := 1;
    repeat
      Strings.Add(TEncoding.UTF8.GetString(FSubject, Offset, InternalGetMatchedOffset - Offset));
      Inc(Count);
      Offset := InternalGetMatchedOffset + InternalGetMatchedLength ;
    until ((Limit > 1) and (Count >= Limit)) or not MatchAgain;
    Strings.Add(TEncoding.UTF8.GetString(FSubject, Offset, Length(FSubject) - Offset));
  end
end;
  ...
  begin
    ...
    repeat
      ...
      begin
        ...
        begin
          LUseOffset := False;
          Strings.Add(TEncoding.UTF8.GetString(FSubject, 0, InternalGetMatchedOffset));
      ...
      begin
        Strings.Add(TEncoding.UTF8.GetString(FSubject, LOffset, InternalGetMatchedOffset - LOffset));
        Inc(Count);
        if Self.GroupCount > 0 then
          Strings.Add(Groups[GroupCount]);
      end;
    ...
    until ((Limit > 1) and (Count >= Limit)) or not MatchAgain;
    Strings.Add(TEncoding.UTF8.GetString(FSubject, LOffset, Length(FSubject) - LOffset));
  end
end;
