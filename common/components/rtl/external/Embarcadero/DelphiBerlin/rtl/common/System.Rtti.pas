...
{$ELSE !MSWINDOWS} // Linux
    ...
    tkRecord:
      Result := (GetTypeData(TypeInfo)^.RecSize > 16) or (CC in [ccReg, ccPascal]); // x86_64 uses RAX/RDX to store 16bytes record
    tkMethod,
    tkVariant:
      Result := True;
{$ENDIF}
...
{$ELSE}
    tkLString: AResult := TValue.From<UTF8String>(UTF8Char(value));
{$ENDIF NEXTGEN}
...
{$IFNDEF NEXTGEN}
    ...
    tkLString:
      begin
        ...
        AResult := TValue.From<AnsiString>(rstr);
      end;
{$ELSE}
    tkLString:
      begin
        SetAnsiString(@rstr, PWideChar(string(value)), 1, CP_UTF8);
        AResult := TValue.From<UTF8String>(rstr);
      end;
{$ENDIF !NEXTGEN}
...
begin
  ...
  if aFieldType = nil then Exit;                                  
  if aFieldType.FHandle = nil then Exit;                                 
...
//  RetBuff: array[0..1] of UInt64;
  ...
  begin
    DataSize := Arg.DataSize;
    // Single / Double
    if (Arg.GetTypeKind = tkFloat) and (Arg.TypeData.FloatType in [ftSingle, ftDouble]) then
    begin
    ...
    // Extended
    else if (Arg.GetTypeKind = tkFloat) and (Arg.TypeData.FloatType in [ftExtended]) then
    begin
      Stack.Align(16);
...
begin
  ...
  begin
    if UseResultPointer(AResultType, IsConstructor, CallingConvention) then
      // do nothing
    else if (AResultType^.Kind = tkFloat) and (AResultType^.TypeData.FloatType in [ftSingle, ftDouble]) then
...
{$IF defined(CPUX64) and not defined(MSWINDOWS)}
  ...
  AFrame^.UseFPU := Int64(FResultFP = ftExtended);
{$ENDIF}
{$IFDEF CPUX86}
...
begin
  ...
  if (FReturnType <> nil) then
  begin
    if (FReturnType^.Kind = tkFloat) and (FReturnType^.TypeData.FloatType in [ftExtended]) then
    begin
