...
function SetToString(PropInfo: PPropInfo; Value: Pointer; Brackets: Boolean = False): string; overload;
function SetToString(TypeInfo: PTypeInfo; Value: Pointer; Brackets: Boolean = False): string; overload;

{ String to Set conversion. }
procedure StringToSet(PropInfo: PPropInfo; const Value: string; Result: Pointer); overload;
...
{$IF     SizeOf(NativeInt) = 4}
  TOnRegisterSet = set of 0..31; // 4bytes * 8
{$ELSEIF SizeOf(NativeInt) = 8}
  TOnRegisterSet = set of 0..63; // 8bytes * 8
{$ELSE }
...
begin
  ...
  PEnumType := SetTypeData.CompType;
  if PEnumType = nil then
    Exit( Sizes[SetTypeData.OrdType] );

  EnumTypeData := PEnumType^.TypeData;
  MinValue := EnumTypeData.MinValue;
  MaxValue := EnumTypeData.MaxValue;

  Result := (MaxValue shr 3) - (MinValue shr 3) + 1;
  case Result of
    3: Result := 4;
{$IFDEF CPU64BITS}
    5..7: Result := 8
...
begin
  if SizeOfSet(PropInfo^.PropType^) <= SizeOf(NativeInt) then
    LargestSet := TPropSet<TOnRegisterSet>.GetProc(Instance, PropInfo)
  else
...
begin
  ...
  begin
    ...
    MaxValue := PEnumInfo^.TypeData.MaxValue;
    EnumOffset := MinValue and (not $7);
    MinIndex := MinValue - EnumOffset;
    MaxIndex := MaxValue - EnumOffset;

    for B := MinIndex to MaxIndex do
...
procedure SetSetProp(Instance: TObject; PropInfo: PPropInfo;
  const Value: string);
type
  TOnRegisterSetProc = procedure (const Value: TOnRegisterSet) of object;
  TIdxOnRegisterSetProc = procedure (Index: Integer; const Value: TOnRegisterSet) of object;
...
{$ENDIF WEAKINSTREF}
      tkArray:
        Exit(HasWeakRef(GetTypeData(Info).ArrayData.elType^));
      tkRecord:
        begin
