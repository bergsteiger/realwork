...
begin
  ...
  begin
    LURI := TURI.Create(TURI.PathRelativeToAbs(LResponse.GetHeaderValue('Location'), LRequest.FURL));  // do not translate
    LRequest.FURL := LURI;
    Result := False;
  end
  else
...
procedure TAndroidHTTPRequest.DoPrepare;
  ...
  function GetURLString(const AURI: TURI): string;
  begin
...
//  if FClient.ProxySettings <> nil then
  ...
  LJURL := TJURL.JavaClass.init(StringToJString(GetURLString(FURL)));
  ...
  if FURL.Scheme.ToUpper = 'HTTPS' then
  begin
...
begin
  ...
  try
    ...
    if FHeaders.TryGetValue(AName, LValue) then
    begin
      LValues := LValue.Split([',']);
      for I := 0 to High(LValues) do
        LList.Add(LValues[I].Trim);
    end;

    LValues := AValue.Split([',']);
    for I := 0 to High(LValues) do
      LList.Add(LValues[I].Trim);
    ...
    begin
      if I > 0 then
        LValue := LValue + ', ';
      LValue := LValue + LList[I];
    end;

    SetHeaderValue(AName, LValue);
  finally
    LList.Free;
  end;
...
begin
  ...
  begin
    DataLength := FSourceStream.Size - FSourceStream.Position;
    ...
    begin
      ...
      try
        ...
        if TOSVersion.Check(4, 4) then
          FJURLConnection.setFixedLengthStreamingMode(DataLength)
        else
          FJURLConnection.setFixedLengthStreamingMode(Integer(DataLength));
        LJStream := FJURLConnection.getOutputStream;
        Written := 0;
        while Written < DataLength do
        begin
          ToWrite := BUFFERSIZE;
          if (DataLength - Written) < ToWrite then
            ToWrite := DataLength - Written;
          Written := Written + Cardinal(FSourceStream.Read(LJBuffer.Data^, ToWrite));
          LJStream.write(LJBuffer, 0, ToWrite);
        end;
      finally
        LJBuffer.Free;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if SameText(LHeader, 'Set-Cookie') then // do not localize
