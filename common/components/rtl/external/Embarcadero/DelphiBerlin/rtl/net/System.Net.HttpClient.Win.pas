...
type
  TCertificateStore = class
    ...
    public
      class function Store: HCERTSTORE; static;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (AResponse <> nil) and SameText(LHeader, 'Set-Cookie') then // do not localize
...
begin
  ...
  begin
    if GetLastError <> ERROR_INSUFFICIENT_BUFFER then
      raise ENetHTTPException.CreateResFmt(@SNetHttpHeadersError, [GetLastError, SysErrorMessage(GetLastError, FWinHttpHandle)]);
...
function TWinHTTPClient.DoGetResponseInstance(const AContext: TObject; const AProc: TProc;
  const AsyncCallback: TAsyncCallback; const AsyncCallbackEvent: TAsyncCallbackEvent;
  const ARequest: IHTTPRequest; const AContentStream: TStream): IHTTPResponse;
begin
...
var
  LFlags: DWORD;
begin
  FWConnect := WinHttpConnect(TWinHTTPClient(FClient).FWSession, PWideChar(AURI.Host), AURI.Port, 0);
  if FWConnect = nil then
    raise ENetHTTPRequestException.CreateResFmt(@SNetHttpRequestConnectError, [AURI.Host]);

  LFlags := 0;
  if AURI.Scheme.ToUpper = 'HTTPS' then
  ...
  if FWRequest = nil then
    raise ENetHTTPRequestException.CreateResFmt(@SNetHttpRequestOpenError, [GetLastError, SysErrorMessage(GetLastError, FWinHttpHandle)]);
  ...
  if WinHttpSetTimeouts(FWRequest, 0, ConnectionTimeout, ResponseTimeout, ResponseTimeout) = False then
    raise ENetHTTPRequestException.CreateResFmt(@SNetHttpRequestSetTimeoutError, [GetLastError,
      SysErrorMessage(GetLastError, FWinHttpHandle)]);
end;
...
begin
  if WinHttpAddRequestHeaders(FWRequest, PWideChar(AName+': '+ AValue), $ffffffff,
    ...
    raise ENetHTTPRequestException.CreateResFmt(@SNetHttpRequestAddHeaderError, [GetLastError, SysErrorMessage(GetLastError), FWinHttpHandle]);
end;
...
begin
  ...
  if GetHeaderValue(AName) = '' then
    ...
    raise ENetHTTPRequestException.CreateResFmt(@SNetHttpRequestRemoveHeaderError, [GetLastError, SysErrorMessage(GetLastError, FWinHttpHandle)]);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if LClient.ProxySettings.UserName <> '' then // we try to use the usual auth scheme to try to avoid a round trip
        WinHttpSetCredentials(FWRequest, WINHTTP_AUTH_TARGET_PROXY, WINHTTP_AUTH_SCHEME_BASIC, PWideChar(LClient.ProxySettings.UserName),
         PWideChar(LClient.ProxySettings.Password), nil);
    end;
  end
  else
  begin
...
class function TCertificateStore.Store: HCERTSTORE;
begin
  if FStore = nil then
