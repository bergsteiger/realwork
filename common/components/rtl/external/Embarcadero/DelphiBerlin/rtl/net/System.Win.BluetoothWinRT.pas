...
type
 ...
 private
  ...
  private
    ...
    FPairedLEDevices: TBluetoothLEDeviceDictionary;
  ...
  private
    ...
    function AddressFromId(const AId: HSTRING): Int64;
  protected
    FId: HSTRING;
    FAddress: UInt64;
...
var
  ...
  LDataBytes: TBytes;
  LPaired, LNew, LDiscovered: Boolean;
begin
  LId := Format('%.12x', [AAddress]);
  LNew := False;
  LPaired := False;
  // Search this device in LastDiscoveredDevices list
  LDevice := TWinRTBluetoothLEManager.GetDeviceInList(LId, FManager.LastDiscoveredDevices);
  if LDevice = nil then
  begin
    LNew := True;
    // Search this device in PairedDevices
    LDevice := FPairedLEDevices.ExtractPair(LId).Value;
    if LDevice <> nil then
      LPaired := True
    else
      // Search this device in ScannedDevices
      if not FScannedLEDevices.TryGetValue(LId, LDevice) then
      begin
        LDevice := TWinRTBluetoothLEDevice.Create(AAddress, Self, True);
        FScannedLEDevices.Add(LId, LDevice);
      end;
  end;
  ...
  if LNew and (FBluetoothLEScanFilterList <> nil) then
    if not DoDeviceOvercomesFilters(LDevice, FBluetoothLEScanFilterList) then
    begin
      LDiscovered := False;
      LNew := False;
    end;

  if LNew and LPaired then
    TWinRTBluetoothLEManager.AddDeviceToList(LDevice, FManager.LastDiscoveredDevices);
  if LDiscovered then
    DoDiscoverDevice(Self, LDevice, ARSSI, LDevice.AdvertisedData);
end;

constructor TWinRTBluetoothLEAdapter.Create(const AManager: TBluetoothLEManager; const ARadioInfo: TBluetoothRadioInfo);
begin
  inherited Create(AManager);
  FPairedLEDevices := TBluetoothLEDeviceDictionary.Create([doOwnsValues]);
  FScannedLEDevices := TBluetoothLEDeviceDictionary.Create([doOwnsValues]);
  ...
  begin
    ...
    if LBLEDevicesFound.Size > 0 then
      ...
      begin
        ...
        FPairedLEDevices.AddOrSetValue(LWinBluetoothLEDevice.Identifier, LWinBluetoothLEDevice);
      end;
  end;
end;
...
begin
  ...
  TWinRTBluetoothLEManager.ClearDevicesFromList(FManager.LastDiscoveredDevices);
  GetBLEPairedDevices;
...
begin
  StopDiscovery;
  DoDiscoveryEnd(Self, TWinRTBluetoothLEAdapter(Self).FManager.LastDiscoveredDevices);
end;

procedure TWinRTBluetoothLEAdapter.DoDiscoveryEnd(const Sender: TObject; const ADeviceList: TBluetoothLEDeviceList);
begin
  inherited DoDiscoveryEnd(Self, TWinRTBluetoothLEManager(FManager).FDiscoveredLEDevices);
end;
...
function TWinRTBluetoothLEDevice.AddressFromId(const AId: HSTRING): Int64;
var
  LTemp: string;
  LDevPos: Integer;
begin
