...
begin
  if not Deleting and
     not ((Destination = Parent) and (Mode = naAddChild)) then
  begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              MCWidth := Length(FormatSettings.ShortDayNames[1]);
              if MCWidth = 1 then
                MCWidth := 2;
              Buffer := TBitmap.Create;
              try
                Buffer.Canvas.Font := TControlClass(Control).Font;
                MCWidth := MCWidth * Buffer.Canvas.TextWidth('W') * 7 + 30;
              finally
                Buffer.Free;
              end;
...
begin
  ...
  begin
    ...
    R1.Left := R1.Right - 17;
    R1.Top := R1.Bottom - 17;
    if (ParentForm <> nil) and not IsZoomed(ParentForm.Handle) then
    begin
