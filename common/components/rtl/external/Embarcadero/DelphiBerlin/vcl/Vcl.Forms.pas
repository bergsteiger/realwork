...
procedure TCustomForm.ScaleForCurrentDpi;
  function ShouldScale: Boolean;
    // this is just to make code more readable
                                                                                    
  begin
    Result := FScaled and not (csDesigning in ComponentState)
      and ((Owner = nil) or not (LowerCase(Owner.ClassName) = 'teditwindow'))  // we are not in an edit window
      and ((Parent = nil) or not (csDesigning in Parent.ComponentState) or not (csFreeNotification in Parent.ComponentState));
  end;
var
  LCurrentPixelsPerInch: Integer;
begin
