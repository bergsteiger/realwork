...
{
    return new TCDirectoryOutline((System::Classes::TComponent *)NULL);
}
//---------------------------------------------------------------------------
...
{
    OutlineStyle = Vcl::Outline::osTreePictureText;
    Options = Vcl::Outline::TOutlineOptions() << Vcl::Outline::ooStretchBitmaps << Vcl::Outline::ooDrawFocusRect;
    PictureLeaf = PictureClosed;
    AssignCaseProc();
}
...
void __fastcall TCDirectoryOutline::AssignCaseProc()
{
    switch (TextCase) {
        case tcLowerCase:
            FCaseFunction = System::Sysutils::AnsiLowerCaseFileName;
            break;
            
        case tcUpperCase:
            FCaseFunction = System::Sysutils::AnsiUpperCaseFileName;
            break;
        default:
            FCaseFunction = NULL;
            break;
...
namespace {
    class FileFind {
        private:
            System::Sysutils::TSearchRec FSRec;
            int FError;
        ...
        public:
            ...
            __property System::Sysutils::TSearchRec SRec = { read = GetSRec };

            System::Sysutils::TSearchRec &GetSRec() { return FSRec; }
...
{
    Vcl::Outline::TOutlineNode *RootNode = Items[RootItem];
    ...
    for (FileFind Find(RootName, faDirectory); Find.Ok; Find.Next()) {
        System::Sysutils::TSearchRec &SRec = Find.GetSRec();
