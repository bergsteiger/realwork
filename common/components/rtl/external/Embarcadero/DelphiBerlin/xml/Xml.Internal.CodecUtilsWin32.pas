...
begin
  if Ch < $10000 then
    ...
    raise EConvertError.CreateFmt(SInvalidCodePointPar, [Ch, '']);
end;
...
begin
  ...
  if Ch < $200000 then // 4-byte sequence
    ...
    raise EConvertError.CreateFmt(SInvalidCodePointPar, [Ord(Ch), 'Unicode']);
end;
...
begin
  ...
  $0000D800..$0000DFFF :
    raise EConvertError.CreateFmt(SInvalidCodePointPar, [Ch, 'UCS-4']);
  $00010000..$0010FFFF :
    begin
      SeqSize := 4;
      if not Assigned(P) or (DestSize <= 0) then
...
begin
  if Ord(P) < $80 then
    ...
    raise EConvertError.CreateFmt(SInvalidCodePointPar, [Ord(P), 'US-ASCII']);
end;
