...
type
  ...
  private
{$IFDEF NEXTGEN}
    function CodeToName_en(Value: TIso639LanguageCode): string;
    function NameToCode_en(Value: string): TIso639LanguageCode;
{$ELSE !NEXTGEN}
...
{$ENDIF NEXTGEN}
  ...
  public
    constructor Create;
{$IFDEF NEXTGEN}
    function CodeToName(const Value: TIso639LanguageCode): string; virtual;
    function CodeToSymbol(const Value: TIso639LanguageCode): string;
      virtual;
    function NameToCode(const Value: string): TIso639LanguageCode; virtual;
    function SymbolToCode(const Value: string): TIso639LanguageCode; virtual;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
function IsSubLanguage(const Sublanguage, Superlanguage: string): Boolean;
{$ELSE !NEXTGEN}
...
{$ENDIF}
  ...
  Xml.Internal.AbnfUtils, System.SysUtils;
...
begin
      ...
      try
        ...
        try
          for I := Ord(Low(TIso639LanguageCode)) to
            ...
            {$IFDEF CLR}
            TStrings(Dest).AddObject(ConvertFromUTF16('US-ASCII',
              CodeToName(TIso639LanguageCode(I))), TObject(I));
            {$ELSE}
              {$IFDEF UNICODE}
            TStrings(Dest).AddObject(
  ...
  begin
      TUtilsWideStringList(Dest).BeginUpdate;
      try
        TUtilsWideStringList(Dest).Clear;
        try
          for I := Ord(Low(TIso639LanguageCode)) to
            Ord(High(TIso639LanguageCode)) do
            TUtilsWideStringList(Dest).AddObject(CodeToName(TIso639LanguageCode(I)),
                              {$IFDEF CLR}TObject(I){$ELSE}Pointer(I){$ENDIF});
        except
          TUtilsWideStringList(Dest).Clear;
          raise;
        end;
      finally
        TUtilsWideStringList(Dest).EndUpdate;
      end;
  end
  else
...
{$IFDEF NEXTGEN}
function TIso639Info.CodeToName(const Value: TIso639LanguageCode): string;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
function TIso639Info.CodeToName_en(Value: TIso639LanguageCode): string;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
function TIso639Info.codeToSymbol(const Value: TIso639LanguageCode): string;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
function TIso639Info.NameToCode(const Value: string): TIso639LanguageCode;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
function TIso639Info.NameToCode_en(Value: string): TIso639LanguageCode;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
function TIso639Info.symbolToCode(const Value: string): TIso639LanguageCode;
{$ELSE !NEXTGEN}
