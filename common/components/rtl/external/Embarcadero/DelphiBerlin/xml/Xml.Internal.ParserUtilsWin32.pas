...
uses
  Xml.Internal.CodecUtilsWin32, System.Classes, System.SysUtils;
...
{$ENDIF NEXTGEN}
  ...
  protected
    ...
    procedure ReadEventHandler(const Sender: TObject; var Buf; Count: Longint; var Ok:
      Boolean); virtual;
  ...
  public
    ...
    destructor Destroy; override;
{$IFDEF NEXTGEN}
    function Match(Ucs2Str: string): Boolean; virtual;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
    function SkipNext(Ucs2Str: string): Integer; virtual;
{$ELSE !NEXTGEN}
...
{$ENDIF NEXTGEN}
  ...
  protected
    FBusy: Boolean;
    procedure CodecReadEventHandler(const Sender: TObject;
      var Buf;
      Count: Longint;
      var Ok: Boolean); virtual;
    procedure CodecWriteEventHandler(const Sender: TObject;
      const Buf;
...
begin
  ...
  try
    try
{$IFDEF NEXTGEN}
      C.DecodeStr(MarshaledString(@S[Low(string)]), S.Length, Result);
{$ELSE !NEXTGEN}
      C.DecodeStr(PAnsiChar(@S[Low(string)]), Length(S), Result);
{$ENDIF NEXTGEN}
    except
...
{$ENDIF NEXTGEN}
  ...
  begin
{$IFDEF NEXTGEN}
    FRemaining := FStream.Read(FBuffer, 0, FBufSize);
{$ELSE !NEXTGEN}
...
begin
{$IFDEF NEXTGEN}
  SetLength(FBuffer, 0); // Remark: If an exception occurs, the destructor is automatically called.
{$ELSE !NEXTGEN}
  FBuffer := nil; // Remark: If an exception occurs, the destructor is automatically called.
{$ENDIF NEXTGEN}
  FStream := nil; // Therefore, we need to initialize critical objects with nil first.
...
begin
  ...
  FInitialUCS4CharData.ByteCount := InitialByteCount;
  FInitialUCS4CharData.CharCount := InitialCharCount;
  FInitialUCS4CharData.CharsInLine := InitialCharsInLine;
  FInitialUCS4CharData.CodePoint := $98; // START OF STRING
  FInitialUCS4CharData.Line := InitialLine;
  FInitialUCS4CharData.Size := 0;
  FInitialUCS4CharData.TabsInLine := InitialTabsInLine;
...
{$IFDEF NEXTGEN}
function TUtilsUCS4Reader.Match(Ucs2Str: string): Boolean;
{$ELSE !NEXTGEN}
...
begin
{$IFDEF NEXTGEN}
  for I := Low(Ucs2Str) to High(Ucs2Str) do
{$ELSE !NEXTGEN}
...
begin
  ...
  FPreviousUCS4Char := FCurrentUCS4Char;
  FCurrentUCS4Char := FNextUCS4Char;
  Inc(FNextUCS4Char.CharCount, 1);
  FNextUCS4Char.CodePoint := TempNextCP;
  Inc(FNextUCS4Char.ByteCount, TempSize);
  FNextUCS4Char.Size := TempSize;
  UpdateLocator(FNextUCS4Char);
end;

procedure TUtilsUCS4Reader.ReadEventHandler(const Sender: TObject; var Buf; Count:
...
{$IFDEF NEXTGEN}
function TUtilsUCS4Reader.SkipNext(Ucs2Str: string): Integer;
{$ELSE !NEXTGEN}
...
begin
  case FNextUCS4Char.CodePoint of
    $A:
      begin // LF
        Inc(UCS4CharData.Line);
        UCS4CharData.CharsInLine := 0;
        UCS4CharData.TabsInLine := 0;
      end;
    $9:
      begin // TAB
        Inc(UCS4CharData.CharsInLine);
        Inc(UCS4CharData.TabsInLine);
      end;
  else
    Inc(UCS4CharData.CharsInLine);
  end;
...
procedure TUtilsCustomTranscoder.CodecReadEventHandler(const Sender: TObject;
  var Buf; Count: LongInt; var Ok: Boolean);
begin
  Ok := False; // By default return False;
end;

procedure TUtilsCustomTranscoder.CodecWriteEventHandler(const Sender: TObject;
  const Buf; Count: LongInt);
begin
...
procedure TUtilsStandardTranscoder.CodecReadEventHandler(const Sender: TObject;
  var Buf; Count: LongInt; var Ok: Boolean);
begin
...
procedure TUtilsStandardTranscoder.CodecWriteEventHandler(const Sender: TObject;
  const Buf; Count: LongInt);
begin
...
procedure TUtilsStreamTranscoder.CodecReadEventHandler(const Sender: TObject;
  var Buf; Count: LongInt; var Ok: Boolean);
begin
  Ok := FReader.Read(Buf, Count);
end;

procedure TUtilsStreamTranscoder.CodecWriteEventHandler(const Sender: TObject;
  const Buf; Count: LongInt);
begin
