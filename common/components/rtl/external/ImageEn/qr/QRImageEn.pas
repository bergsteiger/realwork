...
begin
   ...
   IECopyBitmap(fBitmap,XBitmap);
  	if (XBitmap.Width>0) and (XBitmap.Height>0) then begin
      ...
      with ParentReport.QRPrinter do begin
         ...
         with fBitmapInfoHeader256 do begin
            ...
            biPlanes:=1;
            if Xbitmap.pixelformat=pf1bit then begin
                biBitCount:=1;
         ...
         inc(fOffX,CalcLeft);
         inc(fOffY,CalcTop);
         IEDrawDibDraw(fHDrawDib,canvas.handle,fOffX,fOffY,fExtx,fExty,PBitmapInfoHeader(@fBitmapInfoHeader256)^,Xbitmap.ScanLine[Xbitmap.height-1],
                    o1x,o1y,o2x,o2y,0);
         with Canvas do begin
            Pen.Color := Frame.Color;
            Pen.Width := Frame.Width;
            Pen.Style := Frame.Style;
            // shadow
            if fShadowSize>0 then begin
               Brush.Color:=clBlack;
               Brush.Style:=bsSolid;
               if fImageShadow then begin
                  // ombra sull'immagine
                  IERightShadow(Canvas,fOffX+fExtx,fOffY,fOffX+fExtx+fShadowSize+2,fOffY+fExty+fShadowSize,fShadowType);
                  IEBottomShadow(Canvas,fOffX,fOffY+fExty,fOffX+fExtx+fShadowSize,fOffY+fExty+fShadowSize+1,fShadowType);
               end else begin
                  // ombra sul frame
                  IERightShadow(Canvas,CalcRight+2,CalcTop,CalcRight+2+fShadowSize,CalcBottom+fShadowSize,fShadowType);
                  IEBottomShadow(Canvas,CalcLeft,CalcBottom+1,CalcRight+fShadowSize,CalcBottom+1+fShadowSize,fShadowType);
               end;
               if fImageBorder then begin
                  // bordo sull'immagine
                  moveto(fOffX,fOffY);
...
begin
   ...
   if assigned(fImageEnView) then begin // fImageEnView ora potrebbe anche essere nil
