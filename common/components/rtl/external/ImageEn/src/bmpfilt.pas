...
// BMP
procedure BMPReadStream(fs:TStream; Bitmap:TIEBitmap; BlockDim:integer; var IOParams:TIOParamsVals; var Progress:TProgressRec; Preview:boolean; MissingFileHead:boolean);
procedure BMPWriteStream(fs:TStream; Bitmap:TIEBitmap; var IOParams:TIOParamsVals; var Progress:TProgressRec);
...
begin
   ...
   q:=0;
   ww:=0;
   while q<xImageDim do begin
   	if bits2^=0 then begin
      	// escape
         inc(bits2); inc(q);
         case bits2^ of
         	0: begin
            ...
            2: begin
            		// delta
                  inc(bits2); inc(q); w:=bits2^;
                  inc(bits2); inc(q); dec(y,bits2^);
                  px:=Bitmap.scanline[abs(inverter-y)];
                  inc(px,w);
            	end;
            else begin
            	// absolute packet
               xx:=bits2^;
               for w:=0 to (xx shr 1)-1 do begin
               	inc(bits2); inc(q);
                  WriteLo;
                  WriteHi;
               end;
               if xx and 1 <>0 then begin
               	inc(bits2); inc(q);
                  WriteLo;
               end;
...
var
   ...
   begin
   	if ww<Bitmap.Width then begin
	   	px^:=bits2^ SHR 4;   inc(px); inc(ww);
      end;
   end;
   procedure WriteHi;
   begin
   	if ww<Bitmap.Width then begin
	   	px^:=bits2^ and $0F; inc(px); inc(ww);
      end;
   end;
begin
	y:=Bitmap.height-1;
   ...
   while q<xImageDim do begin
   	if bits2^=0 then begin
         ...
         case bits2^ of
            ...
            2: begin
            		// delta
                  inc(bits2); w:=bits2^; inc(q);
                  inc(bits2); dec(y,bits2^); inc(q);
                  px:=Bitmap.scanline[abs(inverter-y)];
                  inc(px,w);
            	end;
            else begin
...
begin
   ...
   for q:=0 to xImageDim-1 do begin
   	if bits2^=0 then begin
      	// escape
         inc(bits2);
         case bits2^ of
...
///////////////////////////////////////////////////////////////////////
// Legge la bitmap dallo stream
// Bitmap è sempre in pf24bit o pf1bit
procedure BMPReadStream(fs:TStream; Bitmap:TIEBitmap; BlockDim:integer; var IOParams:TIOParamsVals; var Progress:TProgressRec; Preview:boolean; MissingFileHead:boolean);
var
	FileHead:TBITMAPFILEHEADER;
   InfoHead:^TBITMAPINFOHEADER2;
   CoreHead:^TBITMAPCOREHEADER;
   dm:integer;	// dimensione struttura successiva a BITMAPFILEHEADER
   p0:integer;
   xBitCount:integer;
   xCompression:integer;
   xImageDim:integer;	// dim immagine (può essere 0 per imm. non compresse)
...
begin
   ...
   if (FileHead.bfSize>0) and (BlockDim<=0) then
	...
	if FileHead.bfType<>19778 then begin
   	Progress.Aborting^:=true;
      exit;
   end;
	fs.Read(dm,sizeof(dm));	// legge dimensione prossimo header
   ...
   // read headers
	if dm=sizeof(TBITMAPCOREHEADER) then begin
      ...
      if dm>sizeof(TBITMAPINFOHEADER2) then
			fs.seek(dm-40,soFromCurrent);	// salta dati extra
      if dm=64 then
      ...
      if IOParams.DpiX=0 then IOParams.DpiX:=gDefaultDPIX;
		IOParams.DpiY:=round((InfoHead^.biYPelsPerMeter/100)*2.54);
      if IOParams.DpiY=0 then IOParams.DpiY:=gDefaultDPIY;
      // read colormap
      if InfoHead^.biClrUsed>256 then InfoHead^.biClrUsed:=0;
      if (InfoHead^.biClrUsed=0) and (xBitCount<=8) then
        	InfoHead^.biClrUsed:=1 shl xBitCount;	// default
		if (InfoHead^.biClrUsed>0) then begin
         ...
         for q:=0 to InfoHead^.biClrUsed-1 do begin
  	...
  	if (xCompression=BI_RGB) or (xCompression=BI_BITFIELDS) then IOParams.BMP_Compression:=ioBMP_UNCOMPRESSED;
	if (xCompression=BI_RLE4) or (xCompression=BI_RLE8) then IOParams.BMP_Compression:=ioBMP_RLE;
	// read bitmap
   ...
   case xBitCount of
   		1:  begin	// 1 bit per pixel
	         	IOParams.BitsPerSample:=1;
	            IOParams.SamplesPerPixel:=1;
               if not Preview then begin
                  if xCompression=BI_RGB then begin
                  	bitmapheight1:=Bitmap.Height-1;
                     for y:=BitmapHeight1 downto 0 do begin
                        // OnProgress
                        with Progress do
                           if assigned(fOnProgress) then
			...
			4:  begin	// 4 bit per pixel
               ...
               if not Preview then begin
                  ...
                  if xCompression=BI_RGB then begin
                     ...
                     freemem(bits);
                  end else if xCompression=BI_RLE4 then begin
                     // RLE4 compression
         ...
         8:  begin	// 8 bit per pixel
               ...
               if not Preview then begin
               	if IOParams.IsNativePixelFormat then
                     ...
                     for y:=0 to IOParams.ColorMapCount-1 do
                     	Bitmap.Palette[y]:=IOParams.ColorMap[y];
                  if xCompression=BI_RLE8 then begin
                     // RLE8 compression
                     getmem(bits,xImageDim);
                     fs.Read(bits^,xImageDim);
							if IOParams.IsNativePixelFormat then begin
                     ...
                     freemem(bits);
                  end else if xCompression=BI_RGB then begin
                     // no compression
                     ...
                     for y:=bitmapheight1 downto 0 do begin
                        ...
                        if Progress.Aborting^ then
                           break;
                        bits2:=bits; inc(bits2,lw*(Bitmapheight1-y));
								if IOParams.IsNativePixelFormat then begin
                        	// native format
                           Bitmap.PaletteUsed:=256;
         ...
         16: begin	// 16 bit per pixel
               ...
               if not Preview then begin
                  ...
                  if xCompression=BI_RGB then begin
                     ...
                     BitFields[1]:=$03E0;
                     BitFields[2]:=$001F;
                  end; // altrimenti è BI_BITFIELDS valori già caricati in BitFields
...
///////////////////////////////////////////////////////////////////////
// comprime una riga in RLE4
// px: buffer decompresso
// Width: numero bytes di px
// rowbuf: bufer compresso (output)
// rest. dimensione buffer compresso
// nota: ogni byte di px contiene un pixel (nibble)
...
begin
	while p1-pb>0 do begin
      ...
      if q=1 then begin
         ...
         rowbuf^:=0; inc(rowbuf);	// ESC
         rowbuf^:=q; inc(rowbuf);
         for w:=0 to (q shr 1)-1 do begin
...
begin
	...
	p1:=0;
   pb:=0;
	while p1<Width do begin
   	if p1-pb>255 then begin
      	WAbs;
         //pb:=0;
      end;
		if px^[p1]=px^[p1+1] then begin
         ...
         q:=p2-p1;
         rowbuf^:=q; inc(rowbuf);
         rowbuf^:=(px^[p1] SHL 4) or px^[p1]; inc(rowbuf);
         pb:=p2;
         p1:=pb;
      end else
      	inc(p1);
...
begin
	while p1-pb>0 do begin
      ...
      if q=1 then begin
         // uno solo, codifica come run length
         rowbuf^:=1; inc(rowbuf);
         rowbuf^:=px^[pb]; inc(rowbuf);
      end else if q=2 then begin
         // due, ancora run length
         rowbuf^:=1; inc(rowbuf);
         rowbuf^:=px^[pb]; inc(rowbuf);
         rowbuf^:=1; inc(rowbuf);
         rowbuf^:=px^[pb+1]; inc(rowbuf);
      end else begin
         // sono 3 o più, codifica come absolute
         rowbuf^:=0; inc(rowbuf);	// ESC
         rowbuf^:=q; inc(rowbuf);
...
///////////////////////////////////////////////////////////////////////
// note: i bitcount in uscita sarà solo 1,4,8,24
...
var
   ...
   qt:TIEQuantizer;
   zz,q,row:integer;
   p0:integer;	// posizione inizio file
   p1:integer;	// posizione inizio immagine
   p2:integer;	// posizione finale file
   px:PRGB;
...
begin
   ...
   p1:=0;
	...
	with IOParams do begin
      if (BitsPerSample=1) or (BitsPerSample=4) then
         ...
         if SamplesPerPixel=1 then xBitCount:=8 else xBitCount:=24;
   end;
	// Scrive header
	fillchar(FileHead,sizeof(FileHead),0);
	FileHead.bfType:=19778;
   SafeStreamWrite(fs,Progress.Aborting^,FileHead,sizeof(FileHead));
	if IOParams.BMP_Version=ioBMP_BMOS2V1 then begin
   	// OS/2 v1.x
      ...
      if (IOParams.BMP_Compression=ioBMP_RLE) then begin
      	if xBitCount=4 then InfoHead.biCompression:=BI_RLE4;
         if xBitCount=8 then InfoHead.biCompression:=BI_RLE8;
      end;
      ...
      // sono stati omessi i parametri di OS/2 v2, per mancanza di documentazione
      SafeStreamWrite(fs,Progress.Aborting^,InfoHead,InfoHead.biSize);
	end;
	//
   Progress.per1:=100/Bitmap.Height;
   lw:=(((Bitmap.Width*xBitCount)+31) shr 5) shl 2; // larghezza riga in byte
   //
   if (xBitCount=4) or (xBitCount=8) then begin
   	//////////////////////////////////////
   	// 4 e 8 bits per pixel con colormap
      if bitmap.pixelformat=ie1g then begin
      	// converte 1 bit a 24 bit
      ...
      qt:=TIEQuantizer.Create(bwBitmap, IOParams.ColorMap^, zz, -1, 0);
		if IOParams.BMP_Version=ioBMP_BMOS2V1 then
      	// scrive colormap come TRGB (RGBTRIPLE)
         SafeStreamWrite(fs,Progress.Aborting^,IOParams.ColorMap^,zz*3)
      else
      	// scrive colormap come RGBQUAD
         WColorMap4(zz);
      p1:=fs.position;
      // scrive bwbitmap
      getmem(rowbuf,bwBitmap.Width*10);	// buffer compresso (potrebbe essere maggiore del decompresso!)
      getmem(buf,imax(bwBitmap.Width,lw));	// buffer decompresso
      bitmapheight1:=bwBitmap.Height-1;
      bitmapwidth1:=bwBitmap.Width-1;
      for row:=BitmapHeight1 downto 0 do begin
	      // OnProgress
	      with Progress do
	         if assigned(fOnProgress) then
         ...
         if Progress.Aborting^ then
         	break;
         //
         if (IOParams.BMP_Compression=ioBMP_RLE) and (IOParams.BMP_Version<>ioBMP_BMOS2V1) then begin
         	// RLE COMPRESSION
            if bwbitmap.PixelFormat=ie8p then begin
            	// native format
            ...
            if xBitCount=8 then begin
               ...
               if bwbitmap.PixelFormat=ie8p then begin
                  ...
                  q:=0;
                  while q<(bwBitmap.Width shr 1) do begin
                     buf[q]:=px_byte^ SHL 4; inc(px_byte);
                     buf[q]:=buf[q] or px_byte^; inc(px_byte);
                     inc(q);
                  end;
                  ...
                  q:=0;
                  while q<(bwBitmap.Width shr 1) do begin
                     buf[q]:=qt.RGBIndex[px^] SHL 4; inc(px);
                     buf[q]:=buf[q] or qt.RGBIndex[px^]; inc(px);
                     inc(q);
                  end;
                  if bwBitmap.width and 1 <>0 then
   ...
   if xBitCount=1 then begin
		//////////////////////////////////////
      // 1 bit per pixel (con colormap, ma non viene usata...)
      if IOParams.ColorMap<>nil then begin
      	freemem(IOParams.ColorMap);
         IOParams.fColorMap:=nil;
         IOParams.fColorMapCount:=0;
      ...
      if BWBitmap<>nil then begin
         ...
         p1:=fs.position;
         bitmapheight1:=bitmap.height-1;
			for q:=bitmapheight1 downto 0 do begin
		      // OnProgress
		      with Progress do
		         if assigned(fOnProgress) then
   ...
   if xBitCount=24 then begin
		//////////////////////////////////////
      // 24 bit per pixel (include 16 e 24 bitcount)
      if bitmap.pixelformat=ie1g then begin
      	// converte 1 bit a 24 bit
         BWBitmap:=TIEBitmap.Create;
         BWBitmap.Assign(Bitmap);
   ...
   if not Progress.Aborting^ then begin
      ...
      fs.position:=p2;	// posiziona alla fine del file
   end;
end;
...
{$WARNINGS OFF}
	lp:=fs.Position;
	result:=false;
	if fs.Size>(sizeof(TICONDIR)+sizeof(TICONDIRENTRY)) then begin
   	fs.Read(IconDir,sizeof(TICONDIR));
      if (IconDir.idReserved<>0) or (IconDir.idType<>1) or (IconDir.idCount=0) then begin
      	fs.Position:=lp;
      	exit;	// FALSE
      end;
      for q:=0 to IconDir.idCount-1 do begin
      	fs.Read(IconEntry,sizeof(TICONDIRENTRY));
         with IconEntry do
	         if (bReserved<>0) or (dwBytesInRes=0) or (dwImageOffset<fs.Position) then begin
      	      fs.Position:=lp;
   	         exit; // FALSE
         	end;
      end;
...
{$WARNINGS OFF}
	p0:=fs.Position;
	fs.Read(IconDir,sizeof(TICONDIR));
	if IconDir.idReserved=19778 then begin
   	// this is a bmp renamed!
      fs.position:=p0;
      BMPReadStream(fs,Bitmap,fs.size,IOParams,Progress,Preview,false);
      exit;
   end;
   ...
   FillChar(IOParams.ICO_Sizes[0],sizeof(TIOICOSizes),0);
	for q:=0 to IconDir.idCount-1 do begin
   	if q=IEMAXICOIMAGES then break;
   	fs.Read(IconEntry,sizeof(TICONDIRENTRY));
   ...
   if xBitsPerSample=0 then begin
   	// usa ColorCount di IconEntry, altrimenti l'ICO non è valida
   ...
   *)
   colorcount:=1 shl xBitsPerSample;
   if xBitsPerSample<=8 then begin
   ...
   if (IconEntry.bWidth<>InfoHead.biwidth) and (IconEntry.bWidth<>0) and (InfoHead.biWidth<>0)
		and (InfoHead.biwidth<2048) and (InfoHead.biheight<2048) then begin
		Width:=InfoHead.biWidth;
      Height:=InfoHead.biHeight shr 1;
	end;
 	lw:=(((width*xBitsPerSample)+31) shr 5) shl 2;
   lw1:=((width+31) shr 5) shl 2;
	// read colormap (ColorCount è il numero di colori nella colormap, e potrebbe
   // non rispecchiare il valore di xBitsPerSample)
   if IOParams.ColorMap<>nil then begin
   	// remove old colormap
   	freemem(IOParams.ColorMap);
		IOParams.fColorMap:=nil;
   ...
   if xBitsPerSample<=8 then begin
      ...
      for q:=0 to ColorCount-1 do begin
         IOParams.ColorMap[q].r:=ColorMap4[q].rgbRed;
         IOParams.ColorMap[q].g:=ColorMap4[q].rgbGreen;
         IOParams.ColorMap[q].b:=ColorMap4[q].rgbBlue;
      end;
   ...
   case xBitsPerSample of
      1:  begin	// 1 bit per pixel
            ...
            for y:=BitmapHeight1 downto 0 do begin
               ...
               for x:=0 to BitmapWidth1 do begin
                  if _GetPixelbw(amap,x)<>0 then begin
      ...
      4:  begin	// 4 bit per pixel
            ...
            if (Bitmap.Width and 1)<>0 then inc(w);
            getmem(bits,lw*Bitmap.Height);
            fs.Read(bits^,lw*Bitmap.Height);
            fs.Read(andmap^,lw1*Bitmap.Height);
            bitmapheight1:=Bitmap.Height-1;
...
begin
   ...
   for i:=0 to high(sizes) do begin
      ...
      // save xor map
      if (SamplesPerPixel=3) and (BitsPerSample=8) then begin
         ...
         for row:=ie.IEBitmap.Height-1 downto 0 do begin
            px:=ie.IEBitmap.Scanline[row];
            fs.Write(px^,ie.IEBitmap.RowLen);
         end;
      end else if (SamplesPerPixel=4) and (BitsPerSample=8) then begin
         ...
         for row:=ie.IEBitmap.Height-1 downto 0 do begin
            ...
            xrgba:=rgba;
				for col:=0 to ie.IEBitmap.Width-1 do begin
               xrgba^.r:=p_rgb^.r;
               xrgba^.g:=p_rgb^.g;
               xrgba^.b:=p_rgb^.b;
               xrgba^.a:=px^;
         ...
         freemem(rgba);
      end else if (SamplesPerPixel=1) and (BitsPerSample=1) then begin
      ...
      end else if (SamplesPerPixel=1) and (BitsPerSample=4) then begin
         ...
         qt:=TIEQuantizer.Create(ie.IEBitmap, IOParams.ColorMap^, 15, -1, 0);
         ...
         for row:=ie.IEBitmap.Height-1 downto 0 do begin
            ...
            xbuf:=buf;
            col:=0;
            while col<ie.IEBitmap.Width do begin
               ...
               if ie.IEBitmap.HasAlphaChannel and (ie.IEBitmap.Alpha[col,row]<255) then
               	...
               	xbuf^:=xbuf^ or ((qt.RGBIndex[p_rgb^]+1) shl 4);
               inc(col);
               inc(p_rgb);
               inc(xbuf);
            end;
         ...
         freemem(buf);
         qt.free;
      end else (*if (SamplesPerPixel=1) and (BitsPerSample=8) then*) begin
         ...
         qt:=TIEQuantizer.Create(ie.IEBitmap, IOParams.ColorMap^, 255, -1, 0);
      ...
      getmem(buf,l);
      if ie.IEBitmap.HasAlphaChannel and (integer(bc[i])<>24) then begin
         for row:=ie.IEBitmap.Height-1 downto 0 do begin
            fillchar(buf^,l,0);
            px:=ie.IEBitmap.AlphaChannel.ScanLine[row];
            for col:=0 to ie.IEBitmap.Width-1 do begin
   ...
   fs.Position:=p0;
	for i:=0 to high(sizes) do begin
      ...
      IconEntry.bHeight:=sizes[i].cy;
		if (integer(bc[i])=24) or (integer(bc[i])=32) then begin
...
begin
   ...
   for i:=0 to icount-1 do begin
      ...
      if (SamplesPerPixel=3) and (BitsPerSample=8) then begin
         ...
         fs.Write(InfoHead,sizeof(TBITMAPINFOHEADER));
         for row:=Bitmap.Height-1 downto 0 do begin
         ...
         for row:=Bitmap.Height-1 downto 0 do begin
            ...
            xbuf:=buf;
            col:=0;
            while col<Bitmap.Width do begin
               xbuf^:=qt.RGBIndex[p_rgb^]; inc(col); inc(p_rgb);
               xbuf^:=xbuf^ or (qt.RGBIndex[p_rgb^] shl 4); inc(col); inc(p_rgb);
               inc(xbuf);
            end;
            fs.Write(buf^,l);
         end;
      ...
      getmem(buf,l);
      if Bitmap.HasAlphaChannel and (integer(bc[i])<>24) then begin
         for row:=Bitmap.Height-1 downto 0 do begin
            fillchar(buf^,l,0);
            px:=Bitmap.AlphaChannel.ScanLine[row];
            for col:=0 to Bitmap.Width-1 do begin
...
{$WARNINGS OFF}
	lp:=fs.Position;
	result:=false;
	if fs.Size>(sizeof(TCURSORDIR)+sizeof(TCURSORDIRENTRY)) then begin
   	fs.Read(CurDir,sizeof(TCURSORDIR));
      if (CurDir.cdReserved<>0) or (CurDir.cdType<>2) or (CurDir.cdCount=0) then begin
      	fs.Position:=lp;
      	exit;	// FALSE
      end;
      for q:=0 to CurDir.cdCount-1 do begin
      	fs.Read(CurEntry,sizeof(TCURSORDIRENTRY));
         with CurEntry do
	         if (bReserved<>0) or (lBytesInRes=0) or (dwImageOffset<fs.Position) then begin
      	      fs.Position:=lp;
   	         exit; // FALSE
         	end;
      end;
   ...
   *)
   ...
   if InfoHead.biBitCount>0 then
   	// usa BitCount di BITMAPINFOHEADER
		xBitsPerSample:=InfoHead.biBitCount;
   if xBitsPerSample=0 then begin
   	// usa ColorCount di CurEntry, altrimenti la CUR non è valida
...
begin
	...
	repeat
		...
		if c='#' then begin
         ...
         result:='#'+trim(result);
         break;
      end else if ((c>='0') and (c<='9')) or ((c>='a') and (c<='z')) or ((c>='A') and (c<='Z')) then begin
      	// item
			result:=result+c;
         repeat
				if fs.Read(c,1)<1 then
...
begin
	...
	x:=0;
	magic:='';
   width:=0; height:=0; vmax:=0;
	repeat
      ss:=PBMReadItem(fs);
      if copy(ss,1,1)='#' then
      	ss:=ss
      else if magic='' then begin
      	magic:=ss;
...
begin
   ...
   if magic='P1' then begin
      ...
      until false;
   end else if magic='P2' then begin
   	// PGM (8bit) - ASCII
	   IOParams.BitsPerSample:=8;
	   IOParams.SamplesPerPixel:=1;
	   if Preview then
   ...
   end else if magic='P3' then begin
   	// PPM (24bit) - ASCII
	   IOParams.BitsPerSample:=8;
	   IOParams.SamplesPerPixel:=3;
	   if Preview then
   ...
   end else if magic='P4' then begin
   	// PBM (1bit) - RAWBITS
	   IOParams.BitsPerSample:=1;
	   IOParams.SamplesPerPixel:=1;
	   if Preview then
      ...
      _Negative1BitEx(Bitmap);
   end else if magic='P5' then begin
   	// PGM (8bit) - RAWBITS
	   IOParams.BitsPerSample:=8;
	   IOParams.SamplesPerPixel:=1;
	   if Preview then
      ...
      freemem(buf);
   end else if magic='P6' then begin
   	// PPM (24bit) - RAWBITS
	   IOParams.BitsPerSample:=8;
	   IOParams.SamplesPerPixel:=3;
	   if Preview then
...
begin
   ...
   //
	if ((IOParams.BitsPerSample=1) and (IOParams.SamplesPerPixel=1)) or (Bitmap.PixelFormat=ie1g) then begin
   	// P4 (PBM - 1 bit - RAW)
      ...
      freemem(buf);
   end else if (IOParams.BitsPerSample=8) and (IOParams.SamplesPerPixel=1) then begin
      ...
      freemem(buf);
   end else if ((IOParams.BitsPerSample=8) and (IOParams.SamplesPerPixel=3)) or (Bitmap.PixelFormat=ie24RGB) then begin
...
type TWBMPHeader = packed record
	typeField:byte;
   EndOfExt:byte;
   Width:byte;
   Height:byte;
...
begin
	// read header
   fs.Read(header,sizeof(TWBMPHeader));
   if (header.typeField<>0) or (header.EndOfExt<>0) then begin
   	Progress.Aborting^:=true;
      exit;
   end;
   // assign parameters
...
begin
	...
	if Bitmap.PixelFormat<>ie1g then begin
      ...
      wbitmap.PixelFormat:=ie1g;
   	...
   	wbitmap:=Bitmap;
   // resize
   if (wbitmap.Width>255) or (wbitmap.Height>255) then begin
   ...
   if (header.Width and $7)<>0 then
      inc(ll);
   for y:=0 to wbitmap.Height do begin
...
var
   ...
   var
   	j:integer;
   begin
   	px:=Bitmap.scanline[i]; inc(pbyte(px),offset);
      pb:=row;
   ...
   begin
   	pb:=row;
      l:=bitmap.width div 8; if (bitmap.width and 7)<>0 then inc(l);
      ...
      predline:=nil;
   	for j:=0 to bitmap.height-1 do begin
      	case IOParams.PS_Compression of
         	ioPS_G4FAX:
            	CCITTHuffmanPutLineG4(pbyte(Bitmap.Scanline[j]),Bitmap.Width,ms,bwr,bwrl,predline,Progress.Aborting^);
            ioPS_G3FAX2D:
            	CCITTHuffmanPutLineG32D(pbyte(Bitmap.Scanline[j]),Bitmap.Width,ms,bwr,bwrl,predline,Progress.Aborting^);
         end;
      end;
		case IOParams.PS_Compression of
      	ioPS_G4FAX:
	         CCITTHuffmanPutLineG4(nil,0,ms,bwr,bwrl,predline,Progress.Aborting^);
         ioPS_G3FAX2D:
	         CCITTHuffmanPutLineG32D(nil,0,ms,bwr,bwrl,predline,Progress.Aborting^);
      end;
      l:=ms.Size;
...
begin
   ...
   if (ioparams.SamplesPerPixel=1) and (ioparams.BitsPerSample>1) then begin
		ioparams.SamplesPerPixel:=3;
		ioparams.BitsPerSample:=8;
   end;
   if ioparams.SamplesPerPixel>3 then ioparams.SamplesPerPixel:=3;
   ...
   case ioparams.SamplesPerPixel of
      ...
      3:
         begin
         	if IOParams.PS_Compression=ioPS_JPEG then begin
            	ms:=TMemoryStream.Create;
               ...
               freemem(asciiout);
            	...
            	// RLW
               getmem(compressedrow, Bitmap.RowLen * 2);
               getmem(asciiout, bitmap.width*2 );
...
begin
   ...
   if ioparams.SamplesPerPixel=3 then begin
   	// color image
      if ioparams.PS_Compression=ioPS_JPEG then begin
...
begin
   ...
   with rec^ do begin
      // image stream
      image:=nil;
      if (IOParams.BitsPerSample=1) and (IOParams.SamplesPerPixel=1) then begin
      	// black/white
         rl:=bitmap.Width div 8; if (bitmap.Width and 7)<>0 then inc(rl);
         case IOParams.PDF_Compression of
            ...
            ioPDF_G4FAX:
            	begin
                  ...
                  for i:=0 to bitmap.height-1 do
                  	CCITTHuffmanPutLineG4(pbyte(Bitmap.Scanline[i]),Bitmap.Width,ms,bwr,bwrl,predline,Progress.Aborting^);
                  CCITTHuffmanPutLineG4(nil,0,ms,bwr,bwrl,predline,Progress.Aborting^);
            ...
            ioPDF_G3FAX2D:
            	begin
						CCITTCommonCodeBegin;
						for i:=0 to bitmap.height-1 do
                  	CCITTHuffmanPutLineG32D(pbyte(Bitmap.Scanline[i]),Bitmap.Width,ms,bwr,bwrl,predline,Progress.Aborting^);
						CCITTHuffmanPutLineG32D(nil,0,ms,bwr,bwrl,predline,Progress.Aborting^);
						CCITTCommonCodeEnd(1);
               end;
         end;
         ...
         image.dict.items.AddObject('BitsPerComponent',TIEPDFNumericObject.Create( 1 ));
      	...
      	// 24 bit
         if IOParams.PDF_Compression=ioPDF_JPEG then begin
         	// JPEG (DCT)
            ms:=TMemoryStream.Create;
            WriteJpegStream(ms,Bitmap,IOParams,Progress);
