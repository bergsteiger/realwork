...
(*$ifdef IEKYLIX*)
uses iekdef, Types, QGraphics, Classes, hyiedefs,hyieutils;
(*$endif*)
(*$ifdef IEDELPHI*)
uses Windows, {Wintypes, }Graphics, Classes, hyiedefs,hyieutils;
(*$endif*)
...
begin
   ...
   for i := 0 to FFTN-1 do begin
      for j := 0 to FFTN-1 do begin
         temp^[j] := image^[j]^[i];
   ...
   nc := FFTN;
	...
	if (H=nil) then begin
      ...
      for i:=0 to 255 do begin
         if (k <= 0) then
         	k := 0;
         while (k < H[i]) and (j<nr*nc) do begin
            ii := dij[j];
            jj := ii and 1023;
...
begin
   zeromemory(@result,sizeof(TIEComplexColor));
	with result do begin
      if assigned(fftr) then begin
         real_Red:=@fftr^[y]^[x];
         imag_Red:=@fftr^[y]^[x+FFTN];
      end;
      if assigned(fftg) then begin
         real_Green:=@fftg^[y]^[x];
         imag_Green:=@fftg^[y]^[x+FFTN];
      end;
      if assigned(fftb) then begin
         real_Blue:=@fftb^[y]^[x];
         imag_Blue:=@fftb^[y]^[x+FFTN];
      end;
      if assigned(fftgray) then begin
