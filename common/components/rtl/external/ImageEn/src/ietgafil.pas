...
begin
   ...
   if (TGAHead.Imagetype in [1,2,3,9,10,11]) and (TGAHead.Bits in [1,4,8,16,24,32]) and
      (TGAHead.ColorMaptype<2) then begin
      result:=true;
      Stream.Position:=sp;
      Stream.Read(c, 1);
      if c='P' then begin
      	Stream.Read(b, 1);
         if (b-48>01) and (b-48<7) then begin
         	Stream.Read(c, 1);
   ...
   begin
   	with rc^ do begin
         ...
         if FileOk then begin
            ...
            // Read in colormap
            MakeGenPalette(rc^);
            if TGAHead.ColorMaptype=1 then begin
               case TGAHead.ColorMapBits of
                  15,16:
                     for I:=TGAHead.ColorMapStart to TGAHead.ColorMapStart+TGAHead.ColorMapLength-1 do begin
                        W1:=FastGetWord;
               ...
               (*
               for I:=0 to TGAHead.ColorMapLength-1 do begin
                  IOParams.fColorMap^[I].r:=Palette256[I].r;
                  IOParams.fColorMap^[I].g:=Palette256[I].g;
                  IOParams.fColorMap^[I].b:=Palette256[I].b;
               end;
               *)
               IOParams.TGA_GrayLevel:=false;
            end else if BitsPerPixel=8 then begin
            	// gray level image (8bpp but without colormap)
               SetUpMaskGrayPalette(rc^);
               IOParams.TGA_GrayLevel:=true;
            end else if BitsPerPixel=1 then begin
            	// bilevel image
               ...
               if (B1 and $80)<>0 then begin
                  ...
                  case BitsPerPixel of
                     ...
                     24,32:begin
                        ...
                        G:=FastGetByte;
                        R:=FastGetByte;
                        if BitsPerPixel=32 then begin
                           B1:=FastGetByte;
                           if TempArrayAlpha<>nil then
                              TempArrayAlpha[col]:=B1;
                        end;
                        do24;
                     end;
                  end;
   ...
   zeromemory(rc,sizeof(TRC));
	with rc^ do begin
      ...
      if FileOK then begin
         ...
         case BitsPerPixel of
         	1:  begin IOParams.BitsPerSample:=1; IOParams.SamplesPerPixel:=1; end;
            8:  begin IOParams.BitsPerSample:=8; IOParams.SamplesPerPixel:=1; end;
            16: begin IOParams.BitsPerSample:=5; IOParams.SamplesPerPixel:=3; end;
            24: begin IOParams.BitsPerSample:=8; IOParams.SamplesPerPixel:=3; end;
            32: begin IOParams.BitsPerSample:=8; IOParams.SamplesPerPixel:=4; end;
         end;
         if not Preview then begin
            ...
            GetMem(TempArrayD2,LineBytes);
            if (not IgnoreAlpha) and (BitsPerPixel=32) and ((TGAHead.Descriptor and 8)<>0) then begin
               ...
               TempArrayAlpha:=nil;
            if ((ord(TGAHead.Descriptor)and 32)<>32) and ((ord(TGAHead.Descriptor)and 16)<>16) then begin
               StartLine:=Height-1;
               IncLine:=-1;
            end else begin
               StartLine:=0;
...
var
   ...
   qt:TIEQuantizer;
	...
	begin
      ...
      case DestBitsPerPixel of
         ...
         4,8:
         	begin
            	if (DestBitsPerPixel=8) and IOParams.TGA_GrayLevel then begin
               	// gray scaled image
               	TGAHead.ColorMaptype:=0;
                  if IOParams.TGA_Compressed then
                     TGAHead.Imagetype:=11
            ...
            8: // encode 8 bit row (1 byte)
               begin
                  ...
                  repeat
                     ...
                     inc(p);
                     l:=1;
                     while (p<bwidth) and (l8=TempArrayDBIG^[p]) and (l<128) do begin
                        inc(p); inc(l);
                     end;
                     if l>1 then begin
                        // encode as run-length packet
                        warr^:=$80 or (l-1); inc(warr);
                        warr^:=l8; inc(warr);
                     end else begin
                        // encode as raw packet
                        dec(p);
                        l:=0;
                        while (p<bwidth-1) and (TempArrayDBIG^[p]<>TemparrayDBIG^[p+1]) and (l<127) do begin
                           inc(p); inc(l);
                        end;
                        if p=bwidth-1 then begin
                        	inc(p); inc(l);
                        end;
                        warr^:=l-1; inc(warr);
                        CopyMemory(warr,@TempArrayDBIG^[p-l],l); inc(warr,l);
                     end;
                  until p>=bwidth;
               end;
            24:// encode 24 bit row (3 byte)
               begin
                  ...
                  repeat
                     ...
                     inc(p,3);
                     l:=1;
                     while (p<bwidth) and equalrgb(l24,PRGB(@TempArrayDBIG^[p])^) and (l<128) do begin
                        inc(p,3);
                        inc(l);
                     end;
                     if l>1 then begin
                        ...
                        dec(p,3);
                        l:=0;
                        while (p<bwidth-3) and (not equalrgb(PRGB(@TempArrayDBIG^[p])^,PRGB(@TemparrayDBIG^[p+3])^)) and (l<127) do begin
                           inc(p,3);
                           inc(l);
                        end;
                        if p=bwidth-3 then begin
                        	inc(p,3);
                           inc(l);
                        end;
                        warr^:=l-1;
   ...
   var
   	ms:word;
   begin
   	with rc^,Extension do begin
      	zeromemory(@Extension,sizeof(TGAExtension));
         strcopy(AuthorName,PAnsiChar(IOParams.TGA_Author));
         strcopy(JobName,PAnsiChar(IOParams.TGA_ImageName));
	...
	with rc^ do begin
      ...
      if IOParams.SamplesPerPixel=1 then begin
      	case IOParams.BitsPerSample of
         	1:
            	begin
                  if OrigBitsPerPixel=24 then begin
                     XBitmap:=_ConvertTo1bitEx(Bitmap, rgb1,rgb2);
                     if XBitmap=nil then begin
