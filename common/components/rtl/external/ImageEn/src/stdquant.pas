   ...
   with gvar^ do begin
   	with pBoxArr^[0] do begin
         ...
         for y := 0 to DMCoSi.czHei - 1 do begin
            ...
            for x := 0 to DMCoSi.czWid - 1 do begin
...
begin
  with gvar^ do begin
    ...
    for i := RedI to BlueI
    do begin
       if (FindCutPoint(gvar,pn, nBoxes[i, 0], nBoxes[i, 1], i))
...
begin
  with gvar^ do begin
    ...
    do begin
       ...
       if (CutBox(gvar,pBoxArr^[n], pBoxArr^[CurBox])) then inc(CurBox);
    end;
    Result := CurBox;
  end;
end;
...
begin
	with gvar^ do begin
   	for i := 0 to cHBROutCol - 1 do
    		with pBoxArr^[i] do begin
      		for r := Low[RedI] to High[RedI] - 1 do begin
         		rOff := r SHL Bits;
          		for g := Low[GreenI] to High[GreenI] - 1 do begin
            		gOff := (rOff OR g) SHL Bits;
             		for b := Low[BlueI] to High[BlueI] - 1 do begin
               		p := gOff OR b;
               		pMap^[p] := i;
             		end;
          		end;
...
begin
  ...
  with gvar^ do begin
    ...
    pBoxArr := nil;
    pMap    := nil;
    with DMCoSi do begin
       pqBMI := SrcBitmap;
       if pqBMi is TBitmap then begin
          with pqBMI as TBitmap do begin
             czWid := Width;
             czHei := Height;
          end;
       end else if pqBMi is TIEBitmap then begin
          with pqBMI as TIEBitmap do begin
             czWid := Width;
             czHei := Height;
          end;
       end;
