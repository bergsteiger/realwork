...
{**************************************************************************************************}
{                                                                                                  }
{ Last modified: $Date: 2012/04/04 06:37:30 $ }
{ Revision:      $Rev:: 3160                                                                     $ }
{ Author:        $Author: fireton $ }
{                                                                                                  }
{**************************************************************************************************}
...
type
  ...
  public
    ...
    function PeekChar(out Buffer: Char): Boolean; virtual;
    function PeekAnsiChar(out Buffer: AnsiChar): Boolean;
    function PeekWideChar(out Buffer: WideChar): Boolean;
    function ReadChar(out Buffer: Char): Boolean; virtual;
  ...
  public
    ...
    function PeekChar(out Buffer: Char): Boolean; override;
    function ReadChar(out Buffer: Char): Boolean; override;
...
function TJclAnsiStream.PeekChar(out Buffer: Char): Boolean;
var
  Pos: Int64;
begin
  Pos := FPosition;
  FPosition := FPeekPosition;
  Result := Self.Read(Buffer, SizeOf(Buffer)) = SizeOf(Buffer);
  FPeekPosition := FPosition;
  FPosition := Pos;
end;

function TJclAnsiStream.ReadChar(out Buffer: Char): Boolean;
begin
  Result := Self.Read(Buffer, SizeOf(Buffer)) = SizeOf(Buffer);
  FPeekPosition := FPosition;
end;
