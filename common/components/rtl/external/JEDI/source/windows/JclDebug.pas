...
// $Id: JclDebug.pas,v 1.9 2010/12/01 13:11:13 lulin Exp $
...
type
  ...
  public
    ...
    property TimeStamp: DWORD read FTimeStamp;
  end;
...
type
  ...
  public
    property CallerAddr: Pointer read GetCallerAdr;
...
begin
  ...
  if Module > 0 then
  begin
    NtHeaders := PeMapImgNtHeaders32(Pointer(Module));
...
begin
  ...
  try
    try
      ...
      if PeMapImgFindSection32(NtHeaders, JclDbgDataResName) <> nil then
      begin
        Result := True;
        Exit;
      end;
...
begin
  ...
  Result := (PeMapImgFindSection32(PeMapImgNtHeaders32(Pointer(Module)), JclDbgDataResName) <> nil);
  if Result then
...
begin
  ...
  try
   ...
   // - нефига падать при обработке исключений
   // http://mdp.garant.ru/pages/viewpage.action?pageId=219121705 
  end;//try..except
end;
...
begin
  Result := FStackInfo.CallerAddr - DWORD(ModuleFromAddr(CallerAddr));
end;
...
begin
  ...
  begin
    ...
    Item.FStackInfo.CallerAddr := DWORD(FirstCaller);
    Add(Item);
  end;
  if Raw then
    TraceStackRaw
...
begin
  ...
  begin
    ...
    if ValidCodeAddr(StackFrame^.CallerAddr, FModuleInfoList) and ValidStackAddr(StackFrame^.CallersEBP) then
    begin
      Inc(StackInfo.Level);
      ...
      if ValidCallSite(StackFrame^.CallerAddr, CallInstructionSize) then
        StackInfo.CallerAddr := StackFrame^.CallerAddr - CallInstructionSize
      else
        StackInfo.CallerAddr := StackFrame^.CallerAddr;
      if (StackFrame^.CallersEBP > DWORD(StackFrame)) then begin
       StackInfo.DumpSize := StackFrame^.CallersEBP - DWORD(StackFrame);
       StackInfo.ParamSize := (StackInfo.DumpSize - SizeOf(TStackFrame)) div 4;
      end else begin
       StackInfo.DumpSize := 0;
       StackInfo.ParamSize := 0;
      end;//StackFrame^.CallersEBP > DWORD(StackFrame)
