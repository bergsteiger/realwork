...
{**************************************************************************************************}
{                                                                                                  }
{ Last modified: $Date: 2013/05/24 15:59:59 $ }
{ Revision:      $Rev:: 3594                                                                     $ }
{ Author:        $Author: lulin $ }
{                                                                                                  }
{**************************************************************************************************}
...
begin
  ...
  begin
    Current := {$IfDef XE4}System.AnsiStrings.{$EndIf}AnsiStrPos(PAnsiChar(Current), PAnsiChar(SubStr));
    if Current <> nil then
    begin
      Last := Current;
      Inc(Current);
...
begin
  ...
  begin
    ...
    Inc(SPI, Index);
    Dec(SPI);
    SPI := {$IfDef XE4}System.AnsiStrings.{$EndIf}StrPos(SPI, SubP);
    if SPI <> nil then
      Result := SPI - SP + 1
    else
      Result := 0;
...
begin
  ...
  begin
    ...
    {$ENDIF ~SUPPORTS_UNICODE}
    {$IfDef XE4}System.AnsiStrings.{$EndIf}StrPCopy(List[I], S);
  end;
...
begin
  ...
  begin
    ...
    for I := 0 to Count - 1 do
      {$IfDef XE4}System.AnsiStrings.{$EndIf}StrDispose(List[I]);
...
begin
  ...
  for I := 0 to Source.Count - 1 do
    if Source[I] = '' then
      ...
      Inc(TotalLength, {$IfDef XE4}System.AnsiStrings.{$EndIf}StrLen(PAnsiChar(AnsiString(Source[I]))) + 1);
  AllocateMultiSz(Dest, TotalLength);
  P := Dest;
  for I := 0 to Source.Count - 1 do
  begin
    P := {$IfDef XE4}System.AnsiStrings.{$EndIf}StrECopy(P, PAnsiChar(AnsiString(Source[I])));
    Inc(P);
  end;
  P^ := #0;
  Result := Dest;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        Dest.Add(P);
        P := {$IfDef XE4}System.AnsiStrings.{$EndIf}StrEnd(P);
        Inc(P);
      end;
    end;
  finally
...
begin
  ...
  begin
    P := Source;
    repeat
      Inc(Result, {$IfDef XE4}System.AnsiStrings.{$EndIf}StrLen(P) + 1);
      P := {$IfDef XE4}System.AnsiStrings.{$EndIf}StrEnd(P);
      Inc(P);
    until P^ = #0;
    Inc(Result);
  ...
  begin
    ...
    begin
      ...
      if CaseInsensitive then
        Result := {$IfDef XE4}System.AnsiStrings.{$EndIf}AnsiStrLIComp(PAnsiChar(@S1[Cur1]), PAnsiChar(@S2[Cur2]), 1)
      else
        Result := {$IfDef XE4}System.AnsiStrings.{$EndIf}AnsiStrLComp(PAnsiChar(@S1[Cur1]), PAnsiChar(@S2[Cur2]), 1);
      Inc(Cur1);
      Inc(Cur2);
    end;
  end;
