...
var
  NullDWORD: {$IfDef XE4}DWord{$Else}PDWORD{$EndIf};
  Res: DWORD;
begin
  {$IfNDef XE4}NullDWORD := nil;{$EndIf XE4}
  if not ReadFile(PipeInfo.PipeRead, PipeInfo.Buffer[0], BufferSize, NullDWORD{$IfNDef XE4}^{$EndIf}, @Overlapped) then
  begin
    Res := GetLastError;
