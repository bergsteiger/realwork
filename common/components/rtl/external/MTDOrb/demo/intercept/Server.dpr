...
begin
  ...
  bank := TMyObj.Create as IServant;
...
procedure TMyObj.Foo(var str: AnsiString);
begin
  WriteLn(str);
  str := 'Server';
  //ReadLn;
...
begin
   ...
   *)
  WriteLn('server: input message(' + AnsiString(IntToStr(buf.WPosition - buf.RPosition)) + ' bytes)'+ThreadID);
  // simple decryption
...
begin
   ...
   *   data between buf->rpos() and buf->wpos().
   *)
  WriteLn('server: output message (' + AnsiString(IntToStr(buf.WPosition - buf.RPosition)) + ' bytes)'+ThreadID);
  // simple encryption
  for i := buf.RPosition to buf.WPosition - 1 do
    PByteArray(buf._buffer)^[i] := PByteArray(buf._buffer)^[i] xor $37;
  result := INVOKE_CONTINUE;
