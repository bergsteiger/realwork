...
type
  ...
  protected
    function bar(const s1: TS; out s2: TS; var s3: TS): TS; virtual; abstract;
    function bar1(const s1: TS2; out s2: TS2; var s3: TS2): TS2; virtual; abstract;
  ...
  protected
    function bar(const s1: TS; out s2: TS; var s3: TS): TS; virtual;
    function bar1(const s1: TS2; out s2: TS2; var s3: TS2): TS2; virtual;
    function narrow_helper(const str: RepositoryID): Pointer; override;
  end;
...
begin
  result := false;
  if not stat_string.demarshal(dec,@struct.a) then exit;
...
begin
  ...
  stat_string.marshal(enc,@struct.a);
...
// Tfoo_stub
//***********************************************************
function Tfoo_stub.bar(const s1: TS; out s2: TS; var s3: TS): TS;
var
  req: IStaticRequest;
  _s1: IStaticAny;
  _s2: IStaticAny;
...
begin
  ...
  req.add_in_arg(_s1);
  req.add_out_arg(_s2);
  req.add_inout_arg(_s3);
  req.set_result(_result);
  req.invoke;
  dorb_static_throw(req);
end;

function Tfoo_stub.bar1(const s1: TS2; out s2: TS2; var s3: TS2): TS2;
var
  req: IStaticRequest;
  _s1: IStaticAny;
  _s2: IStaticAny;
  ...
  _result: IStaticAny;
begin
  _s1 := StaticAny(S2_marshaller,@s1);
  _s2 := StaticAny(S2_marshaller,@s2);
  _s3 := StaticAny(S2_marshaller,@s3);
...
function Tfoo_stub_clp.bar(const s1: TS; out s2: TS; var s3: TS): TS;
var
  _srv: IServant;
  srv: Ifoo;
begin
...
function Tfoo_stub_clp.bar1(const s1: TS2; out s2: TS2; var s3: TS2): TS2;
var
  _srv: IServant;
  srv: Ifoo;
begin
...
function Tfoo_serv.bar(const s1: TS; out s2: TS; var s3: TS): TS;
begin
  result := FImplementation.bar(s1, s2, s3);
end;

function Tfoo_serv.bar1(const s1: TS2; out s2: TS2; var s3: TS2): TS2;
begin
  result := FImplementation.bar1(s1, s2, s3);
end;
...
begin
  ...
  if req.op_name = 'bar' then
    begin
      ...
      req.add_inout_arg(StaticAny(S_marshaller,@bar_s3) as IStaticAny);
    ...
    begin
      req.add_in_arg(StaticAny(S2_marshaller,@bar1_s1) as IStaticAny);
      req.add_out_arg(StaticAny(S2_marshaller,@bar1_s2) as IStaticAny);
      req.add_inout_arg(StaticAny(S2_marshaller,@bar1_s3) as IStaticAny);
      req.set_result(StaticAny(S2_marshaller,@bar1_result) as IStaticAny);
      if not req.read_args() then  exit;
      bar1_result := bar1(bar1_s1, bar1_s2, bar1_s3);
      req.write_results();
      exit;
    end;
  result := false;
...
initialization
  ...
  _tc_S2 := CreateTypeCodeConst('010000000f00000084000000010000000b00000049444c3a53323a312e3000000300000053320000020000000200000061000000120000'+
    '000000000002000000780000000f00000044000000010000000a00000049444c3a533a312e300000000200000053000000030000000200'+
    '00006100000003000000020000006200000003000000020000006300000009000000');
