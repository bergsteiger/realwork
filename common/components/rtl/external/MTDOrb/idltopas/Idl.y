...
%type <node> type_prefix_dcl attr_get_raises_expr attr_set_raises_expr
...
attr_dcl	: T_ATTRIBUTE param_type_spec simple_declarators
	          {$$ := TNode.Create ( nt_attribute, $2, $3 );}
                | T_ATTRIBUTE param_type_spec simple_declarators attr_get_raises_expr 
                  {$$ := TNode.Create ( nt_attribute, $2, $3, $4, nil);}
                | T_ATTRIBUTE param_type_spec simple_declarators attr_set_raises_expr
                  {$$ := TNode.Create ( nt_attribute, $2, $3, nil, $4 );}
                | T_ATTRIBUTE param_type_spec simple_declarators attr_get_raises_expr attr_set_raises_expr
                  {$$ := TNode.Create ( nt_attribute, $2, $3, $4, $5 );}
                | T_ATTRIBUTE param_type_spec simple_declarators attr_set_raises_expr attr_get_raises_expr 
                  {$$ := TNode.Create ( nt_attribute, $2, $3, $5, $4 );}
                | T_READONLY T_ATTRIBUTE param_type_spec simple_declarators raises_expr
                  {$$ := TNode.Create ( nt_readonly_attribute, $3, $4, $5 );}
		;
attr_get_raises_expr: T_GETRAISES T_LEFT_PARANTHESIS scoped_names T_RIGHT_PARANTHESIS
                  {$$ := TNode.Create ( nt_raises_expr, $3 );}
                ;
attr_set_raises_expr: T_SETRAISES T_LEFT_PARANTHESIS scoped_names T_RIGHT_PARANTHESIS
                  {$$ := TNode.Create ( nt_raises_expr, $3 );}
                ;
