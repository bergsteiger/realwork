...
procedure PrintError(msg: AnsiString);
procedure yyprintln(str: AnsiString='');
...
uses
  {$IFDEF LINUX}Libc,{$ENDIF}Sysutils;
  ...
  begin
    if (bufptr=0) and (yyinput.Position < yyinput.Size) then
      begin
        ...
        yyline := Trim(String(StrPas(PAnsiChar(@str))));
        inc(yylineno); yycolno := 1;
        buf[1] := nl;
        for i := 1 to length(yyline) do
          buf[i+1] := AnsiChar(yyline[length(yyline)-i+1]);
...
procedure reject;
  ...
  begin
    ...
    for i := yyleng+1 to yysleng do
      yytext := ShortString(yytext+AnsiChar(get_char));
    dec(yymatches);
  end(*reject*);
...
procedure yyscan;
  begin
    ...
    yyactchar := get_char;
    inc(yyleng);
    yytext[yyleng] := AnsiChar(yyactchar);
  end(*yyscan*);
...
var
  fulltext : AnsiString;
begin
  fulltext := AnsiString(filename) + '('+ AnsiString(inttostr(yylineno)) + '):' + msg;
{$IFDEF MSWINDOWS}
  FileWrite(GetStdHandle(STD_ERROR_HANDLE), PAnsiChar(fulltext)^, Length(fulltext));
{$ENDIF}
{$IFDEF LINUX}
  FileWrite(STDERR_FILENO, PAnsiChar(fulltext)^, Length(fulltext));
{$ENDIF}
  halt(1);
end;
...
begin
  if str <> '' then
    yyoutput.Write(PAnsiChar(str)^,length(str));
  yyoutput.Write(CRLF,2);
end;
