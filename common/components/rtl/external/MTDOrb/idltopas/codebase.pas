...
type
  ...
  protected
    function in_list(name : AnsiString; list : TStrings) : Boolean;
    procedure add_list(name : AnsiString; list : TStrings);

    function get_name(cont : IContained) : Identifier;
    function declare(name: Identifier) : Boolean;
    function add_marshaller(name: Identifier; it: IIRObject): Boolean;
    function base_sequence_str(seq : ISequenceDef): AnsiString;
  public
    function attribute_str(attr: IAttributeDef; write: Boolean): AnsiString;
    procedure declare_marshaller(cont : IIDLType);
    function operation_str(op: IOperationDef): AnsiString;
    function param_str(p: ParameterDescription): AnsiString;
    function type_str(t: IIRObject; recursive: Boolean = false): AnsiString;
    ...
    procedure declare_all_methods2(int: IInterfaceDef; suffix: AnsiString = ''; except_first: Boolean = false);
    procedure declare_all_methods(int: IInterfaceDef; suffix: AnsiString = ''; except_first: Boolean = false);
    procedure declare_attributes(int: IInterfaceDef; suffix: AnsiString = ''; write_properties: Boolean = false);
    procedure declare_methods(int: IInterfaceDef; suffix: AnsiString = '');
    procedure stub_attribute(name: Identifier;attr: IExtAttributeDef; for_abstract: Boolean);
    procedure stub_method(name: Identifier;op: IOperationDef; for_abstract: Boolean);
    procedure stub_clp_attribute(name: Identifier;attr: IAttributeDef);
    procedure stub_clp_attribute2(name, basename: Identifier; attr: IAttributeDef);
    procedure stub_clp_method(name, basename: Identifier;op: IOperationDef);
    function marshaller_for(it: IIDLType): Identifier;
    procedure method_dispatch(cname: Identifier; int: IInterfaceDef; passenv: Boolean);
    ...
    procedure attr_method_call(attr: IExtAttributeDef; var firstcall: Boolean);
    procedure int_marshaller(int: IInterfaceDef);
    function def_value(it: IIDLType): AnsiString;
    procedure struct_marshaller(cont : IContained; modname : AnsiString);
    ...
    function initializer_decl(init: Initializer; target: IValueDef): AnsiString;
    procedure value_member(vmd: IValueMemberDef; all: boolean);
    procedure marshaller_decl(name : Identifier; it: IIRObject);
    function marshaller_for_array(arr: IArrayDef): AnsiString;
    function marshaller_for_seq(s: ISequenceDef): AnsiString;
    function array_str(arr: IArrayDef): AnsiString;
    procedure ex_members_decl(ename: Identifier; members: StructMemberSeq; mode : TExcept_declare_mode = ed_all);
    procedure except_marshaller(ex : IExceptionDef);
    function expand_name(t: IIDLType; recursive: Boolean = false): AnsiString;
    procedure add_forwards(int: IContained);
    procedure common_servant_methods_declaration;
    procedure serv_method(name: Identifier; op: IOperationDef; const intf: AnsiString = '');
    procedure serv_attribute(name: Identifier; attr: IAttributeDef; const intf: AnsiString = '');
  public
...
//  TCodebase
//*********************************************************************
function TCodebase.get_name(cont : IContained) : AnsiString;
begin
...
function TCodebase.attribute_str(attr: IAttributeDef; write: Boolean): AnsiString;
var
  tstr,name: AnsiString;
begin
...
function TCodebase.operation_str(op: IOperationDef): AnsiString;
var
...
begin
  if op._result.kind = tk_void then
    ...
    result := 'function ';
  result := result + safe_name(op._name);
...
function TCodebase.param_str(p: ParameterDescription): AnsiString;
begin
...
function TCodebase.type_str(t: IIRObject; recursive: Boolean): AnsiString;
var
  tname : AnsiString;
  bound : longint;
begin
  case t.def_kind of
      ...
      if result <> '' then
        ...
        begin
          ...
          if bound = 0 then
            ...
            result := 'Array [0..'+AnsiString(inttostr(bound-1))+'] of '+tname
        end;
    end;
    ...
    dk_Native: result := (t as IContained)._name;
    dk_Primitive :
...
procedure TCodebase.declare_all_methods(int: IInterfaceDef; suffix: AnsiString = ''; except_first: Boolean = false);
var
  i,n : integer;
  base: IInterfaceList;
  tmp: IInterfaceDef;
...
procedure TCodebase.declare_all_methods2(int: IInterfaceDef; suffix: AnsiString = ''; except_first: Boolean = false);
var
  i: integer;
  all : IInterfaceList;
begin
...
procedure TCodebase.declare_attributes(int: IInterfaceDef; suffix: AnsiString; write_properties: Boolean);
var
...
procedure TCodebase.declare_methods(int: IInterfaceDef; suffix: AnsiString);
var
...
var
  name,shift,ename,aname: AnsiString;
  params : ParDescriptionSeq;
...
procedure TCodebase.stub_attribute(name: AnsiString;attr: IExtAttributeDef; for_abstract: Boolean);
var
  stub_name, attrname, tname, ename: AnsiString;
  i, len: Integer;
  ex : IExceptionDef;
begin
  attrname := attr._name;
  ...
  if attr.get_mode = ATTR_NORMAL then
    begin
      ...
      len := Length(attr.setExceptions);
      if len > 0 then begin
        write('  req.set_exceptions_tc([');
        for i := 0 to Pred(len) do begin
          if i > 0 then write(', ');
          ex := attr.setExceptions[i];
      ...
      for i := 0 to Pred(len) do
        begin
          ...
          ex := attr.setExceptions[i];
          ename := get_name(ex as IContained);
          write(ename+'_marshaller, '''+ex._id+'''');
  ...
  len := Length(attr.getExceptions);
  if len > 0 then begin
    write('  req.set_exceptions_tc([');
    for i := 0 to Pred(len) do begin
      if i > 0 then write(', ');
      ex := attr.getExceptions[i];
      ename := get_name(ex as IContained);
      write('_tc_'+ename+'.typecode');
    end;
    writeln(']);');
...
procedure TCodebase.stub_method(name: AnsiString; op: IOperationDef; for_abstract: Boolean);
var
  stub_name,ename, operation: AnsiString;
  i,j,len : integer;
  params: ParDescriptionSeq;
  res: IIDLType;
  ex : IExceptionDef;
begin
  ...
  if for_abstract then
    writeln('  req := StaticRequest(self._to_object,'''+op._name+''');')
  else
    writeln('  req := StaticRequest(self,'''+op._name+''');');
  for i := 0 to Pred(Length(params)) do
...
function TCodebase.marshaller_for(it: IIDLType): AnsiString;
var
  idef: IInterfaceDef;
  seq: ISequenceDef;
  tc: ITypeCode;
...
var
  name,mname,iname: AnsiString;
begin
  if FDB.repoid_defined(int._id) then exit;
  name := get_name(int);
  if in_list( name+'_marshaller', FImplemented) then exit;
  add_list(name+'_marshaller', FImplemented);
  iname := 'I'+name;
...
function TCodebase.def_value(it: IIDLType): AnsiString;
begin
  case it.def_kind of
...
procedure TCodebase.marshaller_decl(name : AnsiString; it: IIRObject);
begin
...
var
  id,name,mname,tstr: AnsiString;
  members: StructMemberSeq;
  member : StructMember;
  i, mlen : integer;
begin
  members := nil;
  if FDB.repoid_defined(ex._id) then exit;
  name := get_name(ex);
  id := ex._id;
  if in_list(name+'_marshaller', FImplemented) then exit;
  add_list(name+'_marshaller', FImplemented);
...
procedure TCodebase.struct_marshaller(cont : IContained; modname : AnsiString);
var
  name,mname: AnsiString;
  members: StructMemberSeq;
  i : integer;
  sdef: IStructDef;
begin
  ...
  writeln('  struct: ' + modname + '_int.T' + safe_name(name) + ';');
  writeln('begin');
  writeln('  struct := ' + modname + '_int.T' + safe_name(name) + '(addr^);');
  for i := 0 to Pred(length(members)) do
  ...
  writeln('  System.Finalize(' + modname + '_int.T' + safe_name(name) + '(addr^));');
  writeln('{$HINTS ON}');
  ...
  writeln('procedure '+mname+'._create(var addr: Pointer);');
  writeln('begin');
  writeln('  addr := AllocMem(sizeOf(' + modname + '_int.T'+safe_name(name)+'));');
  writeln('{$HINTS OFF}'); //some records do not need Initialize
  writeln('  System.Initialize(' + modname + '_int.T' + safe_name(name) + '(addr^));');
  writeln('{$HINTS ON}');
  writeln('end;');
  writeln;
  // _assign
  writeln('procedure '+mname+'._assign(dst, src: Pointer);');
  writeln('begin');
  writeln('  ' + modname + '_int.T' + safe_name(name) + '(dst^) := ' + modname + '_int.T' + safe_name(name) + '(src^);');
  writeln('end;');
  writeln;
  //if FParams.IndexOf('-any') > 0 then
    //begin
      ...
      if member._label.get_type.kind = tk_octet then
        ...
        begin
          if discr.def_kind = dk_Enum then
            ...
            tmp := '''' + AnsiString(member._label.get_variant) + ''''
          else
            tmp := AnsiString(member._label.get_variant);
          write('    '+tmp);
          while (i+1 < cnt) and (members[i+1].name = member.name) do
            begin
              tmp := AnsiString(members[i+1]._label.get_variant);
              write(', '+ tmp);
              inc(i);
            end;
          writeln(' : begin');
...
var
  name,mname,tstr,str: AnsiString;
  bound : longint;
  t: IIDLType;
begin
...
function TCodebase.initializer_decl(init: Initializer; target: IValueDef): AnsiString;
var
  j: integer;
  memb : StructMember;
begin
...
procedure TCodebase.stub_clp_attribute(name: AnsiString;attr: IAttributeDef);
var
  stub_name, attrname, tname: AnsiString;
  val : IIDLType;
begin
  attrname := attr._name;
...
procedure TCodebase.stub_clp_attribute2(name, basename: AnsiString; attr: IAttributeDef);
var
  stub_name, attrname, tname: AnsiString;
  val : IIDLType;
begin
...
procedure TCodebase.stub_clp_method(name, basename: AnsiString; op: IOperationDef);
var
  stub_name, operation: AnsiString;
  i,j : integer;
  params: ParDescriptionSeq;
  res: IIDLType;
begin
  ...
  writeln('      srv := T'+basename+'_serv._narrow(_srv);');
  writeln('      if srv <> nil then');
  operation := 'srv.'+safe_name(op._name);
  res := op.get_result_def;
  if res._type.kind <> orbtypes.tk_void then
...
procedure TCodebase.method_dispatch(cname: AnsiString; int: IInterfaceDef; passenv: Boolean);
var
...
begin
  ...
  for i := 0 to Pred(attributes.Count) do
    begin
      if not firstcall then
        writeln;
      attr_method_call(attributes[i] as IExtAttributeDef,firstcall);
    end;
  for j := 1 to Pred(base.count) do
    begin
      methods := (base[j] as IContainer).contents(dk_Operation);
...
begin
  ...
  // declare array
  writeln('type ');
  writeln('  '+name+' = array [0..' + AnsiString(inttostr(size-1)) + '] of '+ tstr + ';');
  writeln('  P'+name+' = ^'+ name+';');
  writeln;
  writeln('function '+mname+'.demarshal(dec: IDecoder;const addr : Pointer): Boolean;');
  writeln('var');
...
function TCodebase.expand_name(t: IIDLType; recursive: Boolean = false): AnsiString;
var
  otype: IIDLType;
begin
  ...
  case otype.def_kind of
    dk_Array : with otype as IArrayDef  do
       begin
         result := 'A' + AnsiString(inttostr(length)) + '_' + expand_name(element_type_def, true);
    ...
    dk_Sequence : with otype as ISequenceDef  do
      begin
        if bound > 0 then
          result := 'B'+ AnsiString(inttostr(bound)) + '_' + expand_name(element_type_def, true)
        else
...
function TCodebase.marshaller_for_seq(s: ISequenceDef): AnsiString;
begin
  result := expand_name(s);
end;

function TCodebase.marshaller_for_array(arr: IArrayDef): AnsiString;
begin
...
begin
  result := t;
  while result.def_kind = dk_alias do
    result := (result as IAliasDef).original_type_def;
end;

function TCodeBase.array_str(arr: IArrayDef): AnsiString;
begin
  result := 'array [0..' + AnsiString(inttostr(arr.length-1)) + '] of ';
...
function TCodebase.declare(name: Identifier): Boolean;
begin
  if not in_list(name, FDeclared) then
    begin
      add_list(name, FDeclared);
      result := true;
    end
  else
    result :=  false;
...
function TCodebase.in_list(name: AnsiString; list : TStrings) : Boolean;
begin
  Result := list.IndexOf(String(name)) >=0;
end;

procedure TCodebase.add_list(name: AnsiString; list: TStrings);
begin
  if not in_list(name, list) then
      list.Add(String(name));
end;

function TCodebase.add_marshaller(name: AnsiString; it: IIRObject): Boolean;
begin
  if not in_list(name, Marshallers) then
    begin
      Marshallers.addObject(String(name), TObject(it));
      result := true;
    end
  else
    result := false;
...
procedure TCodebase.ex_members_decl(ename: AnsiString; members: StructMemberSeq; mode : TExcept_declare_mode);
var
  i : integer;
  member : StructMember;
  tstr : AnsiString;
...
begin
  ...
  writeln('begin');
  writeln('  result := TValueBase._demarshal(dec,vb,''' + cont._id + ''');');
...
begin
  writeln('    function _primary_interface(const objid: ObjectID; const poa: IPOA): RepositoryID; override;');
...
procedure TCodebase.serv_method(name: AnsiString; op: IOperationDef; const intf: AnsiString);
var
  stub_name, operation: AnsiString;
  params: ParDescriptionSeq;
  i, j : integer;
...
begin
  ...
  writeln('begin');
  if intf <> '' then
    operation := '(FImplementation as ' + intf + ').'+safe_name(op._name)
  else
    operation := 'FImplementation.'+safe_name(op._name);
  res := op.get_result_def;
  if res._type.kind <> orbtypes.tk_void then
    operation := 'result := '+operation;
  params := op.get_params;
...
procedure TCodebase.serv_attribute(name: AnsiString; attr: IAttributeDef; const intf: AnsiString);
var
  stub_name, operation: AnsiString;
  j : integer;
begin
...
procedure TCodebase.attr_method_call(attr: IExtAttributeDef;
  var firstcall: Boolean);
var
  name: AnsiString;

  procedure attr_except_block(get: Boolean);
  var
    rlen, i: Integer;
    exceptions: ExceptionDefSeq;
    shift, aname, ename: AnsiString;
  begin
    if get then
      exceptions := attr.getExceptions
    else
      exceptions := attr.setExceptions;
    shift := '';
    rlen := Length(exceptions);
    if rlen > 0 then
      begin
        write('      req.set_exceptions_tc([');
        for i := 0 to Pred(rlen) do begin
          if i > 0 then write(', ');
          ename := get_name(exceptions[i]);
          write('_tc_'+ename+'.typecode');
        end;
        writeln(']);');
        writeln('      try');
    ...
    if get then
      writeln(shift+'      '+name+'_get_value := _get_'+name+';')
    else
      writeln(shift+'      _set_'+name+'('+name+'_set_value);');
    ...
    if rlen > 0 then
      begin
        ...
        for i := 0 to Pred(rlen) do
          begin
            ename := exceptions[i]._name;
            aname := 'T'+get_name(exceptions[i]);
            writeln('        on '+ ename + ': '+aname+' do');
            writeln('          req.set_exception('+aname+'.Create('+ename+') as IORBException);');
          end;
        writeln('      end;');
...
function TCodebase.base_sequence_str(seq: ISequenceDef): AnsiString;
var
  otype: IIDLType;
begin
