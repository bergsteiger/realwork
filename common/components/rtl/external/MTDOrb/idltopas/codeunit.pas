...
type
  ...
  private
    ...
    procedure InternalGenerate(const AName: AnsiString; const AContents: IInterfaceList);
    procedure GenerateInterface(const AName: AnsiString; const AContents: IInterfaceList);
    procedure GenerateImplementation(const AName: AnsiString; const AContents: IInterfaceList);
    procedure ProcessRmic(const RepoId: AnsiString);
  protected
    // interface unit
    ...
    procedure int_declare_internal_type(cont_name, name: AnsiString; t: IIDLType);
    ...
    procedure code_impl_struct(str: IStructDef; modname : AnsiString);
    procedure code_impl_interface(int: IInterfaceDef; modname : AnsiString);
    procedure code_impl_value(val: IValueDef);
    procedure code_impl_value_box(val: IValueBoxDef);
    procedure code_impl_module(m: IModuleDef; modname : AnsiString);
    procedure code_impl_union(u: IUnionDef; modname : AnsiString);
    ...
    procedure final_attribute(name: AnsiString; attr: IAttributeDef);
    procedure final_method(name: AnsiString; def: IOperationDef);

    procedure impl_code(cont: IIRObject; modname : AnsiString);
    ...
    function get_name(cont : IContained) : AnsiString;  overload;
    function get_name(int: IInterfaceDef) : AnsiString;  overload;
  public
...
function TCodeGen.get_name(cont : IContained) : AnsiString;
begin
   if in_list('-without-prefix', FParams) or name_without_prefix(cont) then
      Result := cont._name
   else
      Result := cont._absolute_name;
end;

function TCodeGen.get_name(int: IInterfaceDef) : AnsiString;
begin
   if in_list('-without-prefix', FParams) or name_without_prefix(int) then
      Result := int._name
   else
...
var
  name,usesstr: AnsiString;
  i,cnt : integer;
  contents: IInterfaceList;

  function _ExtractFileName(str: AnsiString): AnsiString;
  var
    I: Integer;
  begin
    I := LastDelimiter('\:/', String(str));
  ...
  begin
    ...
    for i := 0 to module.Count - 1 do begin
      ...
      for j := 0 to other.Count - 1 do
        if not Fdb.repoid_defined((other[j] as IContained)._id) then begin
          Result := True;
          Break;
        end;
  end;

  procedure GenerateModule(const ACont: IContainer; const AName: AnsiString);
  var
    module, other: IInterfaceList;
    i: Integer;
    name: AnsiString;
  begin
    ...
    for i := 0 to module.Count - 1 do begin
      if {not Fdb.repoid_defined((module[i] as IContained).id)} HaveUndefined(module[i] as IContainer) then begin
        name := AName + '_' + (module[i] as IContained)._name;
        GenerateModule(module[i] as IContainer, name);
      end;
    end;
    FIncludes.Clear;
    dependencies(FRepository.contents());
    if in_list(AName, FIncludes) then
      FIncludes.Delete(FIncludes.IndexOf(String(AName)));
    other := ACont.contents();
...
begin
  try
    name := _ExtractFileName(AnsiString(FParams[0]));
    if Pos('.',String(name)) > 0 then
      name := AnsiString(Copy(String(name),1,Pos('.',String(name))-1));

    if in_list('-ns', FParams) then begin
    ...
    if in_list('-impl', FParams) then
      begin
        assign(output, String(name) + '_impl.pas');
        Rewrite(output);
        write_header();
            ...
            for i := 0 to cnt do
              begin
                if (length(usesstr) > 80) and (i <> cnt) then
                  begin
                    writeln(usesstr + ',');
                    usesstr := '  ' + AnsiString(FIncludes[i]) + '_int';
                  end
                else
                  usesstr := usesstr + ', ' + AnsiString(FIncludes[i]) + '_int';
...
{** calls class implementation method for each
    definition kind }
procedure TCodegen.impl_code(cont: IIRObject; modname : AnsiString);
begin
  case cont.def_kind of
    dk_Interface : code_impl_interface(cont as IInterfaceDef, modname);
    dk_Struct : code_impl_struct(cont as IStructDef, modname);
    ...
    dk_Module : code_impl_module(cont as IModuleDef, modname);
    dk_Union : code_impl_union(cont as IUnionDef, modname);
...
begin
  ...
  writeln('//                                                                            ');
  writeln('// ***************************************************************************');
  write('// Compiled with parameters: "');
...
begin
  if oper._result.kind in [tk_objref,tk_struct,tk_union] then
    begin
      ...
      if (cont <> nil) and not in_list(get_name(cont), FDeclared) then
    ...
    begin
      ...
      if paramdesc._type.kind in [tk_objref,{tk_struct,}tk_union] then
        begin
          ...
          if not in_list(get_name(cont), FDeclared) then
            add_forwards(cont);
        end;
    end;
end;
    ...
    begin
      if members[i].type_def.def_kind in [dk_Interface,dk_Value] then
        begin
          ...
          if defin = cont then
            begin
              if FDB.repoid_defined(cont._id) then exit;
              if FForwards.IndexOf(cont) < 0 then FForwards.add(cont);
            end
          else if not in_list(get_name(cont), FDeclared) then
            add_forwards(cont);
        end;
...
var
  ...
  function _extractfilename(name: AnsiString): AnsiString;
  var
    idx, idx1: Integer;
    str: AnsiString;
  begin
    result := AnsiString(extractfilename(String(name)));
    if FParams.IndexOf('-ns') >= 0 then begin
      idx := pos('/', String(result));
      idx1 := 0;
      ...
      while pos('/', String(result)) > 0 do
        result := copy(result,pos('/', String(result))+1,1000)
  end;
begin
  for i := 0 to pred(contents.count) do
    begin
      id := (contents[i] as IContained)._id;
      if Fdb.repoid_defined(id) then
        begin
          ...
          //if not in_list(fname, FIncludes) then
            add_list(fname, FIncludes);
        end;
...
var
  t : IIDLType;
begin
  if FDB.repoid_defined((cont as IContained)._id) then exit;
...
begin
  if FDB.repoid_defined((enum as IContained)._id) then exit;
...
var
  name,iname,cname,basenames,bname,boname: AnsiString;
  base: IInterfaceList;
  i : integer;
  stub_name,skel_name , mdirective: AnsiString;
  contents : IInterfaceList;
  abstract_base_class : Boolean;
begin
  if FDB.repoid_defined(int._id) then exit;
  ...
  writeln('    function narrow_helper(const str: RepositoryID): Pointer; override;');
  writeln('  public');
  if int.get_is_abstract then
    begin
  ...
  //-----------  stub --------------------
  writeln('  {** stub for interface '+int._id+'} ');
  mdirective := ' override;';
  ...
  if FParams.IndexOf('-poa') < 0 then
    // BOA
    begin
      writeln('  {** skeleton for interface '+int._id+'} ');
    ...
    begin
      //-----------  POA stub --------------------
      writeln('  {** POA stub for interface '+int._id+'} ');
      if base.count = 0 then
      ...
      writeln;
      //-----------  servant --------------------
      writeln('  {** servant for interface '+int._id+'} ');
      if base.count = 0 then
...
begin
  members := nil;
  if FDB.repoid_defined(e._id) then exit;
  ...
  writeln('    function repoid(): RepositoryID; override;');
...
begin
  ...
  if s.bound = 0 then
    ...
    writeln('  '+name+' = Array [0..'+AnsiString(IntToStr(s.bound-1))+'] of '+ tstr+';');
  writeln('  P'+name+' = ^'+ name+';');
  writeln;
...
var
  name, typestr, last_name: AnsiString;
  member: UnionMember;
  members: UnionMemberSeq;
  i : integer;
begin
  members := nil;
  if FDB.repoid_defined((u as IContained)._id) then exit;
...
begin
  members := nil;
  if FDB.repoid_defined((str as IContained)._id) then exit;
...
var
  name, impl_name, int_name, type_name : AnsiString;
begin
  if FDB.repoid_defined((val as IContained)._id) then exit;
  name := get_name(val as IContained);
  impl_name := 'T'+name;
  int_name := 'I'+name;
  writeln('  {** '+(val as IContained)._id+' } ');
  ...
  writeln('    function narrow_helper(const repoid: RepositoryID): Pointer; override;');
  writeln('  public');
...
var
  name, impl_name, int_name : AnsiString;
...
begin
  ...
  if supported.count > 0 then
    begin
      write('I'+(supported[0] as IContained)._name+',');
  ...
  for i := 0 to Pred(mlist.count) do
    begin
      ...
      writeln('    F' + name + '_' + vmd._name + ' : ' + type_str(vmd.get_type_def) + ';');
    end;
  for i := 0 to Pred(mlist.count) do
  ...
  if Length(val.initializers) > 0 then
    begin
      writeln('  {** factory for valuetype '+(val as IContained)._id+'} ');
      ...
      writeln('    function narrow_helper(const repoid: RepositoryID): Pointer;');
...
begin
  ...
  for i := 0 to Pred(list.count) do
    begin
      ...
      write('_'+vmd._name+': '+type_str(vmd.get_type_def));
    end;
  writeln(');');
  writeln('begin');
  writeln('  // TODO');
...
begin
  ...
  writeln('end;');
  writeln;
  writeln('function T'+name+'.repoid(): RepositoryID;');
  writeln('begin');
  writeln('  result := '''+id+''';');
  writeln('end;');
  writeln;
...
procedure TCodegen.code_impl_union(u: IUnionDef; modname : AnsiString);
var
  i,ind : integer;
  members: UnionMemberSeq;
  member: UnionMember;
  typestr,name, val, last_name: AnsiString;
  discr: IIDLType;
  enum : IEnumDef;
  ...
  for i := 0 to Pred(Length(members)) do
    case members[i].type_def.def_kind of
      dk_Sequence : if not is_base_sequence(members[i].type_def as ISequenceDef) then
        seq_marshaller(members[i].type_def as ISequenceDef);
      dk_Array : array_marshaller(members[i].type_def as IArrayDef);
      dk_Struct : struct_marshaller(members[i].type_def as IContained, modname);
    end;
  union_marshaller(u);
  writeln('//***********************************************************');
  writeln('// T'+name);
  ...
  for i := 0 to Pred(Length(members)) do
    begin
      ...
      if member._label.get_type.kind = tk_octet then
        val := AnsiString(get_default_value_for_union(u.discriminator_type_def,members))
        ...
        val := '''' + AnsiString(member._label.get_variant) + ''''
      else
        val := AnsiString(member._label.get_variant);
...
var
  name, cname, tname, repoid : AnsiString;
begin
  if FDB.repoid_defined((val as IContained)._id) then exit;
  name := get_name(val as IContained);
  repoid := (val as IContained)._id;
  cname := 'T'+name;
  if val.original_type_def.def_kind = dk_Sequence then begin
    if is_base_sequence(val.original_type_def as ISequenceDef) then
  ...
  writeln;
  // narrow_helper
  writeln('function '+cname+'.narrow_helper(const repoid: RepositoryID): Pointer;');
  writeln('begin');
  writeln('  result := nil;');
  writeln('  if strcomp(PAnsiChar(repoid),'''+repoid+''') = 0 then');
...
var
  name, cname, tname, mname, fname, repoid : AnsiString;
...
begin
      ...
      for j := 0 to Pred(alist.count) do
        begin
          ...
          if attr.get_mode = ATTR_NORMAL then
            begin
              writeln('procedure '+cname+'._set_'+attr._name+'(const val: '+tname+');');
              writeln('begin');
              writeln;
              writeln('end;');
              writeln;
          ...
          writeln('function '+cname+'._get_'+attr._name+': '+tname+';');
          writeln('begin');
          writeln('  result := '+def_value(res)+';');
          writeln('end;');
          writeln;
  ...
  for i := 0 to Pred(supported.count) do
    begin
      writeln('  else if strcomp(PAnsiChar(repoid),'''+(supported[i] as IInterfaceDef)._id+''') = 0 then');
  ...
  for i := 0 to Pred(list.count) do
    begin
      vmd := list[i] as IValueMemberDef;
      writeln('  '+vmd._name+'(other.'+vmd._name+');');
    end;
  writeln('end;');
  writeln;
  // _copy_value
  ...
  writeln('  try');
  writeln('    result := _create(list,'''+(val as IContained)._id+''');');
  ...
  if Length(iseq) > 0 then
    begin
      ...
      writeln;
      // narrow_helper
      writeln('function '+fname+'.narrow_helper(const repoid: RepositoryID): Pointer;');
      writeln('begin');
      writeln('  result := nil;');
      writeln('  if strcomp(PAnsiChar(repoid),'''+repoid+''')= 0 then');
...
procedure TCodegen.code_impl_module(m: IModuleDef; modname : AnsiString);
var
  i : integer;
  contents : IInterfaceList;
begin
  //if FDB.repoid_defined(m._id) then exit;
  contents := (m as IContainer).contents();
  for i := 0 to Pred(contents.count) do
    impl_code(contents[i] as IContained, modname);
end;

procedure TCodegen.code_impl_interface(int: IInterfaceDef; modname : AnsiString);
var
  id_str, name, cname, iname, biname: AnsiString;
  bid_str, stub_name, skel_name: AnsiString;
  ...
  begin
    ...
    list := (int as IContainer).contents(dk_Attribute);
    for i := 0 to Pred(list.count) do
      stub_attribute(name,list[i] as IExtAttributeDef,int.get_is_abstract);
  end;
  ...
  begin
      ...
      for i := 0 to Pred(all.count) do begin
          ...
          for j:=0 to Pred(list.count) do begin
              stub_attribute(name,list[j] as IExtAttributeDef,int.get_is_abstract);
          end;
      end;
  end;
  ...
  begin
    ...
    writeln('//***********************************************************');
    // narrow_helper
    writeln('function '+cname+'_stub_clp.narrow_helper(const str: RepositoryID): Pointer;');
    writeln('begin');
    ...
    if (base.count > 0) then begin
      ...
      for i := j to Pred(base.count) do begin
        ...
        for j:=0 to Pred(ibase.count) do begin
          writeln('    end');
          writeln('  else if str = '''+(ibase[j] as IInterfaceDef)._id+''' then');
          writeln('    begin');
...
begin
  ...
  base := int.get_base_interfaces;
  // narrow_helper
  writeln('function '+cname+'.narrow_helper(const str: RepositoryID): Pointer;');
  writeln('begin');
  id_str := ''''+int._id+'''';
  ...
  writeln('begin');
  writeln('  result := obj._repoid = '+id_str);
  writeln('end;');
  writeln;
  ...
  if FParams.IndexOf('-poa') < 0 then
    begin
      ...
      for j := 1 to Pred(base.count) do
        begin
          ...
          writeln('  impl := find_impl('''+baseint._id+''','''+get_name(baseint)+''');');
          writeln('  create_ref('''',nil,impl,'''+baseint._id+''');');
        end;
      writeln('  register_dispatcher(IStaticInterfaceDispatcher(self));');
      writeln('end;');
      writeln;
      method_dispatch(skel_name,int,true);
      writeln('function '+skel_name+'.narrow_helper(const str: RepositoryID): Pointer;');
      writeln('begin');
      if base.count = 0 then
        writeln('  result := nil;');
      writeln('  if str = '+id_str+' then');
    ...
    begin
      ...
      writeln;
      // _is_a
      writeln('function '+cname+'_serv._is_a(const repoid: RepositoryID): Boolean;');
      writeln('begin');
      ...
      writeln;
      // _primary_interface
      writeln('function '+cname+'_serv._primary_interface(const objid: ObjectID; const poa: IPOA): RepositoryID;');
      writeln('begin');
      ...
      writeln;
      // narrow_helper
      writeln('function '+cname+'_serv.narrow_helper(const str: RepositoryID): Pointer;');
      writeln('begin');
      if base.count = 0 then
        writeln('  result := nil;');
      writeln('  if str = '+id_str+' then');
...
procedure TCodegen.code_impl_struct(str: IStructDef; modname : AnsiString);
var
  i : integer;
  members : StructMemberSeq;
  contents : IInterfaceList;
begin
  ...
  struct_marshaller(str as IContained, modname)
end;
...
begin
  ...
  if bound = 0 then
    ...
    writeln('Array [0..' + AnsiString(inttostr(bound-1)) + '] of ' + tname + ';')
end;
...
begin
  ...
  case t.def_kind of
    ...
    dk_Interface : begin
        if FDB.repoid_defined((t as IContained)._id) then
          writeln(AnsiString(ChangeFileExt(String(FDB.repoid_file_name((t as IContained)._id)),'_int')) + '.I' + (t as IContained)._name)
        else
...
var
  ...
  contents : IInterfaceList;
begin
  if not FDB.repoid_defined(m._id) then begin
    writeln('// module '+m._name+' types');
    writeln;
  end;
  contents := (m as IContainer).contents();
  for i := 0 to Pred(contents.count) do
...
procedure TCodegen.int_declare_internal_type(cont_name,name: AnsiString; t: IIDLType);
var
  tmp : AnsiString;
begin
  case t.def_kind of
...
begin
  ...
  if base_int.Count > 0 then
    ...
    writeln;
  writeln('  ['''+CreateStringGUID(int._id)+''']');
  declare_methods(int,'');
...
begin
  ...
  writeln('  ['''+CreateStringGUID(val._id)+''']');
  if tname = '' then
    tname := type_str(val.original_type_def);
...
procedure TCodegen.final_method(name: AnsiString; def: IOperationDef);
var
  mstr: AnsiString;
  ind: integer;
  res: IIDLType;
...
procedure TCodegen.final_attribute(name: AnsiString; attr: IAttributeDef);
var
  tstr: AnsiString;
begin
  ...
  if attr.get_mode = ATTR_NORMAL then
    begin
      writeln('procedure T'+name+'_impl._set_'+attr._name+'(const val: '+tstr+');');
      writeln('begin');
      writeln('  // TODO');
      writeln('end;');
      writeln;
    end;
  writeln('function T'+name+'_impl._get_'+attr._name+': '+tstr+';');
  writeln('begin');
  writeln('  // TODO');
...
var
  ...
  wch: widestring;
begin
  if FDB.repoid_defined(con._id) then Exit;
  ...
  case con.type_def._type.kind of
    ...
    tk_char: begin
        ...
        if ord(ch[1]) >= 33 then
          ...
          writeln('  ' + name + ' = #' + AnsiString(IntToStr(ord(char(ch[1])))) + ';')
      end;
    tk_wchar: begin
      wch := val;
      writeln('  ' + name + ': WideChar = #' + AnsiString(IntToStr(ord(wch[1]))) + ';');
    end;
    //WJ: added tk_octet
    tk_octet,tk_short,tk_long,tk_ushort,tk_ulong: writeln('  ' + name + ' = ' + AnsiString(IntToStr(val)) + ';');
    tk_float : writeln('  ' + name + ' = ' + AnsiString(FloatToStrf(val,ffGeneral,7,0)) + ';');
    tk_double : writeln('  ' + name + ' = ' + AnsiString(FloatToStr(val)) + ';');
    tk_boolean: if val = TRUE then
        writeln('  '+name+' = TRUE;')
      else
        writeln('  '+name+' = FALSE;');
    tk_longlong, tk_ulonglong: begin
      int64Val := val;
      writeln('  ' + name + ' = ' + AnsiString(IntToStr(int64Val)) + ';');
    end;
    else
...
procedure TCodegen.InternalGenerate(const AName: AnsiString; const AContents: IInterfaceList);
begin
  try
    ProcessRmic('');
    if in_list(AName, FIncludes) then
      FIncludes.Delete(FIncludes.IndexOf(String(AName)));
    AssignFile(Output, String(AName) + '_int.pas');
    Rewrite(Output);
    GenerateInterface(AName, AContents);
    CloseFile(Output);

    AssignFile(Output, String(AName) + '.pas');
    Rewrite(Output);
...
procedure TCodegen.GenerateImplementation(const AName: AnsiString; const AContents: IInterfaceList);
var
  usesstr, temp: AnsiString;
  cnt, i: Integer;
  it : IIDLType;
begin
    // implementations
    ...
    writeln('uses');
    write('  Classes, orb_int, orb, req_int, code_int, env_int, stdstat, std_seq,');
    //really ugly :(
    if AName <> 'imr' then
      write('imr, imr_int, ');
    writeln('');
    ...
    if FIncludes.count > 0 then
      begin
        ...
        for i := 0 to cnt do
          begin
            if not already_included(AnsiString(FIncludes[i])) then begin
              ...
              if (length(usesstr) > 80) and (i <> cnt) then
                begin
                  writeln(usesstr+',');
                  usesstr := '  '+ AnsiString(FIncludes[i]);
                end
              else
                usesstr := usesstr + ', ' + AnsiString(FIncludes[i]);
            end;
            if i = cnt then usesstr := usesstr+';';
          end
      end
        ...
        if FParams.IndexOf('-any') > 0 then
          for i := 0 to Pred(Marshallers.count) do
            begin
              ...
              writeln('  function ' + AnsiString(Marshallers[i]) + '_to_any(const val' + temp + '): IAny;');
              writeln('  function any_to_' + AnsiString(Marshallers[i]) + '(const a: IAny; var val' + temp + '): Boolean;');
              writeln;
            end;
        writeln('var');
        for i := 0 to Pred(Marshallers.count) do
    ...
    // implementation
    for i := 0 to Pred(AContents.count) do
      impl_code(AContents[i] as IIRObject, AName);
          ...
          begin
            for i := 0 to Pred(Marshallers.count) do
              begin
                ...
                writeln('  stat: IStaticAny;');
                writeln('begin');
                writeln('  stat := StaticAny(' + AnsiString(Marshallers[i]) + '_marshaller,@val);');
                ...
                begin
                  ...
                  while length(temp) > 120 do
                    begin
                      ...
                      write('    ');
                      temp := copy(temp,111,length(temp) - 110);
                    end;
                  writeln(''''+temp+''');');
                end;
            end;
...
procedure TCodegen.GenerateInterface(const AName: AnsiString; const AContents: IInterfaceList);
var
  usesstr: AnsiString;
  cnt, i: Integer;

  function addWord( line : AnsiString; w : AnsiString ) : AnsiString;
  begin
      if Length(Trim(String(line)))>0 then
          Result := line + ', ' + w
      else
          Result := line + w;
  end;
begin
    ...
    FDeclared.Clear;
    usesstr := '  orbtypes';
    if not already_included('code_int') then
      usesstr := addWord(usesstr, 'code_int');
    if not already_included('orb_int') then
      usesstr := addWord(usesstr, 'orb_int');
    if FIncludes.count > 0 then
      begin
        cnt := FIncludes.count-1;
        for i := 0 to cnt do
          begin
            if not already_included(AnsiString(FIncludes[i])) then begin
              if (length(usesstr) > 80) {and (i <> cnt)} then
                begin
                  writeln(usesstr+',');
                  usesstr := addWord('  ', AnsiString(FIncludes[i]) + '_int');
                end
              else
                usesstr := addWord(usesstr, AnsiString(FIncludes[i]) + '_int');
            end;
            if i = cnt then usesstr := usesstr+';';
          end
      end
    ...
    if FConstants.Count > 0 then
      begin
        ...
        while i < FConstants.count do begin
          if not FDB.repoid_defined((FConstants[i] as IConstantDef)._id) then Break;
          i := Succ(i);
        end; { while }
...
procedure TCodegen.ProcessRmic(const RepoId: AnsiString);
var
  i, idx: Integer;
  b: Boolean;
begin
