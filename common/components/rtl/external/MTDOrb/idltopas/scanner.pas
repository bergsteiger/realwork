...
uses
  ...
  ir_int,ir,Classes,parser,orbtypes,orb_int,any,tcode,Math,idldb{$IFDEF LINUX}
  ,Libc{$ENDIF};
...
type
  ...
  public
    ...
    function gen_repo_id(node: TNode): AnsiString;
    function gen_ident(node: TNode): AnsiString;
    procedure scan(con: IContainer; node: TNode);
    procedure add_forward_dcl(id: AnsiString);
    procedure remove_forward_dcl(id: AnsiString);
    function forward_exists(id: AnsiString): Boolean;
    function interface_exists(id: AnsiString): Boolean;
...
begin
  raise Exception.Create(String(node.file_name)+'('+inttostr(node.line_number)+'): '+mess);
end;
...
begin
  raise Exception.Create(String(node.file_name)+'('+inttostr(node.line_number)+'): '+Format(ErrorMsg[AErrNum], [mess]));
end;
...
function TScanner.gen_ident(node: TNode): AnsiString;
begin
  result := FDB.get_id_prefix+ node.getIdent;
end;

function TScanner.gen_repo_id(node: TNode): AnsiString;
var
  absname, name: AnsiString;
begin
  name := node.getIdent;
  if name[1] = ':' then
...
procedure TScanner.add_forward_dcl(id: AnsiString);
begin
  FForwardList.add(String(id));
end;

procedure TScanner.remove_forward_dcl(id: AnsiString);
begin
  if FForwardList.IndexOf(String(id)) >= 0 then
    FForwardList.Delete(FForwardList.IndexOf(String(id)));
  FInterfaceList.add(String(id));
end;

function TScanner.interface_exists(id: AnsiString): Boolean;
begin
  result := FInterfaceList.IndexOf(String(id)) >= 0;
end;

function TScanner.forward_exists(id: AnsiString): Boolean;
begin
  result := FForwardList.IndexOf(String(id)) >= 0;
end;
...
var
  ...
  params : IInterfaceList;
begin
  nid := gen_repo_id(node);
  if forward_exists(nid) then  exit;
  if interface_exists(nid) then exit;
  params := TInterfaceList.Create;
  int := IContained(con.create_value(nid, node.getIdent, FVersion,false,is_abstract,nil,false,
...
begin
  ...
  if (list.count > 0) then
    begin
      if not forward_exists(IContained(list[0])._id) then
    ...
    begin
      ...
      while snames <> nil do
        begin
          ...
          if FForwardList.IndexOf(String(idef._id)) >= 0 then
        ...
        begin
          ...
          if tmp.node_type = nt_scoped_name then
            begin
              ...
              if FForwardList.IndexOf(String((bvalue  as IContained)._id)) >= 0 then
      ...
      while hdr <> nil do
        begin
          ...
          if FForwardList.IndexOf(String((base as IContained)._id)) >= 0 then
  ...
  while elements <> nil do
    begin
      ...
      if element.node_type = nt_init_dcl then
        begin
          init.name := AnsiString(element.getIdent);
          ...
          while pdecls <> nil do
            begin
              ...
              init.members[i].name := AnsiString(pdecl.getBranch(2).getIdent);
...
begin
  ...
  while scoped_names <> nil do
    begin
      ...
      if forward_exists(IContained(base)._id) then
        scanner_exception(node,'Interface not defined');
      if int.get_is_abstract and not base.get_is_abstract then
        scanner_exception(node,'interface '+String(IContained(int)._name)+' can`t be abstract');
      baseint.Add(base);
...
begin
  ...
  while params <> nil do
    begin
      ...
      if (omode = OP_ONEWAY) and (pmode <> PARAM_IN) then
        scanner_exception(18, node, String(node.getIdent));
      ptype := get_type(con,param.getBranch(2));
      paramdesc.name := AnsiString(param.getBranch(3).getIdent);
  ...
  if (omode = OP_ONEWAY) and (Length(exceptions) <> 0) then
    scanner_exception(18, node, String(node.getIdent));
  name := node.getIdent;
  id := 'IDL:'+(con as IContained)._name+'/'+name+':'+FVersion;
  try
    if idef <> nil then
    ...
    nt_scoped_name: begin
        ...
        if cont <> nil then
          begin
            ...
            if result = nil then
              scanner_exception(node,'scoped name ''' +String(node.getIdent)+ ''' not defined');
          end
...
begin
  ...
  case node.node_type of
    nt_simple_declarator: begin
        m.name := AnsiString(node.getIdent);
        m.type_def := _type;
        m._type := _tc_void;
        mem^[i] := m;
      end;
...
var
  ...
  i : integer;
begin
  str := copy(String(node.getIdent),6,1000);
  i := pos(' ',str);
  if i < 1 then exit;
...
begin
  if node.node_type() = nt_typeid then begin
    ptype := 'ID';
    name := String(node.getBranch(1).getIdent());
    txt := String(node.getIdent());
  end
    ...
    txt := trim(copy(trim(String(node.getIdent)),8,200));
    i := pos(' ',txt);
    ...
    if ptype = 'prefix' then
      begin
        ...
        i := pos('"',txt);
        if i < 1 then scanner_exception(node,'invalid prefix declaration');
        FDB.switch_prefix(AnsiString(copy(txt,1,i-1)));
        exit;
      end;
    i := pos(' ',txt);
    if i < 1 then scanner_exception(node,'invalid prefix declaration');
    ...
    if ptype = 'version' then
      begin
        ...
        FDB.set_version(AnsiString(name),AnsiString(copy(txt,1,i)));
      end
      ...
      begin
        ...
        i := pos('"',txt);
        if i < 1 then scanner_exception(node,'invalid prefix declaration');
        FDB.set_repo_id(AnsiString(name),AnsiString(copy(txt,1,i-1)));
      end
    else
      assert(false,'not implemented');
    Exit;
  ...
  if (Length(name) > 0) and (name[1] <> ':') then begin
    ...
    c := con.find(AnsiString(name));
    if c = nil then
      //The name wasn't defined. Assume its relative to the
      //current scope
      name := String(FDB.get_current_scope()) + name
    else
      name := String(c.absolute_name());
  end;
  if ptype = 'prefix' then begin
    if name = '' then
      FDB.switch_prefix(AnsiString(txt))
    else
      FDB.set_prefix(AnsiString(name), AnsiString(txt));
  end
  else if ptype = 'ID' then
    FDB.set_repo_id(AnsiString(name), AnsiString(txt));
end;
...
begin
  ...
  if t.node_type = nt_scoped_name then
    begin
      ...
      if ct = nil then scanner_exception(node,'type not found: '+ String(t.getIdent));
      alias := TAliasDef._narrow(ct as IORBObject);
      if ct = nil then scanner_exception(node,'alias not found: '+ String(t.getIdent));
      while alias <> nil do
        begin
  ...
  if not (ctype._type.kind in [
    ...
    tk_longlong, tk_ulonglong, tk_longdouble]) then
      scanner_exception(node,'bad constant type: '+String(cname));
  ...
  case ctype._type.kind of
    ...
    tk_string : a.put_string(AnsiString(val));
    tk_wstring : a.put_wstring(WideString(val));
    tk_char : a.put_char(PAnsiChar(AnsiString(val))^);
    tk_octet : a.put_octet(Octet(val));
    tk_wchar : a.put_wchar(PWideChar(WideString(val))^);
    tk_longlong: a.put_longlong(val);
    tk_ulonglong: a.put_ulonglong(val);
...
var
  ...
  itype: IIDLType;
  p : TNode;
  name,repoid : AnsiString;
  attr : IAttributeDef;
  getexceptions: ExceptionDefSeq;
...
begin
  ...
  while true do
    begin
      ...
      if Assigned(node.getBranch(3)) then
        raises_dcl(con, node.getBranch(3),@getexceptions);
      if Assigned(node.getBranch(4)) then
        raises_dcl(con, node.getBranch(4),@setexceptions);
      if idef <> nil then
        attr := idef.create_attribute(repoid, name, FVersion,itype,AttributeMode(integer(readonly)),
          getexceptions, setexceptions)
      else
        attr := vdef.create_attribute(repoid, name, FVersion, itype, AttributeMode(integer(readonly)),
          getexceptions, setexceptions);
      p := p.getBranch(2);
    end;
end;
...
var
  ...
  begin
    ...
    case node.node_type of
      nt_simple_declarator : begin
          member.name := AnsiString(node.getIdent);
      ...
      begin
        ...
        case t.def_kind of
          dk_Enum : begin
              ...
              if pos('::',String(scoped_name)) > 0 then
                begin
                  str := PAnsiChar(_label);
              ...
              for i := 0 to Pred(enum_tc.member_count) do
                begin
                  if enum_tc.member_name(i) = Identifier(_label) then
                    begin
          ...
          dk_Primitive : begin
              ...
              case prim.kind of
                ...
                pk_char: member._label.put_char(AnsiString(val)[1]);
...
begin
  ...
  while node <> nil do
    begin
      ...
      if c = nil then
        scanner_exception(node,'exception not found '+String(node.getIdent));
      ex := TExceptionDef._narrow(c as IORBObject);
      if ex = nil then
        scanner_exception(node,'can`t cast '+String(node.getIdent)+' to exception');
...
var
  ...
  old_yyleng  : Byte;
  old_yystate    : Integer;
  old_yyactchar  : AnsiChar;
  old_yylastchar : AnsiChar;
  old_yyrule     : Integer;
...
begin
  ...
  to_be_imported := String(node.getIdent);
  ...
  try
    ...
    yyinput := codeutil.OpenFile(AnsiString(real_file_name));
    try
      yyprintln('#line 1 "'+AnsiString(real_file_name)+'"');
      yyinput.position := 0;
      cpp.yylex;
    finally
      yyinput := nil;
    ...
    {$ENDIF}
    {$IFDEF LINUX}
    yyoutput := THandleStream.Create({STDOUT_FILENO}1);{FIXME}
    {$ENDIF}
