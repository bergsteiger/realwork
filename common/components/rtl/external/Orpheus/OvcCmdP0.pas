...
uses
  ...
  Buttons, Classes, Controls, Dialogs, Forms,
  {$IfDef Delphi6}
  DesignIntf,
  DesignEditors,
  {$Else  Delphi6}
  DsgnIntf,
  {$EndIf Delphi6}
  Graphics, Menus, Messages, StdCtrls, SysUtils, Tabs,
  OvcData, OvcCmd, OvcHelp, OvcMisc, OvcCmdP1, OvcExcpt, OvcConst, ExtCtrls;

type
  THotEdit = class(TEdit)
  protected
...
type
  ...
  TOvcControllerEditor = class(TDefaultEditor)
  public
...
procedure THotEdit.CMDialogChar(var Msg : TCMDialogChar);
begin
...
procedure THotEdit.WMGetDlgCode(var Msg: TWMGetDlgCode);
begin
  inherited;
  Msg.Result := Msg.Result or DLGC_WANTALLKEYS;
end;

procedure THotEdit.WMKeyDown(var Msg : TWMKeyDown);
begin
...
procedure THotEdit.WMSysKeyDown(var Msg : TWMSysKeyDown);
begin
  Text := KeyToText(Lo(Msg.CharCode), GetShiftFlags);
  Msg.Result := 0;
end;

procedure THotEdit.WMChar(var Msg : TWMChar);
begin
  Msg.Result := 0;
end;
...
begin
  {create THotEdit controls in the same place as the invisible TEdits}
  HotEdit1 := THotEdit.Create(Self);
...
begin
  ...
  {set flag if this is a wordstar command}                             {!!.12}
  if ckWordStar.Checked then                                           {!!.12}
    CR.SS2 := ss_WordStar;                                             {!!.12}
...
procedure TOvcfrmCmdTable.btnHelpClick(Sender: TObject);
begin
  ShowHelpContext(hcCommandProcessor);
end;
