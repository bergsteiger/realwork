...
uses
  ...
  Controls, Forms, Messages, StdCtrls, SysUtils,
  OvcConst, StDate, StSrMgr;
...
const
  ...
  pmUser1       = AnsiChar('1');         {User-defined picture mask characters}
  pmUser2       = '2';
  pmUser3       = '3';
  pmUser4       = '4';
  pmUser5       = '5';
...
const
  ...
  MaxWord        = 65535;     {largest Word value}
...
{$IfNDef XE}
var
  OrphStr : TStStringResource;
{$EndIf XE}

function GetOrphStr(Index : Word) : string;
begin
  {$IfDef XE}
  Result := Format('Orpheus string: %d', [Index]);
  {$Else   XE}
  Result := OrphStr[Index];
  {$EndIf  XE}
end;
...
initialization
  InitAlphaCharSet;
  {$IfNDef XE}
  OrphStr := TStStringResource.Create(HInstance, 'ORPHEUS_STRINGS_ENGLISH');
  {$EndIf XE}
