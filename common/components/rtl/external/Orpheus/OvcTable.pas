...
{$IFDEF Win32}
  {$R OVCTABLE.R32} {Glyphs for cells; cursors for table}
{$ELSE}
  {$R OVCTABLE.R16} {Glyphs for cells; cursors for table}
{$ENDIF}
...
uses
  ...
  SysUtils, Messages,
  l3Interfaces,
  afwInterfaces, afwFont,

  evdTextStyle_Const,
  Graphics, Classes, Controls, Forms, StdCtrls, Menus, DIALOGS,
  OvcMisc, OvcData, OvcConst, OvcBase, OvcCmd,
  OvcTCmmn, OvcSpAry, OvcTCAry, OvcTSelL, OvcTCell, OvcTCHdr,
  OvcTGRes, OvcTGPns, OvcTbClr, OvcTbRws, OvcTbCls;
...
type
  TOvcCustomTable = class(TOvcTableAncestor
    ...
    protected {private}
      ...
      fUseStyleTable      : Boolean;
      fStyleId            : Tl3StyleId;

      fPreventRecurse_StopEditingState : boolean;
...
//     function CheckStamp(const aGUID: TGUID): Boolean;
    ...
    {$IfNDef DesignTimeLibrary}
     procedure StyleTableChanged;
      {-}
    {$EndIf DesignTimeLibrary}
     procedure SetStyleId(const aValue : Tl3StyleId);
     procedure SetUseStyleTable(const aValue : Boolean);
      ...
      function  tbCalcRequiresVSBar : boolean;                  {!!.11}
      ...
      procedure ctimKillFocus(var Msg : TWMKey); message ctim_KillFocus;
  ...
  TOvcTable = class(TOvcCustomTable)
    ...
    published
      ...
      property ColCount stored false; {it's stored elsewhere}
...
uses
 l3Base,
 evStyleTableSpy,  evStyleTableTools,
 afwVcl;
...
constructor TOvcCustomTable.Create(AOwner : TComponent);
 begin
  ...
  {$EndIf DesignTimeLibrary}
    ...
    FAccess := tbDefAccess;
    FAdjust := tbDefAdjust;
    tbCellAttrFont := TafwFont.Create;
    ...
    tbColMoveCursor := LoadCursor(hInstance, PChar(crColumnMove));
    tbRowMoveCursor := LoadCursor(hInstance, PChar(crRowMove));
    ...
    //tbCmdTable := NewStr(scGridTableName);
...
procedure TOvcCustomTable.Cleanup;
begin
  {$IfNDef DesignTimeLibrary}
...
procedure TOvcCustomTable.Loaded;
  begin
    ...
    tbSelList.SetColCount(ColCount);                            {!!.10}
    tbSelList.SetRowCount(RowLimit);                            {!!.10}
    {$ENDIF}
  end;
{====================================================================}
...
(*function TOvcCustomTable.CheckStamp(const aGUID: TGUID): Boolean;
begin
...
{$IfNDef DesignTimeLibrary}
procedure TOvcCustomTable.StyleTableChanged;
//var
// l_C : TColor;
begin
 if UseStyleTable then
 begin
  evGetStyleFont(Font, fStyleId);
  {l_C := evGetStyleBackColor(fStyleID);
...
procedure TOvcCustomTable.SetStyleId(const aValue : Tl3StyleId);
{-}
begin
 ...
 begin
  ...
  StyleTableChanged;
  {$EndIf DesignTimeLibrary}
 end;//if fStyleId <> aValue then
end;//SetStyleId

procedure TOvcCustomTable.SetUseStyleTable(const aValue : Boolean);
{-}
begin
 if (fUseStyleTable <> aValue) then
 begin
  fUseStyleTable := aValue;
  {$IfNDef DesignTimeLibrary}
  StyleTableChanged;
  {$EndIf DesignTimeLibrary}
 end;//fUseStyleTable <> aValue
end;
...
procedure TOvcCustomTable.tbWriteColData(Writer : TWriter);
  var
    ...
    S : AnsiString;
  begin
    if tbMustFinishLoading then begin                          {!!.16}
      tbFinishLoadingCellList;                                 {!!.16}
      tbFinishLoadingDefaultCells;                             {!!.16}
      tbMustFinishLoading := false;                            {!!.16}
    end;                                                       {!!.16}
...
procedure TOvcCustomTable.SetActiveCell(RowNum : TRowNum; ColNum : TColNum);
  begin
    DoActiveCellMoving(ccNone, RowNum, ColNum);                 {!!.01}
    tbSetActiveCellWithSel(RowNum, ColNum);                     {!!.01}
  end;
{--------}
    ...
    try
      ...
      if (RowNum <> FActiveRow) then
        begin
          ...
          (*InvalidateRowHeading(FActiveRow);*)
      ...
      if (ColNum <> FActiveCol) then
        begin
          ...
          (*InvalidateColumnHeading(FActiveCol);*)
                    ...
                    if tbCalcActiveCellRect(CellRect) then
                      begin
                        ...
                        if MustFocus then
                          ...
                          {$ELSE}
                          WinProcs.SetFocus(tbActCell.EditHandle);
                          {$ENDIF}
                      end
                  end
                else
          ...
          begin
            ...
            if (otsEditing in tbState) then
              begin
                ...
                {$ELSE}
                MustFocus := tbEditCellHasFocus(WinProcs.GetFocus);
                {$ENDIF}
...
procedure TOvcCustomTable.SetCols(CS : TOvcTableColumns);
  begin
    ...
    try
      ...
      FCols := CS;
      FCols.Table := Self;
      FCols.OnColumnChanged := tbColChanged;                    {!!.11}
      tbColChanged(FCols, 0, 0, taGeneral);                     {!!.11}
    finally
      AllowRedraw := true;
    end;{try..finally}
  end;
      ...
      begin
        if tbHasVSBar and HandleAllocated then
          begin
            ...
            if (tbLastTopRow < 16*1024) then
              if tbCalcRequiresVSBar then                                        {!!.11}
                SetScrollRange(Handle, SB_Vert, LockedRows, tbLastTopRow, false) {!!.11}
              else                                                               {!!.11}
                SetScrollRange(Handle, SB_Vert, LockedRows, LockedRows, false)   {!!.11}
            else
              begin
                Divisor := succ(tbLastTopRow div $400);
                SetScrollRange(Handle, SB_Vert,
                                        LockedRows, tbLastTopRow div Divisor,
                                        false)
              end;
          end
      end
...
function TOvcCustomTable.FilterKey(var Msg : TWMKey) : TOvcTblKeyNeeds;
  ...
  begin
    Result := otkDontCare;
    Cmd := Controller.EntryCommands.TranslateUsing([scGridTableName{tbCmdTable^}], TMessage(Msg), GetTickCount);
  ...
  begin
    ...
    try
      ...
      if Assigned(tbActCell) then
        begin
          l3FillChar(CellAttr, sizeof(CellAttr), 0);
...
function TOvcCustomTable.StopEditingState(SaveValue : boolean) : boolean;
  var
    ...
    R : TRect;
    SaveState : TOvcTblStates;

  begin
   if fPreventRecurse_StopEditingState then
    Exit;
   fPreventRecurse_StopEditingState := True;
   try
    Result := true;
    ...
    try
      ...
      MustFocus := tbEditCellHasFocus(WinProcs.GetFocus);
      {$ENDIF}
      if not MustFocus then                                    {!!.13}
        MustFocus := Focused;                                  {!!.13}
      DoEnteringColumn(ActiveCol);
      DoEnteringRow(ActiveRow);
      DoGetCellData(ActiveRow, ActiveCol, Data, cdpForSave);
      ...
      try                                                      {!!.17}
        ...
        if MustFocus then
          ...
          tbState := tbState - [otsFocused] + [otsUnfocused];  {!!.13}
      end;{try..finally}                                       {!!.17}
    finally
      AllowRedraw := true;
    end;{try..finally}
   finally
    fPreventRecurse_StopEditingState := False;
   end;
  end;
{====================================================================}
...
procedure TOvcCustomTable.tbUpdateSelection(RowNum : TRowNum; ColNum : TColNum;
  ...
  begin
    ...
    try
      ...
      FreeAndNil(NewInvCells);
      FreeAndNil(DeselCells);
    end;{try..finally}
  end;
{====================================================================}
...
procedure TOvcCustomTable.tbColChanged(Sender : TObject; ColNum1, ColNum2 : TColNum;
  ...
  begin
    ...
    if not HandleAllocated then begin                          {!!.15}
      tbSelList.SetColCount(ColCount);                         {!!.15}
      Exit;
    end;                                                       {!!.15}
    ...
    try
      ...
      with tbColNums^ do
        case Action of
          ...
          taSingle  : begin
                        ...
                        if not DoIt then                                 {!!.01}
                          DoIt := (ColNum1 > Ay[pred(Count)].Number) and {!!.01}
                                  (ClientWidth > Ay[Count].Offset);      {!!.01}
                        DoColumnsChanged(ColNum1, -1, taSingle);         {!!.10}
                      end;
          taAll     : DoIt := true;
          taInsert  : begin
                        DoIt := (Ay[0].Number <= ColNum1) and
      ...
      if not DoIt then
        begin
          ...
          tbSelList.SetColCount(ColCount);                      {!!.01}
          tbSetScrollRange(otsbHorizontal);
          tbSetScrollPos(otsbHorizontal);
          Exit;
        end;
      ...
      tbSetScrollRange(otsbHorizontal);
      tbSetScrollPos(otsbHorizontal);
      if (LeftCol > tbLastLeftCol) then                         {!!.10}
        LeftCol := tbLastLeftCol;                               {!!.10}
    finally
      AllowRedraw := true;
    end;{try..finally}
  end;
...
procedure TOvcCustomTable.tbRowChanged(Sender : TObject; RowNum1, RowNum2 : TRowNum;
  ...
  begin
    ...
    {similarly don't bother if we have no handle}
    if not HandleAllocated then begin                          {!!.15}
      tbSelList.SetRowCount(RowLimit);                         {!!.15}
      Exit;
    end;                                                       {!!.15}
    {make sure there's no flicker}
    AllowRedraw := false;
    try
      ...
      with tbRowNums^ do
        case Action of
          ...
          taSingle  : begin
                        ...
                        {check for unhiding a row after all others}       {!!.01}
                        if not DoIt then                                  {!!.01}
                          DoIt := (RowNum1 > Ay[pred(Count)].Number) and  {!!.01}
                                  (ClientHeight > Ay[Count].Offset);      {!!.01}
                        DoRowsChanged(RowNum1, -1, taSingle);             {!!.12}
                      end;
          taAll     : DoIt := true;
          taInsert  : begin
                        DoIt := (Ay[0].Number <= RowNum1) and
      ...
      if not DoIt then
        begin
          ...
          tbSelList.SetRowCount(RowLimit);                      {!!.01}
          tbSetScrollRange(otsbVertical);
          tbSetScrollPos(otsbVertical);
          Exit;
        end;
...
function TOvcCustomTable.tbCalcCellsFromRect(const UR : TRect; var GR : TRect) : integer;
  ...
  begin
    ...
    if (Region = otrInUnused) then
      begin
        Result := 2;
        l3FillChar(GR, sizeof(GR), $FF); {set 'em all to -1}
        Exit;
      end;
...
procedure TOvcCustomTable.tbCalcColsOnLastPage;
  ...
  begin
    ...
    try
      ...
      if tbLastLeftCol < LeftCol then                           {!!.11}
        LeftCol := tbLastLeftCol;                               {!!.11}
    finally
...
procedure TOvcCustomTable.tbCalcRowsOnLastPage;
  ...
  begin
    ...
    try
      ...
      if tbLastTopRow < TopRow then                             {!!.11}
        TopRow := tbLastTopRow;                                 {!!.11}
    finally
...
{--------}
function TOvcCustomTable.tbCalcRequiresVSBar : boolean;         {!!.11}
  var
    Row : TRowNum;
  begin
...
procedure TOvcCustomTable.tbMoveActCellBotOfPage;
  ...
  begin
    ...
    if (ActiveRow <> NewActiveRow) or (ActiveCol <> NewActiveCol) then
      begin
        tbSetActiveCellWithSel(NewActiveRow, NewActiveCol);     {!!.01}
      end;
  end;
{--------}
      ...
      begin
        ...
        with Canvas do
          begin
            ...
            {$ELSE}
            WinProcs.SetBkColor(Handle, ColorToRGB(BrushColor));
            {$ENDIF}
...
procedure TOvcCustomTable.tbDrawUnusedBit;
  ...
  begin
    ...
    {$ELSE}
    WinProcs.GetClientRect(Handle, CR);
    {$ENDIF}
    with R, tbColNums^ do
      begin
...
procedure TOvcCustomTable.Paint;
  ...
  begin
    ...
    {$ELSE}
    WinProcs.GetClipBox(Canvas.Handle, UR);
    {$ENDIF}
...
procedure TOvcCustomTable.ctimStartEdit(var Msg : TMessage);
  begin
    if not StartEditingState then                               {!!.11}
      begin                                                     {!!.11}
        AllowRedraw := false;                                   {!!.11}
        InvalidateCell(ActiveRow, ActiveCol);                   {!!.11}
        AllowRedraw := true;                                    {!!.11}
      end;                                                      {!!.11}
    Msg.Result := 1;
  end;
...
procedure TOvcCustomTable.ctimKillFocus(var Msg : TWMKey); {V}
 Begin
  if not StopEditingState(true) then SetFocus;
 end;
        ...
        case Region of
          ...
          otrInLocked :
            begin
              ...
              if InEditingState then                            {!!.12}
                if not StopEditingState(true) then              {!!.12}
                  Exit;                                         {!!.12}
              AllowRedraw := false;
              try
              ...
              if DoingCol then
                begin
                  ...
                  if ActiveCol = ColFrom then
                    ActiveCol := ColTo
                  else if (ColTo > ColFrom) then begin                     {!!.15}
                    if (ColFrom < ActiveCol) and (ActiveCol <= ColTo) then {!!.15}
                      ActiveCol := IncCol(ActiveCol, -1);                  {!!.15}
                  end                                                      {!!.15}
                  else begin                                               {!!.15}
                    if (ColTo <= ActiveCol) and (ActiveCol < ColFrom) then {!!.15}
                      ActiveCol := IncCol(ActiveCol, +1);                  {!!.15}
                  end;                                                     {!!.15}
                end
              else {doing rows}
                begin
                  ...
                  if ActiveRow = RowFrom then
                    ActiveRow := RowTo
                  else if (RowTo > RowFrom) then begin                     {!!.15}
                    if (RowFrom < ActiveRow) and (ActiveRow <= RowTo) then {!!.15}
                      ActiveRow := IncRow(ActiveRow, -1);                  {!!.15}
                  end                                                      {!!.15}
                  else begin                                               {!!.15}
                    if (RowTo <= ActiveRow) and (ActiveRow < RowFrom) then {!!.15}
                      ActiveRow := IncRow(ActiveRow, +1);                  {!!.15}
                  end;                                                     {!!.15}
                end;
            finally
  ...
  begin
    ...
    if (not OnGridLine) and
      ...
      begin                                                     {!!.01}
        ...
        if InMoveArea then                                      {!!.01}
          if IsColMove then                                     {!!.01}
            InMoveArea := otoAllowColMoves in Options           {!!.01}
          else                                                  {!!.01}
            InMoveArea := otoAllowRowMoves in Options;          {!!.01}
      end;                                                      {!!.01}
    {now set the cursor}
    if InMoveArea then
      begin
