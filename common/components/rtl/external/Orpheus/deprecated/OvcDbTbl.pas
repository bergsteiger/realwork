...
uses
  ...
  Buttons, Classes, Controls, DB, DBTables, Forms, Graphics,
  Menus, Messages, SysUtils, StdCtrls,
  OvcBase, OvcCmd, OvcConst, OvcData, OvcExcpt, OvcMisc,
  OvcTCBEF, OvcTCell, OvcTBClr, OvcTBCls, OvcTCmmn, OvcTGPns,
  STDate;
...
const
  dtDefColorUnused        = clWindow;
  dtDefColumnCount        = 1;
  dtDefDateOrTime         = dtUseDate;
  dtDefDefaultMargin      = 4;
  dtDefHeaderHeight       = 21;
  dtDefHeaderOptions      = [hoShowHeader];
  dtDefHitMargin          = 4;
  dtDefIndicatorWidth     = 11;
  dtDefLeftColumn         = 0;
  dtDefMapSize            = 16;
  dtDefOptions            = [dtoShowIndicators];
  dtDefParentColor        = False;
  dtDefRowHeight          = 21;
  dtDefTabStop            = True;
  dtDefTextStyle          = tsFlat;
  dtDefWantTabs           = False;
...
type
  ...
  public
    ...
    property Column[Index : Integer] : TOvcDbTableColumn
      read GetColumn;
      default;
  ...
  protected {private}
    ...
    FOnLeftColumnChanged : TNotifyEvent;        {left column changed}  {!!.13}
    ...
    tbIndicatorWidth     : Integer;             {width of indicator column or 0 if not displayed}
    ...
    {cell-table messages}
    procedure ctimKillFocus(var Msg : TMessage);
      message ctim_KillFocus;
    procedure ctimQueryOptions(var Msg : TMessage);
      message ctim_QueryOptions;
    procedure ctimQueryColor(var Msg : TMessage);
      message ctim_QueryColor;
    procedure ctimQueryFont(var Msg : TMessage);
      message ctim_QueryFont;
    ...
    procedure ctimRemoveCell(var Msg : TMessage);
      message ctim_RemoveCell;
    procedure ctimSetFocus(var Msg : TMessage);
      message ctim_SetFocus;
    procedure ctimStartEdit(var Msg : TMessage);
      message ctim_StartEdit;
    procedure ctimStartEditMouse(var Msg : TWMMouse);
      message ctim_StartEditMouse;
    procedure ctimStartEditKey(var Msg : TWMKey);
      message ctim_StartEditKey;
    ...
    function tbAdjustIntegralHeight : Boolean;
      {-adjust for integral height if enabled}
    ...
    function tbCanModify : Boolean;
      {-enter db edit mode and return True if in edit mode}
    procedure tbCellChanged(Sender : TObject);
      override;
      {-notification of a cell change}
    function tbCellModified(Cell : TOvcBaseTableCell) : Boolean;
      {-return True if cell was modified}
    ...
    procedure tbColumnChanged(Sender : TObject; C1, C2 : Integer; Action : TOvcTblActions);
    ...
    procedure tbGetFieldValue(AField : TField; ACell : TOvcBaseTableCell;
    ...
    function tbIsSibling(HW : hWnd) : Boolean;                         {!!.11}
      {-return True if the window HW one of our siblings}
    function tbLoading(DS : TDataSource) : Boolean;                    {!!.11}
    ...
    function tbPaintBitmapCell(ACanvas : TCanvas; PaintRect : TRect;
    ...
    procedure tbQueryColData(ColNum : Integer; var W : Integer;
    ...
    procedure tbSetFieldValue(AField : TField; ACell : TOvcBaseTableCell;
              Data : Pointer; Size : Integer);
      {-set the db field value}
    procedure tbSetFocus(AHandle : hWnd);
  ...
  protected
    ...
    procedure DoOnActiveCellChanged(RowNum, ColNum : Integer);
      dynamic;
    procedure DoOnIndicatorClick(RowNum, ColNum : Integer);
      dynamic;
    procedure DoOnLeftColumnChanged;                                   {!!.13}
      dynamic;
    procedure DoOnLockedCellClick(RowNum, ColNum : Integer);
      dynamic;
    procedure DoOnPaintUnusedArea;
      virtual;
    procedure DoOnUnusedAreaClick(RowNum, ColNum : Integer);
      dynamic;
    procedure DoOnUserCommand(Cmd : Word);
      dynamic;

    {internal properties}
    property DataLink : TOvcDbTableDataLink
      read FDataLink;

    property VisibleColumns : TOvcVisibleColumns
  ...
  {.Z-}
    ...
    property ColorUnused : TColor
      read FColorUnused
      write SetColorUnused
      default dtDefColorUnused;
    ...
    property DefaultMargin : Integer
      read FDefaultMargin
      write SetDefaultMargin
      default dtDefDefaultMargin;
    ...
    property OnLeftColumnChanged : TNotifyEvent
      read FOnLeftColumnChanged
      write FOnLeftColumnChanged;

    property OnLockedCellClick : TCellNotifyEvent
      read FOnLockedCellClick
      write FOnLockedCellClick;

    property OnPaintUnusedArea : TNotifyEvent
      read FOnPaintUnusedArea
      write FOnPaintUnusedArea;

    property OnUnusedAreaClick : TCellNotifyEvent
      read FOnUnusedAreaClick
      write FOnUnusedAreaClick;
  ...
  public
    ...
    {public methods}
    function CalcRowCol(X, Y : Integer; var RowNum, ColNum : Integer) : TOvcTblRegion;
    ...
    procedure GetColumnOrder(var Cols : array of string);              {!!.13}
      {-obtain the field names for each of the table columns}
    procedure GetColumnWidths(var Cols : array of Integer);            {!!.13}
      {-obtain the widths of the table columns}
    {.Z+}
    function IncCol(ColNum, Delta : Integer) : Integer;
      {-increase/decrease the column number and verify}
    ...
    procedure InvalidateNotLocked;                                     {!!.01}
    ...
    procedure InvalidateUnusedArea;                                    {!!.01}
    ...
    procedure Scroll(Delta : Integer);
      {-scroll the table by Distance (signed) by moving the active row}
    procedure SetActiveCell(RowNum, ColNum : Integer);
      {-change the active cell to the specified row and column}
    {.Z+}
    function SetCellProperties(AField : TField; ACell : TOvcBaseTableCell) : Boolean;
      {-set the cell properties to match the field data type}
    {.Z-}
    procedure SetColumnOrder(const Cols : array of string);            {!!.13}
      {-set the order of the table columns using field names}
    procedure SetColumnWidths(const Cols : array of Integer);          {!!.13}
    ...
    property LeftColumn : Integer
      read FLeftColumn
      write SetLeftColumn
      default dtDefLeftColumn;

    property VisibleColumnCount : Integer
      read GetVisibleColumnCount;

    property VisibleRowCount : Integer
      read FVisibleRowCount;
  end;
  ...
  published
    ...
    property Options;
    property ParentColor default dtDefParentColor;
    ...
    property ShowHint;
    property TabOrder;
    property TabStop default dtDefTabStop;
    property TextStyle;
    property Visible;
    ...
    property OnKeyPress;
    property OnKeyUp;
    property OnLeftColumnChanged;                                      {!!.13}
...
{$IFDEF TRIALRUN} OrTrial, {$ENDIF}
  Dialogs,                                                             {!!.01}
  OvcNF, OvcPF, OvcSF,
  OvcTCBmp, OvcTCBox, OvcTCCbx, OvcTCEdt, OvcTCGly,
  OvcTCHdr, OvcTCIco, OvcTCNum, OvcTCPic, OvcTCSim;
...
begin
  ...
  {$IFDEF Win32}
  {Exclude(FComponentStyle, csInheritable);}                           {!!.01}
  {$ENDIF}

  {set default inherited properties}
  Color               := tbDefTableColor;
  Height              := tbDefHeight;
  ParentColor         := dtDefParentColor;
  TabStop             := dtDefTabStop;
  Width               := tbDefWidth;
  ...
  FColorUnused        := dtDefColorUnused;
  FDefaultMargin      := dtDefDefaultMargin;
  FHeaderHeight       := dtDefHeaderHeight;
  FHeaderOptions      := dtDefHeaderOptions;
  FIndicatorWidth     := dtDefIndicatorWidth;
  FLockedColumns      := 0;
  FLeftColumn         := dtDefLeftColumn;
  FOptions            := dtDefOptions;
  FRowHeight          := dtDefRowHeight;
  FScrollBars         := tbDefScrollBars;
  FTextStyle          := dtDefTextStyle;

  {initialize internal items}
  tbCmdTable := GetOrphStr(SCGridTableName);
  tbCellAttrFont := TFont.Create;
  tbColMoveCursor := LoadCursor(hInstance, PAnsiChar(crColumnMove));
  tbState := [otsNormal];
  ...
  FColumns := TOvcTableColumns.Create(Self, dtDefColumnCount, TOvcDbTableColumn);
  ...
  try
    ...
    {$ELSE}
    tbIndicators := TImageList.Create(Bmp.Width, Bmp.Height);
    {$ENDIF}
...
begin
  ...
  with Params do
    Style := LongInt(Style) or OvcData.ScrollBarStyles[FScrollBars]    {!!.D4}
...
begin
  ...
  if (dtoShowIndicators in Options) then
    tbIndicatorWidth := FIndicatorWidth
  else
    tbIndicatorWidth := 0;
...
begin
  ...
  if not tbIsSibling(Msg.wParam) then                                  {!!.11}
    Exit;                                                              {!!.11}
  ...
  if (Msg.wParam <> LongInt(Handle)) and (Handle <> GetFocus)then      {!!.D4}
...
begin
  ...
  if (Delta = 0) then
    if not tbIsColumnHidden(Result) then                               {!!.10}
      Exit;
...
begin
  ...
  tbCalcRowColData(LeftColumn);                                        {!!.11}
...
begin
  if tbIsDbActive then with DataLink do begin
    case Command of
      ...
      ccLeft          :
        begin
          if InEditingState then                                       {!!.14}
            tbActiveCell.CellEditor.Perform(CM_EXIT, 0, 0);            {!!.14}
...
begin
  ...
  if (Operation = opRemove) and (FDataLink <> nil) then begin
    ...
    if (AComponent = tbDataSource) then begin                          {!!.11}
      VisibleColumns.Clear;                                            {!!.11}
      DataSource := nil
    ...
    if (FDataLink <> nil) and not tbLoading(DataSource) then           {!!.11}
    ...
    if (hoShowHeader in HeaderOptions) then begin
      ...
      if Bool(IntersectRect(IR, TR, Clip)) then begin
        ...
        if Assigned(FHeaderCell) and (hoUseHeaderCell in HeaderOptions) then
          ...
          CellAttr.caFont.Assign(Font);
          CellAttr.caFontColor := Colors.LockedText;                   {!!.01}
          CellAttr.caFontHiColor := Colors.SelectedText;               {!!.01}
    ...
    for R := 0 to Pred(VisibleRowCount) do begin
      ...
      if (dtoShowIndicators in Options) then begin
        ...
        if tbIsDbActive and IsActiveRow then begin
          ...
          {$ENDIF}
          tbIndicators.Draw(Canvas, FIndicatorWidth-tbIndicators.Width-3,
      ...
      for ColIdx := 0 to Pred(VisibleColumnCount) do begin
        ...
        if not (InEditingState and IsActiveRow and (Col.Number = ActiveColumn)) then begin
          ...
          if (Cell <> nil) then begin
            ...
            end else if DataLink.DefaultFields and (Col.Number > -1)   {!!.12}
                  and not (csDesigning in ComponentState) then begin   {!!.12}
              if (DataLink.DataSet <> nil) and (FieldCount > 0) then   {!!.13}
                case Fields[Col.Number].Alignment of                   {!!.12}
                  taLeftJustify  : CellAttr.caAdjust := otaCenterLeft; {!!.12}
                  taRightJustify : CellAttr.caAdjust := otaCenterRight;{!!.12}
                  taCenter       : CellAttr.caAdjust := otaCenter;     {!!.12}
                end;                                                   {!!.12}
          ...
          {paint the cell}
          if Assigned(Fld) and Assigned(Cell) and
             (Cell is TOvcTCBitmap) then begin
...
begin
  ...
  if (ACell is TOvcTCString) then begin
    ...
    TOvcTCComboBox(ACell).UseRunTimeItems := False;                    {!!.01}
    ...
    case AField.DataType of
      ftString   :
        begin
          ...
          if Length(TOvcTCPictureField(ACell).PictureMask) >           {!!.13}
             AField.DisplayWidth then                                  {!!.13}
            TOvcTCPictureField(ACell).PictureMask :=                   {!!.13}
              Copy(TOvcTCPictureField(ACell).PictureMask, 1,           {!!.13}
                   AField.DisplayWidth);                               {!!.13}
...
begin
  ...
  if not tbLoading(Value) then                                         {!!.11}
    for I := 0 to Pred(ColumnCount) do
  ...
  if (csDesigning in ComponentState) then begin
    if (not Active) or (tbDataSource <> FDataLink.DataSource) then
      for I := 0 to Pred(ColumnCount) do begin
        ...
        if (Value = nil) then                                          {!!.11}
          Columns[I].DefaultCell := nil;                               {!!.11}
      end;
  end;
...
procedure TOvcCustomDbTable.SetIndicatorWidth(Value : Integer);
begin
...
begin
  ...
  if Value <> FLeftColumn then begin
    if Value > FLeftColumn then                                        {!!.01}
      tbScrollTableLeft(Value)                                         {!!.01}
    else                                                               {!!.01}
      tbScrollTableRight(Value);                                       {!!.01}

    tbUpdateHScrollBar;                                                {!!.01}
  end;
end;
...
begin
  if Value <> FOptions then begin
    ...
    {see if dtoAlwaysEditing was turned off}                           {!!.10}
    if (dtoAlwaysEditing in Old) and                                   {!!.10}
       not (dtoAlwaysEditing in FOptions) then begin                   {!!.10}
      Application.ProcessMessages;                                     {!!.10}
      if InEditingState then                                           {!!.10}
        StopEditing(True);                                             {!!.10}
    end;                                                               {!!.10}
...
begin
  ...
  {is this one of our supported field types}
  Fld := TOvcDbTableColumn(Columns[ActiveColumn]).Field;
  if (not Assigned(Fld) or not (Fld.DataType in SupportedFieldTypes)) then begin
    tbActiveCell := nil;
    Exit;
  end;
...
begin
  ...
  if (FDataLink.DataSet = nil) then                                    {!!.12}
    Exit;                                                              {!!.12}
...
begin
  ...
  {avoid forcing window handle creation}                               {!!.11}
  if HandleAllocated then                                              {!!.11}
    FullWidth := ClientWidth                                           {!!.11}
  else                                                                 {!!.11}
    FullWidth := Width;                                                {!!.11}
...
begin
  ...
  if VisibleColumnCount = 0 then                                       {!!.11}
    Exit;                                                              {!!.11}
  ...
  case Action of
    ...
    taSingle  :
      begin                                                            {!!.01}
        ...
        {check for unhiding a column after all others}                 {!!.01}
        if not DoIt then                                               {!!.01}
          DoIt :=                                                      {!!.01}
            (C1 > VisibleColumns[Pred(VisibleColumnCount)].Number) and {!!.01}
            (ClientWidth > VisibleColumns.RightEdge);                  {!!.01}
      end;                                                             {!!.01}
    taAll     :
      DoIt := True;
    taInsert  :
      begin
        ...
        tbCalcRowColData(LeftColumn);                                  {!!.12}
        tbLayOutChanged;                                               {!!.12}
      end;
    taDelete  :
      begin
  ...
  {adjust left column if all columns will fit}                         {!!.10}
  if (LeftColumn > tbLastLeftCol) then                                 {!!.10}
    LeftColumn := tbLastLeftCol;                                       {!!.10}
...
begin
  ...
  {see if any field names have changed}
  ...
  if InEditingState then begin
    ...
    if (Idx = -1) or not tbCalcActiveCellRect(CellRect) then
      ...
      if (tbActiveCell is TOvcTCComboBox) then begin
        ...
        R := tbCalcColumnRect(ActiveColumn);                           {!!.10}
        R.Bottom := {Client}Height;                                    {!!.13}
        ValidateRect(Handle, @R);

        {this is needed under NT 3.51}                                 {!!.13}
        InvalidateRect(tbActiveCell.EditHandle, nil, False);           {!!.13}
        UpdateWindow(tbActiveCell.EditHandle);                         {!!.13}
      end;
    end;
  end else begin
...
function  TOvcCustomDbTable.tbEditCellHasFocus(FocusHandle : hWnd) : Boolean;
  {-return True if the edit cell has the focus}
var
  ChildHandle : hWnd;
begin
  ...
  if not Assigned(tbActiveCell) then                                   {!!.01}
    Exit;                                                              {!!.01}
...
begin
  ...
  InEditMode := (DataLink.DataSet <> nil) and                          {!!.01}
                (DataLink.DataSet.State in [dsEdit, dsInsert]);        {!!.01}
...
begin
  ...
  if Assigned(AField) then begin
    ...
    if (ACell is TOvcTCString) then
      ...
      end else begin                                    {block revised}{!!.01}
        S := AField.AsString;
        {Idx := -1;}                                                   {!!.13}
        {if TOvcTCComboBox(ACell).Style <> csDropDown then}            {!!.13}
        if S = '' then
          Idx := -1
        else
...
begin
  ...
  LockedColsOffset := tbIndicatorWidth;
  Idx := 0;
...
{!!.11} {added}
function TOvcCustomDbTable.tbIsSibling(HW : hWnd) : Boolean;
  {-return True if the window HW one of our siblings}
var
  C : TWinControl;
  H : hWnd;
...
begin
  ...
  if WordWrap then begin
    ...
    Wd := Size.cX;
    {$ELSE}
    HtWd := GetTextExtent(Canvas.Handle, Buf, Len);
    Wd := LH(HtWd).L;
    {$ENDIF}
...
begin
  ...
  {restore row and column data}                                        {!!.01}
  tbCalcRowColData(CurLeftCol);                                        {!!.01}
  LeftColumn := NewLeftCol;
end;
...
begin
  ...
  Idx := VisibleColumns.IndexOf(OldLeftCol);                           {!!.12}
  if Idx > -1 then                                                     {!!.12}
    OrigOfs := VisibleColumns[Idx].Offset                              {!!.12}
  else                                                                 {!!.12}
    OrigOfs := 0;                                                      {!!.12}
  FLeftColumn := NewLeftCol;
  tbCalcRowColData(LeftColumn);
...
begin
  ...
  if Assigned(AField) then begin
    ...
    if (ACell is TOvcTCComboBox) then begin
      ...
      else begin                                        {block revised}{!!.01}
        if TOvcTCComboBox(ACell).Style in [csDropDown, csSimple] then
          AField.AsString := PCellComboBoxInfo(Data)^.St
        else begin
          Idx := PCellComboBoxInfo(Data)^.Index;
          if Idx < 0 then
            AField.AsString := ''
          else
            AField.AsString := TOvcTCComboBox(ACell).Items[Idx];
        end;
      end;
    ...
    if (ACell is TOvcTCString) then                                    {!!.10}
      AField.AsString := S                                             {!!.10}
    else if (ACell is TOvcTCSimpleField) or
            (ACell is TOvcTCPictureField) or
            (ACell is TOvcTCNumericField) then begin
      case AField.DataType of
        ftString   : AField.AsString := S;
...
procedure TOvcCustomDbTable.tbSetFocus(AHandle : hWnd);
  {-call window's SetFocus routine}
begin
  ...
  try
    with FDataLink do begin
      if Active and (RecordCount > 0) then begin
        ...
        {this update may have been triggered by a scroll action}       {!!.01}
        tbScrolling := True;                                           {!!.01}
        try                                                            {!!.01}
          tbUpdateActive;
        finally                                                        {!!.01}
          tbScrolling := False;                                        {!!.01}
        end;                                                           {!!.01}
      end else                                                         {!!.13}
        FVisibleRowCount := 1;                                         {!!.13}
    end;
  except
    {ignore errors}
  end;
...
begin
  if tbHasHSBar and HandleAllocated then begin
    ...
    for ColNum := LockedColumns to tbLastLeftCol do
      if not tbIsColumnHidden(ColNum) then                             {!!.10}
        Inc(I);
...
begin
  ...
  if tbIsDbActive and HandleAllocated and tbHasVSBar then              {!!.12}
...
{$ENDIF}
      ...
      CBN_DROPDOWN :
        begin
          ...
          {$ELSE}
          SendMessage(Msg.lParamLo, CB_GETDROPPEDCONTROLRECT, 0, LongInt(@R));
          {$ENDIF}
          {validate area occupied by combobox list}
          R.TopLeft := ScreenToClient(R.TopLeft);
          R.BottomRight := ScreenToClient(R.BottomRight);
...
begin
  ...
  if (FDataLink.DataSet = nil) then                                    {!!.17}
    Exit;                                                              {!!.17}
  ...
  if (dtoWantTabs in Options) and (Msg.CharCode = VK_TAB) then begin
    ...
    if (ShiftFlags = 0) then
      Cmd := ccRight
    else if (ShiftFlags and ss_Shift) <> 0 then                        {!!.10}
      Cmd := ccLeft                                                    {!!.10}
    else
      Cmd := ccNone;
  end else
  ...
  if InEditingState then begin
    ...
    (
      (Cmd > ccLastCmd) and (Cmd < ccUserFirst) and                    {!!.01}
      (
...
begin
  ...
  if (otsFocused in tbState) and (otsNormal in tbState) then begin
    ...
    {if the datasource is in edit or insert mode, update the record}   {!!.12}
    if FDataLink.DataSet <> nil then                                   {!!.11}
      if (FDataLink.DataSet.State in [dsEdit, dsInsert]) then begin    {!!.11}
        Idx := VisibleColumns.IndexOf(Col);                            {!!.12}
        if (Idx = -1) or (VisibleColumns[Idx].DefaultCell = nil) then  {!!.12}
          FDataLink.DataSet.Post;                                      {!!.12}
      end;
    ...
    case Region of
      ...
      otrInLocked :
        begin
          ...
          if (Msg.XPos <= tbIndicatorWidth) then
...
begin
  ...
  if tbIsDbActive then with DataLink do begin
    case Msg.ScrollCode of
      ...
      SB_LINEDOWN      :
        if (dtoPageScroll in Options) then
          Scroll(RecordCount - ActiveRecord)
        else
          Scroll(+1);
      SB_PAGEUP        :
        Scroll(-VisibleRowCount);
