...
uses
  ...
  OvcData, OvcIntl, {OvcRLbl, }OvcStr, OvcUser, STDate;
...
const
  DefCtrlColor         = clRed;
  DefParentColor       = True;
  DefPictureMask       = 'XXXXXXXXXX';
  DefTransparent       = False;
  DefUseIntlMask       = False;
...
begin
  ...
  FPictureMask    := DefPictureMask;
  FUseIntlMask    := DefUseIntlMask;
...
procedure TOvcCustomPictureLabel.PaintTo(DC : hDC; X, Y : Integer);
var
  R : TRect;
begin
  R.Left := X;
...
var
  ...
  begin
    if NeedFloat then begin
      Dec(CurLeftLen);
      Result[I] := IntlSupport.CurrencyLtStr[CurLeftLen];
...
begin
  ...
  if I > 0 then begin
    ...
    for N := K downto I do
      if J > 0 then begin
        Dec(J);
        Result[N] := IntlSupport.CurrencyLtStr[J];
      end else
        Result[N] := ' ';
  end;
  ...
  if I > 0 then begin
    ...
    while (I <= ResultLen) and (FPictureMask[I] = pmCurrencyRt) do begin
      if J < K then begin
        Result[I] := IntlSupport.CurrencyRtStr[J];
        Inc(J);
      end else
        Result[I] := ' ';
      Inc(I);
          ...
          if PicChar = pmComma then begin
            if (SrcLen > 0) then begin                                 {!!.10}
              if IntlSupport.CommaChar <> #0 then                      {!!.10}
                Result[I] := IntlSupport.CommaChar                     {!!.10}
              else                                                     {!!.10}
                System.Delete(Result, I, 1);                           {!!.10}
            end else                                                   {!!.10}
              HandleOtherCases;
