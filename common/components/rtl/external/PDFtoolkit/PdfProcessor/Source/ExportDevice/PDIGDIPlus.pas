...
******************************************************************************}
{*****************************************************************************
             The following two functionalities are added by Gnostice for use
             in DLL projects.
             1] procedure StartIGDIPlus;
             2] procedure StopIGDIPlus;

             As stated in MSDN GdiplusStartup or GdiplusShutdown should not be called
             in DllMain or in any function that is called by DllMain. It is also
             observed that the process hangs at the point of unloading the DLL.
             Please refer the article below for more information:
             http://msdn.microsoft.com/en-us/library/ms534077(VS.85).aspx

             © 2002-2009 Gnostice Information Technologies Private Limited
             www.gnostice.com
******************************************************************************}
...
//--------------------------------------------------------------------------
  ...
  IGPBitmap = interface( IGPImage )
    ...
    property Pixels[ X, Y : Integer ] : TGPColor read GetPixel write SetPixelProp;
...
//  TMatrixArray = array[0..5] of Single;
  ...
  IGPMatrix = interface
    ...
    function TransformPoints( pts : array of TGPPoint ) : TGPMatrix;

    function TransformVectorsF( pts : array of TGPPointF ) : TGPMatrix;
    function TransformVectors( pts : array of TGPPoint ) : TGPMatrix;
...
\**************************************************************************)
  ...
  IGPGraphics = interface
    ...
    function TransformPoints(destSpace: TGPCoordinateSpace; srcSpace: TGPCoordinateSpace;
      pts: PGPPointF; count: Integer) : TGPGraphics; overload;
    function TransformPoints(destSpace: TGPCoordinateSpace; srcSpace: TGPCoordinateSpace;
      pts: PGPPoint; count: Integer) : TGPGraphics; overload;
    //------------------------------------------------------------------------
    ...
    // MeasureString
    function GetStringSizeF(string_: WideString; length: Integer; font: IGPFont;
    ...
    property Transform          : IGPMatrix read GetTransform       write SetTransformProp;
    property Clip               : IGPRegion read GetClip;
...
\**************************************************************************)
  ...
  function TGPMatrix.TransformPoints( pts : array of TGPPoint ) : TGPMatrix;
  begin
    ErrorCheck( GdipTransformMatrixPointsI(FNativeMatrix, @pts[ 0 ], Length( pts )));
    Result := Self;
  end;

  function TGPMatrix.TransformVectorsF( pts : array of TGPPointF ) : TGPMatrix;
  begin
    ErrorCheck( GdipVectorTransformMatrixPoints( FNativeMatrix, @pts[ 0 ], Length( pts )));
    Result := Self;
  end;

  function TGPMatrix.TransformVectors( pts : array of TGPPoint ) : TGPMatrix;
  begin
  ...
  function TGPMatrix.Equals(matrix: IGPMatrix): Boolean;
  var
    AValue : BOOL;
...
\**************************************************************************)
  ...
  function TGPRegion.Equals(region: IGPRegion; g: IGPGraphics): Boolean;
  var
    booln: BOOL;
  begin
    booln := False;
...
//       codepointsFitted: PInteger = nil; linesFilled: PInteger = nil);
  function TGPGraphics.GetStringSizeF(string_: WideString; length: Integer; font: IGPFont;
      const layoutRectSize: TGPSizeF; stringFormat: IGPStringFormat;
      codepointsFitted: PInteger = nil; linesFilled: PInteger = nil) : TGPSizeF;
  var
...
{******************************************************************************
            Added by Gnostice
            * Call StartIGDIPlus before making call to the IGDIPlus routines.
            * Call StopIGDIPlus after you are done.
*******************************************************************************}
procedure StartIGDIPlus;
begin
