...
type
  ...
  public
  { IStrCollection }
    function Add(const AString: AnsiString): Boolean; overload;
    ...
    function Remove(const AString: AnsiString): Boolean; overload;
    function RemoveAll(ACollection: IStrCollection): Boolean;
    function RetainAll(ACollection: IStrCollection): Boolean;
    function Size: Integer;
  public
  { IStrList }
    procedure Add(Index: Integer; const AString: AnsiString); overload;
...
procedure TStrItr.Add(const AString: AnsiString);
begin
  with FOwnList do
  begin
    System.Move(Items[FCursor], Items[FCursor + 1],
      (FOwnList.FCount - FCursor) * SizeOf(AnsiString));
    FCapacity := Length(Items);
...
function TStrItr.GetString: AnsiString;
begin
  Result := FOwnList.Items[FCursor];
end;
...
function TStrItr.Next: AnsiString;
begin
  Result := FOwnList.Items[FCursor];
  FLastRet := FCursor;
  Inc(FCursor);
...
function TStrItr.Previous: AnsiString;
begin
  Dec(FCursor);
  FLastRet := FCursor;
  Result := FOwnList.Items[FCursor];
...
begin
  ...
  begin
    ...
    System.Move(Items[FCursor + 1], Items[FCursor],
      (FSize - FCursor) * SizeOf(AnsiString));
  end;
  Dec(FOwnList.FCount);
  Dec(FSize);
end;

procedure TStrItr.SetString(const AString: AnsiString);
begin
{
  if FLastRet = -1 then
...
procedure TStrVector.Add(Index: Integer; const AString: AnsiString);
begin
  System.Move(Items[Index], Items[Index - 1],
...
function TStrVector.Add(const AString: AnsiString): Boolean;
begin
  if FCount = FCapacity then
    Grow;
...
function TStrVector.Contains(const AString: AnsiString): Boolean;
var
  I: Integer;
begin
  Result := False;
...
function TStrVector.GetString(Index: Integer): AnsiString;
begin
  if (Index < 0) or (Index >= FCount) then
  begin
    Result := '';
...
function TStrVector.IndexOf(const AString: AnsiString): Integer;
var
  I: Integer;
begin
  Result := -1;
...
function TStrVector.LastIndexOf(const AString: AnsiString): Integer;
var
  I: Integer;
begin
  Result := -1;
...
function TStrVector.Remove(const AString: AnsiString): Boolean;
var
  I: Integer;
begin
  Result := False;
  ...
  for I := FCount - 1 downto 0 do
    ...
    begin
      ...
      System.Move(Items[I + 1], Items[I],
        (FCount - I) * SizeOf(AnsiString));
      Dec(FCount);
      Result := True;
    end;
end;

function TStrVector.Remove(Index: Integer): AnsiString;
begin
  if (Index < 0) or (Index >= FCount) then
    raise EDCLOutOfBounds.Create(SOutOfBounds);
  Result := Items[Index];
...
procedure TStrVector.SetString(Index: Integer; const AString: AnsiString);
begin
  if (Index < 0) or (Index >= FCount) then
    raise EDCLOutOfBounds.Create(SOutOfBounds);
  Items[Index] := AString;
...
function TVector.Capacity: Integer;
begin
  Result := FCapacity;
end;
