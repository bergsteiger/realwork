...
begin
  ...
  while Len > 0 do begin
    ...
    if aLen<=Len then begin
      l3Move(buffer^,Data.Buffer[Data.Index],aLen);
      dec(Len,aLen);
      inc(PtrInt(buffer),aLen);
      Compress;
      Data.Index := 0;
    end else begin
      l3Move(buffer^,Data.Buffer[Data.Index],Len);
      inc(Data.Index,Len);
      break;
    end;
  end;
...
begin
  ...
  if L<sizeof(tmp) then begin
    l3Fillchar(tmp,sizeof(tmp),L); // add some salt to unweak password
    if L>0 then
      l3Move(p^,tmp,L);
...
//  assert(KeySize div 8>=AESBlockSize);
  ...
  l3Move(pIn^,pOut^,n); // pIn=pOut is tested in move()
...
begin
  ...
  try
    ...
    if KeySize<128 then begin
      ...
      if LastLen<>0 then
      if Encrypt then begin
        l3Fillchar(Last,AESBlockSize,0);
        Read(@Last,LastLen);
        Crypt.Encrypt(Last);
...
begin
  ...
  if not AES.Initialized then // if error in KeySize -> default fast XorOffset()
    ...
    if BufCount>0 then begin
      ...
      if Len>Count then
        Len := Count;
      l3Move(Buffer,Buf[BufCount],Len);
      inc(BufCount,Len);
...
begin
  Finalize;
  l3Move(buf,result,sizeof(result));
end;
...
begin
  ...
  if count < 0 then begin  //  Padding forces an extra block 
    l3FillChar(p^,count + 8, 0);
    MD5Transform(buf,in_);
    p := @in_;
    count := 56;
  end;
  l3FillChar(p^, count, 0);
...
begin
  ...
  Update(Buffer^,Len);
  Finalize;
  l3Move(buf,Digest,sizeof(Digest));
end;
...
begin
  ...
  if t>len then begin
    l3Move(p^, Pointer(Cardinal(@in_) + 64 - t)^, len);
    Exit;
  end;
  // First chunk is an odd size
  l3Move(p^, Pointer(Cardinal(@in_) + 64 - t)^, t);
  MD5Transform(buf, in_);
  inc(PtrUInt(p), t);
  dec(len, t);
...
begin
  ...
  l3Move(Pointer(Input)^,pointer(result)^,len);
  l3FillChar(PByteArray(result)^[len],padding,padding);
  // encryption
...
begin
  ...
  l3Move(aKey,fKey,fKeySizeBytes);
end;
...
begin
  l3Move(Backup,key,sizeof(key));
end;

procedure TRC4.SaveKey(out Backup: TRC4InternalKey);
begin
  l3Move(key,Backup,sizeof(key));
end;
...
begin
  if Key='' then
    l3FillChar(CompressShaAesKey,sizeof(CompressShaAesKey),0) else
    SHA256Weak(Key,CompressShaAesKey);
  if IV='' then
    l3FillChar(CompressShaAesIV,sizeof(CompressShaAesIV),0) else begin
    SHA256Weak(IV,IV256);
    l3Move(IV256,CompressShaAesIV,sizeof(CompressShaAesIV));
  end;
end;
