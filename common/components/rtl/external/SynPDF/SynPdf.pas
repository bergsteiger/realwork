...
{$endif}
  ...
  TPdfGDIComment =
    (pgcOutline, pgcBookmark, pgcLink, pgcStartAAC, pgcEndAAC);
...
const
 cnAACDataLen = 2;
 cnCommentTypePos = 0;
 cnBlockTypePos = cnAACDataLen - 1;
 cnEndAACRectLen = cnAACDataLen + SizeOf(TRect);
 cnEndAACDataLen = cnEndAACRectLen + SizeOf(TColor);
 cnColorPos = cnEndAACRectLen - 1;
...
procedure GDICommentStatAAC(MetaHandle: HDC; aBlockType: TspBlockType);

procedure GDICommentEndAAC(MetaHandle: HDC; const aRect: TRect; aColor: TColor);
...
begin // high(TPdfGDIComment)<$47 so it will never begin with GDICOMMENT_IDENTIFIER
  ...
  l3Move(pointer(aBookmarkName)^,D[1],L);
...
begin // high(TPdfGDIComment)<$47 so it will never begin with GDICOMMENT_IDENTIFIER
  ...
  SetLength(Data,L+2);
  D := pointer(Data);
  D[cnCommentTypePos] := AnsiChar(pgcOutline);
  D[cnBlockTypePos] := AnsiChar(aLevel);
  l3Move(pointer(aTitle)^,D[2],L);
...
begin // high(TPdfGDIComment)<$47 so it will never begin with GDICOMMENT_IDENTIFIER
  ...
  D^ := AnsiChar(pgcLink);
  PRect(D+1)^ := aRect;
  l3Move(pointer(aBookmarkName)^,D[1+sizeof(TRect)],L);
  Windows.GdiComment(MetaHandle,L+(1+sizeof(TRect)),D);
end;

procedure GDICommentStatAAC(MetaHandle: HDC; aBlockType: TspBlockType);
var
 l_Data : RawByteString;
 l_PChar: PAnsiChar;
begin
 SetLength(l_Data, cnAACDataLen);
 l_PChar := Pointer(l_Data);
 l_PChar[cnCommentTypePos] := AnsiChar(pgcStartAAC);
 l_PChar[cnBlockTypePos] := AnsiChar(aBlockType);
 Windows.GdiComment(MetaHandle, cnAACDataLen, l_PChar);
end;
...
var
 ...
 l_PChar: PAnsiChar;
begin
 SetLength(l_Data, cnEndAACDataLen);
 l_PChar := Pointer(l_Data);
 l_PChar^ := AnsiChar(pgcEndAAC);
 (l_PChar + cnBlockTypePos)^ := AnsiChar(sp_btNone);
 PRect(l_PChar + cnAACDataLen)^ := aRect;
 PColor(l_PChar + cnEndAACRectLen)^ := aColor;
 Windows.GdiComment(MetaHandle, cnEndAACDataLen, l_PChar);
end;
...
//  assert(DigitCount<high(t));
  ...
  StrInt32(@t[15],Value);
  inc(DigitCount);
  l3Move(t[16-DigitCount],B^,DigitCount);
  inc(B,DigitCount);
  result := self;
end;
...
begin
 ...
 if TTF <> nil then
  TTF := TTF.WinAnsiFont
 else // we expect the WinAnsi font in the code below
  if Ansi < 0 then
...
begin
 with LogFont do
  ...
  end; // if (FontType = TRUETYPE_FONTTYPE) and (lfFaceName[0] <> '@') then
  Result := 1;
end;
...
begin
  ...
  l3FillChar(LFont, sizeof(LFont), 0);
...
begin
  ...
  if ForceNoBitmapReuse then
    l3Fillchar(Hash,sizeof(Hash),0) else begin
    row := PERROW[B.PixelFormat];
    if row=0 then begin
      B.PixelFormat := pf24bit;
      row := 24;
    end;
    l3Fillchar(Hash,sizeof(Hash),row);
...
procedure InitializeLogFontW(const aFontName: RawUTF8; aStyle: TFontStyles;
  var aFont: TLogFontW);
begin
  l3Fillchar(AFont,sizeof(AFont),0);
  with aFont do begin
    lfHeight := -1000;
    if fsBold in AStyle then
...
{$IFDEF Garant_Fix}
 procedure lp_GarantFontNameCorrection;
 const
  csGarantFontEnd = ' CYR';
  cnGarantFontEndLen = 4;
 var
  l_Pos: Integer;
 begin
  l_Pos := Pos(csGarantFontEnd, AName);
  if l_Pos > 0 then
   Delete(AName, l_Pos, cnGarantFontEndLen);
 end;
{$ENDIF Garant_Fix} 
...
begin
  ...
  if result<n then
    l3Move(fUsedWide[result],fUsedWide[result+1],(n-result)*4);
...
begin
  ...
  try
    ...
    begin
      ...
      if fDoc.PDFA1 or not WinAnsiFont.fFixedWidth then
      begin
    ...
    begin
      ...
      if fDoc.PDFA1 or (fDoc.EmbeddedTTF and
         ((fDoc.fEmbeddedTTFIgnore = nil) or (fDoc.fEmbeddedTTFIgnore.IndexOf(fDoc.FTrueTypeFonts[fTrueTypeFontsIndex - 1]) < 0))) then
      begin
        fDoc.GetDCWithFont(self);
        ...
        begin
          ...
          if GetFontData(fDoc.FDC, 0, 0, pointer(ttf), ttfSize) <> GDI_ERROR then
          begin
            fFontFile2 := TPdfStream.Create(fDoc);
            if not fDoc.fEmbeddedWholeTTF then
            begin
              ...
              begin
                ...
                with fUsedWideChar do
                  for i := 0 to Count-1 do
                    Used.Add(Values[i]);
                if CreateFontPackage(Pointer(ttf), ttfSize, SubSetData, SubSetMem, SubSetSize,
                    TTFCFP_FLAGS_SUBSET, 0, TTFMFP_SUBSET, 0, TTFCFP_MS_PLATFORMID, TTFCFP_DONT_CARE,
                    Pointer(Used.Values), Used.Count, @lpfnAllocate, @lpfnReAllocate, @lpfnFree, nil) = 0 then
                begin
...
type
  ...
  TPdfEnum = class
  private
    f_AACBlocType: TspBlockType;
    f_SaveContent: TPdfStream;
    f_BackRect: TRect;
    f_FillColor: TColor;
...
begin
    ...
    case PEMRBitBlt(R)^.dwRop of // we only handle PATCOPY = fillrect
      ...
      begin
        ...
        if (Brush.Color <> clWhite) then
         if (E.f_AACBlocType = sp_btNone) then
        {$ENDIF Garant_Fix}
  ...
  EMR_GDICOMMENT:
    with PEMRGDICOMMENT(R)^ do
      if cbData>1 then
        E.HandleComment(TPdfGDIComment(Data[cnCommentTypePos]),PAnsiChar(@Data)+1,cbData-1);
...
begin
  ...
  with obj[aLogFont^.ihFont-1] do begin
    kind := OBJ_FONT;
    l3Move(aLogFont^.elfw.elfLogFont,LogFont,sizeof(LogFont));
...
procedure TPdfEnum.RestoreContent;
begin
 SaveBackRect;
 Assert(f_SaveContent <> nil);
 Canvas.FContents.FWriter.Save;
 f_SaveContent.FWriter.Add(THeapMemoryStream(Canvas.FContents.FWriter.fDestStream).Memory, Canvas.FContents.FWriter.fDestStream.Size);
 FreeAndNil(Canvas.FContents);
 Canvas.FContents := f_SaveContent;
 f_SaveContent := nil;
 f_BackRect := Rect(0, 0, 0, 0);
end;

procedure TPdfEnum.SaveContent;
begin
 f_BackRect := Rect(0, 0, 0, 0);
 Canvas.FContents.Writer.Save;
 f_SaveContent := Canvas.FContents;
 Canvas.FContents := TPdfStream.Create(Canvas.Doc, True);
end;
  ...
  with DC[nDC] do begin
    ...
    if cardinal(iObject-1)<cardinal(length(Obj)) then // avoid GPF
      ...
      case Kind of // ignore any invalid reference
        ...
        OBJ_FONT: begin
          font.spec := FontSpec;
          l3Move(LogFont,font.LogFont,sizeof(LogFont));
        end;
      end;
  end;
end;
...
begin
  try
    case Kind of
      ...
      pgcStartAAC: begin
       Assert(f_AACBlocType = sp_btNone);
       f_AACBlocType := TspBlockType(P[cnBlockTypePos - 1]);
       Assert(f_AACBlocType <> sp_btNone);
       SaveContent;
      end;
      pgcEndAAC: begin
       f_AACBlocType := sp_btNone;
       f_BackRect := PRect(P + cnBlockTypePos)^;
       f_FillColor := PColor(P + cnColorPos)^;
       RestoreContent;
      end;
    end;
  except
...
begin
  try
    ...
    if not ClipRgnNull then begin
      ...
      l3Fillchar(ClipRgn,sizeof(ClipRgn),0);
      ClipRgnNull := True;
    end;
  except
    on E: Exception do ; // ignore any error (continue EMF enumeration)
...
{$IFDEF Garant_Fix}
 function lp_IsFormulaSymbol(aOutPrecision: Byte): Boolean;
 const
  cnSize = 5;
  csFormulaChars: array [0..cnSize] of WideChar = (#$007B, #$0028, #$29, #$007D, #$005B, #$007D);
 var
  i          : Integer;
  l_FirstChar: WideChar;
 begin
  Result := (aOutPrecision = OUT_TT_PRECIS) and (R.emrtext.nChars = 1);
  if Result then
  begin
   Result := False;
   l_FirstChar := tmp[0];
   for i := 0 to cnSize do
    if (l_FirstChar = csFormulaChars[i]) then
    begin
     Result := True;
     Break;
    end;
...
{$ENDIF Garant_Fix}
 ...
 function lp_BoundsIsNull: Boolean;
 begin
  Result := (R.rclBounds.Top = 0) and (R.rclBounds.Left = 0) and (R.rclBounds.Right = -1) and (R.rclBounds.Bottom = -1);
 end;

var
 l_NeedClip     : Boolean;
 l_SinglePosY   : Single;
 l_CanvasFont   : TPdfFont;
{$IFDEF Garant_Fix}
 l_FormulaSymbol: Boolean;
{$ENDIF Garant_Fix}
begin
 if (R.emrtext.nChars > 0) and not lp_BoundsIsNull then
  with DC[nDC] do
  begin
    SetLength(tmp, R.emrtext.nChars + 1); // faster than WideString for our purpose
    l3Move(pointer(PtrUInt(@R) + R.emrtext.offString)^, tmp[0], R.emrtext.nChars * 2);
    ASignY := 1;
    ASignX := 1;
...
{$IFDEF Garant_Fix}
    l_NeedClip := False;
    l_FormulaSymbol := lp_IsFormulaSymbol(font.LogFont.lfOutPrecision);
{$ELSE}
    l_NeedClip := not DC[nDC].ClipRgnNull;
{$ENDIF Garant_Fix}
...
{$IFDEF Garant_Fix}
    if (font.LogFont.lfPitchAndFamily and TMPF_FIXED_PITCH) = 0 then
...
{$ENDIF Garant_Fix}      
    ...
    begin
      ...
      begin
       for i := 0 to R.emrtext.nChars - 1 do
         if (tmp[i] = ' ') or (tmp[i] = #160) then
           Inc(nspace);
       if ((nspace = 0) or ((W - measW) < nspace)) then
           Positioning := tpKerningFromAveragePosition;
      end; // if (Positioning = tpSetTextJustification) then    
    ...
    bOpaque := (not brush.null) and (brush.Color <> clWhite) and
       ((R.emrtext.fOptions and ETO_OPAQUE <> 0) or
        ((font.BkMode = OPAQUE) and (font.BkColor = brush.color))) and (f_AACBlocType = sp_btNone);
    if bOpaque then
      if WithClip then
        backRect := R.emrtext.rcl
      else
      begin
...
{$IFDEF Garant_Fix}
        backRect.Bottom := Max(backRect.Bottom, R.rclBounds.Bottom - R.rclBounds.Top);
{$ENDIF Garant_Fix}        
      end;
...
{$endif}
    ...
    if (R.emrtext.fOptions and ETO_GLYPH_INDEX) <> 0 then
      ...
      begin
        ...
        repeat
          ...
          Canvas.EndText;
          Canvas.BeginText;
          Canvas.MoveTextPoint(Canvas.I2X(PosX), l_SinglePosY);
          Inc(cur);
        until false;
      end
      else
      begin
       if (R.emrtext.nChars = 1) and (tmp[0] = #61623) then
        Canvas.ShowText(cc_Middot)
       else
    ...
    if WithClip or l_NeedClip then
    begin
      Canvas.GRestore;
...
procedure TPdfEnum.SaveBackRect;
var
 l_SavedContent: TPdfStream;
begin
 Canvas.FContents.FWriter.Save;
 l_SavedContent := Canvas.FContents;
 Canvas.FContents := f_SaveContent;
 try
  FillColor := f_FillColor;
  FillRectangle(f_BackRect);
 finally
  FillColor := DC[0].font.color;
  f_SaveContent := Canvas.FContents;
  Canvas.FContents := l_SavedContent;
 end;
end;
...
begin
  ...
  if L>SizeOf(dest) then
    L := SizeOf(dest) else
    l3Move(PDF_PADDING,dest[L],SizeOf(dest)-L);
  l3Move(pointer(tmp)^,dest,L);
end;
...
begin
  ...
  l3Move(Digest,fInternalKey[0],HASHSIZE[fLevel]);
  ...
  elRC4_128: begin  // Algorithm 3.5 in PDF reference doc
    ...
    l3Move(Digest,fUserPass,SizeOf(Digest));
    l3Move(Digest,fUserPass[SizeOf(Digest)],SizeOf(Digest));
  end;
  end;
...
begin
  if (fDoc.fCurrentObjectNumber<>fLastObjectNumber) or (fDoc.fCurrentGenerationNumber<>fLastGenerationNumber) then
   ComputeNewRC4Key
  else
