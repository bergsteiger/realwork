...
const
  ...
  StDecDigitString  : string[13] = '0123456789';
  ...
  protected
    ...
    procedure AddTokenToReplace(var PatRec : PStPatRecord;
                                LastPatRec : PStPatRecord;
                                     Token : TStTokens;
                                     S     : ShortString);
    function  AppendS(Dest, S1 : PAnsiChar; Count : Cardinal) : PAnsiChar;
    ...
    procedure SubLine(Buf : PAnsiChar; FindBuf : PAnsiChar);
...
begin
  ...
  if (Remaining < Count) then Count := Remaining;
  //Dest := Dest + I;
  Move(S1[0], Dest[I], Count);
  //I := I + Count;
  Dest[I + Count] := #0;
end;
    ...
    if (AChar = Any) then
      ...
      if (AChar = Esc) then begin
        ...
        case AChar of
          ...
          lWord      : AddTokenToPattern(PatRec, LastPatRec, tknNegCharClass, StWordDelimString);
          lHex       : AddTokenToPattern(PatRec, LastPatRec, tknCharClass, StHexDigitString);
          lDec       : AddTokenToPattern(PatRec, LastPatRec, tknCharClass, StDecDigitString);
        else
...
begin
  if FIgnoreCase then
   begin
    GetMem(FindBuf, Len+10);
    StrCopy(FindBuf, Buf);
    CharUpperBuff(FindBuf, Len);
   end
  else FindBuf := Buf;
 //Buf := FindBuf;
 try
  ...
  try
    if (FSelAvoidPatPtr <> nil) then begin
      if (not Avoid) then
        Result := FindMatch(FindBuf, FSelAvoidPatPtr, REPosition)
      else if (Avoid) then
        Result := not(FindMatch(FindBuf, FSelAvoidPatPtr, REPosition))
      else
        Result := True;
    end else
      Result := True;
    ...
    if Result then begin
      ...
      if ((FReplacePatPtr <> nil) and (not CheckOnly)) then begin
        if (ooModified in FOutputOptions) then begin
          ...
          Result := FindMatch(FindBuf, FMatchPatPtr, REPosition);
        end;
        if Result then begin
          Tmp[0] := #0;
          SubLine(Buf,FindBuf);
    ...
    if (Token = tknDitto) then begin
      ...
      if (TagNum = 0) then begin
        ...
        if SubLineFindTag(Buf, I, IEnd, TagNum, Flags, IStart, IStop) then begin
          ...
          AppendS(S, @Buf[IStart], IStop-IStart);
        end else begin
           {else couldn't find tagged word, don't append anything}
        end;
      end;
    end else if (Token = tknLitChar) then
      AppendS(S, @PatRec^.OneChar, 1);
    PatRec := PatRec^.NextPattern;
  end;
end;

procedure TStStreamRegEx.SubLine(Buf : PAnsiChar; FindBuf : PAnsiChar);
var
  I,
  M,
  NumToAdd,
...
begin
  ...
  try
    ...
    while (Buf[I] <> EndStr) do begin
      ...
      M := SubLineMatchPattern(FindBuf, Flags, TagOn, TagNum, I, FMatchPatPtr);
      if (M > -1) and (M <> I) and (LastM <> M) then begin
        ...
        LastM := M;
        AppendS(FOutLineBuf, ALine, StrLen(ALine));
      end;
      ...
      if (M = -1) or (M = I) then begin
        {no match or null match, append the character}
          if (Buf[I] = #13) then
            ...
            NumToAdd := 1;
        AppendS(FOutLineBuf, @Buf[I], NumToAdd);
        I := I + NumToAdd;
