...
begin
  NewList := TStringList.Create;
  try
    ErrorD := StNetQueryDisplayInformation(FServer, 1, 0, DWord(-1), DWord(-1),
                                           EntriesRead, Buffer);
    if ErrorD = NERR_SUCCESS then begin
      try
        for Index := 0 to EntriesRead-1 do begin
...
begin
  ...
  try
    ...
    ErrorD := StNetQueryDisplayInformation(FServer, 3, 0, DWord(-1), DWord(-1),
                                           EntriesRead, Buffer);
    if ErrorD = NERR_SUCCESS then begin
      try
        for Index := 0 to EntriesRead-1 do begin
...
begin
  NewList := TStringList.Create;
  try
    StNetQueryDisplayInformation(FServer, 3, 0, DWord(-1), DWord(-1),
                                 EntriesRead, Buffer);
