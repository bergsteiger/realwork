...
uses
  ...
  vgComponent, vgCustomResources, vgCustomObject, vgCustomControl,
  vgScenePrim, vgObject, vgVisualObject;
...
begin
  ...
  begin
    ...
    if vgStyleDesigner.DesignScene.Root <> nil then
      TvgObject(vgStyleDesigner.DesignScene.Root).DeleteChildren;
    ...
    if (vgStyleDesigner.DesignScene.Root <> nil) and (TvgObject(vgStyleDesigner.DesignScene.Root).FindResource(Current) <> nil) then
      vgDesigner.SelectObject(nil, TvgObject(vgStyleDesigner.DesignScene.Root).FindResource(Current), []);
    ...
    {$ENDIF}
    FreeAndNil(vgStyleDesigner.vgScene1);
    FreeAndNil(vgStyleDesigner.DesignScene);
    FreeAndNil(vgStyleDesigner.vgScene2);
    FreeAndNil(vgStyleDesigner.vgScene3);

    FreeAndNil(vgStyleDesigner);
    vgStyleDesigner := nil;

    FreeAndNil(vgDesigner);
 ...
 begin
   ...
   begin
     ...
     if Obj.IsVisual then
       N.IsChecked := not TvgVisualObject(Obj.Visual).Locked;
   end
   else
   begin
     N := TvgTreeViewItem.Create(Self);
   ...
   begin
     TvgVisualObject(Obj.Visual).DesignHide := true;
     // center
     with Obj.Visual do
     begin
...
begin
  ...
  begin
    ...
    ProcessObject(TvgObject(DesignScene.Root), nil);
...
begin
  ...
  begin
    ...
    begin
      // center
      TvgVisualObject(New.Visual).Position.X := round((DesignScene.Width - TvgVisualObject(New.Visual).Width) / 2);
      TvgVisualObject(New.Visual).Position.Y := round((DesignScene.Height - TvgVisualObject(New.Visual).Height) / 2);
      //
      if (Old <> nil) and (Old.IsVisual) then
      begin
        if Old <> DesignScene.Root then
          TvgVisualObject(Old.Visual).DesignHide := true
      end
      else
        if (Old <> nil) and (Old.Parent <> nil) and (Old.Parent.IsVisual) then
        begin
          if Old.Parent <> DesignScene.Root then
            TvgVisualObject(Old.Parent.Visual).DesignHide := true;
        end;
      if New <> DesignScene.Root then
        TvgVisualObject(New.Visual).DesignHide := false;
    end
    else
    begin
      if (New <> nil) and (New.Parent <> nil) and (New.Parent.IsVisual) then
      begin
        TvgVisualObject(New.Parent.Visual).DesignHide := false;
      end;
    end;
...
begin
  ...
  S := TMemoryStream.Create;
  try
    Root := TvgObject(DesignScene.Root);
    Str.Clear;
    for i := 0 to Root.ChildrenCount - 1 do
      if Root.Children[i].isVisual then
      begin
...
begin
  if DestItem = nil then
    TvgObject(SourceItem.TagObject).Parent := TvgObject(DesignScene.Root)
  else
