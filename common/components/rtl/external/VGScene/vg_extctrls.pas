...
procedure TvgIPhoneButton.Cleanup;
begin
...
begin
  ...
  try
    ...
    List := TvgVisualObjectList.Create;
    try
     for i := 0 to FChildren.Count - 1 do
     begin
       if not TvgObject(FChildren[i]).isVisual then Continue;
       Assert(FChildren[i] Is TvgVisualObject);
       ...
       begin
         ...
         List.Insert(j, TvgVisualObject(FChildren[i]));
       end
       else
         List.Add(TvgVisualObject(FChildren[i]));
     end;
     { align }
     ...
     begin
       ...
       begin
         List[i].Position.Y := Height - MinSize;
         List[i].Position.X := (Width / 2) - (MaxWidth / 2) + (i * MinSize);
         List[i].Height := MinSize;
         List[i].Width := MinSize;
       end;
     end
     else
     begin
       ...
       for i := 0 to List.Count - 1 do
       begin
         if (FMousePos.X >= List[i].Position.X) and
            (FMousePos.X <= List[i].Position.X + List[i].Width) then
         begin
           hot := true;
           Break;
         end
       ...
       begin
         if (FMousePos.X >= List[i].Position.X) and
            (FMousePos.X <= List[i].Position.X + List[i].Width) then
         begin
           List[i].Width := MaxSize;
           List[i].Height := MaxSize;
           MaxWidth := MaxWidth + (MaxSize - MinSize);
         end
         else
         begin
           dist := (List[i].Position.X + (List[i].Width / 2)) - FMousePos.X;
           if (Abs(dist) < Amplitude) and (hot) then
           begin
             List[i].Width := MinSize + Sin((Pi / 2) + ((dist / Amplitude) * (Pi / 2))) * (MaxSize - MinSize);
             List[i].Height := List[i].Width;
             MaxWidth := MaxWidth + (List[i].Width - MinSize);
           end
           else
           begin
             List[i].Width := MinSize;
       ...
       for i := 0 to List.Count - 1 do
       begin
         List[i].Position.Y := Height - List[i].Height - 1;
         List[i].Position.X := Pos;
         Pos := Pos + List[i].Height;
       end;
     end;
     { }
    finally
...
begin
  x := 0;
  y := 0;
  TvgCanvas(Canvas).Stroke.Assign(FLineFill);
  while x < Width / 2 do
  begin
    if (x = 0) then
    begin
      TvgCanvas(Canvas).StrokeThickness := 2;
      TvgCanvas(Canvas).Stroke.SolidColor := FLineFill.SolidColor
    end
    else
    begin
      if (frac(x) = 0) and (frac(x / frequency / marks) = 0) then
        ...
        TvgCanvas(Canvas).Stroke.SolidColor := vgOpacity(FLineFill.SolidColor, 0.4);
      TvgCanvas(Canvas).StrokeThickness := 1;
    end;

    TvgCanvas(Canvas).DrawLine(vgPoint(round(Width / 2) + x + (TvgCanvas(Canvas).StrokeThickness / 2), 0), vgPoint(round(Width / 2) + x + (TvgCanvas(Canvas).StrokeThickness / 2), Height), AbsoluteOpacity);
    if x <> 0 then
      TvgCanvas(Canvas).DrawLine(vgPoint(round(Width / 2) - x + (TvgCanvas(Canvas).StrokeThickness / 2), 0), vgPoint(round(Width / 2) - x + (TvgCanvas(Canvas).StrokeThickness / 2), Height), AbsoluteOpacity);
    x := x + FFrequency;
  end;
  ...
  begin
    ...
    TvgCanvas(Canvas).DrawLine(vgPoint(0, round(Height / 2) + y + (TvgCanvas(Canvas).StrokeThickness / 2)), vgPoint(Width, round(Height / 2) + y + (TvgCanvas(Canvas).StrokeThickness / 2)), AbsoluteOpacity);
    if y <> 0 then
      TvgCanvas(Canvas).DrawLine(vgPoint(0, round(Height / 2) - y + (TvgCanvas(Canvas).StrokeThickness / 2)), vgPoint(Width, round(Height / 2) - y + (TvgCanvas(Canvas).StrokeThickness / 2)), AbsoluteOpacity);
    y := y + FFrequency;
  end;
...
procedure TvgImageViewer.Cleanup;
begin
  inherited;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Resource := 'labelstyle';
      for i := 1 to 12 do
        Items.Add({$IfDef XE}FormatSettings.{$EndIf}LongMonthNames[i]);
...
begin
  ...
  try
    ...
    { week days }
    for i := 0 to 6 do
      TvgLabel(FWeek.Children[i]).Text := {$IfDef XE}FormatSettings.{$EndIf}ShortDayNames[1 + ((7 + i + FFirstDayOfWeekNum) mod 7)];
    { days }
    DecodeDate(FDateTime, AYear, AMonth, ADay);
    PreMonth := AMonth - 1;
    if PreMonth < 1 then
...
procedure TvgCalendarBox.Cleanup;
begin
  inherited;
end;
...
begin
  ...
  TvgCanvas(Canvas).Fill.Assign(FontFill);
  TvgCanvas(Canvas).FillText(ARect, ARect, Text, false, AbsoluteOpacity, TextAlign, vgTextAlignCenter);
end;
...
procedure TvgCalendarTextBox.Cleanup;
begin
  inherited;
end;
