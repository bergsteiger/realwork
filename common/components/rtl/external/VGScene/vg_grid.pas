...
type
  ...
  protected
    ...
    procedure DoContentPaint2(Sender: TObject; Canvas: TvgCustomCanvas; const ARect: TvgRect);
    { data }
...
procedure TvgPopupColumn.Cleanup;
begin
  FreeAndNil(FItems);
  inherited;
end;
...
procedure TvgCustomGrid.Cleanup;
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          TvgCanvas(Canvas).FillRect(vgRect(P.X, P.Y, P.X + FContent.Width, P.Y + FRowHeight), 0, 0, [], AbsoluteOpacity);
        end;
      end;
    end;
  end;
  ...
  begin
    ...
    if FShowHorzLines then
      ...
      begin
        P := FContent.LocalToAbsolute(vgPoint(0, (TopRow + i) * FRowHeight));
        P := TvgVisualObject(Sender).AbsoluteToLocal(P);
        TvgCanvas(Canvas).DrawLine(vgPoint(P.X, P.Y - 0.5), vgPoint(P.X + FContent.Width, P.Y - 0.5), AbsoluteOpacity);
      end;
    if FShowVertLines then
      ...
      begin
        ...
        P := TvgVisualObject(Sender).AbsoluteToLocal(P);
        TvgCanvas(Canvas).DrawLine(vgPoint(P.X + 0.5, P.Y), vgPoint(P.X + 0.5, P.Y + (Min(RowCount, VisibleRows) * FRowHeight)), AbsoluteOpacity);
      end;
...
procedure TvgStringColumn.Cleanup;
begin
  inherited;
end;
...
procedure TvgStringGrid.Cleanup;
begin
  inherited;
end;
