...
const
 def_MouseTracking = {$IfDef Nemesis}false{true}{$Else}false{$EndIf};
...
type
  ...
  published
    ...
    property MouseTracking: boolean
     read FMouseTracking
     write FMouseTracking
     default def_MouseTracking;
...
{$IFNDEF NOVCL}
  ...
  protected
   ...
   function DoTab(const aScene   : IvgScene;
                  aControl : TControl;
                  aShift   : Boolean): Boolean; override;
...
begin
  ...
  if (Parent <> nil) and (Parent is TvgScrollBox) and (TvgScrollBox(Parent).ContentLayout <> nil) then
  begin
    TvgScrollBox(Parent).ContentPaint(TvgCanvas(Canvas), LocalRect);
  end;
...
begin
  ...
  fMouseTracking := def_MouseTracking;
end;

procedure TvgScrollBox.Cleanup;
begin
  FContent := nil;
  inherited;
end;
...
begin
  ...
  if (B <> nil) and (B.IsVisual) then
    TvgVisualObject(B.Visual).Visible := FShowSizeGrip;
  ...
  begin
    ...
    if (B <> nil) and (B.IsVisual) then
     TvgVisualObject(B.Visual).Visible := false;
  ...
  if (B <> nil) and (B.IsVisual) then
    FContentLayout := TvgVisualObject(B.Visual);
...
begin
  ...
  begin
    ...
    FContent.FNeedRecalcUpdateRect := true; // need to recalc
  end;
end;
...
procedure TvgScrollBox.Realign;
 ...
 begin
    ...
    if ContentLayout.Parent <> nil then
      TvgVisualObject(ContentLayout.Parent.Visual).BeginUpdate;
    ...
    if ContentLayout.Parent <> nil then
    begin
      TvgVisualObject(ContentLayout.Parent.Visual).EndUpdate;
      TvgVisualObject(ContentLayout.Parent.Visual).Realign;
    end;
    // align scrollbars
    if (VScrollBar <> nil) then
    begin
...
procedure TvgScaledLayout.Cleanup;
begin
  inherited;
end;
...
begin
  ...
  if not (Assigned(FScene)
          and {$IfDef vgDesign}(FScene.GetDesignTime){$Else}false{$EndIf}) then
  begin
    NeedRecalcAbsolute;
...
procedure TvgSplitLayout.Cleanup;
begin
  inherited;
end;
...
begin
  ...
  CanFocused := true;
  // http://mdp.garant.ru/pages/viewpage.action?pageId=253659765
end;

procedure TvgNonVGLayout.SetFocus;
begin
 inherited;
 if IsFocused then
...
procedure TvgNonVGLayout.Cleanup;
begin
...
function TvgNonVGLayout.DoTab(const aScene   : IvgScene;
                              aControl : TControl;
...
var
 ...
 State : TShiftState;
begin
 Result := inherited DoTab(aScene, aControl, aShift);
 if not Result then
 begin
  if (FControl = aControl) then
  begin
   aScene.SetFocused(Self);
   Key := VK_TAB;
   Char := #0;
   if aShift then
    State := [ssShift]
    ...
    State := []; 
   aScene.UnicodeKeyDown(Key, Char, State);
   if not (FControl Is TWinControl) OR TWinControl(FControl).Focused then
    aScene.SetFocus;
   Result := true;
  end;//FControl = aControl
 end;//not Result
end;

function TvgNonVGLayout.IsAcceptableControlForTabNavigation(aControl: TWinControl): Boolean;
begin
 Result := inherited IsAcceptableControlForTabNavigation(aControl);
 if Result then
 begin
  if (FControl = aControl) then
   Result := false; 
...
begin
  ...
  begin
    ...
    FControl := Value;
    CanFocused := (FControl Is TWinControl);
    Realign;
  end;
end;
