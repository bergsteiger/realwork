...
begin
  ...
  begin
    ...
    if FListbox.ItemHeight > 0 then
      // При выборе верхнего или нижнего Item-а в списке, если он виден частично, то TvgComboBox прокручивает
      // весь список так, чтоб текущий Item стал виден полностью. Если прокрутить надо всего на пару пиксел,
      // то выглядит как непонятное пользователю "дерганье". Даже написали ошибку:
      // http://mdp.garant.ru/pages/viewpage.action?pageId=259892454
      // Делаем высоту выпадающего списка для умещения целого кол-ва Item-ов, чтобы избежать прокрутки частями.
...
begin
 ...
 begin
  ...
  ClearSavedItemIndex; //исключаем повторное срабатывание на всякий случай
 end;
end;
   ...
   begin
    ...
    // http://mdp.garant.ru/pages/viewpage.action?pageId=269069309&focusedCommentId=296627220#comment-296627220
     ...
     // http://mdp.garant.ru/pages/viewpage.action?pageId=273598887&focusedCommentId=274465738#comment-274465738
      ...
      // - никогда не говори никогда, или пиши Assert
...
// - это всё равно не поможет, т.к. Alt-Стрелка - не доходит       
       begin
         If ItemIndex < Count - 1 then
          ItemIndex := ItemIndex + 1;
...
begin
 ...
 //ClearJustClosedAndSynchronizeIndex;
 // - нельзя так делать - http://mdp.garant.ru/pages/viewpage.action?pageId=292914902
...
begin
  if FListBox <> nil then
  begin
    ClearSavedItemIndex; //если меняется индекс, то уже нет смысла восстанавливать его в KillFocus
