...
procedure TvgMemo.Cleanup;
begin
  if FUnwrapLines <> nil then
    FreeAndNil(FUnwrapLines);
...
begin
  ...
  TvgCanvas(Canvas).MeasureText(R, R, Str, false, TextAlign, vgTextAlignCenter);
...
begin
  ...
  WholeTextWidth := 0;
  if TvgCanvas(Canvas) = nil then Exit;
...
procedure TvgMemo.DoContentPaint(Sender: TObject; Canvas: TvgCustomCanvas; const ARect: TvgRect);
var
  R, TmpRect: TvgRect;
  LSelArea : TSelArea;
  CurSelRect : integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      TvgCanvas(Canvas).FillText(vgRect(TmpPt.X - R.Left, TmpPt.Y - R.Top, $FFFF, TmpPt.Y - R.Top + (GetLineHeight * 1.25)), LocalRect, Lines[CurLine], false, 1, vgTextAlignNear, vgTextAlignNear);
      Inc(CurLine);
    end;
...
begin
  ...
  TvgCanvas(Canvas).Font.Assign(Font);
//  if Selected then
//    TvgCanvas(Canvas).Font.Color := clHighlightText;
//  TvgCanvas(Canvas).Brush.Style := bsClear;
//  TvgCanvas(Canvas).TexTvgRect(LRect, SymbolRect.Left, SymbolRect.Top, PasswordChar);
end;
...
begin
  ...
  begin
    LWidth := TvgCanvas(Canvas).TextWidth(TmpS);
...
begin
  ...
  begin
    ...
    if LLongestLineWidth < TvgCanvas(Canvas).TextWidth(TmpS) then
      FWidesLineIndex := Length(FLinesBegs) - 1;
end;
...
begin
  ...
  begin
    Result := TvgCanvas(Canvas).TextWidth(Lines[LineNum]);
  end
  else
    Result := 0;
end;
