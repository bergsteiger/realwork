...
begin
  ...
  TvgCanvas(Canvas).Stroke.Assign(FStroke);
  TvgCanvas(Canvas).StrokeThickness := FStrokeThickness;
  TvgCanvas(Canvas).StrokeCap := FStrokeCap;
  TvgCanvas(Canvas).StrokeJoin := FStrokeJoin;
  TvgCanvas(Canvas).StrokeDash := FStrokeDash;
end;
...
begin
  case FLineType of
    vgLineHorizontal: TvgCanvas(Canvas).DrawLine(GetShapeRect.TopLeft, vgPoint(GetShapeRect.Right, GetShapeRect.Top), AbsoluteOpacity);
    vgLineVertical: TvgCanvas(Canvas).DrawLine(GetShapeRect.TopLeft, vgPoint(GetShapeRect.Left, GetShapeRect.Bottom), AbsoluteOpacity);
  else
    TvgCanvas(Canvas).DrawLine(GetShapeRect.TopLeft, GetShapeRect.BottomRight, AbsoluteOpacity);
  end;
end;
...
begin
  TvgCanvas(Canvas).FillEllipse(GetShapeRect, AbsoluteOpacity);
  TvgCanvas(Canvas).DrawEllipse(GetShapeRect, AbsoluteOpacity);
end;
...
begin
  ...
  P.ClosePath;
  TvgCanvas(Canvas).FillPath(P, P.GetBounds, AbsoluteOpacity);
  TvgCanvas(Canvas).DrawPath(P, P.GetBounds, AbsoluteOpacity);
  FreeAndNil(P);
end;
...
begin
  ...
  begin
    ...
    P.LineTo(vgPoint(Width / 2, Height / 2));
    P.ClosePath;
    Result := TvgCanvas(Canvas).PtInPath(AbsoluteToLocal(vgPoint(X, Y)), P.GetBounds, P);
    FreeAndNil(P);
  end
  else
...
begin
  TvgCanvas(Canvas).FillArc(vgPoint(Width / 2, Height / 2), vgPoint((Width - StrokeThickness) / 2, ((Height - StrokeThickness) / 2)), FStartAngle, FEndAngle, AbsoluteOpacity);
  TvgCanvas(Canvas).DrawArc(vgPoint(Width / 2, Height / 2), vgPoint((Width - StrokeThickness) / 2, ((Height - StrokeThickness) / 2)), FStartAngle, FEndAngle, AbsoluteOpacity);
end;
...
begin
  ...
  begin
    ...
    if not (vgSideRight in FSides) then
      R.Right := R.Right + Off;
    TvgCanvas(Canvas).FillRect(R, xRadius, yRadius, FCorners, AbsoluteOpacity, CornerType);
    TvgCanvas(Canvas).DrawRectSides(GetShapeRect, xRadius, yRadius, FCorners, AbsoluteOpacity, Sides, CornerType);
  end
  else
  begin
    TvgCanvas(Canvas).FillRect(R, xRadius, yRadius, FCorners, AbsoluteOpacity, CornerType);
    TvgCanvas(Canvas).DrawRect(R, xRadius, yRadius, FCorners, AbsoluteOpacity, CornerType);
  end;
end;
...
procedure TvgBlurRectangle.Cleanup;
begin
  if FBuffer <> nil then
    FreeAndNil(FBuffer);
  inherited;
...
begin
  ...
  begin
    ...
    FBuffer.Clear(0);
    TvgCanvas(FBuffer.Canvas).Fill.Assign(Fill);
    TvgCanvas(FBuffer.Canvas).FillRect(R, xRadius, yRadius, FCorners, AbsoluteOpacity, CornerType);
    TvgCanvas(FBuffer.Canvas).Stroke.Assign(Stroke);
    TvgCanvas(FBuffer.Canvas).DrawRect(R, xRadius, yRadius, FCorners, AbsoluteOpacity, CornerType);
    Blur(nil, FBuffer, Radius);
    FRecreate := false;
  end;
  TvgCanvas(Canvas).DrawBitmap(FBuffer, LocalRect, LocalRect, AbsoluteOpacity, true);
end;
...
begin
  ...
  TvgCanvas(Canvas).FillRect(GetShapeRect, Radius, Radius, FCorners, AbsoluteOpacity);
  TvgCanvas(Canvas).DrawRect(GetShapeRect, Radius, Radius, FCorners, AbsoluteOpacity);
end;
...
procedure TvgBlurRoundRect.Cleanup;
begin
  if FBuffer <> nil then
    FreeAndNil(FBuffer);
  inherited;
...
begin
  ...
  begin
    ...
    FBuffer.Clear(0);
    TvgCanvas(FBuffer.Canvas).Fill.Assign(Fill);
    TvgCanvas(FBuffer.Canvas).FillRect(R, rectRadius, rectRadius, FCorners, AbsoluteOpacity);
    TvgCanvas(FBuffer.Canvas).Stroke.Assign(Stroke);
    TvgCanvas(FBuffer.Canvas).DrawRect(R, rectRadius, rectRadius, FCorners, AbsoluteOpacity);
    Blur(nil, FBuffer, Radius);
    FRecreate := false;
  end;
  TvgCanvas(Canvas).DrawBitmap(FBuffer, LocalRect, LocalRect, AbsoluteOpacity, true);
...
procedure TvgCalloutRectangle.Cleanup;
begin
  FreeAndNil(FPath);
  inherited;
end;
...
begin
  CreatePath;
  R := FPath.GetBounds;
  TvgCanvas(Canvas).FillPath(FPath, R, AbsoluteOpacity);
  TvgCanvas(Canvas).DrawPath(FPath, R, AbsoluteOpacity);
end;
...
var
  ...
  M: TvgMatrix;
begin
  TvgCanvas(Canvas).Font.Assign(FFont);
  if FStretch then
  begin
    R := vgRect(0, 0, 1000, 1000);
    TvgCanvas(Canvas).MeasureText(R, R, FText, false, FHorzTextAlign, FVertTextAlign);
    ...
    TvgCanvas(Canvas).MultyMatrix(M);
    vgInflateRect(R, Font.Size / 3, Font.Size / 3);
    TvgCanvas(Canvas).FillText(R, R, FText, false, AbsoluteOpacity, FHorzTextAlign, FVertTextAlign);
  end
  else
    TvgCanvas(Canvas).FillText(LocalRect, LocalRect, FText, FWordWrap, AbsoluteOpacity, FHorzTextAlign, FVertTextAlign, f_DrawWithEllipsis);
end;
...
procedure TvgText.pm_SetDrawWithEllipsis(aValue: Boolean);
begin
...
begin
  ...
  FDisableAlign := true;
  if FAutoSize and (TvgCanvas(Canvas) <> nil) and (FText <> '') then
  begin
    TvgCanvas(Canvas).Font.Assign(FFont);
    if WordWrap then
      ...
      R := vgRect(0, 0, 1000, 1000);
    C := TvgCanvas(Canvas);
//    C.MeasureText(R, R, FText, WordWrap, vgTextAlignNear, vgTextAlignNear);     http://mdp.garant.ru/pages/viewpage.action?pageId=407736218
    C.MeasureText(R, R, FText + ' ', WordWrap, vgTextAlignNear, vgTextAlignNear);
  ...
  FDisableAlign := false;
  if FAutoSize and (C <> nil) and (FText <> '') then
...
begin
  ...
  begin
    ...
    TvgCanvas(Canvas).DrawBitmap(B, vgRect(0, 0, FBitmapMargins.Left, FBitmapMargins.Top), R, AbsoluteOpacity, true);
    { top }
    R := vgRect(FBitmapMargins.Left, 0, Width - FBitmapMargins.Right, FBitmapMargins.Top);
    TvgCanvas(Canvas).DrawBitmap(B, vgRect(FBitmapMargins.Left, 0, B.Width - FBitmapMargins.Right, FBitmapMargins.Top), R, AbsoluteOpacity, true);
    { righttop }
    R := vgRect(Width - FBitmapMargins.Right, 0, Width, FBitmapMargins.Top);
    TvgCanvas(Canvas).DrawBitmap(B, vgRect(B.Width - FBitmapMargins.Right, 0, B.Width, FBitmapMargins.Top), R, AbsoluteOpacity, true);
    { left }
    R := vgRect(0, FBitmapMargins.Top, FBitmapMargins.Left, Height - FBitmapMargins.Bottom);
    TvgCanvas(Canvas).DrawBitmap(B, vgRect(0, FBitmapMargins.Top, FBitmapMargins.Left, B.Height - FBitmapMargins.Bottom), R, AbsoluteOpacity, true);
    { center }
    R := vgRect(FBitmapMargins.Left, FBitmapMargins.Top, Width - FBitmapMargins.Right, Height - FBitmapMargins.Bottom);
    TvgCanvas(Canvas).DrawBitmap(B, vgRect(FBitmapMargins.Left, FBitmapMargins.Top, B.Width - FBitmapMargins.Right, B.Height - FBitmapMargins.Bottom), R, AbsoluteOpacity, true);
    { right }
    R := vgRect(Width - FBitmapMargins.Right, FBitmapMargins.Top, Width, Height - FBitmapMargins.Bottom);
    TvgCanvas(Canvas).DrawBitmap(B, vgRect(B.Width - FBitmapMargins.Right, FBitmapMargins.Top, B.Width, B.Height - FBitmapMargins.Bottom), R, AbsoluteOpacity, true);
    { leftbottom }
    R := vgRect(0, Height - FBitmapMargins.Bottom, FBitmapMargins.Left, Height);
    TvgCanvas(Canvas).DrawBitmap(B, vgRect(0, B.Height - FBitmapMargins.Bottom, FBitmapMargins.Left, B.Height), R, AbsoluteOpacity, true);
    { bottom }
    R := vgRect(FBitmapMargins.Left, Height - FBitmapMargins.Bottom, Width - FBitmapMargins.Right, Height);
    TvgCanvas(Canvas).DrawBitmap(B, vgRect(FBitmapMargins.Left, B.Height - FBitmapMargins.Bottom, B.Width - FBitmapMargins.Right, B.Height), R, AbsoluteOpacity, true);
    { rightbottom }
    R := vgRect(Width - FBitmapMargins.Right, Height - FBitmapMargins.Bottom, Width, Height);
    TvgCanvas(Canvas).DrawBitmap(B, vgRect(B.Width - FBitmapMargins.Right, B.Height - FBitmapMargins.Bottom, B.Width, B.Height), R, AbsoluteOpacity, true);
  end
  else
  begin
    case FWrapMode of
      vgImageOriginal:
        begin
          ...
          TvgCanvas(Canvas).DrawBitmap(B, vgRect(0, 0, B.Width, B.Height), R, AbsoluteOpacity, true);
      ...
      vgImageFit:
        begin
          ...
          TvgCanvas(Canvas).DrawBitmap(B, vgRect(0, 0, B.Width, B.Height), R, AbsoluteOpacity, DisableInterpolation)
        end;
      vgImageStretch:
        begin
          R := LocalRect;
...
procedure TvgCustomPath.Cleanup;
begin
  FreeAndNil(FData);
  inherited;
end;
...
begin
  ...
  {$EndIf vgDesign}
  if (TvgCanvas(Canvas) <> nil) and (FData.ResourcePath <> nil) and (Length(FData.ResourcePath.PathData) > 1) then
  begin
    Result := TvgCanvas(Canvas).PtInPath(AbsoluteToLocal(vgPoint(X, Y)), GetShapeRect, FData.ResourcePath)
  end
  else
  if (TvgCanvas(Canvas) <> nil) and (Length(FData.PathData) > 1) then
  begin
    Result := TvgCanvas(Canvas).PtInPath(AbsoluteToLocal(vgPoint(X, Y)), GetShapeRect, FData)
  end
  else
    Result := inherited PointInObject(X, Y);
end;
...
begin
    ...
    case FWrapMode of
      vgPathOriginal:
        begin
          ...
          TvgCanvas(Canvas).FillPath(P, R, AbsoluteOpacity);
          TvgCanvas(Canvas).DrawPath(P, R, AbsoluteOpacity);
      ...
      vgPathStretch:
        begin
          TvgCanvas(Canvas).FillPath(P, ShapeRect, AbsoluteOpacity);
          TvgCanvas(Canvas).DrawPath(P, ShapeRect, AbsoluteOpacity);
        end;
      vgPathTile:
        begin
          State := TvgCanvas(Canvas).SaveCanvas;
  ...
  begin
    ...
    TvgCanvas(Canvas).DrawRect(R, 0, 0, AllCorners, AbsoluteOpacity);
    TvgCanvas(Canvas).StrokeDash := vgDashSolid;
  end;//Assigned(Scene) and Scene.GetDesignTime and not FLocked and not FInPaintTo
  {$EndIf vgDesign}
end;
...
begin
  ...
  if Assigned(FOnPaint) then
    FOnPaint(Self, TvgCanvas(Canvas));
end;
...
procedure TvgSelection.Cleanup;
begin
  inherited;
end;
...
begin
  ...
  if FLeftTopHot then
    ...
    TvgCanvas(Canvas).Fill.SolidColor := $FFFFFFFF;
  TvgCanvas(Canvas).Fillellipse(vgRect(R.Left - (GripSize), R.Top - (GripSize), R.Left + (GripSize), R.Top + (GripSize)), AbsoluteOpacity);
  TvgCanvas(Canvas).DrawEllipse(vgRect(R.Left - (GripSize), R.Top - (GripSize), R.Left + (GripSize), R.Top + (GripSize)), AbsoluteOpacity);
  R := LocalRect;
  if FRightTopHot then
    TvgCanvas(Canvas).Fill.SolidColor := $FFFF0000
  else
    TvgCanvas(Canvas).Fill.SolidColor := $FFFFFFFF;
  TvgCanvas(Canvas).Fillellipse(vgRect(R.Right - (GripSize), R.Top - (GripSize), R.Right + (GripSize), R.Top + (GripSize)), AbsoluteOpacity);
  TvgCanvas(Canvas).DrawEllipse(vgRect(R.Right - (GripSize), R.Top - (GripSize), R.Right + (GripSize), R.Top + (GripSize)), AbsoluteOpacity);
  R := LocalRect;
  if FLeftBottomHot then
    TvgCanvas(Canvas).Fill.SolidColor := $FFFF0000
  else
    TvgCanvas(Canvas).Fill.SolidColor := $FFFFFFFF;
  TvgCanvas(Canvas).Fillellipse(vgRect(R.Left - (GripSize), R.Bottom - (GripSize), R.Left + (GripSize), R.Bottom + (GripSize)), AbsoluteOpacity);
  TvgCanvas(Canvas).DrawEllipse(vgRect(R.Left - (GripSize), R.Bottom - (GripSize), R.Left + (GripSize), R.Bottom + (GripSize)), AbsoluteOpacity);
  R := LocalRect;
  if FRightBottomHot then
    TvgCanvas(Canvas).Fill.SolidColor := $FFFF0000
  else
    TvgCanvas(Canvas).Fill.SolidColor := $FFFFFFFF;
  TvgCanvas(Canvas).FillEllipse(vgRect(R.Right - (GripSize), R.Bottom - (GripSize), R.Right + (GripSize), R.Bottom + (GripSize)), AbsoluteOpacity);
  TvgCanvas(Canvas).DrawEllipse(vgRect(R.Right - (GripSize), R.Bottom - (GripSize), R.Right + (GripSize), R.Bottom + (GripSize)), AbsoluteOpacity);
end;
...
procedure TvgSelectionPoint.Cleanup;
begin
  inherited;
end;
...
begin
  ...
  TvgCanvas(Canvas).FillEllipse(vgRect(- (GripSize), - (GripSize), (GripSize), (GripSize)), AbsoluteOpacity);
  TvgCanvas(Canvas).DrawEllipse(vgRect(- (GripSize), - (GripSize), (GripSize), (GripSize)), AbsoluteOpacity);
end;
...
begin
  ...
  begin
    ...
    P := TvgVisualObject(Parent.Visual).AbsoluteToLocal(P);
    if ParentBounds then
    begin
      if P.X < 0 then P.X := 0;
      if P.Y < 0 then P.Y := 0;
      if P.X > TvgVisualObject(Parent.Visual).Width then
       P.X := TvgVisualObject(Parent.Visual).Width;
      if P.Y > TvgVisualObject(Parent.Visual).Height then
       P.Y := TvgVisualObject(Parent.Visual).Height;
    end;
    Position.X := P.X;
    Position.Y := P.Y;
    if Assigned(FOnTrack) then FOnTrack(Self);
...
procedure TvgDesignFrame.Cleanup;
begin
  inherited;
end;
