...
(*  IvgScene = interface
    ...
    procedure BeginVCLDrag(Source: TObject; ABitmap: TvgCustomBitmap);
  ...
  TvgCanvas = class;
  TvgObject = vgObject.TvgObject;
(*  TvgObject = class;*)
  TvgVisualObject = vgVisualObject.TvgVisualObject;
  //TvgVisualObject = class;
  TvgObjectClass = class of TvgObject;
  TvgResources = class;
  TvgControl = class;
  TvgBitmap = vgBitmap.TvgBitmap;
  //TvgBitmap = class;

  {$I vg_widestr.inc}

  TvgStorageItem = class(TvgPersistent)
  private
    FType: TValueType;
  ...
  TvgStorage = class(TvgPersistent)
  private
  ...
  TvgSpline = class(Tl3ProtoObject)
  private
...
{$EndIf Nemesis}
  ...
  TvgTransform = class(TvgPersistent)
  private
    FMatrix: TvgMatrix;
    FRotateAngle: single;
  ...
  protected
    ...
    procedure SetPosition(aValue : TvgPosition);
    procedure SetScale(aValue : TvgPosition);
    {$IfNDef DesignTimeLibrary}
    class function IsCacheable : Boolean; override;
    {$EndIf  DesignTimeLibrary}
  public
  ...
  published
    ...
    property Scale: TvgPosition read FScale write SetScale;
...
(*  TvgGradientPoints = class(TCollection)
  ...
  TvgGradient = class(TvgPersistent)
  private
...
(*  TvgVisual = class(TvgPersistent)
  ...
  TvgBrushResource = class(TvgPersistent)
  protected
  ...
  TvgWrapMode = vgTypes.TvgWrapMode;
...
(*  TvgBitmapObject = class;
  ...
  TvgPathData = class(TvgPersistent)
  private
    FOnChanged: TNotifyEvent;
  protected
  ...
  protected
    ...
    FParent: THandle;
    FScene: IvgScene;
    FForceUseSystemDefaultTextRendering: Boolean;
  ...
  public
    ...
    procedure FillText(const ARect, AClipRect: TvgRect; const AText: WideString; const WordWrap: boolean;
      ...
      aDrawWithEllipsis : Boolean = false); virtual; abstract;
    ...
    property ForceUseSystemDefaultTextRendering: Boolean
     read FForceUseSystemDefaultTextRendering write FForceUseSystemDefaultTextRendering;
  published
  end;
  TvgCanvasClass = class of TvgCanvas;

  TvgMouseEvent = vgTypes.TvgMouseEvent;
(*  TvgMouseEvent = procedure(Sender: TObject; Button: TMouseButton;
    Shift: TShiftState; X, Y: single) of object;*)
  TvgMouseMoveEvent = vgTypes.TvgMouseMoveEvent;
(*  TvgMouseMoveEvent = procedure(Sender: TObject; Shift: TShiftState;
    X, Y, Dx, Dy: single) of object;*)
  TvgMouseWheelEvent = vgTypes.TvgMouseWheelEvent;
...
(*  TvgAnimationType = (
  ...
  TvgInterpolationType = vgTypes.TvgInterpolationType;
(*  TvgInterpolationType = (
  ...
  TvgObjectSortCompare = vgTypesPrim.TvgObjectSortCompare;
...
(*  TvgObject = class(TvgCustomObject)
  ...
  public
    ...
    procedure AddObjectsToList(const AList: TvgObjectList);
    procedure AddControlsToList(const AList: TvgObjectList);
  ...
  TvgAnimation = vgAnimation.TvgAnimation;
...
(*  TvgAlign = (
  ...
  TOnPaintEvent = vgInterfaces.TOnPaintEvent;
//  TOnPaintEvent = procedure (Sender: TObject; const Canvas: TvgCustomCanvas; const ARect: TvgRect) of object;

  TvgDragMode = vgTypes.TvgDragMode;
(*  TvgDragMode = (
    vgDragManual,
    vgDragAutomatic
  );*)

  TvgDragObject = vgTypes.TvgDragObject;
...
(*  TvgVisualObject = class(TvgObject)
  ...
  protected
    ...
    procedure pm_SetOnClick(const Value: TNotifyEvent); virtual;
  public
    ...
    { }
    procedure GetTabOrderList(List: TvgObjectList; Children: boolean);
...
{$IFNDEF NOVCL}
  ...
  TvgOnSceneKeyDownMessage = procedure(aSender: TvgCustomScene; var aMessage: TWMKey) of object;

  TvgCustomScene = class(TvgScenePrim, IvgScene{$IFDEF WINDOWS}, IDropTarget{$ENDIF})
  private
    {$IFDEF FPCWIN}
    PrevWndProc: WNDPROC;
    FWStyle: string;
    ...
    {$IfDef vgDesign}
    FDesignRoot: TvgVisualObject;
    {$EndIf vgDesign}
    {$IfDef vgDesign}
    FSelected: TvgVisualObject;
    {$EndIf vgDesign}
    FCaptured, FHovered, FFocused: TvgVisualObject;
    {$IfDef vgDesign}
    ...
    FMoving : Boolean;
    {$EndIf vgDesign}
    FLeftTop, FRightTop, FLeftBottom, FRightBottom, FTop, FBottom, FLeft, FRight, FRotate: boolean;
    ...
    FShowTimer: TvgTimer;
    FDBCSLeadChar: Word;
    FStyle: TvgCustomResources{TvgResources};
    ...
    f_InDoTab : Boolean;
    FTabList: TvgVisualObjectList;
    //http://mdp.garant.ru/pages/viewpage.action?pageId=508187704
    f_RecreateCanvas: Boolean;
    f_OnMouseLeave: TNotifyEvent;
    f_ForceUseSystemDefaultTextRendering: Boolean;
    f_DisableFocusByClick: Boolean;
    f_ActivateWindowOnClick: Boolean;
    f_OnSceneKeyDownMessage: TvgOnSceneKeyDownMessage;
    ...
    {$EndIf vgDesign}
    function GetRoot: TvgCustomObject;
    procedure SetFocused(Value: TvgCustomObject{TvgVisualObject});
    {$IfDef vgDesign}
    procedure DoDesignSelect(AObject: TObject);
    {$EndIf vgDesign}
    ...
    {$EndIf vgDesign}
    procedure SetStyle(Value: TvgCustomResources{TvgResources});
    { IvgScene }
    function GetActiveControl: TvgCustomObject;
    function GetDisableUpdate: boolean;
    {$IfDef vgDesign}
    ...
    function GetComponent: TComponent;
    {$IfDef vgDesign}
    function GetSelected: TvgCustomObject{TvgVisualObject};
    {$EndIf vgDesign}
    {$IfDef vgDesign}
    function GetDesignPlaceObject: TvgCustomObject{TvgVisualObject};
    {$EndIf vgDesign}
    procedure SetDisableUpdate(Value: boolean);
    function GetUpdateRectsCount: integer;
    function GetUpdateRect(Index: integer): TvgRect;
    procedure SetCaptured(Value: TvgCustomObject{TvgVisualObject});
    function GetCaptured: TvgCustomObject{TvgVisualObject};
    function GetFocused: TvgCustomObject{TvgVisualObject};
    {$IfDef vgDesign}
    procedure SetDesignRoot(Value: TvgCustomObject{TvgVisualObject});
    {$EndIf vgDesign}
    function GetMousePos: TvgPoint;
    function GetStyle: TvgCustomResources{TvgResources};
    function GetTransparency: boolean;
    procedure SetTransparency(const Value: boolean);
    procedure BeginVCLDrag(Source: TObject; ABitmap: TvgCustomBitmap{TvgBitmap});
    procedure EndDragEvent(Sender, Target: TObject; X, Y: Integer);
    function GetAnimatedCaret: boolean;
    function LocalToScreen(const Point: TvgPoint): TvgPoint;
    function ScreenToLocal(const Point: TvgPoint): TvgPoint;
    function ShowKeyboardForControl(AObject: TvgCustomObject{TvgObject}): boolean;
    function HideKeyboardForControl(AObject: TvgCustomObject{TvgObject}): boolean;
   function DoTab(aControl: TControl;
    aShift: Boolean): Boolean;
    procedure FocusHook(aControl : TWinControl; aGot: Boolean);
    procedure ControlMessageHook(aControl : TWinControl; const aMessage : TMessage);
    function GetSceneControl: TControl;
      ...
      procedure WMMouseActivate(var Msg : TWMMouseActivate);
        ...
        {-}
    procedure pm_SetForceUseSystemDefaultTextRendering(aValue: Boolean);
  protected
  ...
  protected  
    ...
    { }
    {$IfNDef DesignTimeLibrary}
    function IsSomePopupWasClosed: Boolean; override;
    {$EndIf  DesignTimeLibrary}
  public {V}
    function ObjectByPoint(X, Y: single; aForHint: Boolean = False): TvgVisualObject;
  public
    FPopupPos: TPoint;
    constructor Create(AOwner: TComponent); override;
    procedure Cleanup; override;
    procedure AddTabStop(aControl : TvgVisualObject);
    procedure RemoveTabStop(aControl : TvgVisualObject);
    procedure GetEffectiveTabOrderList(aList: TvgVisualObjectList);
    ...
    { children }
    procedure AddObject(AObject: TvgCustomObject{TvgObject});
    procedure RemoveObject(AObject: TvgCustomObject{TvgObject});
    ...
    property Root: TvgCustomObject read GetRoot;
    ...
    property MousePos: TvgPoint read FMousePos; {V}
    property Hovered: TvgVisualObject read FHovered;
    property Focused: TvgCustomObject{TvgVisualObject} read GetFocused write SetFocused;
    ...
    property Style: TvgCustomResources{TvgResources} read FStyle write SetStyle;
    property TabStop;
    //http://mdp.garant.ru/pages/viewpage.action?pageId=508187704
    property RecreateCanvas: Boolean read f_RecreateCanvas write f_RecreateCanvas;
    property OnMouseLeave: TNotifyEvent read f_OnMouseLeave write f_OnMouseLeave;
    property ForceUseSystemDefaultTextRendering: Boolean
     read f_ForceUseSystemDefaultTextRendering write pm_SetForceUseSystemDefaultTextRendering;
    property DisableFocusByClick: Boolean read f_DisableFocusByClick write f_DisableFocusByClick;
    property ActivateWindowOnClick: Boolean read f_ActivateWindowOnClick write f_ActivateWindowOnClick;
    property OnSceneKeyDownMessage: TvgOnSceneKeyDownMessage read f_OnSceneKeyDownMessage write f_OnSceneKeyDownMessage;
  published
  end;//TvgCustomScene
...
{$ENDIF}
  ...
  private
    ...
    procedure MoveNonVGLayoutControls(aScene: TvgCustomScene);
  protected
  ...
  public
    ...
    property JustClosed: DWORD read f_JustClosed write f_JustClosed; 
  published
...
(*var
  g_ResourceList: TvgObjectList = nil;*)
...
begin
  ...
  begin
   g_ResourceList := TvgObjectList.Create;
   g_ResourceList.Capacity := 100;
  end;
  if g_ResourceList.IndexOf(AObject) < 0 then
    g_ResourceList.Add(AObject);
end;
...
begin
  Result := nil;
  if (g_ResourceList <> nil) then
    for i := g_ResourceList.Count - 1 downto 0 do
      if (g_ResourceList[i] <> nil) and TvgObject(g_ResourceList[i]).Stored then
        if CompareText(TvgObject(g_ResourceList[i]).ResourceName, AResource) = 0 then
        begin
          Result := TvgObject(g_ResourceList[i]);
          Break;
        end;
end;
...
{ Scenes }
var
  SceneList: TvgSceneList;
  NeedResetLang: boolean;

procedure AddScene(const AScene: IvgScene);
begin
  if SceneList = nil then
    SceneList := TvgSceneList.Create;
  if SceneList.IndexOf(AScene) < 0 then
    SceneList.Add(AScene);
end;

procedure RemoveScene(const AScene: IvgScene);
begin
  SceneList.Remove(AScene);
end;
...
procedure vgMoveLongword(const Src: Pointer; Dst: Pointer; Count: Integer);
begin
  l3Move(Src^, Dst^, Count * 4); // move() is optimized and fast
end;
...
var
  S: char;
begin
  S := {$IfDef XE}FormatSettings.{$EndIf}DecimalSeparator;
  try
    ...
    {$IfDef XE}FormatSettings.{$EndIf}DecimalSeparator := S;
  end;
end;
...
procedure TvgStorageItem.Cleanup;
begin
  if FStream <> nil then
    FreeAndNil(FStream);
  inherited;
...
procedure TvgTransform.Cleanup;
begin
...
begin
  ...
  begin
    TvgHackPosition(FPosition).FX := TvgHackPosition(TvgTransform(Source).Position).FX;
    TvgHackPosition(FPosition).FY := TvgHackPosition(TvgTransform(Source).Position).FY;
    TvgHackPosition(FScale).FX := TvgHackPosition(TvgTransform(Source).Scale).FX;
    TvgHackPosition(FScale).FY := TvgHackPosition(TvgTransform(Source).Scale).FY;
    TvgHackPosition(FSkew).FX := TvgHackPosition(TvgTransform(Source).Skew).FX;
    TvgHackPosition(FSkew).FY := TvgHackPosition(TvgTransform(Source).Skew).FY;
    TvgHackPosition(FRotateCenter).FX := TvgHackPosition(TvgTransform(Source).RotateCenter).FX;
    TvgHackPosition(FRotateCenter).FY := TvgHackPosition(TvgTransform(Source).RotateCenter).FY;
    MatrixChanged(Self);
  end
  else
    inherited
...
procedure TvgTransform.SetPosition(aValue : TvgPosition);
begin
 fPosition.Assign(aValue);
end;

procedure TvgTransform.SetScale(aValue : TvgPosition);
begin
 fScale.Assign(aValue);
end;

{$IfNDef DesignTimeLibrary}
class function TvgTransform.IsCacheable : Boolean;
begin
 Result := true;
end;
{$EndIf  DesignTimeLibrary}
...
{$IfNDef DesignTimeLibrary}
class function TvgGradient.IsCacheable : Boolean;
begin
 Result := true;
end;
{$EndIf  DesignTimeLibrary}
...
procedure TvgBrushResource.Cleanup;
begin
...
{$IfNDef DesignTimeLibrary}
class function TvgBrushResource.IsCacheable : Boolean;
begin
 Result := true;
end;
{$EndIf  DesignTimeLibrary}
...
procedure TvgBrushBitmap.Cleanup;
begin
  FOnChanged := nil;
  FWrapMode:= Low(TvgWrapMode);
  FreeAndNil(FBitmap);
  inherited;
end;
...
{$IfNDef DesignTimeLibrary}
class function TvgBrushBitmap.IsCacheable : Boolean;
begin
 Result := true;
end;
{$EndIf  DesignTimeLibrary}
...
{$IfNDef DesignTimeLibrary}
class function TvgBrush.IsCacheable : Boolean;
begin
 Result := true;
end;
{$EndIf  DesignTimeLibrary}
...
procedure TvgFont.Cleanup;
begin
 FStyle := Low(TvgFontStyle);
 FOnChanged := nil;
  inherited;
end;
...
begin
  ...
  begin
    ...
    if fsBold in TFont(Source).Style then
      FStyle := vgFontBold;
...
{$IfNDef DesignTimeLibrary}
class function TvgFont.IsCacheable : Boolean;
begin
 Result := true;
end;
{$EndIf  DesignTimeLibrary}
...
begin
  ...
  begin
    ...
    for i := 0 to FHeight - 1 do
      l3Move(Scanline[i]^, TBitmap(Dest).Scanline[i]^, Width * 4);
  end
  else
  {$ENDIF}
  {$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      ...
      l3Move(TvgPathData(Source).PathData[0], PathData[0], SizeOf(TvgPathPoint) * Length(PathData));
      if Assigned(FOnChanged) then
        FOnChanged(Self);
    end;
  end
...
begin
  ...
  begin
    ...
    l3Move(PathData[0], OldPathData[0], Length(PathData) * SizeOf(PathData[0]));
    SetLength(PathData, 0);
...
begin
  ...
  begin
    ...
    begin
      ...
      FreeAndNil(FSaveData[i].Stroke);
      FreeAndNil(FSaveData[i].Fill);
      FreeAndNil(FSaveData[i].Font);
    end;
...
begin
  ...
  begin
    ...
    if FBuffered then
      for i := 0 to FHeight - 1 do
        l3Move(PvgColorArray(FBufferBits)[(i) * FWidth], TvgBitmap(Dest).Scanline[i]^, FWidth * 4);
  end
  else
    inherited;
end;
...
{  if (g_ResourceList <> nil) then
  begin
   i := g_ResourceList.IndexOf(Self);
   if i >= 0 then
     g_ResourceList[i] := nil;
  end;//g_ResourceList <> nil}
  { }
  if FParent <> nil then
...
begin
  if FNotifyList = nil then
    FNotifyList := TvgObjectList.Create;
...
procedure TvgObject.AddObjectsToList(const AList: TvgObjectList);
var
  i: integer;
begin
  AList.Add(Self);
...
procedure TvgObject.AddControlsToList(const AList: TvgObjectList);
var
  i: integer;
begin
...
begin
  ...
  if IsVisual and AObject.IsVisual and (TvgVisualObject(AObject.Visual).Align <> vaNone) then
    TvgVisualObject(Self.Visual).FNeedAlign := true;
  if IsVisual and (AObject is TvgEffect) and (TvgEffect(AObject).Enabled) then
  begin
    TvgVisualObject(Visual).FHasEffect := true;
    TvgVisualObject(Visual).UpdateEffects;
  end;
  if IsVisual and AObject.IsVisual then
  begin
    if TvgVisualObject(Visual).TempCanvas <> nil then
      TvgVisualObject(AObject.Visual).TempCanvas := TvgVisualObject(Visual).TempCanvas;
    if Self.IsVisual then
      TvgVisualObject(AObject).FUpdating := TvgVisualObject(Self).FUpdating;
    TvgVisualObject(AObject.Visual).RecalcOpacity;
    TvgVisualObject(AObject.Visual).RecalcAbsolute;
    TvgVisualObject(AObject.Visual).RecalcUpdateRect;
    if TvgVisualObject(Visual).FHasEffect then
    begin
      TvgVisualObject(Visual).UpdateEffects;
    ...
    if TvgVisualObject(Self).FTabList = nil then
      TvgVisualObject(Self).FTabList := TvgObjectList.Create;
...
{    if AObject.isVisual and (TvgVisualObject(AObject).Align <> vaNone) then
  ...
  if IsVisual then
    TvgVisualObject(Visual).RecalcHasEffect;
  if IsVisual and AObject.IsVisual then
  begin
    TvgVisualObject(Self).RecalcNeedAlign;
    if TvgVisualObject(AObject.Visual).TempCanvas <> nil then
      TvgVisualObject(AObject.Visual).TempCanvas := nil;
  end;
end;
...
var
  i: integer;
begin
  if (Parent <> nil) and (Parent.IsVisual) and not (TvgVisualObject(Parent.Visual).ClipChildren) and
     ((Position.X < 0) or (Position.Y < 0) or
      (Position.X + Width < TvgVisualObject(Parent.Visual).Width) or
      (Position.Y + Height < TvgVisualObject(Parent.Visual).Height))
    then
      TvgVisualObject(Parent.Visual).FRecalcUpdateRect := true;
...
begin
  ...
  begin
    ...
    begin
      FAbsoluteMatrix := vgMatrixMultiply(vgMatrixMultiply(FLocalMatrix, TvgVisualObject(FParent.Visual).GetChildrenMatrix), TvgVisualObject(FParent.Visual).AbsoluteMatrix);
    end
    else
...
begin
  if FTempCanvas <> nil then
    ...
    if FScene <> nil then
      Result := TvgCanvas(FScene.GetCanvas)
    else
      Result := nil;
end;
...
begin
  ...
  if (Assigned(FScene) and not (FScene.GetDesignTime)) and
     CanFocused and
     not FIsFocused and
...
procedure TvgVisualObject.GetTabOrderList(List: TvgObjectList; Children: boolean);
var
  i: integer;
  Control: TvgObject;
begin
  if (FTabList <> nil) then
    ...
    begin
      ...
      List.Add(Control);
      if Children then
        TvgVisualObject(Control).GetTabOrderList(List, Children);
...
procedure TvgVisualObject.pm_SetOnClick(const Value: TNotifyEvent);
begin
 FOnClick := Value;
end;*)
...
procedure TvgBrushObject.Cleanup;
begin
  FreeAndNil(FBrush);
  inherited;
end;
...
procedure TvgPathObject.Cleanup;
begin
  FreeAndNil(FPath);
  inherited;
end;
...
begin
  ...
  if (Operation = opRemove) and
     (FBindingObjects <> nil) and
     (AComponent Is TvgObject) and
     (FBindingObjects.IndexOf(TvgObject(AComponent)) >= 0) then
    FBindingObjects.Remove(TvgObject(AComponent));
  {$IFNDEF NOVCL}
...
begin
  if FBindingObjects = nil then
    FBindingObjects := TvgObjectList.Create;
...
begin
  ...
  begin
    { style }
    Obj := nil;
    if Assigned(FScene) and (FScene.GetStyle <> nil) and (TvgResources(FScene.GetStyle).FRoot <> nil) then
      Obj := TvgVisualObject(TvgResources(FScene.GetStyle).FRoot.FindResource(FResource));
    if Obj = nil then
  ...
  begin
    ...
    begin
      ...
      begin
        StyleName := FResource;
        ResourceObject := TvgVisualObject(TvgResources(FScene.GetStyle).FRoot.FindResource(StyleName));
        if ResourceObject <> nil then
...
begin
  ...
  begin
    ...
    begin
      ...
      TvgHackVisualObject(ResourceObject).FAlign := vaContents;
      {$IfDef vgDesign}
...
procedure TvgBackground.Cleanup;
begin
  FreeAndNil(FFill);
  inherited;
end;
...
begin
  ...
  begin
    TvgCanvas(Canvas).Fill.Assign(FFill);
...
begin
  ...
  begin
    if (FResourceLink <> nil) then
      TvgHackVisualObject(FResourceLink.Visual).FVisible := false;
    inherited;
    if FResourceLink <> nil then
      TvgHackVisualObject(FResourceLink.Visual).FVisible := true;
  end
  else
...
begin
  ...
  FSceneList := TvgSceneList.Create;
end;

procedure TvgResources.Cleanup;
begin
  if FRoot <> nil then
    FreeAndNil(FRoot);
...
begin
  ...
  begin
    ...
    if FRoot.IsVisual then
      TvgHackVisualObject(FRoot.Visual).UpdateDesignHide(false);
    {$EndIf vgDesign}  
    UpdateScenes;
  end;
end;
...
{$IfDef DesignTimeLibrary}
procedure TvgResources.WriteRes;
 ...
 function vcmGetTAModule(const aName: String): IOTAModule;
   {-}
 var
 ...
 function pm_GetModule: IOTAModule;
   {-}
 begin
  Result := vcmGetTAModule(ChangeFileExt(ExtractFileName(pm_GetUnitName), ''));
 end;
...
begin
 if (FRoot <> nil) then
 begin
  l_SE := pm_GetSourceEditor;
  if (l_SE <> nil) then
  begin
   l_Name := ChangeFileExt(l_SE.FileName, '.vgstyle');
   l_S := TFileStream.Create(l_Name, fmCreate);
   try
    FRoot.SaveToStream(l_S);
   finally
    FreeAndNil(l_S);
   end;//try..finally
  end;//l_SE <> nil
...
procedure TvgResources.WriteState(Writer: TWriter);
  //override;
  {-}
begin
 WriteRes;
...
procedure TvgLang.Cleanup;
var
  i: integer;
begin
  for i := 0 to FResources.Count - 1 do
    TObject(FResources.Objects[i]).Free;
...
procedure TvgFrame.Cleanup;
begin
  if not (Assigned(FScene) and
          {$IfDef vgDesign}(FScene.GetDesignTime){$Else}false{$EndIf}) then
...
begin
  ...
  begin
    FNeedClone := false;
    if not (Assigned(FScene) and {$IfDef vgDesign}(FScene.GetDesignTime){$Else}false{$EndIf}) and
       (FSceneObject <> nil) then
    begin
      ...
      begin
        ...
        Obj := TvgObject(FSceneObject.Root);
        Obj.Parent := Self;
      ...
      end;//(FSceneObject <> nil) and (FSceneObject.FOpenInFrame = nil) and (FSceneObject.Root <> nil)
    end;
  end;//FNeedClone
  {$IfDef vgDesign}
  if (FScene <> nil) and
     (Assigned(FScene) and (FScene.GetDesignTime)) and (SceneObject <> nil) and (SceneObject.Root <> nil) then
  begin
    Obj := TvgObject(SceneObject.Root);
    if Obj.isVisual then
    begin
      if FBuffer = nil then
      ...
      TvgCanvas(Canvas).DrawBitmap(FBuffer, vgRect(0, 0, FBuffer.Width, FBuffer.Height),
  ...
  end;//(FScene <> nil) and (Assigned(FScene) and (FScene.GetDesignTime)) and (SceneObject <> nil) and (SceneObject.Root <> nil)
  {$EndIf vgDesign}
end;
...
begin
  ...
  begin
    ...
    if not (Assigned(FScene) and
            {$IfDef vgDesign}(FScene.GetDesignTime){$Else}false{$EndIf}) and
       (FSceneObject <> nil) then
    begin
      if (FSceneObject <> nil) and
         (FChildren <> nil) and (FChildren.Count > 0) then
      begin
        FSceneObject.AddObject(TvgObject(FChildren[0]));
        FSceneObject.FOpenInFrame := nil;
        if (FSceneObject.FCloneFrame <> nil) and not (csDestroying in FSceneObject.ComponentState) then
          FreeAndNil(FSceneObject.FCloneFrame);
      end;//(FSceneObject <> nil) and (FChildren <> nil) and (FChildren.Count > 0)
    end;//not (Assigned(FScene)..
    ...
    case GetEventKind(inEvent) of
      kEventControlDraw:
        begin
          ...
          { draw }
          if TvgCustomScene(inUserData).TvgCanvas(Canvas).FBuffered then
            TvgCustomScene(inUserData).TvgCanvas(Canvas).Parent := THandle(myContext)
          else
            TvgCustomScene(inUserData).TvgCanvas(Canvas).Handle := THandle(myContext);
{          if TvgCustomScene(inUserData).Transparency then
          ...
          if TvgCustomScene(inUserData).TvgCanvas(Canvas).FBuffered then
            TvgCustomScene(inUserData).TvgCanvas(Canvas).Parent := 0
          else
            TvgCustomScene(inUserData).TvgCanvas(Canvas).Handle := 0;
          Result := noErr;
        end;
...
begin
  ...
  //http://mdp.garant.ru/pages/viewpage.action?pageId=508187704
  f_RecreateCanvas := False;
  f_DisableFocusByClick := False;
  f_ActivateWindowOnClick := False;
end;

procedure TvgCustomScene.Cleanup;
begin
  if HandleAllocated then
   Tl3ProcessMessagesManager.Instance.Unsubscribe(Handle);  
  ...
  if (FStyle <> nil) then
   TvgResources(FStyle).RemoveSceneUpdater(Self);
...
begin
  inherited;
  Tl3ProcessMessagesManager.Instance.Subscribe(Handle);  
  ...
  TvgCanvas(Canvas).Parent := Handle;
  {$ENDIF}
  {$IFDEF LINUX}
  TvgCanvas(Canvas).Parent := Integer(GetFixedWidget(pgtkwidget(Handle)));
  {$ENDIF}
  {$ENDIF}
  TvgCanvas(Canvas).ResizeBuffer(Width, Height);
  RealignRoot;
  AddUpdateRect(vgRect(0, 0, Width, Height));
  { Add Hook }
  {$IFNDEF UCL}
  if FTransparency {$IfDef vgDesign}and not (GetDesignTime){$EndIf vgDesign} then
  begin
    ...
    begin
      ...
      {$IFDEF WINDOWS}
      if FTransparency
         {$IfDef vgDesign}and not (GetDesignTime){$EndIf vgDesign} and
...
begin
  ...
  {$IFNDEF UCL}
  {$IFDEF WINDOWS}
  // Пересоздаем канву при создании окна
  // http://mdp.garant.ru/pages/viewpage.action?pageId=508187704
  if (FCanvas = nil) then
  begin
    FCanvas := DefaultCanvasClass.Create(Width, Height);
    FCanvas.ForceUseSystemDefaultTextRendering := f_ForceUseSystemDefaultTextRendering;
    FCanvas.Scene := Self;    
  end;
...
begin
  {$ifdef windows}
  // http://mdp.garant.ru/pages/viewpage.action?pageId=508187704
  if (TvgCanvas(Canvas) <> nil) and f_RecreateCanvas then
    FreeAndNil(FCanvas);
    //TvgCanvas(Canvas).Handle := 0;
  {$endif}
  {$IFNDEF UCL}
  {$IFdef WINDOWS}
  if HandleAllocated then
  ...
  begin
    if Container.CheckParentVisible then
      ...
      begin
        ...
        Control := TvgVisualObject(Container.Children[I].Visual);
...
begin
  ...
  Width := AWidth;
  Height := AHeight;
  TvgCanvas(Canvas).ResizeBuffer(AWidth, AHeight);
  RealignRoot;
...
{$IFNDEF UCL}
{$IFDEF WINDOWS}
 procedure lp_PaintChildControls(hDC: THandle);
 var
  i: Integer;
  lControl: TWinControl;
 begin//lp_PaintChildControls
   for i := 0 to ControlCount-1 do
    if Controls[i] is TWinControl then
    begin
     lControl := TWinControl(Controls[i]);
     lControl.PaintTo(hDC, lControl.Left, lControl.Top);
    end;
 end;//lp_PaintChildControls
var
  ...
  begin
    ...
    if not Parent.Visible then
     if (Width <= 0) OR (Height <= 0) then
      Exit;
      (*
Александр: http://mdp.garant.ru/pages/viewpage.action?pageId=376438963
этот тест попробуй
...
*)        
    ...
    Assert(Canvas <> nil);
    // save alpha
    l3System.GetLocalMem(SaveBits, Width * Height * SizeOf(TvgColorRec));
    try
     Assert(SaveBits <> nil);
     Assert(TvgCanvas(Canvas).FBufferBits <> nil);
     vgMoveLongword(TvgCanvas(Canvas).FBufferBits, SaveBits, Width * Height);
     Assert(TvgCanvas(Canvas).Handle <> 0);
     // paint
     PaintControls(TvgCanvas(Canvas).Handle, nil);
     Assert(TvgCanvas(Canvas).Handle <> 0);
     lp_PaintChildControls(TvgCanvas(Canvas).Handle);
     // restore alpha
     for j := 0 to Height - 1 do
       for i := 0 to Width - 1 do
         PvgColorRecArray(TvgCanvas(Canvas).FBufferBits)[i + (j * Width)].A := SaveBits[i + (j * Width)].A;
    finally
     l3System.FreeLocalMem(SaveBits{, Width * Height * 4});
    end;//try...finally
  end;//ControlCount > 0

  UpdateLayeredWindow(Parent.Handle, 0, @Origin, @Size, TvgCanvas(Canvas).Handle, @BitmapOrigin, $00000000, @Blend, ULW_ALPHA);
  {$ENDIF}
  {$ENDIF}
end;
...
begin
  ...
  if csDestroying in ComponentState then Exit;
  if (TvgCanvas(Canvas).FBuffered) and (TvgCanvas(Canvas).FBufferBits = nil) then Exit;
  if (vgRectWidth(R) = 0) or (vgRectHeight(R) = 0) then Exit; 

  l_R := vgRect(Trunc(R.Left), Trunc(R.Top), Trunc(R.Right) + 1, Trunc(R.Bottom) + 1);
  if not vgIntersectRect(l_R, vgRect(0, 0, Width, Height)) then Exit;
  ...
  begin
    ...
    FUpdateRects[High(FUpdateRects)] := l_R;
...
{$IFDEF LINUX}
   GR.x := Trunc(l_R.Left);
   GR.y := Trunc(l_R.Top);
   GR.width := Trunc(vgRectWidth(l_R));
   GR.height := Trunc(vgRectHeight(l_R));
   SetLength(FUpdateRects, Length(FUpdateRects) + 1);
   FUpdateRects[High(FUpdateRects)] := l_R;
...
{$ENDIF}
{$IFDEF DARWIN}
   HIViewSetNeedsDisplayInRect(HiViewRef(TCarbonWidget(Handle).Widget), CGRectFromRect(l_R), true);
{$ENDIF}
{$IFdef WINDOWS}
   if Transparency and (Parent is TCustomForm) and {$IfDef vgDesign}not FDEsigntime{$Else}true{$EndIf} then
   begin
     SetLength(FUpdateRects, Length(FUpdateRects) + 1);
     FUpdateRects[High(FUpdateRects)] := l_R;
     PostMessage(Handle, WM_ADDUPDATERECT, 0, 0); // only for layered
   ...
   begin
     WR := Rect(Trunc(l_R.Left), Trunc(l_R.Top), Trunc(l_R.Right), Trunc(l_R.Bottom));
...
begin
  if {$IfDef vgDesign}not (FDesignTime){$Else}true{$EndIf} and (FOpenInFrame <> nil) then Exit;
  ...
  begin
    ...
    try
      ...
      { draw back }
      if TvgCanvas(Canvas).BeginScene then
      begin
        TvgCanvas(Canvas).ResetClipRect;
        ScaleMatrix := IdentityMatrix;
        TvgCanvas(Canvas).SetMatrix(ScaleMatrix);
        TvgCanvas(Canvas).SetClipRects(FUpdateRects);
        ...
        begin
          ...
          begin
            { apply cliprgn }
            {$IFdef WINDOWS}
            j := SaveDC(TvgCanvas(Canvas).Handle);
            ...
            SelectClipRgn(TvgCanvas(Canvas).Handle, Rgn);
            DrawParentImage(Self, TvgCanvas(Canvas).Handle);
            RestoreDC(TvgCanvas(Canvas).Handle, j);
            {$ENDIF}
          end
          else
          begin
            TvgCanvas(Canvas).Fill.Assign(FFill);
            TvgCanvas(Canvas).FillRect(vgRect(-1, -1, Width + 1, Height + 1), 0, 0, AllCorners, 1);
          end;
        end
        else
        begin
          ...
          begin
            ...
            begin
              if FTransparency then
                TvgCanvas(Canvas).ClearRect(FUpdateRects[i], 0)
              else
                TvgCanvas(Canvas).ClearRect(FUpdateRects[i], FFill.SolidColor and $FFFFFF);
            end;
          end
          else
          begin
        ...
        if FChildren <> nil then
          ...
          begin
            ...
            for j := 0 to High(FUpdateRects) do
              ...
              begin
                TvgCanvas(Canvas).SetMatrix(vgMatrixMultiply(ScaleMatrix, TvgVisualObject(FChildren[i]).AbsoluteMatrix));
                TvgHackVisualObject(FChildren[i]).BeforePaint;
                TvgHackVisualObject(FChildren[i]).Paint;
                TvgHackVisualObject(FChildren[i]).AfterPaint;
                TvgHackVisualObject(FChildren[i]).PaintChildren;
                if Assigned(TvgVisualObject(FChildren[i]).OnPaint) then
                begin
                  TvgCanvas(Canvas).SetMatrix(vgMatrixMultiply(ScaleMatrix, TvgVisualObject(FChildren[i]).AbsoluteMatrix));
                  TvgVisualObject(FChildren[i]).OnPaint(TvgVisualObject(FChildren[i]), TvgCanvas(Canvas), TvgVisualObject(FChildren[i]).LocalRect);
                end;
                Break;
              end;
          end;
  ...
  {        for i := Trunc((-FDesignScroll.X) / (FSnapGridSize)) - 1 to Trunc((-FDesignScroll.X + Width) / (FSnapGridSize)) + 1 do
          begin
            if i mod 5 = 0 then
              TvgCanvas(Canvas).Stroke.SolidColor := $50505050
            else
              TvgCanvas(Canvas).Stroke.SolidColor := $50303030;
            TvgCanvas(Canvas).DrawLine(vgPoint(i * FSnapGridSize + 0.5, -FDesignScroll.Y + 0.5), vgPoint(i * FSnapGridSize + 0.5, -FDesignScroll.Y + Height + 0.5), 1);
          end;
          ...
          begin
            if j mod 5 = 0 then
              ...
              TvgCanvas(Canvas).Stroke.SolidColor := $50303030;
            TvgCanvas(Canvas).DrawLine(vgPoint(-FDesignScroll.X + 0.5, j * FSnapGridSize + 0.5), vgPoint(-FDesignScroll.X + Width + 0.5, j * FSnapGridSize + 0.5), 1)
          end;}
        end;//FSnapGridShow and (FSnapGridSize <> 0)
        {$IfDef vgDesign}
        { design }
        ...
        begin
          ...
          TvgCanvas(Canvas).StrokeThickness := 1;
          TvgCanvas(Canvas).Stroke.Style := vgBrushSolid;
          TvgCanvas(Canvas).Stroke.SolidColor := $FF1072C5;
          ScaleMatrix := vgTypes.IdentityMatrix;
          TvgCanvas(Canvas).SetMatrix(vgMatrixMultiply(ScaleMatrix, FSelected.AbsoluteMatrix));
          R := FSelected.BoundsRect;
          vgInflateRect(R, -0.5, -0.5);
          TvgCanvas(Canvas).StrokeDash := vgDashDash;
          TvgCanvas(Canvas).DrawRect(R, 0, 0, AllCorners, 1);
          TvgCanvas(Canvas).StrokeDash := vgDashSolid;
          begin
          { rotate }
          if FRotateHot then
            TvgCanvas(Canvas).Fill.SolidColor := $FFFF0000
          else
            TvgCanvas(Canvas).Fill.SolidColor := $FFFFFFFF;
          R := FSelected.BoundsRect;
          vgInflateRect(R, -0.5, -0.5);
          TvgCanvas(Canvas).DrawLine(vgPoint((R.Left + R.Right) / 2, R.Top), vgPoint((R.Left + R.Right) / 2, R.Top - RotSize), 1);
          TvgCanvas(Canvas).Fillellipse(vgRect((R.Left + R.Right) / 2 - (GripSize), R.Top - RotSize - (GripSize),
            (R.Left + R.Right) / 2 +(GripSize), R.Top - RotSize + (GripSize)), Opaque);
          TvgCanvas(Canvas).DrawEllipse(vgRect((R.Left + R.Right) / 2 - (GripSize), R.Top - RotSize - (GripSize),
            (R.Left + R.Right) / 2 +(GripSize), R.Top - RotSize + (GripSize)), Opaque);
          { angles }
          if FLeftTopHot then
            TvgCanvas(Canvas).Fill.SolidColor := $FFFF0000
          ...
          R := FSelected.BoundsRect;
          vgInflateRect(R, -0.5, -0.5);
          TvgCanvas(Canvas).Fillellipse(vgRect(R.Left - (GripSize), R.Top - (GripSize), R.Left + (GripSize), R.Top + (GripSize)), Opaque);
          TvgCanvas(Canvas).DrawEllipse(vgRect(R.Left - (GripSize), R.Top - (GripSize), R.Left + (GripSize), R.Top + (GripSize)), Opaque);
          ...
          R := FSelected.BoundsRect;
          vgInflateRect(R, -0.5, -0.5);
          TvgCanvas(Canvas).Fillellipse(vgRect(R.Right - (GripSize), R.Top - (GripSize), R.Right + (GripSize), R.Top + (GripSize)), Opaque);
          TvgCanvas(Canvas).DrawEllipse(vgRect(R.Right - (GripSize), R.Top - (GripSize), R.Right + (GripSize), R.Top + (GripSize)), Opaque);
          ...
          R := FSelected.BoundsRect;
          vgInflateRect(R, -0.5, -0.5);
          TvgCanvas(Canvas).Fillellipse(vgRect(R.Left - (GripSize), R.Bottom - (GripSize), R.Left + (GripSize), R.Bottom + (GripSize)), Opaque);
          TvgCanvas(Canvas).DrawEllipse(vgRect(R.Left - (GripSize), R.Bottom - (GripSize), R.Left + (GripSize), R.Bottom + (GripSize)), Opaque);
          ...
          R := FSelected.BoundsRect;
          vgInflateRect(R, -0.5, -0.5);
          TvgCanvas(Canvas).FillEllipse(vgRect(R.Right - (GripSize), R.Bottom - (GripSize), R.Right + (GripSize), R.Bottom + (GripSize)), Opaque);
          TvgCanvas(Canvas).DrawEllipse(vgRect(R.Right - (GripSize), R.Bottom - (GripSize), R.Right + (GripSize), R.Bottom + (GripSize)), Opaque);
          { lines }
          ...
          begin
            ...
            R := FSelected.BoundsRect;
            vgInflateRect(R, -0.5, -0.5);
            TvgCanvas(Canvas).FillRect(vgRect(R.Left + vgRectWidth(R)/2 - (GripSize), R.Top - (GripSize), R.Left + vgRectWidth(R)/2 + (GripSize), R.Top + (GripSize)), 0, 0, [], Opaque);
            TvgCanvas(Canvas).DrawRect(vgRect(R.Left + vgRectWidth(R)/2 - (GripSize), R.Top - (GripSize), R.Left + vgRectWidth(R)/2 + (GripSize), R.Top + (GripSize)), 0, 0, [], Opaque);
            if FBottomHot then
              TvgCanvas(Canvas).Fill.SolidColor := $FFFF0000
            else
              TvgCanvas(Canvas).Fill.SolidColor := $FFFFFFFF;
            R := FSelected.BoundsRect;
            vgInflateRect(R, -0.5, -0.5);
            TvgCanvas(Canvas).FillRect(vgRect(R.Left + vgRectWidth(R)/2 - (GripSize), R.Bottom - (GripSize), R.Left + vgRectWidth(R)/2 + (GripSize), R.Bottom + (GripSize)), 0, 0, [], Opaque);
            TvgCanvas(Canvas).DrawRect(vgRect(R.Left + vgRectWidth(R)/2 - (GripSize), R.Bottom - (GripSize), R.Left + vgRectWidth(R)/2 + (GripSize), R.Bottom + (GripSize)), 0, 0, [], Opaque);
          end;
          ...
          begin
            ...
            R := FSelected.BoundsRect;
            vgInflateRect(R, -0.5, -0.5);
            TvgCanvas(Canvas).FillRect(vgRect(R.Left - (GripSize), R.Top + vgRectHeight(R)/2 - (GripSize), R.Left + (GripSize), R.Top + vgRectHeight(R)/2 + (GripSize)), 0, 0, [], Opaque);
            TvgCanvas(Canvas).DrawRect(vgRect(R.Left - (GripSize), R.Top + vgRectHeight(R)/2 - (GripSize), R.Left + (GripSize), R.Top + vgRectHeight(R)/2 + (GripSize)), 0, 0, [], Opaque);
            if FRightHot then
              TvgCanvas(Canvas).Fill.SolidColor := $FFFF0000
            else
              TvgCanvas(Canvas).Fill.SolidColor := $FFFFFFFF;
            R := FSelected.BoundsRect;
            vgInflateRect(R, -0.5, -0.5);
            TvgCanvas(Canvas).FillRect(vgRect(R.Right - (GripSize), R.Top + vgRectHeight(R)/2 - (GripSize), R.Right + (GripSize), R.Top + vgRectHeight(R)/2 + (GripSize)), 0, 0, [], Opaque);
            TvgCanvas(Canvas).DrawRect(vgRect(R.Right - (GripSize), R.Top + vgRectHeight(R)/2 - (GripSize), R.Right + (GripSize), R.Top + vgRectHeight(R)/2 + (GripSize)), 0, 0, [], Opaque);
          end;
          { grid lines }
          ...
          begin
            ScaleMatrix := vgTypes.IdentityMatrix;
            TvgCanvas(Canvas).SetMatrix(vgMatrixMultiply(ScaleMatrix, TvgVisualObject(FSelected.Parent).AbsoluteMatrix));
            TvgCanvas(Canvas).StrokeDash := vgDashDash;
            ...
            begin
              ...
              begin
                TvgCanvas(Canvas).DrawLine(vgPoint(FSelected.Position.X + 0.5, Trunc(FSelected.Position.Y + (FSelected.Height / 2)) + 0.5),
              ...
              begin
                TvgCanvas(Canvas).DrawLine(vgPoint(Trunc(FSelected.Position.X + (FSelected.Width / 2)) + 0.5, FSelected.Position.Y + 0.5),
              ...
              begin
                TvgCanvas(Canvas).DrawLine(vgPoint(FSelected.Position.X + 0.5, FSelected.Position.Y + 0.5), vgPoint(FDesignGridLines[i].Position.X + 0.5, FDesignGridLines[i].Position.Y + 0.5), 1);
              end;
              if (FSelected.Position.X + FSelected.Width = FDesignGridLines[i].Position.X) then
              begin
                TvgCanvas(Canvas).DrawLine(vgPoint(FDesignGridLines[i].Position.X + 0.5, FSelected.Position.Y + 0.5), vgPoint(FDesignGridLines[i].Position.X + 0.5, FDesignGridLines[i].Position.Y + 0.5), 1);
              end;
              if (FSelected.Position.Y + FSelected.Height = FDesignGridLines[i].Position.Y) then
              begin
                TvgCanvas(Canvas).DrawLine(vgPoint(FSelected.Position.X + 0.5, FDesignGridLines[i].Position.Y + 0.5), vgPoint(FDesignGridLines[i].Position.X + 0.5, FDesignGridLines[i].Position.Y + 0.5), 1);
              end;
              if (FSelected.Position.X = FDesignGridLines[i].Position.X + FDesignGridLines[i].Width) then
              begin
                TvgCanvas(Canvas).DrawLine(vgPoint(FDesignGridLines[i].Position.X + FDesignGridLines[i].Width + 0.5, FSelected.Position.Y + 0.5),
              ...
              begin
                TvgCanvas(Canvas).DrawLine(vgPoint(FSelected.Position.X + 0.5, FDesignGridLines[i].Position.Y + FDesignGridLines[i].Height + 0.5),
          ...
          begin
            ...
            TvgCanvas(Canvas).Stroke.SolidColor := $FF5B91DE;
            TvgCanvas(Canvas).DrawRect(R, 0, 0, AllCorners, 1);
            TvgCanvas(Canvas).Font.Family := 'Tahoma';
            TvgCanvas(Canvas).Font.Style := vgFontRegular;
            TvgCanvas(Canvas).Font.Size := 9;
            R.Bottom := R.Top;
            R.Top := R.Bottom - 19;
            R.Right := R.Left + 160;
            TvgCanvas(Canvas).Fill.SolidColor := $FF5B91DE;
            TvgCanvas(Canvas).FillRect(R, 0, 0, AllCorners, 1);
            TvgCanvas(Canvas).Fill.SolidColor := $FFFFFFFF;
            vgInflateRect(R, -2, -2);
            if FDesignPlaceObject.Name <> '' then
              TvgCanvas(Canvas).FillText(R, R, 'ALT-drag to place into [' + FDesignPlaceObject.Name + ']', false, 1, vgTextAlignNear, vgTextAlignCenter)
            else
              TvgCanvas(Canvas).FillText(R, R, 'ALT-drag to place into [' + FDesignPlaceObject.ClassName + ']', false, 1, vgTextAlignNear, vgTextAlignCenter);
          end;
        end;//(FSelected <> nil) and not FSelected.DisableDesignResize
        {$EndIf vgDesign}
        { design modes }
        {$IfDef vgDesign}
        if FDesignTime then
      ...
      begin
        { buffer }
        {$IFDEF WINDOWS}
        if (not FTransparency) or (FTransparency and not (Parent is TCustomForm))
           or {$IfDef vgDesign}(FDesignTime){$Else}false{$EndIf}
           then
        {$ENDIF}
        begin
          for i := 0 to High(FUpdateRects) do
          begin
            R := FUpdateRects[i];
            {$IFDEF WINDOWS}
            TvgCanvas(Canvas).FlushBufferRect(0, 0, FDC, FUpdateRects[i]);
            {$ELSE}
            TvgCanvas(Canvas).FlushBufferRect(0, 0, 0, R);
            {$ENDIF}
          end;
        end;
        { Transparancy }
...
procedure TvgCustomScene.BeginVCLDrag(Source: TObject; ABitmap: TvgCustomBitmap{TvgBitmap});
begin
...
begin
  if TObject(Pointer(VCLDragSource.Tag)) is TvgVisualObject then
    TvgHackVisualObject(Pointer(VCLDragSource.Tag)).DragEnd;
  VCLDragSource.Tag := 0;
end;
...
begin
  ...
  P := vgPoint(X, Y);
  NewTarget := TvgVisualObject(TvgObject(Root).Visual).FindTarget(P, Data);
  ...
  if FTarget <> nil then
  begin
    TvgHackVisualObject(FTarget).DragOver(Data, P, Accept);
  end;
  if (NewTarget <> FTarget) then
  begin
    if FTarget <> nil then
    ...
    if FTarget <> nil then
    begin
      TvgHackVisualObject(FTarget).DragEnter(Data, P);
    end;
  end;
  if FTarget = nil then
    Accept := false;
...
begin
  ...
  begin
    ...
    if (VCLDragSource <> nil) and (VCLDragSource.Tag <> 0) then
      ...
      Data.Source := Source;
    TvgHackVisualObject(FTarget).DragDrop(Data, vgPoint(X, Y));
  end;
  FTarget := nil;
end;
...
begin
  try
    if (Root = nil) and not (TvgObject(Root).IsVisual) then
    begin
...
begin
  try
    ...
    with ScreenToClient(pt) do
      P := vgPoint(X, Y);
    NewTarget := TvgVisualObject(TvgObject(Root).Visual).FindTarget(P, GetDataObject);
    if (NewTarget <> FTarget) then
    begin
      if FTarget <> nil then
        TvgHackVisualObject(FTarget).DragLeave;
      ...
      if FTarget <> nil then
      begin
        TvgHackVisualObject(FTarget).DragEnter(GetDataObject, P);
      end;
    end;
...
begin
  try
    ...
    begin
      with ScreenToClient(pt) do
        P := vgPoint(X, Y);
      TvgHackVisualObject(FTarget).DragDrop(GetDataObject, P);
    end;
    if (FDropTargetHelper <> nil) then
...
begin
  {$IFDEF LINUX}
  TvgCanvas(Canvas).Handle := Integer(gdk_cairo_create(TGtkDeviceContext(inherited TvgCanvas(Canvas).Handle).Drawable));

  if FTransparency then
  begin
    cairo_set_operator(Pcairo_t(TvgCanvas(Canvas).Handle), CAIRO_OPERATOR_SOURCE);
    cairo_set_source_rgba(Pcairo_t(TvgCanvas(Canvas).Handle), 0, 0, 0, 0);
    cairo_paint(Pcairo_t(TvgCanvas(Canvas).Handle));
    cairo_set_operator(Pcairo_t(TvgCanvas(Canvas).Handle), CAIRO_OPERATOR_OVER);
  end;
//  if Length(FUpdateRects) = 0 then
  begin
    // update all area
  ...
  Draw;
  cairo_destroy(Pcairo_t(TvgCanvas(Canvas).Handle));
  TvgCanvas(Canvas).Handle := 0;
  {$ENDIF}
end;

function TvgCustomScene.ObjectByPoint(X, Y: single; aForHint: Boolean = False): TvgVisualObject;
var
  i: integer;
  Obj, NewObj: TvgObject;
  l_VisualObj: TvgVisualObject;
begin
  Result := nil;
  ...
  begin
    ...
    if not TvgVisualObject(Obj).Visible and
       {$IfDef vgDesign}not (FDesignTime){$Else}true{$EndIf} then Continue;

    l_VisualObj := TvgVisualObject(Obj);
    if (not aForHint) then
     NewObj := l_VisualObj.ObjectByPoint(X, Y)
    else
     NewObj := l_VisualObj.ObjectByPointForHint(X, Y);
    if NewObj <> nil then
    begin
...
begin
  ...
  if ({$IfDef vgDesign}FMoving or{$EndIf vgDesign}
      FLeftTop or FRightTop or FLeftBottom or FRightBottom) then
  begin
    Msg.Result := 1;
    Exit;
  end;
...
begin
  ...
  begin
    Obj := ObjectByPoint(CursorPos.X, CursorPos.Y, True);
...
begin
 ...
 try
  ...
  if Assigned(f_OnMouseLeave) then
   f_OnMouseLeave(Self);
 finally
  afw.EndOp;
 end;
...
var
  ...
  rgn: HRgn;
begin
  if (Msg.DC <> 0) and (TvgCanvas(Canvas) <> nil) then
  begin
    rgn := CreateRectRgn(0, 0, 1, 1);
    rgnStatus := GetUpdateRgn(Handle, rgn, false);
    if (rgnStatus = 1) then
    begin
      TvgCanvas(Canvas).FlushBuffer(0, 0, Msg.DC);
    end;
    DeleteObject(rgn);
  end;
  Msg.Result := 1;
...
begin
  ...
  // VK.  Если в фокусе находятся vg-контрол (ex. TvgCheckBox, TvgButton, и др…), который
  // не предназначен для ввода символов, то Сцена не должна возвращать DLGC_WANTCHARS,
  // т.к. это мешает обработке клавиш-акселераторов.
  if (FFocused is TvgCheckbox) or (FFocused is TvgButton) then
...
begin
  ...
  if Assigned(f_OnSceneKeyDownMessage) then
   f_OnSceneKeyDownMessage(Self, Message);
end;
...
var
  ...
  b: TBitmap;
begin
  l3FillChar(LogFont, sizeof(LogFont){, 0});
  b := TBitmap.Create;
  b.Width := 1;
  b.Height := 1;
...
begin
  ...
  begin
    ...
    if Focused <> nil then
    begin
      with TvgVisualObject(Focused).LocalToAbsolute(vgPoint(0, 0)) do
...
function vgIsAcceptableControlForTabNavigation(aControl: TWinControl): Boolean;
...
begin
 ...
 begin
  Result := true;
  if (SceneList <> nil) then
   for l_Index := 0 to Pred(SceneList.Count) do
    if not SceneList[l_Index].IsAcceptableControlForTabNavigation(aControl) then
    begin
     Result := false;
     Exit;
    end;//SceneList[l_Index].DoTab(aControl, aShift)
 end;//aControl Is TvgCustomScene
end;
...
begin
 if (GetRoot = nil) then
  ...
  Result := TvgHackObject(GetRoot).IsAcceptableControlForTabNavigation(aControl);
end;

procedure TvgCustomScene.SetFocus;
var
 l_List: TvgVisualObjectList;
begin
 inherited;
 if (FFocused = nil) then
 begin
  l_List := TvgVisualObjectList.Create;
  try
   GetEffectiveTabOrderList(l_List);
   if not l_List.Empty then
   begin
    if (GetKeyState(VK_SHIFT) >= 0) then
     l_List.First.SetFocus
    else
     l_List.Last.SetFocus;
...
procedure TvgCustomScene.AddTabStop(aControl : TvgVisualObject);
begin
 if (aControl = nil) then
  Exit;
 if (FTabList = nil) then
  FTabList := TvgVisualObjectList.Create;
 aControl.CanFocused := true;
 FTabList.Remove(aControl);
 FTabList.Add(aControl);
end;

procedure TvgCustomScene.RemoveTabStop(aControl : TvgVisualObject);
begin
 if (aControl = nil) then
  Exit;
 if (FTabList <> nil) then
...
procedure TvgCustomScene.GetEffectiveTabOrderList(aList: TvgVisualObjectList);
var
 i: integer;
 Control: TvgVisualObject;
 l_List : TvgObjectList;
begin
 if (Self.FTabList <> nil) then
 begin
  for I := 0 to FTabList.Count - 1 do
  begin
   Control := FTabList[I];
   if (Control.CheckParentVisible and
       Control.CanFocused and
       Control.AbsoluteEnabled) then
    aList.Add(Control);
  end;//for I
 end//Self.FTabList <> nil
 else
 begin
  l_List := TvgObjectList.Create;
  try
   TvgVisualObject(Root).GetTabOrderList(l_List, true, true);
   for i := 0 to l_List.Count - 1 do
   begin
    Assert(l_List[i].IsVisual);
    aList.Add(TvgVisualObject(l_List[i]));
   end;//for i
  finally
   FreeAndNil(l_List);
  end;//try..finally
...
var
  g_PopupList: TvgObjectList;

{$IfNDef DesignTimeLibrary}
function TvgCustomScene.IsSomePopupWasClosed: Boolean; //override;
var
  i: integer;
begin
 //Result := inherited IsSomePopupWasClosed;
 Result := false;
 if (g_PopupList <> nil) then
  if (g_PopupList.Count > 0) then
  begin
    for i := g_PopupList.Count - 1 downto 0 do
    begin
     if TvgPopup(g_PopupList[i]).IsOpen AND
        not TvgPopup(g_PopupList[i]).StaysOpen then
     begin
      Result := true;
      TvgPopup(g_PopupList[i]).IsOpen := false;
     end;//TvgPopup(g_PopupList[i]).IsOpen..
    end;//for i
  end;//g_PopupList.Count > 0
end;
{$EndIf DesignTimeLibrary}
...
procedure TvgCustomScene.UnicodeKeyDown(var Key: Word; var Char: System.WideChar;
 ...
 procedure DoTabPrim(aForward : Boolean; AllowExitFromForm : Boolean);

  function CheckDockedForm : Boolean;
  var
   l_PF : TCustomForm;
   l_C  : TWinControl;
  begin//CheckDockedForm
   Result := false;
   if AllowExitFromForm then
    if (Owner Is TCustomForm) then
     if (TCustomForm(Owner).Parent <> nil) then
     // - форма куда-то придочена
     begin
      l_PF := afw.GetParentForm(TCustomForm(Owner).Parent);
      Assert(l_PF <> nil);
      Self.Focused := nil;
      l_C := Self;
      f_InDoTab := true;
      try
       while true do
       begin
        THackControl(l_PF).SelectNext(l_C, aForward, true);
        l_C := FindControl(GetFocus);
        if (l_C = nil) then
         break;
        if (afw.GetParentForm(l_C) <> Self.Owner) then
         break;
       end;//while true
      finally
       f_InDoTab := false;
      ...
      Result := true;
     end;//TCustomForm(Owner).Parent <> nil
  end;//CheckDockedForm
 ...
 var
  ...
  function CheckListItem: Boolean;
  begin//CheckListItem
   Result := (CurIdx >= 0) AND (CurIdx < l_List.Count);
   if Result then
    Assert(l_List[CurIdx].CheckParentVisible and
           l_List[CurIdx].CanFocused and
           l_List[CurIdx].AbsoluteEnabled);
  end;//CheckListItem

 var
  l_OtherIndex : Integer;

  procedure SetupIndexes;
  begin//SetupIndexes
   if not aForward then
   begin
    l_OtherIndex := l_List.Count - 1;
    if (FFocused <> nil) then
     CurIdx := l_List.IndexOf(FFocused) - 1
    else
     CurIdx := l_OtherIndex;
   end//not aForward
   else
 ...
 begin//DoTabPrim
  ...
  try
   ...
   // - перемещаться нам больше некуда
    CheckDockedForm
   else
   begin
    SetupIndexes;
    if CheckListItem then
     TvgVisualObject(l_List[CurIdx]).SetFocus
    else
    begin
     if not CheckDockedForm then
     begin
      Assert(CurIdx <> l_OtherIndex);
      if (CurIdx <> l_OtherIndex) then
      // - чтобы два раза одно и то же не проверять
      begin
       CurIdx := l_OtherIndex;
       if CheckListItem then
        TvgVisualObject(l_List[CurIdx]).SetFocus;
      end;//CurIdx <> l_OtherIndex
     end;//not CheckDockedForm
    end;//CheckListItem
   end;//l_List.Count < 2
  finally
   FreeAndNil(l_List);
 ...
 procedure DoTab;
 begin//
  DoTabPrim(not (ssShift in Shift), true);
 end;//DoTab
...
begin
 ...
 if (FFocused <> nil) then
 begin
  TvgHackVisualObject(FFocused).KeyDown(Key, Char, Shift);
  if (Key = 0) then
   Exit;
 end;//FFocused <> nil
 Case Key of
  VK_LEFT,
  VK_UP:
   DoTabPrim(false, false);
  VK_RIGHT,
  VK_DOWN:
   DoTabPrim(true, false);
 end;//Case Key
end;
...
begin
 ...
 { focused handler }
 if (FFocused <> nil) then
  TvgHackVisualObject(FFocused).KeyUp(Key, Char, Shift);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FChildren.Delete(0);
      TvgHackVisualObject(Child).FParent := nil;
      TvgHackVisualObject(Child).FScene := nil;
      FreeAndNil(Child);
      //FChildren[0] := nil;
    end;
    FreeAndNil(FChildren);
...
begin
  if AObject = nil then Exit;
  if TvgObject(AObject).Parent <> nil then
    TvgObject(AObject).Parent := nil;
  if FChildren = nil then
    FChildren := TvgObjectList.Create;
  FChildren.Add(TvgObject(AObject));
  TvgHackVisualObject(AObject).SetNewScene(Self);
  if TvgObject(AObject).IsVisual and not (csDestroying in ComponentState) then
  begin
    TvgHackVisualObject(AObject).NeedRecalcUpdateRect;
    TvgHackVisualObject(AObject).NeedRecalcAbsolute;
...
begin
  ...
  begin
    Invalidate;
    TvgHackVisualObject(AObject).SetNewScene(nil);
    Notification(AObject, opRemove);
    FChildren.Remove(TvgObject(AObject));
  end;
end;
...
procedure TvgCustomScene.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
begin
 ...
 try
  ...
  if {$IfDef vgDesign}not (FDesignTime){$Else}true{$EndIf} and
     not Assigned(FOnFlush) and
  ...
  begin
   ...
   l_Obj := TvgVisualObject(ObjectByPoint(FMousePos.X, FMousePos.Y));
   if (l_Obj <> nil) AND l_Obj.HitTest AND (not DisableFocusByClick) then
    SetFocus;
  end;//not (FDesignTime)..
  { translate coord }
  ...
  begin
    ...
    if ((FChildren = nil) or (FChildren.Count = 0)) then
    begin
      l_Obj := TvgBackground.Create(Owner);
      if vgDesigner <> nil then
        l_Obj.Name := vgDesigner.UniqueName(Owner, 'Root');
      AddObject(l_Obj);
      RealignRoot;
    end;
    { Popup }
    ...
    { Change Selected }
    l_Obj := ObjectByPoint(FUnsnapMousePos.X, FUnsnapMousePos.Y);
    if (l_Obj <> nil) and (l_Obj = FSelected) and (ssDouble in Shift) then
    begin
      {$IfDef vgDesign}
      TvgHackVisualObject(l_Obj).DesignClick;
      {$EndIf vgDesign}
    end;
    if (l_Obj <> nil) then
    begin
      if (ssCtrl in Shift) and (l_Obj <> Selected) then
      begin
        { check is exists }
        for i := 0 to High(FSelection) do
          if FSelection[i] = l_Obj then
          begin
        ...
        begin
          ...
          FSelection[High(FSelection)] := l_Obj;
        end;
        if vgDesigner <> nil then
      ...
      begin
        TvgHackVisualObject(FSelected).NeedRecalcUpdateRect;
        FSelected.Repaint;
      end;
      FSelected := l_Obj;
      ...
      { Select in IDE }
      if vgDesigner <> nil then
        vgDesigner.SelectObject(Owner, l_Obj, []);
      if Assigned(FDesignChangeSelection) then
        FDesignChangeSelection(Self);
      { }
      TvgHackVisualObject(FSelected).NeedRecalcUpdateRect;
  ...
  begin
    ...
    if (l_Obj <> nil) then
    begin
      if (FFocused <> l_Obj) AND (FFocused Is TvgComboBox) then
       if (TvgComboBox(FFocused).Popup <> nil) then
        TvgComboBox(FFocused).Popup.JustClosed := 0;
      if (TvgHackVisualObject(l_Obj).QueryInterface(IvgSizeGrip, SG) = 0) and
         (Assigned(SG)) then
      begin
        BeginResize;
      end
      ...
      begin
        P := l_Obj.AbsoluteToLocal(vgPoint(FMousePos.X, FMousePos.Y));
        TvgHackVisualObject(l_Obj).MouseDown(Button, Shift, P.X, P.Y);
        if (TvgHackVisualObject(l_Obj).FDragMode = vgDragAutomatic) then
        begin
          TvgHackVisualObject(l_Obj).BeginAutoDrag;
        end;
      end;//TvgHackVisualObject(l_Obj).QueryInterface(IvgSizeGrip, SG)
    end//l_Obj <> nil
    {$IfNDef Nemesis}
    else
      BeginDrag
...
var
  ...
  NewCursor: TCursor;
begin
 afw.BeginOp;   // http://mdp.garant.ru/pages/viewpage.action?pageId=586352222
 try
  inherited;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if FDesignPlaceObject <> nil then
        begin
          TvgHackVisualObject(FDesignPlaceObject).NeedRecalcUpdateRect;
          FDesignPlaceObject.Repaint;
        end;
      ...
      end//(Obj <> nil) and (Obj <> FSelected.Parent) and (Obj <> FSelected)
      else
      begin
        if FDesignPlaceObject <> nil then
          FDesignPlaceObject.Repaint;
...
//    WheelDelta := WheelDelta * 40;
    ...
    if (FCaptured <> nil) then
    begin
      TvgHackVisualObject(FCaptured).MouseWheel(Shift, WheelDelta, Result);
      Exit;
    end;
    Obj := TvgVisualObject(ObjectByPoint(FMousePos.X, FMousePos.Y));
    while (Obj <> nil) do
    begin
      TvgHackVisualObject(Obj).MouseWheel(Shift, WheelDelta, Result);
    ...
    begin
      ...
      if CurIdx > 0 then
          for i := CurIdx - 1 downto 0 do
            if List[i].isVisual and
               (TvgVisualObject(List[i]).CheckParentVisible) and
               (TvgVisualObject(List[i]).CanFocused) and
               (TvgVisualObject(List[i]).AbsoluteEnabled) then
            begin
...
begin
  inherited;
  if (TvgCanvas(Canvas) <> nil) and (HandleAllocated) then
  begin
    TvgCanvas(Canvas).ResizeBuffer(AWidth, AHeight);
    RealignRoot;
  end;
...
begin
 ...
 begin
  if (FTabList <> nil) AND (AComponent Is TvgVisualObject) then
   FTabList.Remove(TvgVisualObject(AComponent));
  if (AComponent = FActiveControl) then
    FActiveControl := nil;
  if (AComponent = FCaptured) then
    FCaptured := nil;
  {$IfDef vgDesign}
  if (AComponent = FSelected) then
    FSelected := nil;
  {$EndIf vgDesign}  
  if (AComponent = FHovered) then
    FHovered := nil;
  if (AComponent = FFocused) then
    FFocused := nil;
  {$IfDef vgDesign}
  if (AComponent = FDesignPlaceObject) then
    FDesignPlaceObject := nil;
  {$EndIf vgDesign}  
  if (AComponent = FStyle) then
    Style := nil;
  if (AComponent = FOpenInFrame) then
    FOpenInFrame := nil;
...
begin
  ...
  begin
    ...
    begin
      if (FSelected = nil) and (Root <> nil) and (TvgObject(Root).IsVisual) then
        FSelected := TvgVisualObject(TvgObject(Root).Visual);
      if FSelected <> nil then
      begin
        try
          ...
          if OldSel.IsVisual then
            TvgVisualObject(OldSel.Visual).Realign;
        finally
        end;
      end
      else
...
procedure TvgCustomScene.doDesignPopupPaste(Sender: TObject);
 ...
 begin
   Obj.Name := vgDesigner.UniqueName(Owner, Obj.ClassName);
   if Obj.IsVisual and TvgVisualObject(Obj.Visual).Locked then Exit;
   for i := 0 to Obj.ChildrenCount - 1 do
...
var
  ...
  LinkedList: TvgCustomObjectList;
  i: integer;
begin
  { Edit Style }
  if FSelected = nil then Exit;
  ...
  begin
    ...
    R := TvgResources(FSelected.Scene.GetStyle);
  ...
  begin
    ...
    LinkedList := TvgCustomObjectList.Create;
    try
...
begin
  ...
  List.BeginUpdate;
  List.Clear;
  L := TvgObjectList.Create;
  TvgVisualObject(FSelected.Parent).GetTabOrderList(L, false, false);
  try
...
begin
  ...
  begin
    ...
    FFocused := TvgVisualObject(Value);
    if FFocused <> nil then
    begin
...
begin
  if (AObject <> nil) and
     (AObject is TvgVisualObject) and
     (TvgHackVisualObject(AObject).FScene.GetComponent = Self) then
  begin
...
begin
  with ScreenToClient(Classes.Point(Trunc(Point.X), Trunc(Point.Y))) do
    Result := vgPoint(x, y);
end;
...
begin
  FCaptured := TvgVisualObject(Value);
...
begin
  FDesignRoot := TvgVisualObject(Value);
end;
{$EndIf vgDesign}
...
begin
  ...
  begin
    ...
    FStyle := Value;
    if FStyle <> nil then
      TvgResources(FStyle).AddSceneUpdater(Self);
    UpdateResource;
  end;
end;

function TvgCustomScene.ShowKeyboardForControl(AObject: TvgCustomObject{TvgObject}): boolean;
begin
  Result := false;
end;

function TvgCustomScene.HideKeyboardForControl(AObject: TvgCustomObject{TvgObject}): boolean;
begin
  Result := false;
end;
...
begin
 ...
 if (GetRoot = nil) then
  ...
  Result := TvgHackObject(TvgObject(GetRoot)).DoTab(Self, aControl, aShift);
end;

procedure TvgCustomScene.FocusHook(aControl : TWinControl; aGot: Boolean);
begin
 if aGot then
  if (FFocused <> nil) then
  begin
   if (FFocused Is TvgComboBox) then
    if (TvgComboBox(FFocused).Popup <> nil) then
     TvgComboBox(FFocused).Popup.JustClosed := 0;
   if (FFocused Is TvgNonVGLayout) then
    if (TvgNonVGLayout(FFocused).NonVGControl = aControl) then
     Exit;
   Self.Focused := nil;
  end;//FFocused <> nil
end;

procedure TvgCustomScene.ControlMessageHook(aControl : TWinControl; const aMessage : TMessage);

 procedure TryControl(aControl : TWinControl);
 begin//TryControl
  if (aControl = nil) then
   Exit;
  if (aControl.Parent = Self) then
   ...
   TryControl(aControl.Parent);
 end;//TryControl

begin
 if (Transparency = true) then
  if ((aMessage.Msg = CM_INVALIDATE) or (aMessage.Msg = WM_SETCURSOR)) then
  begin
   if (aControl.Owner = Self) then
    Refresh
   else
    TryControl(aControl);
  end;//aMessage.Msg = CM_INVALIDATE
end;
...
procedure TvgCustomScene.WMSetFocus(var Msg : TWMSetFocus);
  //message WM_SETFOCUS;
  {-}
begin
 inherited;
...
procedure TvgCustomScene.WMKillFocus(var Msg : TWMKillFocus);
  //message WM_KILLFOCUS;
  {-}
begin
 inherited;
...
procedure TvgCustomScene.WMMouseActivate(var Msg : TWMMouseActivate);
  //message WM_MOUSEACTIVATE;
  {-}
var
 l_Obj : TvgVisualObject;
...
begin
 inherited;
 GetCursorPos(l_Pt);
 l_Pt := ScreenToClient(l_Pt);
 l_Obj := TvgVisualObject(ObjectByPoint(l_Pt.X, l_Pt.Y));
 if (l_Obj = nil) OR not l_Obj.HitTest then
  Msg.Result := MA_NOACTIVATE;
end;

procedure TvgCustomScene.pm_SetForceUseSystemDefaultTextRendering(aValue: Boolean);
begin
 // Запил для http://mdp.garant.ru/pages/viewpage.action?pageId=518765917
 // Это нужно потому, что сцена всегда рисует текст с антиалиасингом.
 // Если выводить текст с антиалиасингом на "стекло" в Vista/W7 -
 // текст выводится смазаный. В результате, не срасталось с вкладками под Aero
 if (aValue <> f_ForceUseSystemDefaultTextRendering) then
 begin
  f_ForceUseSystemDefaultTextRendering := aValue;
  if (FCanvas <> nil) then
   FCanvas.ForceUseSystemDefaultTextRendering := f_ForceUseSystemDefaultTextRendering;
  Repaint;
 end;//(aValue <> f_ForceUseSystemDefaultTextRendering)
end;
...
begin
  ...
  begin
    for i := g_PopupList.Count - 1 downto 0 do
      TvgPopup(g_PopupList[i]).ClosePopup;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if (g_PopupList.IndexOf(TvgPopupForm(C).FPopup) >= 0) and
         (g_PopupList.IndexOf(FPopup) > g_PopupList.IndexOf(TvgPopupForm(C).FPopup)) then
        FPopup.ClosePopup;
      Exit;
    end;
    CloseAllPopups;
...
var
  ...
  Pos: TPoint;
  l_CurrentMonitor: TMonitor;
  m: Integer;
begin
  Target := PlacementTarget;

  if (Target = nil) and (Parent <> nil) then
    Target := TvgVisualObject(Parent.Visual);
  if Target = nil then
  begin
    case Placement of
      vgPlacementBottom:
...
{            FPopupForm.Left := Round(FPlacementRectangle.Left);
  ...
  begin
    ...
    FPopupForm.Left := Pos.X;
    FPopupForm.Top := Pos.Y;
    l_CurrentMonitor := nil;
    if (FPopupForm.Monitor<>nil) and (FPopupForm.Monitor.Left <> 0) then
      THackCustomForm(FPopupForm).DefaultMonitor := dmDesktop;
    for m := 0 to Screen.MonitorCount - 1 do
      if PtInRect(Screen.Monitors[m].BoundsRect, Pos) then
      begin
        l_CurrentMonitor := Screen.Monitors[m];
        Break;
      end;
    if l_CurrentMonitor <> nil then
    begin
      if FPopupForm.Left + FPopupForm.Width > l_CurrentMonitor.Left + l_CurrentMonitor.Width then
        FPopupForm.Left := l_CurrentMonitor.Left + l_CurrentMonitor.Width - FPopupForm.Width;
      if FPopupForm.Top + FPopupForm.Height > l_CurrentMonitor.Top + l_CurrentMonitor.Height then
        FPopupForm.Top := l_CurrentMonitor.Top + l_CurrentMonitor.Height - FPopupForm.Height;
    end;
    {$ELSE}
  ...
  begin
    if (Parent <> nil) and (Parent.IsVisual) then
      AbsolutePos := TvgVisualObject(Parent.Visual).AbsoluteToLocal(AbsolutePos);
  ...
  case PlacementScene of
    vgPlacementSceneNew:
      begin
        ...
        { show }
        FPopupForm.Show;
        MoveNonVGLayoutControls(FPopupScene);
...
//        TvgForm(FPopupScene).ShowActivated := false;
  ...
  if g_PopupList = nil then
    g_PopupList := TvgObjectList.Create;
  if not StaysOpen then
    g_PopupList.Add(Self);
  { trigger }
  FIsOpen := true;
...
begin
  ...
  // - это не поможет т.к. Esc не доходит из-за VCM
  begin
    IsOpen := false;
    f_JustClosed := 0;
    Key := 0;
...
procedure TvgPopup.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: single);
begin
 inherited;
end;
...
procedure TvgPopup.MoveNonVGLayoutControls(aScene: TvgCustomScene);
 procedure lp_CheckChildren(aVGObject: TvgObject);
 var
  i: Integer;
 begin
  if aVGObject is TvgNonVGLayout then
   with TvgNonVGLayout(aVGObject) do
    if NonVGControl <> nil then
     NonVGControl.Parent := aScene;
  for i := 0 to aVGObject.ChildrenCount-1 do
   lp_CheckChildren(aVGObject.Children[i]);
 end;
begin
 lp_CheckChildren(Self);
end;
...
procedure TvgPopupItem.Cleanup;
begin
  inherited;
end;
...
begin
  ...
  try
   ...
   for i := 0 to List.Count - 1 do
     if (List[i] is TvgPopupItem) then
...
begin
  ...
  begin
    ...
    begin
      GetCursorPos(P);
      with TvgCustomScene(FScene.GetComponent).ScreenToClient(P) do
      begin
        if FPopup.PointInObject(x, y) then Exit;
...
procedure TvgMessagePopup.Cleanup;
begin
  inherited;
end;
...
begin
  ...
  begin
    SaveEnabled := TvgVisualObject(TvgObject(Scene.GetRoot).Visual).Enabled;
    TvgVisualObject(TvgObject(Scene.GetRoot).Visual).Enabled := false;
  end;
  Result := inherited PopupModal;
  if (Scene <> nil) and (Scene.GetRoot <> nil) and FDisableScene then
  begin
    TvgVisualObject(TvgObject(Scene.GetRoot).Visual).Enabled := SaveEnabled;
  end;
end;
...
begin
  ...
  begin
    SaveEnabled := TvgVisualObject(TvgObject(AOwner.GetRoot).Visual).Enabled;
    TvgVisualObject(TvgObject(AOwner.GetRoot).Visual).Enabled := false;
  end;
  try
    ...
    if (AStyle = nil) and (AOwner <> nil) and (AOwner.GetStyle <> nil) then
      PopupCtrl.Style := TvgResources(AOwner.GetStyle);
    ...
    FreeAndNil(PopupCtrl);
  finally
    if (AOwner <> nil) and (AOwner.GetRoot <> nil) and ADisableScene  then
      TvgVisualObject(TvgObject(AOwner.GetRoot).Visual).Enabled := SaveEnabled;
  end;
end;
...
function vgDoTab(aControl : TControl; aShift : Boolean) : Boolean;
var
 l_Index : Integer;
begin
 Result := false;
 if (SceneList <> nil) then
  for l_Index := 0 to Pred(SceneList.Count) do
   if SceneList[l_Index].DoTab(aControl, aShift) then
   begin
    Result := true;
    Exit;
   end;//SceneList[l_Index].DoTab(aControl, aShift)
...
procedure _afwFocusHook(aControl : TWinControl; aGot: Boolean);
var
 l_Index : Integer;
begin
 if (SceneList <> nil) then
  for l_Index := 0 to Pred(SceneList.Count) do
   SceneList[l_Index].FocusHook(aControl, aGot);
end;

procedure _afwControlMessageHook(aControl : TWinControl; const aMessage : TMessage);
var
 l_Index : Integer;
begin
 if (SceneList <> nil) then
  for l_Index := 0 to Pred(SceneList.Count) do
   SceneList[l_Index].ControlMessageHook(aControl, aMessage);
end;
...
initialization
  Assert(not Assigned(afwFacade.g_FocusHook));
  afwFacade.g_FocusHook := _afwFocusHook;
  Assert(not Assigned(afwFacade.g_MessageHook));
  afwFacade.g_MessageHook := _afwControlMessageHook;
  Assert(not Assigned(afwFacade.g_DoTabHook));
  afwFacade.g_DoTabHook := vgDoTab;
  Assert(not Assigned(afwFacade.g_IsAcceptableControlForTabNavigationHook));
  afwFacade.g_IsAcceptableControlForTabNavigationHook := vgIsAcceptableControlForTabNavigation;
  {$IFDEF DARWIN}
