...
procedure TvgCustomTextBox.Cleanup;
begin
...
procedure TvgCustomTextBox.DoContentPaint(Sender: TObject; Canvas: TvgCustomCanvas;
  const ARect: TvgRect);
var
  i: integer;
  R: TvgRect;
...
begin
  ...
  SaveIdx := TvgCanvas(Canvas).SaveCanvas;
  TvgCanvas(Canvas).IntersectClipRect(ARect);

  TvgCanvas(Canvas).Font.Assign(Font);
  TvgCanvas(Canvas).Fill.Assign(FFontFill);
  if FPassword then
  begin
    R := ARect;
  ...
  begin
    ...
    TvgCanvas(Canvas).FillText(R, R, Copy(Text, FFirstVisibleChar, Length(Text) - FFirstVisibleChar + 1),
  ...
  begin
    ...
    if SelLength > 0 then
    begin
      TvgCanvas(Canvas).Fill.Assign(FSelectionFill);
      R := GetSelRect;
      with ContentRect do
  ...
  TvgCanvas(Canvas).RestoreCanvas(SaveIdx);
end;
...
begin
  ...
  begin
    ...
    TvgCanvas(Canvas).MeasureText(R, R, Str, false, vgTextAlignNear, vgTextAlignCenter);
...
begin
  ...
  if Text = '' then
    T := 'a';
  TvgCanvas(Canvas).MeasureText(R, R, T, false, vgTextAlignNear, vgTextAlignCenter);
  ...
  begin
    ...
    begin
      R := ContentRect;
      TvgCanvas(Canvas).MeasureText(R, R, Copy(Text, FFirstVisibleChar, a - FFirstVisibleChar + 1), false, vgTextAlignNear, vgTextAlignCenter);
      Result := R.Right;
    end
    else
    begin
...
procedure TvgNumberBox.Cleanup;
begin
  inherited;
end;
...
begin
  ...
  begin
    ...
    TvgCanvas(Canvas).Fill.SolidColor := $AA505050;
    R := LocalRect;
    if FPressedVert then
    begin
      vgInflateRect(R, -1, -1);
      R.Left := R.Right - 5;
      TvgCanvas(Canvas).FillRect(R, 1, 1, AllCorners, AbsoluteOpacity);
      vgInflateRect(R, -1, -1);
{      if FPressedInc > 0 then
      begin
        TvgCanvas(Canvas).Fill.SolidColor := $AA202020;
        R.Top := (vgRectHeight(R) / 2);
        R.Bottom := R.Top + (Height / 2.1);
        TvgCanvas(Canvas).FillRect(R, 1, 1, AbsoluteOpacity);
      end;
      if FPressedInc < 0 then
      begin
        TvgCanvas(Canvas).Fill.SolidColor := $AA202020;
...
begin
  ...
  begin
    ...
    if (frac(FValue) = 0) or (FValueType = vgValueInteger) then
      ...
      FText := Format('%' + {$IfDef XE}FormatSettings.{$EndIf}DecimalSeparator + IntToStr(FDecimalDigits) + 'f', [FValue]);
    SelLength := 0;
    Repaint;
  end;
end;
...
procedure TvgComboTextBox.Cleanup;
begin
  FreeAndNil(FItems);
  inherited;
end;
...
procedure TvgComboTrackBar.Cleanup;
begin
  inherited;
end;
...
procedure TvgTextBoxClearBtn.Cleanup;
begin
  inherited;
end;
...
initialization
  RegisterVGObjects('Text Edits', [TvgTextBox, TvgRoundTextBox, TvgNumberBox, TvgSpinBox, TvgComboTextBox,
                                   {$IfNDef Nemesis}
                                   TvgComboTrackBar,
                                   {$EndIf  Nemesis}
                                   TvgTextBoxClearBtn]);
  RegisterVGObjects('HUD', [TvgHudTextBox, TvgHudNumberBox, TvgHudRoundTextBox, TvgHudSpinBox, TvgHudComboTextBox
                            {$IfNDef Nemesis}
                            , TvgHudComboTrackBar
                            {$EndIf  Nemesis}
                           ]);
