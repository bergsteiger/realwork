...
{End !!.52}
  ...
  TXpBaseCharFilter = class(Tl3Base)
    protected
      FBufSize : Longint;
      FBuffer  : PAnsiChar;
      FBufPos  : Longint;
    ...
    public
      constructor Create(aStream : TStream; const aBufSize : Longint); reintroduce; virtual;
...
{End !!.52}
  ...
  TXpInCharFilter = class(TXpBaseCharFilter)
    ...
    protected
      ...
      function csGetWin1251Char : TXpUcs4Char;
...
{--------}
procedure TXpBaseCharFilter.Cleanup;
begin
...
{Begin !!.52}
{--------}
procedure TXpInCharFilter.Cleanup;
begin
  FreeAndNil(FDirContext);
  inherited;
end;
{End !!.52}
{--------}
  ...
  {otherwise get a character from the buffer; this depends on the
   format of the stream of course}
    ...
    case Format of
      ...
      sfWindows : aCh := csGetWin1251Char;
    else
...
function TXpInCharFilter.csGetWin1251Char : TXpUcs4Char;
var
 l_C  : AnsiChar;
 l_WC : WideChar;
begin
  ...
  {get the first byte character from the buffer}
  l_C := FBuffer[FBufPos];
  MultiByteToWideChar(CodePage, 0, @l_C, 1, @l_WC, 1);
  Result := Integer(l_WC);
  //XPIso88591ToUcs4(FBuffer[FBufPos], Result);
  FBufPos := FBufPos + 1;                                              {!!.55}
end;
...
begin
  ...
  begin                                  {!!.51}
   ...
   // переносим "хвостик" в начало буфера и дочитываем разницу из потока
   // сделано, чтобы избежать произвольного Seek по потоку (который может этого не поддерживать)
   l_Tail := FBufEnd - FBufPos;
   l3Move((FBuffer + FBufPos)^, FBuffer^, l_Tail);
   FBufEnd := FStream.Read((FBuffer + l_Tail)^, FBufSize - l_Tail) + l_Tail;
  end
  else
  begin
...
begin
  ...
  csGetNextBuffer;
  if FBufSize > 2 then
    if (FBuffer[0] = #$FE) and (FBuffer[1] = #$FF) then
    begin
      FFormat := sfUTF16BE;
      FBufPos := 2;
    end
    ...
    if (FBuffer[0] = #$3C) and // "<" - опознаем XML по началу тега
       (FBuffer[1] = #$00) then
     FFormat := sfUTF16LE  // UTF16 Little Endian без BOM
    else
      FFormat := sfUTF8
  else
    FFormat := sfUTF8;
...
{--------}
procedure TXpOutCharFilter.Cleanup;
begin
