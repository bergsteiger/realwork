...
{== Event types ======================================================}
  ...
  TXpValueEvent = procedure(oOwner : TObject;
                            const sValue : DOMString) of object;
...
{== TXpAttributeInfo =================================================}
  TXpAttributeInfo = class(Tl3Base)
  private
    FType      : Integer;
    FValue     : DOMString;
    FValueType : Integer;
...
{--------}
procedure TXpElementInfo.Cleanup;
var
  i : Integer;
begin
...
begin
  Cleanup;
{Begin !!.51}
  FreeAndNil(FTagAttributes);
  FreeAndNil(FNotationInfo);
  FreeAndNil(FEntityInfo);
  FreeAndNil(FElementInfo);
  FreeAndNil(FAttributeType);
  FreeAndNil(FErrors);
...
begin
  ...
  if (NewStrLen > 0) then begin
    ...
    l3Move(sVal[1], FDataBuffer[FDataBufferPos + 1], NewStrLen * 2);
    ...
    {$IFDEF XpUseInet}
      xpdcHTTP :
        begin
          ...
          oFileStream := XpLoadHTTP(oContext.Dir + oContext.Filename, oErrors);
...
//        oStream := FFilter.Stream;                                   {!!.52}{!!.53}
          FreeAndNil(FFilter);                                                {!!.52}
//        FreeAndNil(oStream);                                                {!!.52}{!!.53}
        end;                                                           {!!.52}
        PopDocument;                                                   {!!.52}
      end;                                                             {!!.52}
      raise;                                                         
  ...
  {if IsWhiteSpace(ReadChar(False)) then begin}                        {!!.51}
   { TempChar := ReadChar(True);}                                      {!!.51}
    if TryRead(Xpc_Encoding) then begin
      ...
      if AnsiStartsText(cWin, sValue) then
      begin
       l_S := sValue;
       Delete(l_S, 1, Length(cWin));
       FFilter.Format := sfWindows;
       try
        FFilter.CodePage := StrToInt(l_S);
       except
        on EConvertError do
         FFilter.CodePage := 1251;
       end;//try..except
      end;//AnsiStartsText..
  {end else}                                                           {!!.51}
