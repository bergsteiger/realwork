...
type
  {$IFDEF ADX_VCL5}
  TadxCommandBarEditor = class(TForm, IUnknown, IDesignNotification, IDesignWindowActions, IDesignEditQuery)
  {$ELSE}
  TadxCommandBarEditor = class(TForm, IUnknown, IDesignNotification, IDesignWindow, IEditHandler, IActivatable)
  {$ENDIF}
  ...
  private
    ...
    {$IFDEF ADX_DESIGNER}
    FCmdBarDesigner: ADXDesign_TLB.TAdxCommandBar;
...
procedure ShowCommandBarEditor(ADesigner: IDesigner; AComponent: TComponent;
...
var
  CommandBarEditorsList: TList = nil;
...
var
  i: Integer;
  frm: TadxCommandBarEditor;
begin
  if CommandBarEditorsList = nil then CommandBarEditorsList := TList.Create;
  for i := 0 to CommandBarEditorsList.Count - 1 do begin
    frm := TadxCommandBarEditor(CommandBarEditorsList[i]);
    with frm do
  ...
  frm := TadxCommandBarEditor.Create(Application);
  with frm do
  try
    {$IFDEF ADX_VCL5}
...
constructor TadxCommandBarEditor.Create(AOwner: TComponent);
begin
  ...
  try
    ...
    FCmdBarDesigner := ADXDesign_TLB.TAdxCommandBar.Create(nil);
    ...
    FCmdBarDesigner.CreateCommandBar(panDesign.Handle);
...
destructor TadxCommandBarEditor.Destroy;
begin
  {$IFNDEF ADX_VCL5}
  ...
  if Assigned(FCmdBarDesigner) then begin
    ...
    FCmdBarDesigner.DestroyCommandBar();
...
procedure TadxCommandBarEditor.DoControlSelect(ASender: TObject; const ControlID: WideString);
...
procedure TadxCommandBarEditor.WndProc(var Message: TMessage);
begin
...
function TadxCommandBarEditor.GetEditState: TEditState;
begin
  Result := [];
end;

{$IFDEF ADX_VCL5}
procedure TadxCommandBarEditor.EditAction(Action: TEditAction);
begin
end;
{$ELSE}
function TadxCommandBarEditor.EditAction(Action: TEditAction): Boolean;
begin
  Result := False;
end;
{$ENDIF}

procedure TadxCommandBarEditor.WindowHide;
begin
  if Visible then
    ShowWindow(Handle, SW_HIDE);
end;

procedure TadxCommandBarEditor.WindowShow;
const
...
{$IFDEF ADX_VCL5}
procedure TadxCommandBarEditor.ItemDeleted(const AItem: IPersistent);
...
procedure TadxCommandBarEditor.ItemInserted(const AItem: IPersistent);
begin
end;

procedure TadxCommandBarEditor.SelectionChanged(const ASelection: IDesignerSelections);
begin
end;

procedure TadxCommandBarEditor.DesignerInitialized(const ADesigner: IUnknown);
begin
end;

procedure TadxCommandBarEditor.DesignerClosed(const ADesigner: IUnknown);
begin
  if Designer = ADesigner then
    CloseEditor;
end;

procedure TadxCommandBarEditor.ItemsModified(const ADesigner: IUnknown);
begin
...
{$ELSE}
procedure TadxCommandBarEditor.ItemDeleted(const ADesigner: IDesigner; Item: TPersistent);
...
procedure TadxCommandBarEditor.ItemInserted(const ADesigner: IDesigner; Item: TPersistent);
begin
end;

procedure TadxCommandBarEditor.SelectionChanged(const ADesigner: IDesigner; const ASelection: IDesignerSelections);
begin
end;

procedure TadxCommandBarEditor.DesignerOpened(const Designer: IDesigner; AResurrecting: Boolean);
begin
end;

procedure TadxCommandBarEditor.DesignerClosed(const ADesigner: IDesigner; AGoingDormant: Boolean);
begin
  if Designer = ADesigner then
    CloseEditor;
end;

procedure TadxCommandBarEditor.ItemsModified(const Designer: IDesigner);
begin
  if FClosing then Exit;
  if Collection <> nil then begin
...
procedure TadxCommandBarEditor.Activated;
{$IFDEF ADX_VCL5}
var
  Msg: TMessage;
{$ENDIF}
...
procedure TadxCommandBarEditor.WMActivate(var Msg: TWMActivate);
begin
  inherited;
...
procedure TadxCommandBarEditor.CloseEditor;
begin
  FClosing := True;
  Collection := nil;
  Component := nil;
...
procedure TadxCommandBarEditor.UpdateProps(const Ctrl: TadxCommandBarControl; ANode: TTreeNode);
var
  id: string;
  ...
  if panDesign.Visible and Assigned(FCmdBarDesigner) then begin
    if FCmdBarIDs.IndexOfObject(Ctrl) >= 0 then begin
      ...
      if id <> Ctrl.OfficeTag then begin
        FCmdBarDesigner.ToolBar.UpdateProp(id, tp_ID, Ctrl.OfficeTag); // tp_ID
        FCmdBarIDs[FCmdBarIDs.IndexOfObject(Ctrl)] := Ctrl.OfficeTag;
      end;
    end;
    FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_BeginGroup, Ctrl.BeginGroup);
    FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_Caption, Ctrl.Caption);
    FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_Enabled, Ctrl.Enabled);
    if not (Ctrl is TadxCustomCommandBarComboBox) then
      FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_Height, Ctrl.Height);
    FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_ToolTip, Ctrl.TooltipText);
    FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_Visible, Ctrl.Visible);
    FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_Width, Ctrl.Width);

    if (Ctrl is TadxCommandBarButton) then begin
      FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_Pressed, Ctrl.AsButton.State);
      FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_Style, Ctrl.AsButton.Style);
      IPict := nil;
      ...
      FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_Glyph, IPict);
    end;

    if (Ctrl is TadxCustomCommandBarComboBox) then begin
      FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_Style, TadxCustomCommandBarComboBoxAccess(Ctrl).Style);
      FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_Text, TadxCustomCommandBarComboBoxAccess(Ctrl).Text);
    end;
    if (Ctrl is TadxCommandBarDropdownList) or (Ctrl is TadxCommandBarComboBox) then begin
      FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_Items, TadxCustomCommandBarComboBoxAccess(Ctrl).Items.Text);
      FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_SelectedItemIndex, TadxCustomCommandBarComboBoxAccess(Ctrl).ListIndex);
      FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_DropDownLines, TadxCustomCommandBarComboBoxAccess(Ctrl).DropdownLines);
      FCmdBarDesigner.ToolBar.UpdateProp(Ctrl.OfficeTag, tp_DropDownWidth, TadxCustomCommandBarComboBoxAccess(Ctrl).DropdownWidth);
    end;
  end;
end;
{$ENDIF}
...
procedure TadxCommandBarEditor.GetSelection;
{$IFDEF ADX_DESIGNER}
var
...
begin
  if Tree.Selected <> nil then begin
    ...
    if Tree.Selected.Data = nil then begin
      (*
      if Component is adxAddin.TadxCommandBar then
        Tree.Selected.Text := adxAddin.TadxCommandBar(Component).CommandBarName + ' (' + adxAddin.TadxCommandBar(Component).ClassName + ')';
      *)
    ...
    // collectionitem
      ...
      if panDesign.Visible and Assigned(FCmdBarDesigner) then begin
        ...
        FCmdBarDesigner.ToolBar.BeginUpdate(Ctrl.OfficeTag);
        UpdateProps(Ctrl, Tree.Selected);
        FCmdBarDesigner.ToolBar.EndUpdate(Ctrl.OfficeTag);
      end;
      {$ENDIF}
    end;
  end;
...
procedure TadxCommandBarEditor.SetSelection;
var
  {$IFDEF ADX_VCL5}
  ...
  try
    ...
    begin
      if Tree.Selected.Data = nil then begin
        if Designer <> nil then begin
          ...
          if panDesign.Visible then
            FCmdBarDesigner.ToolBar.SelectControl('ROOT');
        end;
        Exit;
      end;
      {$IFDEF ADX_VCL5}
      ...
      if Designer <> nil then begin
        ...
        if panDesign.Visible then
          FCmdBarDesigner.ToolBar.SelectControl(TadxCommandBarControl(Tree.Selected.Data).OfficeTag);
      end;
    end
    else
...
procedure TadxCommandBarEditor.SetCollectionPropertyName(const Value: string);
const
...
function TadxCommandBarEditor.GetRegKey: string;
begin
  {$IFDEF ADX_VCL5}
...
procedure TadxCommandBarEditor.FormCreate(Sender: TObject);
begin
  CommandBarEditorsList.Add(Self);
...
procedure TadxCommandBarEditor.FormDestroy(Sender: TObject);
begin
  if CommandBarEditorsList <> nil then
    CommandBarEditorsList.Remove(Self);
end;

procedure TadxCommandBarEditor.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TadxCommandBarEditor.BuildControlsList;
  ...
  begin
    for i := 0 to ACtrls.Count - 1 do begin
      ...
      {$IFDEF ADX_DESIGNER}
      if panDesign.Visible and Assigned(FCmdBarDesigner) then begin
        FCmdBarDesigner.ToolBar.AddControl(
          GetType(ACtrls[i]),
...
var
  ...
  Obj: TPersistent;
begin
  if (Component is TadxMainMenu) or
    (Component is TadxOLExplorerMainMenu) or
    (Component is TadxOLInspectorMainMenu) then
  begin
  ...
  try
    {$IFDEF ADX_DESIGNER}
    if Assigned(FCmdBarDesigner) then FCmdBarDesigner.ToolBar.BeginUpdate('');
    {$ENDIF}
    (*
    Root := Tree.Items.AddChildFirst(nil, adxAddin.TadxCommandBar(TOwnedCollectionAccess(TadxCommandBarControls(Collection)).GetOwner).CommandBarName + ' (' + adxAddin.TadxCommandBar(TOwnedCollectionAccess(TadxCommandBarControls(Collection)).GetOwner).ClassName + ')');
    *)
    ...
    if panDesign.Visible and Assigned(FCmdBarDesigner) then begin
      ...
      if (Obj is TadxMainMenu) or (Obj is TadxOlExplorerMainMenu) or (Obj is TadxOlInspectorMainMenu) then
        FCmdBarDesigner.ToolBar.AddControl(tc_MainMenu, '', 'ROOT', '')
      else
        FCmdBarDesigner.ToolBar.AddControl(tc_Group, '', 'ROOT', '');
      (*
      FCmdBarIDs.AddObject('ROOT', adxAddin.TadxCommandBar(TOwnedCollectionAccess(TadxCommandBarControls(Collection)).GetOwner));
      *)
    ...
    {$IFDEF ADX_DESIGNER}
    if Assigned(FCmdBarDesigner) then FCmdBarDesigner.ToolBar.EndUpdate('');
    {$ENDIF}
    Tree.Items.EndUpdate;
  end;
end;

procedure TadxCommandBarEditor.CommonClick(Sender: TObject);
  ...
  case (Sender as TComponent).Tag of
    ...
    110: if MessageDlg('Are you sure you want to delete the selected item?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
           ...
           try
            {$IFDEF ADX_DESIGNER}
            if panDesign.Visible and Assigned(FCmdBarDesigner) then begin
              FCmdBarDesigner.ToolBar.BeginUpdate('');
              FCmdBarDesigner.ToolBar.DeleteControl(TadxCommandBarControl(Tree.Selected.Data).OfficeTag);
              FCmdBarDesigner.ToolBar.EndUpdate('');
            end;
            {$ENDIF}
    ...
    120: begin
           ...
           try
             ...
             if panDesign.Visible and Assigned(FCmdBarDesigner) then begin
               (*
               if TOwnedCollectionAccess(TadxCommandBarControl(Tree.Selected.Data).Collection).GetOwner is adxAddin.TadxCommandBar then
               *)
               ...
               FCmdBarDesigner.ToolBar.BeginUpdate(ParentID);
               FCmdBarDesigner.ToolBar.SetParent(ParentID,
                 TadxCommandBarControl(Tree.Selected.Data).OfficeTag,
                 TadxCommandBarControl(Tree.Selected.Data).Index);
               FCmdBarDesigner.ToolBar.EndUpdate(ParentID);
             end;
             {$ENDIF}
             Tree.Items.BeginUpdate;
             try
    ...
    // add a control
      ...
      try
        ...
        if panDesign.Visible and Assigned(FCmdBarDesigner) then begin
          (*
          if TOwnedCollectionAccess(Item.Collection).GetOwner is adxAddin.TadxCommandBar then
          *)
          ...
          FCmdBarDesigner.ToolBar.BeginUpdate(ParentID);
          FCmdBarDesigner.ToolBar.AddControl(GetType(Item), ParentID, Item.OfficeTag, Item.Caption);
...
procedure TadxCommandBarEditor.TreeEditing(Sender: TObject; Node: TTreeNode;
  var AllowEdit: Boolean);
begin
  AllowEdit := False;
end;

procedure TadxCommandBarEditor.TreeChange(Sender: TObject; Node: TTreeNode);
begin
  SetSelection;
  UpdateToolBarAndMenu(Node);
end;

procedure TadxCommandBarEditor.UpdateToolBarAndMenu(Node: TTreeNode);
...
procedure TadxCommandBarEditor.TreeContextPopup(Sender: TObject; MousePos: TPoint; var Handled: Boolean);
var
  Node: TTreeNode;
begin
...
procedure TadxCommandBarEditor.TreeDragOver(Sender, Source: TObject; X,
...
begin
  ...
  if Tree.DropTarget.Data = nil then
    ...
    (*
    if DstParent is adxAddin.TadxCommandBar then
    *)
...
procedure TadxCommandBarEditor.TreeDragDrop(Sender, Source: TObject; X, Y: Integer);
...
begin
  if Assigned(Tree.DropTarget) and Assigned(Tree.Selected) then begin
    ...
    if Tree.DropTarget.Data = nil then begin
      (*
      if Component is adxAddin.TadxCommandBar then begin
      *)
      if Component is adxAddin.TadxCustomCommandBar then begin
        DstCtrl := nil;
        DstType := adxCommandBar;
        (*
        DstCollection := adxAddin.TadxCommandbar(Component).Controls;
        *)
    ...
    if DstCtrl = nil then begin
      ...
      try
        ...
        {$IFDEF ADX_DESIGNER}
        if panDesign.Visible and Assigned(FCmdBarDesigner) then begin
          FCmdBarDesigner.ToolBar.BeginUpdate('ROOT');
          FCmdBarDesigner.ToolBar.SetParent('ROOT', SrcCtrl.OfficeTag, SrcCtrl.Index);
          FCmdBarDesigner.ToolBar.EndUpdate('ROOT');
        end;
        {$ENDIF}
        Designer.Modified;
        SetSelection;
      ...
      {$IFDEF ADX_DESIGNER}
      if panDesign.Visible and Assigned(FCmdBarDesigner) then begin
        FCmdBarDesigner.ToolBar.BeginUpdate(DstCtrl.OfficeTag);
        if isNodeDown then
          FCmdBarDesigner.ToolBar.SetParent(DstCtrl.OfficeTag, SrcCtrl.OfficeTag, SrcCtrl.Index)
        else begin
          (*
          if TOwnedCollectionAccess(DstCtrl.Collection).GetOwner is adxAddin.TadxCommandBar then
          *)
          if TOwnedCollectionAccess(DstCtrl.Collection).GetOwner is adxAddin.TadxCustomCommandBar then
            FCmdBarDesigner.ToolBar.SetParent('ROOT', SrcCtrl.OfficeTag, SrcCtrl.Index)
          else
            FCmdBarDesigner.ToolBar.SetParent(
              TadxCommandBarControl(TOwnedCollectionAccess(DstCtrl.Collection).GetOwner).OfficeTag,
              SrcCtrl.OfficeTag, SrcCtrl.Index);
        end;
        FCmdBarDesigner.ToolBar.EndUpdate(DstCtrl.OfficeTag);
      end;
      {$ENDIF}
...
procedure TadxCommandBarEditor.TreeKeyPress(Sender: TObject; var Key: Char);
begin
  {$IFDEF UNICODE}
...
procedure TadxCommandBarEditor.cbOfficeChange(Sender: TObject);
begin
  {$IFDEF ADX_DESIGNER}
  if panDesign.Visible and Assigned(FCmdBarDesigner) then 
    case cbOffice.ItemIndex of
      0: FCmdBarDesigner.ToolBar.UpdateProp('', tp_Style, 4);
      1: FCmdBarDesigner.ToolBar.UpdateProp('', tp_Style, 2);
      2: FCmdBarDesigner.ToolBar.UpdateProp('', tp_Style, 1);
    end;
  {$ENDIF}
end;
...
finalization
  if Assigned(CommandBarEditorsList) then begin
    CommandBarEditorsList.Free;
    CommandBarEditorsList := nil;
  end;
