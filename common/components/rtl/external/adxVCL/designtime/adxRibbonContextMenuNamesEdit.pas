...
unit adxRibbonContextMenuNamesEdit;
...
type
  ...
  TadxRibbonContextMenuNamesEditorGrid = class(TStringGrid)
  private
    FContextMenuNames: TStrings;
    FEdit: TInplaceEdit;
  ...
  TadxRibbonContextMenuNamesEditor = class(TForm)
    Panel1: TPanel;
    Panel2: TPanel;
    btnOK: TButton;
    btnCancel: TButton;
  ...
  private
    FGrid: TadxRibbonContextMenuNamesEditorGrid;
    FNames: TStringList;
    FSourceList: TStringList;
    FRegKey: string;
  public
    constructor Create(Source: TStringList; Names: TStringList; RegKey: string); reintroduce;
    destructor Destroy; override;
  end;

var
  adxRibbonContextMenuNamesEditor: TadxRibbonContextMenuNamesEditor;
...
{ TadxRibbonContextMenuNamesEditor }

constructor TadxRibbonContextMenuNamesEditor.Create(Source: TStringList; Names: TStringList; RegKey: string);
begin
...
destructor TadxRibbonContextMenuNamesEditor.Destroy;
begin
  FSourceList := nil;
  if Assigned(FNames) then FNames.Free;
  inherited Destroy;
end;

procedure TadxRibbonContextMenuNamesEditor.FormCreate(Sender: TObject);
var
  i: Integer;
begin
  FGrid := TadxRibbonContextMenuNamesEditorGrid.Create(Self, FNames);
  FGrid.Parent := Panel2;
  ...
  for i := 0 to FSourceList.Count - 1 do
    FGrid.Cells[0, i + 1] := FSourceList[i];
  with TRegIniFile.Create(FRegKey) do
    try
      Left := ReadInteger(Self.ClassName, 'Left', Left);
      Top := ReadInteger(Self.ClassName, 'Top', Top);
      FGrid.ColWidths[0] := ReadInteger(Self.ClassName, 'Column0', 350);
    finally
      Free;
    end;
end;

procedure TadxRibbonContextMenuNamesEditor.FormClose(Sender: TObject;
...
begin
  with TRegIniFile.Create(FRegKey) do
    try
      ...
      WriteInteger(Self.ClassName, 'Column0', FGrid.ColWidths[0]);
    finally
      Free;
    end;
end;

procedure TadxRibbonContextMenuNamesEditor.btnOKClick(Sender: TObject);
var
  i: Integer;
begin
  FSourceList.Clear;
  for i := 1 to FGrid.RowCount - 1 do
    if FGrid.Cells[0, i] <> '' then
      FSourceList.Add(FGrid.Cells[0, i]);
end;
  ...
  if not ListVisible then begin
    ...
    if ActiveList = FPickList then begin
      ...
      if PickList.Items.Count = 0 then
        PickList.Height := TPopupListbox(PickList).ItemHeight + 4;
      if Text = '' then
...
begin
  ...
  FEditStyle := TadxRibbonContextMenuNamesEditorGrid(Grid).GetEditStyle(TAccessCustomGrid(Grid).Col, TAccessCustomGrid(Grid).Row);
...
{ TadxRibbonContextMenuNamesEditorGrid }

constructor TadxRibbonContextMenuNamesEditorGrid.Create(AOwner: TComponent; const AStrings: TStrings);
begin
  inherited Create(AOwner);
  FContextMenuNames := TStringList.Create;
  FContextMenuNames.AddStrings(AStrings);
  ColCount := 1;
  ...
  Cells[0, 0] := 'Context Menu Names';
end;

destructor TadxRibbonContextMenuNamesEditorGrid.Destroy;
begin
  if Assigned(FContextMenuNames) then FreeAndNil(FContextMenuNames);
  inherited Destroy;
end;

function TadxRibbonContextMenuNamesEditorGrid.CreateEditor: TInplaceEdit;
begin
...
procedure TadxRibbonContextMenuNamesEditorGrid.EditListGetItems(ACol, ARow: Integer; Items: TStrings);
begin
  Items.Clear;
  Items.AddStrings(FContextMenuNames);
end;

function TadxRibbonContextMenuNamesEditorGrid.GetEditStyle(ACol, ARow: Integer): TEditStyle;
begin
  Result := esPickList;
end;

procedure TadxRibbonContextMenuNamesEditorGrid.KeyDown(var Key: Word; Shift: TShiftState);
...
begin
  case Key of
    ...
    VK_INSERT:
      if (Shift = []) and (Cells[0, RowCount - 1] <> '') then begin
        RowCount := RowCount + 1;
        Row := RowCount - 1;
        Col := 0;
        Key := 0;
...
procedure TadxRibbonContextMenuNamesEditorGrid.DeleteRow(ARow: Integer);
begin
...
function TadxRibbonContextMenuNamesEditorGrid.IsEmptyRow: Boolean;
begin
  Result := Cells[0, Row] = '';
end;

function TadxRibbonContextMenuNamesEditorGrid.RestoreCurrentRow: Boolean;
