...
procedure StartIGDIPlus(var gdiplusToken: ULONG);
procedure StopIGDIPlus(gdiplusToken: ULONG);
...
type
  ...
  public
    ...
    function IsVisible(const point: TGPPoint; g: TGPGraphics = nil)
      : BOOL; overload;
    function IsVisible(x, y: Single; g: TGPGraphics = nil): BOOL; overload;
    function IsVisible(const point: TGPPointF; g: TGPGraphics = nil)
      : BOOL; overload;
    function IsVisible(x, y, width, height: Integer; g: TGPGraphics)
      : BOOL; overload;
    function IsVisible(const rect: TGPRect; g: TGPGraphics = nil)
      : BOOL; overload;
    function IsVisible(x, y, width, height: Single; g: TGPGraphics = nil)
      : BOOL; overload;
    function IsVisible(const rect: TGPRectF; g: TGPGraphics = nil)
      : BOOL; overload;
  ...
  protected
    ...
    lastResult: TStatus;
    function SetStatus(status: TStatus): TStatus;
    constructor Create(nativeOrig: GpFontFamily; status: TStatus);
  ...
  protected
    ...
    function SetStatus(status: TStatus): TStatus;
    constructor Create(nativeImage: GpImage; status: TStatus);
      reintroduce; overload;
  public
    ...
    function SaveAdd(newImage: TGPImage; encoderParams: PEncoderParameters)
      : TStatus; overload;
    ...
    function GetThumbnailImage(thumbWidth, thumbHeight: UINT;
      callback: GetThumbnailImageAbort = nil; callbackData: Pointer = nil)
      : TGPImage;
  ...
  public
    ...
    function Clone(x, y, width, height: Integer; format: TPixelFormat)
      : TGPBitmap; overload;
    function Clone(rect: TGPRectF; format: TPixelFormat): TGPBitmap; overload;
    function Clone(x, y, width, height: Single; format: TPixelFormat)
      : TGPBitmap; overload;
    ...
    function FromResource(hInstance: HMODULE; bitmapName: WideString)
      : TGPBitmap;
  ...
  public
    ...
    function SetGamma(gamma: Single;
      type_: TColorAdjustType = ColorAdjustTypeDefault): TStatus;
    function ClearGamma(type_
      : TColorAdjustType = ColorAdjustTypeDefault): TStatus;
    function SetNoOp(type_: TColorAdjustType = ColorAdjustTypeDefault): TStatus;
    function ClearNoOp(type_
      : TColorAdjustType = ColorAdjustTypeDefault): TStatus;
  ...
  public
    ...
    constructor Create; reintroduce; overload;
    constructor Create(m11, m12, m21, m22, dx, dy: Single);
      reintroduce; overload;
    ...
    function TransformPoints(pts: PGPPointF; count: Integer = 1)
      : TStatus; overload;
    function TransformPoints(pts: PGPPoint; count: Integer = 1)
      : TStatus; overload;

    function TransformVectors(pts: PGPPointF; count: Integer = 1)
      : TStatus; overload;
    function TransformVectors(pts: PGPPoint; count: Integer = 1)
      : TStatus; overload;
  ...
  public
    constructor Create(image: TGPImage; wrapMode: TWrapMode = WrapModeTile);
      reintroduce; overload;
    constructor Create(image: TGPImage; wrapMode: TWrapMode; dstRect: TGPRectF);
      reintroduce; overload;
  ...
  protected
    ...
    constructor Create(nativePen: GpPen; status: TStatus); reintroduce;
      overload;
  public
    constructor Create(color: TGPColor; width: Single = 1.0);
      reintroduce; overload;
    constructor Create(brush: TGPBrush; width: Single = 1.0);
      reintroduce; overload;
    destructor Destroy; override;
  ...
  public
    constructor Create(path: TGPGraphicsPath); reintroduce; overload;
    constructor Create(fillMode: TFillMode = FillModeAlternate);
      reintroduce; overload;
    ...
    function AddArc(rect: TGPRectF; startAngle, sweepAngle: Single)
      : TStatus; overload;
    function AddArc(x, y, width, height, startAngle, sweepAngle: Single)
      : TStatus; overload;
    function AddArc(rect: TGPRect; startAngle, sweepAngle: Single)
      : TStatus; overload;
    ...
    function AddBezier(x1, y1, x2, y2, x3, y3, x4, y4: Single)
      : TStatus; overload;
    ...
    function AddBezier(x1, y1, x2, y2, x3, y3, x4, y4: Integer)
      : TStatus; overload;
    ...
    function AddCurve(points: PGPPointF; count: Integer; tension: Single)
      : TStatus; overload;
    function AddCurve(points: PGPPointF;
      count, offset, numberOfSegments: Integer; tension: Single)
      : TStatus; overload;
    function AddCurve(points: PGPPoint; count: Integer): TStatus; overload;
    function AddCurve(points: PGPPoint; count: Integer; tension: Single)
      : TStatus; overload;
    function AddCurve(points: PGPPoint;
      count, offset, numberOfSegments: Integer; tension: Single)
      : TStatus; overload;

    function AddClosedCurve(points: PGPPointF; count: Integer)
      : TStatus; overload;
    function AddClosedCurve(points: PGPPointF; count: Integer; tension: Single)
      : TStatus; overload;
    function AddClosedCurve(points: PGPPoint; count: Integer): TStatus;
      overload;
    function AddClosedCurve(points: PGPPoint; count: Integer; tension: Single)
      : TStatus; overload;
    ...
    function AddPie(rect: TGPRectF; startAngle, sweepAngle: Single)
      : TStatus; overload;
    function AddPie(x, y, width, height, startAngle, sweepAngle: Single)
      : TStatus; overload;
    function AddPie(rect: TGPRect; startAngle, sweepAngle: Single)
      : TStatus; overload;
    ...
    function AddString(string_: WideString; length: Integer;
      family: TGPFontFamily; style: Integer; emSize: Single; origin: TGPPointF;
    ...
    function IsOutlineVisible(x, y: Single; pen: TGPPen; g: TGPGraphics = nil)
      : BOOL; overload;
    function IsOutlineVisible(point: TGPPoint; pen: TGPPen;
      g: TGPGraphics = nil): BOOL; overload;
    function IsOutlineVisible(x, y: Integer; pen: TGPPen; g: TGPGraphics = nil)
      : BOOL; overload;
  end;
  ...
  public
    ...
    destructor Destroy; override;
    function NextSubpath(out startIndex, endIndex: Integer; out isClosed: BOOL)
      : Integer; overload;
    function NextSubpath(path: TGPGraphicsPath; out isClosed: BOOL)
      : Integer; overload;
    ...
    procedure Rewind;
    function Enumerate(points: PGPPointF; types: PBYTE; count: Integer)
      : Integer;
  ...
  public
    ...
    function SetCompositingQuality(compositingQuality
      : TCompositingQuality): TStatus;
    ...
    function TransformPoints(destSpace: TCoordinateSpace;
      srcSpace: TCoordinateSpace; pts: PGPPointF; count: Integer)
      : TStatus; overload;
    function TransformPoints(destSpace: TCoordinateSpace;
      srcSpace: TCoordinateSpace; pts: PGPPoint; count: Integer)
      : TStatus; overload;
    // ------------------------------------------------------------------------
    ...
    function DrawLine(pen: TGPPen; const pt1, pt2: TGPPointF): TStatus;
      overload;
    function DrawLines(pen: TGPPen; points: PGPPointF; count: Integer)
      : TStatus; overload;
    ...
    function DrawLines(pen: TGPPen; points: PGPPoint; count: Integer)
      : TStatus; overload;
    ...
    function DrawBezier(pen: TGPPen; const pt1, pt2, pt3, pt4: TGPPointF)
      : TStatus; overload;
    function DrawBeziers(pen: TGPPen; points: PGPPointF; count: Integer)
      : TStatus; overload;
    ...
    function DrawBezier(pen: TGPPen; const pt1, pt2, pt3, pt4: TGPPoint)
      : TStatus; overload;
    function DrawBeziers(pen: TGPPen; points: PGPPoint; count: Integer)
      : TStatus; overload;

    // DrawRectangle(s)
    function DrawRectangle(pen: TGPPen; const rect: TGPRectF): TStatus;
      overload;
    function DrawRectangle(pen: TGPPen; x, y, width, height: Single)
      : TStatus; overload;
    function DrawRectangles(pen: TGPPen; rects: PGPRectF; count: Integer)
      : TStatus; overload;
    function DrawRectangle(pen: TGPPen; const rect: TGPRect): TStatus; overload;
    function DrawRectangle(pen: TGPPen; x, y, width, height: Integer)
      : TStatus; overload;
    function DrawRectangles(pen: TGPPen; rects: PGPRect; count: Integer)
      : TStatus; overload;

    // DrawEllipse
    function DrawEllipse(pen: TGPPen; const rect: TGPRectF): TStatus; overload;
    function DrawEllipse(pen: TGPPen; x, y, width, height: Single)
      : TStatus; overload;
    function DrawEllipse(pen: TGPPen; const rect: TGPRect): TStatus; overload;
    function DrawEllipse(pen: TGPPen; x, y, width, height: Integer)
      : TStatus; overload;
    ...
    // DrawPolygon
    function DrawPolygon(pen: TGPPen; points: PGPPointF; count: Integer)
      : TStatus; overload;
    function DrawPolygon(pen: TGPPen; points: PGPPoint; count: Integer)
      : TStatus; overload;
    ...
    function DrawCurve(pen: TGPPen; points: PGPPointF;
      count, offset, numberOfSegments: Integer; tension: Single = 0.5)
      : TStatus; overload;
    function DrawCurve(pen: TGPPen; points: PGPPoint; count: Integer)
      : TStatus; overload;
    function DrawCurve(pen: TGPPen; points: PGPPoint; count: Integer;
      tension: Single): TStatus; overload;
    function DrawCurve(pen: TGPPen; points: PGPPoint;
      count, offset, numberOfSegments: Integer; tension: Single = 0.5)
      : TStatus; overload;
    ...
    // FillRectangle(s)
    function FillRectangle(brush: TGPBrush; const rect: TGPRectF)
      : TStatus; overload;
    function FillRectangle(brush: TGPBrush; x, y, width, height: Single)
      : TStatus; overload;
    function FillRectangles(brush: TGPBrush; rects: PGPRectF; count: Integer)
      : TStatus; overload;
    function FillRectangle(brush: TGPBrush; const rect: TGPRect)
      : TStatus; overload;
    function FillRectangle(brush: TGPBrush; x, y, width, height: Integer)
      : TStatus; overload;
    function FillRectangles(brush: TGPBrush; rects: PGPRect; count: Integer)
      : TStatus; overload;
    ...
    // FillEllipse
    function FillEllipse(brush: TGPBrush; const rect: TGPRectF)
      : TStatus; overload;
    function FillEllipse(brush: TGPBrush; x, y, width, height: Single)
      : TStatus; overload;
    function FillEllipse(brush: TGPBrush; const rect: TGPRect)
      : TStatus; overload;
    ...
    function DrawString(string_: WideString; length: Integer; Font: TGPFont;
      const origin: TGPPointF; stringFormat: TGPStringFormat; brush: TGPBrush)
      : TStatus; overload;
    ...
    function MeasureCharacterRanges(string_: WideString; length: Integer;
      ...
      regionCount: Integer; const regions: array of TGPRegion)
      : TStatus; overload;
    ...
    function DrawImage(image: TGPImage; const point: TGPPointF)
      : TStatus; overload;
    ...
    function DrawImage(image: TGPImage; const rect: TGPRectF): TStatus;
      overload;
    function DrawImage(image: TGPImage; x, y, width, height: Single)
      : TStatus; overload;
    function DrawImage(image: TGPImage; const point: TGPPoint)
      : TStatus; overload;
    ...
    function DrawImage(image: TGPImage; x, y, width, height: Integer)
      : TStatus; overload;
    ...
    function DrawImage(image: TGPImage; destPoints: PGPPointF; count: Integer)
      : TStatus; overload;
    function DrawImage(image: TGPImage; destPoints: PGPPoint; count: Integer)
      : TStatus; overload;
    ...
    function BeginContainer(const dstRect, srcRect: TGPRectF; unit_: TUnit)
      : GraphicsContainer; overload;
    function BeginContainer(const dstRect, srcRect: TGPRect; unit_: TUnit)
      : GraphicsContainer; overload;
  ...
  public
    ...
    function GetMetafileHeader(hEmf: HENHMETAFILE; header: TMetafileHeader)
      : TStatus; overload; // ok
    function GetMetafileHeader(filename: WideString; header: TMetafileHeader)
      : TStatus; overload; // ok
    function GetMetafileHeader(stream: IStream; header: TMetafileHeader)
      : TStatus; overload; // ok
    ...
    function SetDownLevelRasterizationLimit(metafileRasterizationLimitDpi
      : UINT): TStatus;
...
function TGPImageAttributes.Reset
  (type_: TColorAdjustType = ColorAdjustTypeDefault): TStatus;
begin
...
function TGPImageAttributes.SetNoOp
  (type_: TColorAdjustType = ColorAdjustTypeDefault): TStatus;
begin
  result := SetStatus(GdipSetImageAttributesNoOp(nativeImageAttr, type_, TRUE));
end;

function TGPImageAttributes.ClearNoOp
  (type_: TColorAdjustType = ColorAdjustTypeDefault): TStatus;
begin
  result := SetStatus(GdipSetImageAttributesNoOp(nativeImageAttr,
    type_, FALSE));
end;
...
function TGPImageAttributes.SetOutputChannelColorProfile(colorProfileFilename
  : WideString; type_: TColorAdjustType = ColorAdjustTypeDefault): TStatus;
begin
...
procedure TGPImageAttributes.SetNativeImageAttr(nativeImageAttr
  : GpImageAttributes);
begin
...
begin
  result := SetStatus(GdipSetMatrixElements(nativeMatrix, m11, m12, m21,
    m22, dx, dy));
end;
...
function TGPMatrix.Multiply(matrix: TGPMatrix;
  order: TMatrixOrder = MatrixOrderPrepend): TStatus;
begin
  result := SetStatus(GdipMultiplyMatrix(nativeMatrix,
...
function TGPMatrix.Translate(OffsetX, OffsetY: Single;
  order: TMatrixOrder = MatrixOrderPrepend): TStatus;
begin
  result := SetStatus(GdipTranslateMatrix(nativeMatrix, OffsetX,
    OffsetY, order));
end;
...
begin
  ...
  begin
    ...
    result := SetStatus(GdipTranslateMatrix(nativeMatrix, -center.x,
      -center.y, order));
  end
  else
  begin
...
function TGPMatrix.TransformVectors(pts: PGPPointF; count: Integer = 1)
  : TStatus;
begin
  result := SetStatus(GdipVectorTransformMatrixPoints(nativeMatrix,
    pts, count));
end;

function TGPMatrix.TransformVectors(pts: PGPPoint; count: Integer = 1): TStatus;
begin
  result := SetStatus(GdipVectorTransformMatrixPointsI(nativeMatrix,
    pts, count));
end;
...
begin
  ...
  if assigned(g) then
    ...
    GPX := nil;
  SetStatus(GdipIsVisibleRegionPointI(nativeRegion, point.x, point.y,
    GPX, booln));
  result := booln;
end;
...
begin
  ...
  if assigned(g) then
    ...
    GPX := nil;
  SetStatus(GdipIsVisibleRegionRectI(nativeRegion, x, y, width, height,
    GPX, booln));
  result := booln;
end;
...
begin
  ...
  if assigned(g) then
    ...
    GPX := nil;
  SetStatus(GdipIsVisibleRegionRect(nativeRegion, x, y, width, height,
    GPX, booln));
  result := booln;
end;
...
function TGPPen.MultiplyTransform(matrix: TGPMatrix;
  order: TMatrixOrder = MatrixOrderPrepend): TStatus;
begin
  result := SetStatus(GdipMultiplyPenTransform(nativePen,
    matrix.nativeMatrix, order));
end;
...
begin
  if (count <= 0) then
    ...
    result := SetStatus(GdipGetPenCompoundArray(nativePen,
      compoundArray, count));
end;
...
function TGPTextureBrush.ScaleTransform(sx, sy: Single;
  order: TMatrixOrder = MatrixOrderPrepend): TStatus;
begin
  result := SetStatus(GdipScaleTextureTransform(GpTexture(nativeBrush), sx,
    sy, order));
end;
...
function TGPLinearGradientBrush.SetLinearColors(color1,
  color2: TGPColor): TStatus;
begin
  result := SetStatus(GdipSetLineColors(GpLineGradient(nativeBrush),
    color1, color2));
end;

function TGPLinearGradientBrush.GetLinearColors(out color1,
  color2: TGPColor): TStatus;
var
...
function TGPLinearGradientBrush.SetGammaCorrection(useGammaCorrection
  : BOOL): TStatus;
begin
...
function TGPLinearGradientBrush.SetBlendBellShape(focus: Single;
  Scale: Single = 1.0): TStatus;
begin
  result := SetStatus(GdipSetLineSigmaBlend(GpLineGradient(nativeBrush),
    focus, Scale));
end;
...
begin
  brush := nil;
  lastResult := GdipCreateHatchBrush(Integer(hatchStyle), foreColor,
    backColor, brush);
  SetNativeBrush(brush);
end;
...
function TGPImage.GetPropertyIdList(numOfProperty: UINT; list: PPropID)
  : TStatus;
begin
...
function TGPImage.GetPropertyItem(propId: propId; propSize: UINT;
  buffer: PPropertyItem): TStatus;
begin
  result := SetStatus(GdipGetPropertyItem(nativeImage, propId,
    propSize, buffer));
end;

function TGPImage.GetPropertySize(out totalBufferSize, numProperties
  : UINT): TStatus;
begin
...
begin
  bitmap := nil;
  lastResult := GdipCreateBitmapFromScan0(width, height, stride, format,
    scan0, bitmap);
...
begin
  bitmap := nil;
  lastResult := GdipCreateBitmapFromScan0(width, height, 0, format,
    nil, bitmap);
...
function TGPBitmap.Clone(x, y, width, height: Integer; format: TPixelFormat)
  : TGPBitmap;
var
  bitmap: TGPBitmap;
...
function TGPBitmap.Clone(x, y, width, height: Single; format: TPixelFormat)
  : TGPBitmap;
var
  bitmap: TGPBitmap;
  gpdstBitmap: GpBitmap;
...
begin
  result := SetStatus(GdipBitmapSetResolution(GpBitmap(nativeImage),
    xdpi, ydpi));
end;
...
begin
  ...
  lastResult := GdipCreateBitmapFromResource(hInstance,
    PWideChar(bitmapName), bitmap);
  SetNativeImage(bitmap);
end;

function TGPBitmap.FromDirectDrawSurface7(surface: IDirectDrawSurface7)
  : TGPBitmap;
begin
...
function TGPBitmap.FromResource(hInstance: HMODULE; bitmapName: WideString)
  : TGPBitmap;
begin
...
function TGPGraphics.SetCompositingMode(compositingMode
  : TCompositingMode): TStatus;
begin
...
function TGPGraphics.SetCompositingQuality(compositingQuality
  : TCompositingQuality): TStatus;
begin
...
function TGPGraphics.SetInterpolationMode(interpolationMode
  : TInterpolationMode): TStatus;
begin
...
function TGPGraphics.SetPixelOffsetMode(pixelOffsetMode
  : TPixelOffsetMode): TStatus;
begin
...
function TGPGraphics.TranslateTransform(dx, dy: Single;
  order: TMatrixOrder = MatrixOrderPrepend): TStatus;
begin
  result := SetStatus(GdipTranslateWorldTransform(nativeGraphics, dx,
    dy, order));
end;
...
begin
  result := SetStatus(GdipDrawLine(nativeGraphics, pen.nativePen, x1,
    y1, x2, y2));
end;
...
function TGPGraphics.DrawLines(pen: TGPPen; points: PGPPointF;
  count: Integer): TStatus;
begin
  result := SetStatus(GdipDrawLines(nativeGraphics, pen.nativePen,
    points, count));
end;

function TGPGraphics.DrawLine(pen: TGPPen; x1, y1, x2, y2: Integer): TStatus;
begin
  result := SetStatus(GdipDrawLineI(nativeGraphics, pen.nativePen, x1,
    y1, x2, y2));
end;
...
function TGPGraphics.DrawLines(pen: TGPPen; points: PGPPoint;
  count: Integer): TStatus;
begin
  result := SetStatus(GdipDrawLinesI(nativeGraphics, pen.nativePen,
    points, count));
end;
...
begin
  result := DrawBezier(pen, pt1.x, pt1.y, pt2.x, pt2.y, pt3.x, pt3.y,
    pt4.x, pt4.y);
end;
...
function TGPGraphics.DrawBeziers(pen: TGPPen; points: PGPPointF;
  count: Integer): TStatus;
begin
  result := SetStatus(GdipDrawBeziers(nativeGraphics, pen.nativePen,
    points, count));
end;
...
function TGPGraphics.DrawBezier(pen: TGPPen; x1, y1, x2, y2, x3, y3, x4,
  y4: Integer): TStatus;
begin
  result := SetStatus(GdipDrawBezierI(nativeGraphics, pen.nativePen, x1, y1, x2,
    y2, x3, y3, x4, y4));
end;

function TGPGraphics.DrawBezier(pen: TGPPen;
  const pt1, pt2, pt3, pt4: TGPPoint): TStatus;
begin
  result := DrawBezier(pen, pt1.x, pt1.y, pt2.x, pt2.y, pt3.x, pt3.y,
    pt4.x, pt4.y);
end;
...
function TGPGraphics.DrawBeziers(pen: TGPPen; points: PGPPoint;
  count: Integer): TStatus;
begin
  result := SetStatus(GdipDrawBeziersI(nativeGraphics, pen.nativePen,
    points, count));
end;
...
function TGPGraphics.DrawRectangle(pen: TGPPen;
  x, y, width, height: Single): TStatus;
begin
...
function TGPGraphics.DrawRectangles(pen: TGPPen; rects: PGPRectF;
  count: Integer): TStatus;
begin
  result := SetStatus(GdipDrawRectangles(nativeGraphics, pen.nativePen,
    rects, count));
end;
...
function TGPGraphics.DrawRectangles(pen: TGPPen; rects: PGPRect;
  count: Integer): TStatus;
begin
  result := SetStatus(GdipDrawRectanglesI(nativeGraphics, pen.nativePen,
    rects, count));
end;
...
function TGPGraphics.DrawEllipse(pen: TGPPen;
  x, y, width, height: Single): TStatus;
begin
...
function TGPGraphics.DrawPolygon(pen: TGPPen; points: PGPPointF;
  count: Integer): TStatus;
begin
  result := SetStatus(GdipDrawPolygon(nativeGraphics, pen.nativePen,
    points, count));
end;
...
function TGPGraphics.DrawPolygon(pen: TGPPen; points: PGPPoint;
  count: Integer): TStatus;
begin
  result := SetStatus(GdipDrawPolygonI(nativeGraphics, pen.nativePen,
    points, count));
end;
...
function TGPGraphics.DrawCurve(pen: TGPPen; points: PGPPointF;
  count: Integer): TStatus;
begin
  result := SetStatus(GdipDrawCurve(nativeGraphics, pen.nativePen,
    points, count));
end;
...
function TGPGraphics.DrawCurve(pen: TGPPen; points: PGPPointF;
  count, offset, numberOfSegments: Integer; tension: Single = 0.5): TStatus;
begin
...
function TGPGraphics.DrawCurve(pen: TGPPen; points: PGPPoint;
  count: Integer): TStatus;
begin
  result := SetStatus(GdipDrawCurveI(nativeGraphics, pen.nativePen,
    points, count));
end;
...
begin
  result := SetStatus(GdipFillPolygon(nativeGraphics, brush.nativeBrush, points,
    count, fillMode));
end;
...
function TGPGraphics.FillEllipse(brush: TGPBrush; const rect: TGPRectF)
  : TStatus;
begin
...
begin
  ...
  if assigned(brush) then
    ...
    nBrush := nil;
  result := SetStatus(GdipDrawString(nativeGraphics, PWideChar(string_), length,
    nFont, @layoutRect, nStringFormat, nBrush));
end;
...
begin
  ...
  if assigned(brush) then
    ...
    nBrush := nil;
  result := SetStatus(GdipDrawString(nativeGraphics, PWideChar(string_), length,
    nFont, @rect, nStringFormat, nBrush));
end;
...
function TGPGraphics.MeasureString(string_: WideString; length: Integer;
  Font: TGPFont; const layoutRectSize: TGPSizeF; stringFormat: TGPStringFormat;
  out size: TGPSizeF; codepointsFitted: PInteger = nil;
  linesFilled: PInteger = nil): TStatus;
var
...
begin
  ...
  status := SetStatus(GdipMeasureCharacterRanges(nativeGraphics,
    PWideChar(string_), length, nFont, @layoutRect, nStringFormat, regionCount,
    nativeRegions));
...
function TGPGraphics.DrawImage(image: TGPImage; const point: TGPPointF)
  : TStatus;
begin
...
begin
  if assigned(image) then
    ...
    nImage := nil;
  result := SetStatus(GdipDrawImageRect(nativeGraphics, nImage, x, y,
    width, height));
end;
...
begin
  if assigned(image) then
    ...
    nImage := nil;
  result := SetStatus(GdipDrawImageRectI(nativeGraphics, nImage, x, y,
    width, height));
end;
...
begin
  ...
  if assigned(image) then
    ...
    nImage := nil;
  result := SetStatus(GdipDrawImagePoints(nativeGraphics, nImage,
    destPoints, count));
end;
...
begin
  ...
  if assigned(image) then
    ...
    nImage := nil;
  result := SetStatus(GdipDrawImagePointsI(nativeGraphics, nImage,
    destPoints, count));
end;

function TGPGraphics.DrawImage(image: TGPImage;
  x, y, srcx, srcy, srcwidth, srcheight: Single; srcUnit: TUnit): TStatus;
var
  nImage: GpImage;
begin
  if assigned(image) then
    ...
    nImage := nil;
  result := SetStatus(GdipDrawImagePointRect(nativeGraphics, nImage, x, y, srcx,
    srcy, srcwidth, srcheight, srcUnit));
end;
...
function TGPGraphics.DrawImage(image: TGPImage;
  x, y, srcx, srcy, srcwidth, srcheight: Integer; srcUnit: TUnit): TStatus;
var
  nImage: GpImage;
begin
  if assigned(image) then
...
begin
  ...
  if assigned(imageAttributes) then
    ...
    nimageAttributes := nil;
  result := SetStatus(GdipDrawImageRectRectI(nativeGraphics, nImage, destRect.x,
    destRect.y, destRect.width, destRect.height, srcx, srcy, srcwidth,
    srcheight, srcUnit, nimageAttributes, callback, callbackData));
end;
...
begin
  ...
  if assigned(imageAttributes) then
    ...
    nimageAttributes := nil;
  result := SetStatus(GdipEnumerateMetafileDestPointI(nativeGraphics, nMetafile,
    @destPoint, callback, callbackData, nimageAttributes));
end;
...
begin
  ...
  if assigned(imageAttributes) then
    ...
    nimageAttributes := nil;
  result := SetStatus(GdipEnumerateMetafileDestPoints(nativeGraphics, nMetafile,
    destPoints, count, callback, callbackData, nimageAttributes));
end;
...
function TGPGraphics.EnumerateMetafile(metafile: TGPMetafile;
  destPoints: PGPPoint; count: Integer; const srcRect: TGPRect; srcUnit: TUnit;
  callback: EnumerateMetafileProc; callbackData: Pointer = nil;
  imageAttributes: TGPImageAttributes = nil): TStatus;
var
  nMetafile: GpMetafile;
...
begin
  SetStatus(GdipBeginContainer(nativeGraphics, @dstRect, @srcRect,
    unit_, result));
end;
...
function TGPGraphics.BeginContainer(const dstRect, srcRect: TGPRect;
  unit_: TUnit): GraphicsContainer;
begin
  SetStatus(GdipBeginContainerI(nativeGraphics, @dstRect, @srcRect,
    unit_, result));
end;
...
begin
  result := SetStatus(GdipPrivateAddMemoryFont(nativeFontCollection,
    memory, length));
end;
...
function TGPGraphicsPath.AddBezier(x1, y1, x2, y2, x3, y3, x4,
  y4: Single): TStatus;
begin
  result := SetStatus(GdipAddPathBezier(nativePath, x1, y1, x2, y2, x3,
    y3, x4, y4));
end;
...
function TGPGraphicsPath.AddBezier(x1, y1, x2, y2, x3, y3, x4,
  y4: Integer): TStatus;
begin
  result := SetStatus(GdipAddPathBezierI(nativePath, x1, y1, x2, y2, x3,
    y3, x4, y4));
end;
...
function TGPGraphicsPath.AddCurve(points: PGPPointF;
  count, offset, numberOfSegments: Integer; tension: Single): TStatus;
begin
...
function TGPGraphicsPath.AddRectangles(rects: PGPRectF; count: Integer)
  : TStatus;
begin
...
begin
  ...
  if assigned(format) then
    gpsf := format.nativeFormat;
  result := SetStatus(GdipAddPathStringI(nativePath, PWideChar(string_), length,
    gpff, style, emSize, @rect, gpsf));
end;
...
function TGPGraphicsPath.GetBounds(out bounds: TGPRect; matrix: TGPMatrix = nil;
  pen: TGPPen = nil): TStatus;
var
  nativeMatrix: GpMatrix;
  nativePen: GpPen;
begin
  ...
  result := SetStatus(GdipGetPathWorldBoundsI(nativePath, @bounds, nativeMatrix,
    nativePen));
end;
...
begin
  ...
  if assigned(path) then
    nativePath := path.nativePath;
  SetStatus(GdipPathIterNextSubpathPath(nativeIterator, resultCount, nativePath,
    isClosed));
  result := resultCount;
end;
...
begin
  SetStatus(GdipPathIterEnumerate(nativeIterator, result, points,
    types, count));
end;
...
procedure TGPGraphicsPathIterator.SetNativeIterator(nativeIterator
  : GpPathIterator);
begin
...
function TGPPathGradientBrush.SetGammaCorrection(useGammaCorrection
  : BOOL): TStatus;
begin
...
function TGPPathGradientBrush.SetBlendBellShape(focus: Single;
  Scale: Single = 1.0): TStatus;
begin
  result := SetStatus(GdipSetPathGradientSigmaBlend(GpPathGradient(nativeBrush),
    focus, Scale));
end;
...
function TGPPathGradientBrush.GetFocusScales(out xScale,
  yScale: Single): TStatus;
begin
...
// // Initialize GDI+
// // GdiplusStartup(gdiplusToken, @StartupInput, nil);
// // code is added to resolve application hanging issue
// GdiplusStartup(gdiplusToken, @StartupInput, @StartupOutput);
// {$IFDEF WIN32}  //in 64 bit Delphi XE2 and XE3 the following code throws access violation while dropping tiff engine
// Stat := StartupOutput.NotificationHook(gdiplusToken);
// assert(Stat = Ok);
// {$ENDIF}
// end;
//
