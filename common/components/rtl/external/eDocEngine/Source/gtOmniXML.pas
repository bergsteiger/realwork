...
uses
  Classes, SysUtils
{$IFDEF MSWINDOWS}, gtGpTextStream {$ENDIF}
    , gtOmniXML_Types, gtOmniXML_Dictionary;
...
type
  ...
  public
    ...
    constructor CreateParseError(const DOMCode, XMLCode: Integer;
...
type
  ...
  IUnicodeStream = interface
    ...
    property OutputFormat: TOutputFormat read GetOutputFormat
    ...
    function GetNextString(var ReadString: XmlString;
...
{$ENDIF}
    ...
    FOutBuffer: PXmlChar;
    FOutBufferPos, FOutBufferSize: Integer;
    // undo support
  ...
  public
    property OutputFormat: TOutputFormat read GetOutputFormat
      write SetOutputFormat;
    property CodePage: Word read GetCodePage write SetCodePage;
    constructor Create(const Stream: TStream; const Mode: TStreamMode;
      const Encoding: Word = CP_UTF16);
  ...
  IXMLNode = interface
    ...
    procedure SelectSingleNode(Pattern: XmlString;
      var Result: IXMLNode); overload;
    ...
    function SelectSingleElementCreate(aTagName: XmlString): IXMLElement;
    // ONDREJ
    procedure SelectNodes(Pattern: XmlString;
      var Result: IXMLNodeList); overload;
    function SelectNodes(Pattern: XmlString): IXMLNodeList; overload;
    function GetXML: XmlString; // ondrej
    function GetXMLFormatted(const OutputFormat: TOutputFormat): XmlString;
    // ondrej
    property XML: XmlString read GetXML;

    function GetAttribute(const Name: XmlString): XmlString; // ondrej
    function GetAttributeDef(const Name, aDefValue: XmlString): XmlString;
    // ondrej
    function SetAttribute(const Name, Value: XmlString): IXMLNode;
    // ondrej; returns self
    function RemoveAttribute(const Name: XmlString): IXMLNode;
    // ondrej; returns self
  ...
  IXMLAttrList = interface(IXMLCustomList)
    ...
    property Values[const Name: XmlString]: XmlString read GetValue
      write SetValue; default; // ondrej
  end;
  ...
  *)
  ...
  TXMLWhiteSpaceHandling = (wsTrim, wsPreserveAll, wsPreserveInTextOnly,
    wsAutoTag);
  ...
  IXMLDocument = interface(IXMLNode)
    ...
    property DocumentElement: IXMLElement read GetDocumentElement
    ...
    function CreateProcessingInstruction(const Target, Data: XmlString)
    ...
    function LoadFromStream(const Stream: TStream;
      const ForceXMLEncoding: Word = 0): Boolean;
    procedure Save(const FileName: string;
      const OutputFormat: TOutputFormat = ofNone);
    procedure SaveToStream(const OutputStream: TStream;
      const OutputFormat: TOutputFormat = ofNone);
    ...
    property WhiteSpaceHandling: TXMLWhiteSpaceHandling
      read GetWhiteSpaceHandling write SetWhiteSpaceHandling;
  end;
...
{$EXTERNALSYM   TXMLNodeList}

  TXMLNodeList = class;
{$EXTERNALSYM  TXMLAttrList}
  TXMLAttrList = class;

  TXMLDocument = class;
 {$EXTERNALSYM  TXMLAttr }
  TXMLAttr = class;
{$EXTERNALSYM  TXMLElement}
  TXMLElement = class;
{$EXTERNALSYM   TXMLText }
  TXMLText = class;
{$EXTERNALSYM  TXMLComment}
  TXMLComment = class;
{$EXTERNALSYM  TXMLCDATASection }
  TXMLCDATASection = class;
{$EXTERNALSYM   TXMLProcessingInstruction}
  TXMLProcessingInstruction = class;
{$EXTERNALSYM   TXMLNode }
  ...
  protected
    ...
    procedure InternalWriteToStream(const OutputStream
      : IUnicodeStream); virtual;
    procedure ReadFromStream(const Parent: TXMLNode;
      const InputStream: IUnicodeStream); virtual;
    ...
    procedure SelectNodes(Pattern: XmlString; var Result: IXMLNodeList);
      overload; virtual;
    procedure SelectSingleNode(Pattern: XmlString; var Result: IXMLNode);
      overload; virtual;
  public
    ...
    property OwnerDocument: IXMLDocument read GetOwnerDocument;
    property Text: XmlString read GetText write SetText;
    constructor Create(const AOwnerDocument: TXMLDocument); overload;
    destructor Destroy; override;
    function InsertBefore(const NewChild, RefChild: IXMLNode): IXMLNode;
    function ReplaceChild(const NewChild, OldChild: IXMLNode): IXMLNode;
    function RemoveChild(const OldChild: IXMLNode): IXMLNode;
    ...
    function GetXMLFormatted(const OutputFormat: TOutputFormat)
      : XmlString; overload;
    property XML: XmlString read GetXML;
  ...
  *)
  {$EXTERNALSYM    TXMLEntityReference }
  TXMLEntityReference = class(TXMLNode, IXMLEntityReference);
{$EXTERNALSYM    TXMLDocumentFragment  }

  TXMLDocumentFragment = class(TXMLNode, IXMLDocumentFragment)
  protected
    function GetNodeName: XmlString; override;
    procedure ReadFromStream(const Parent: TXMLNode;
      const InputStream: IUnicodeStream); override;
  public
    constructor Create(const OwnerDocument: TXMLDocument); overload;
  end;
{$EXTERNALSYM    TXMLCustomList  }
...
{$EXTERNALSYM    TXMLAttrList }
  ...
  private
    Parent: Pointer;
    // IXMLNode;//Ondrej -> BS: memory leak (nodes would not be destroyed - use as pointers - reference count is not incremented)
  public
    function GetNamedItem(const Name: XmlString): IXMLAttr;
    function SetNamedItem(const Arg: IXMLAttr): IXMLAttr;
    function RemoveNamedItem(const Name: XmlString): IXMLAttr;
    ...
    constructor Create(const aParent: IXMLNode); overload; // IXMLNode
    destructor Destroy; override;
  ...
  public
    property TagName: XmlString read GetTagName;
    constructor CreateElement(const OwnerDocument: TXMLDocument;
      const TagName: XmlString);
  ...
  public
    property Target: XmlString read GetTarget;
    property Data: XmlString read GetData;
    constructor CreateProcessingInstruction(const OwnerDocument: TXMLDocument;
      const Target, Data: XmlString);
  end;
    {$EXTERNALSYM     TXMLAttr}
  ...
  public
    ...
    property Specified: Boolean read GetSpecified;
    property Value: XmlString read GetValue write SetValue;
    constructor CreateAttr(const OwnerDocument: TXMLDocument;
      const Name: XmlString);
  end;
 {$EXTERNALSYM     TXMLCharacterData }
  ...
  public
    property Data: XmlString read GetData write SetData;
    property Length: Integer read GetLength;
    constructor CreateCharacterData(const OwnerDocument: TXMLDocument;
      const Data: XmlString); virtual;
    function SubstringData(const Offset, Count: Integer): XmlString;
    procedure AppendData(const Arg: XmlString);
    procedure InsertData(const Offset: Integer; const Arg: XmlString);
    procedure DeleteData(const Offset, Count: Integer);
...
{$EXTERNALSYM     TXMLComment }
  ...
  protected
    ...
    procedure ReadFromStream(const Parent: TXMLNode;
      const InputStream: IUnicodeStream); override;
  public
    constructor CreateComment(const OwnerDocument: TXMLDocument;
      const Data: XmlString); overload; virtual;
  end;
{$EXTERNALSYM     TXMLCDATASection  }
  ...
  protected
    ...
    procedure ReadFromStream(const Parent: TXMLNode;
      const InputStream: IUnicodeStream); override;
  public
    constructor CreateCDATASection(const OwnerDocument: TXMLDocument;
      const Data: XmlString); overload; virtual;
  end;
{$EXTERNALSYM     TXMLDocumentType}
  ...
  protected
    ...
    procedure ReadFromStream(const Parent: TXMLNode;
      const InputStream: IUnicodeStream); override;
  public
    constructor CreateDocumentType(const OwnerDocument: TXMLDocument;
      const Data: XmlString); overload; virtual;
  end;

  TXMLAttrClass = class of TXMLAttr;
{$EXTERNALSYM     TXMLCDATASectionClass}
...
{$EXTERNALSYM      TXMLDocument}
  ...
  protected
    ...
    function InternalCreateCDATASection(const Data: XmlString)
      : TXMLCDATASection;
    ...
    function InternalCreateEntityReference(const Name: XmlString)
      : TXMLEntityReference;
    function InternalCreateProcessingInstruction(const Target, Data: XmlString)
  ...
  public
    ...
    function LoadFromStream(const Stream: TStream;
      const ForceXMLEncoding: Word = 0): Boolean;
    procedure Save(const FileName: string;
      const OutputFormat: TOutputFormat = ofNone); virtual;
    procedure SaveToStream(const OutputStream: TStream;
      const OutputFormat: TOutputFormat = ofNone);
...
const
  CodePages: TCodePages = ((CodePage: 932; Alias: 'shift-jis'),
    // Japanese (Shift-JIS)
    (CodePage: CP_UTF16; Alias: 'utf-16'),
    // Central European Alphabet (Windows)
    (CodePage: 1250; Alias: 'windows-1250'),
    // Central European Alphabet (Windows)
    ...
    (CodePage: ISO_8859_2; Alias: 'iso-8859-2'),
...
function CreateXMLDoc(const aRootNodeName, aProcessingInstruction: String)
  : IXMLDocument; overload;
function CreateXMLDoc(const aRootNodeName: String;
  const aAddUTF8ProcessingInstruction: Boolean): IXMLDocument; overload;
// Unicode functions
...
uses
  gtOmniXML_LookupTables, gtOmniXMLXPath;

const
  MAX_OUTPUTBUFFERSIZE = 256;
  // initial output buffer size (it only stores one tag at once!)
  OUTPUT_INDENT = 2;
...
var
  XMLErrorInfoList: array [MSG_E_NOTEXT .. XML_BAD_ENCODING] of TErrorInfo = (
    (
      ID: MSG_E_NOTEXT; Text: '%s'), (ID: MSG_E_FORMATINDEX_BADINDEX;
    Text: 'The value passed in to formatIndex needs to be greater than zero.'),
    (ID: MSG_E_FORMATINDEX_BADFORMAT; Text: 'Invalid format string.'),
    (ID: MSG_E_SYSTEM_ERROR; Text: 'System error: %s.'),
    (ID: MSG_E_MISSINGEQUALS;
    Text: 'Missing equals sign between attribute and attribute value.'),
    (ID: MSG_E_EXPECTED_TOKEN; Text: 'Expected token %s found %s.'),
    (ID: MSG_E_UNEXPECTED_TOKEN; Text: 'Unexpected token %s.'),
    (ID: MSG_E_MISSINGQUOTE;
    Text: 'A string literal was expected, but no opening quote character was found.'),
    (ID: MSG_E_COMMENTSYNTAX; Text: 'Incorrect syntax was used in a comment.'),
    (ID: MSG_E_BADSTARTNAMECHAR;
    Text: 'A name started with an invalid character.'), (ID: MSG_E_BADNAMECHAR;
    Text: 'A name contained an invalid character.'), (ID: MSG_E_BADCHARINSTRING;
    Text: 'The character "<" cannot be used in an attribute value.'),
    (ID: MSG_E_XMLDECLSYNTAX; Text: 'Invalid syntax for an xml declaration.'),
    (ID: MSG_E_BADCHARDATA;
    Text: 'An invalid character was found in text content.'),
    ...
    (ID: MSG_E_BADCHARINDECL;
    Text: 'An invalid character was found inside a DTD declaration.'),
    (ID: MSG_E_MISSINGSEMICOLON; Text: 'A semi colon character was expected.'),
    (ID: MSG_E_BADCHARINENTREF;
    Text: 'An invalid character was found inside an entity reference.'),
    (ID: MSG_E_UNBALANCEDPAREN; Text: 'Unbalanced parentheses.'),
    (ID: MSG_E_EXPECTINGOPENBRACKET;
    Text: 'An opening "[" character was expected.'), (ID: MSG_E_BADENDCONDSECT;
    Text: 'Invalid syntax in a conditional section.'), (ID: MSG_E_INTERNALERROR;
    Text: 'Internal error: %s'), (ID: MSG_E_UNEXPECTED_WHITESPACE;
    Text: 'Whitespace is not allowed at this location.'),
    (ID: MSG_E_INCOMPLETE_ENCODING;
    Text: 'End of file reached in invalid state for current encoding.'),
    (ID: MSG_E_BADCHARINMIXEDMODEL;
    Text: 'Mixed content model cannot contain this character.'),
    (ID: MSG_E_MISSING_STAR;
    Text: 'Mixed content model must be defined as zero or more("*").'),
    (ID: MSG_E_BADCHARINMODEL; Text: 'Invalid character in content model.'),
    (ID: MSG_E_MISSING_PAREN; Text: 'Missing parenthesis.'),
    (ID: MSG_E_BADCHARINENUMERATION;
    Text: 'Invalid character found in ATTLIST enumeration.'),
    (ID: MSG_E_PIDECLSYNTAX;
    Text: 'Invalid syntax in processing instruction declaration.'),
    (ID: MSG_E_EXPECTINGCLOSEQUOTE;
    Text: 'A single or double closing quote character ('' or ") is missing.'),
    (ID: MSG_E_MULTIPLE_COLONS;
    Text: 'Multiple colons are not allowed in a name.'),
    (ID: MSG_E_INVALID_DECIMAL; Text: 'Invalid character for decimal digit.'),
    (ID: MSG_E_INVALID_HEXADECIMAL;
    Text: 'Invalid character for hexadecimal digit.'),
    (ID: MSG_E_INVALID_UNICODE;
    Text: 'Invalid Unicode character value for this platform.'),
    (ID: MSG_E_WHITESPACEORQUESTIONMARK; Text: 'Expecting white space or "?".'),
    (ID: MSG_E_SUSPENDED; Text: 'The parser is suspended.'), (ID: MSG_E_STOPPED;
    Text: 'The parser is stopped.'), (ID: MSG_E_UNEXPECTEDENDTAG;
    Text: 'End tag was not expected at this location.'), (ID: MSG_E_UNCLOSEDTAG;
    Text: 'The following tags were not closed: %s.'),
    (ID: MSG_E_DUPLICATEATTRIBUTE; Text: 'Duplicate attribute.'),
    (ID: MSG_E_MULTIPLEROOTS;
    Text: 'Only one top level element is allowed in an XML document.'),
    (ID: MSG_E_INVALIDATROOTLEVEL;
    Text: 'Invalid character at the top level of the document.'),
    (ID: MSG_E_BADXMLDECL; Text: 'Invalid XML declaration.'),
    (ID: MSG_E_MISSINGROOT;
    Text: 'XML document must have a top level element.'),
    (ID: MSG_E_UNEXPECTEDEOF; Text: 'Unexpected end of file.'),
    (ID: MSG_E_BADPEREFINSUBSET;
    Text: 'Parameter entities cannot be used inside markup declarations in an internal subset.'),
    (ID: MSG_E_PE_NESTING;
    Text: 'The replacement text for a parameter entity must be properly nested with parenthesized groups.'),
    (ID: MSG_E_INVALID_CDATACLOSINGTAG;
    Text: 'The literal string "]]>" is not allowed in element content.'),
    (ID: MSG_E_UNCLOSEDPI; Text: 'Processing instruction was not closed.'),
    (ID: MSG_E_UNCLOSEDSTARTTAG; Text: 'Element was not closed.'),
    (ID: MSG_E_UNCLOSEDENDTAG;
    Text: 'End element was missing the character ">".'),
    ...
    (ID: MSG_E_UNCLOSEDMARKUPDECL;
    Text: 'A markup declaration was not closed.'), (ID: MSG_E_UNCLOSEDCDATA;
    Text: 'A CDATA section was not closed.'), (ID: MSG_E_BADDECLNAME;
    Text: 'Declaration has an invalid name.'), (ID: MSG_E_BADEXTERNALID;
    Text: 'External ID is invalid.'), (ID: MSG_E_BADELEMENTINDTD;
    Text: 'An XML element is not allowed inside a DTD.'),
    (ID: MSG_E_RESERVEDNAMESPACE;
    Text: 'The namespace prefix is not allowed to start with the reserved string "xml".'),
    (ID: MSG_E_EXPECTING_VERSION;
    Text: 'The version attribute is required at this location.'),
    (ID: MSG_E_EXPECTING_ENCODING;
    Text: 'The encoding attribute is required at this location.'),
    (ID: MSG_E_EXPECTING_NAME;
    Text: 'At least one name is required at this location.'),
    (ID: MSG_E_UNEXPECTED_ATTRIBUTE;
    Text: 'The specified attribute was not expected at this location. The attribute may be case-sensitive.'),
    (ID: MSG_E_ENDTAGMISMATCH;
    Text: 'End tag %s does not match the start tag %s.'),
    (ID: MSG_E_INVALIDENCODING;
    Text: 'System does not support the specified encoding.'),
    (ID: MSG_E_INVALIDSWITCH;
    Text: 'Switch from current encoding to specified encoding not supported.'),
    ...
    (ID: MSG_E_INVALIDXMLSPACE;
    Text: 'XML space attribute has invalid value. Must specify "default" or "preserve".'),
    (ID: MSG_E_MULTI_ATTR_VALUE;
    Text: 'Multiple names found in attribute value when only one was expected.'),
    (ID: MSG_E_INVALID_PRESENCE;
    Text: 'Invalid ATTDEF declaration. Expected #REQUIRED, #IMPLIED, or #FIXED.'),
    (ID: MSG_E_BADXMLCASE;
    Text: 'The name "xml" is reserved and must be lowercase.'),
    (ID: MSG_E_CONDSECTINSUBSET;
    Text: 'Conditional sections are not allowed in an internal subset.'),
    (ID: MSG_E_CDATAINVALID; Text: 'CDATA is not allowed in a DTD.'),
    (ID: MSG_E_INVALID_STANDALONE;
    Text: 'The standalone attribute must have the value "yes" or "no".'),
    (ID: MSG_E_UNEXPECTED_STANDALONE;
    Text: 'The standalone attribute cannot be used in external entities.'),
    (ID: MSG_E_DOCTYPE_IN_DTD;
    Text: 'Cannot have a DOCTYPE declaration in a DTD.'),
    (ID: MSG_E_MISSING_ENTITY; Text: 'Reference to an undefined entity.'),
    (ID: MSG_E_ENTITYREF_INNAME;
    Text: 'Entity reference is resolved to an invalid name character.'),
    (ID: MSG_E_DOCTYPE_OUTSIDE_PROLOG;
    Text: 'Cannot have a DOCTYPE declaration outside of a prolog.'),
    (ID: MSG_E_INVALID_VERSION; Text: 'Invalid version number.'),
    (ID: MSG_E_DTDELEMENT_OUTSIDE_DTD;
    Text: 'Cannot have a DTD declaration outside of a DTD.'),
    (ID: MSG_E_DUPLICATEDOCTYPE;
    Text: 'Cannot have multiple DOCTYPE declarations.'), (ID: MSG_E_RESOURCE;
    Text: 'Error processing resource %s.'), (ID: MSG_E_INVALID_OPERATION;
    Text: 'This operation can not be performed with a Node of type %s.'),
    (ID: MSG_E_WRONG_DOCUMENT;
    Text: 'NewChild was created from a different document than the one that created this node.'),

    (ID: XML_IOERROR; Text: 'Error opening input file: ''%s''.'),
    (ID: XML_ENTITY_UNDEFINED; Text: 'Reference to undefined entity %s.'),
    (ID: XML_INFINITE_ENTITY_LOOP;
    Text: 'Entity %s contains an infinite entity reference loop.'),
    (ID: XML_NDATA_INVALID_PE;
    Text: 'Cannot use the NDATA keyword in a parameter entity declaration.'),
    (ID: XML_REQUIRED_NDATA;
    Text: 'Cannot use a general parsed entity ''%s'' as the value for attribute ''%s''.'),
    (ID: XML_NDATA_INVALID_REF;
    Text: 'Cannot use unparsed entity %s in an entity reference.'),
    (ID: XML_EXTENT_IN_ATTR;
    Text: 'Cannot reference an external general parsed entity %s in an attribute value.'),
    (ID: XML_STOPPED_BY_USER; Text: 'XML parser stopped by user.'),
    (ID: XML_PARSING_ENTITY; Text: 'Error while parsing entity %s. %s.'),
    (ID: XML_E_MISSING_PE_ENTITY;
    Text: 'Parameter entity must be defined before it is used.'),
    (ID: XML_E_MIXEDCONTENT_DUP_NAME;
    Text: 'The same name must not appear more than once in a single mixed-content declaration: %s.'),
    (ID: XML_NAME_COLON;
    Text: 'Entity, EntityRef, PI, Notation names, or NMToken cannot contain a colon.'),
    (ID: XML_ELEMENT_UNDECLARED;
    Text: 'The element %s is used but not declared in the DTD/Schema.'),
    (ID: XML_ELEMENT_ID_NOT_FOUND;
    Text: 'The attribute %s references the ID %s, which is not defined anywhere in the document.'),
    (ID: XML_DEFAULT_ATTRIBUTE;
    Text: 'Error in the default attribute value defined in DTD/Schema.'),
    (ID: XML_XMLNS_RESERVED;
    Text: 'Reserved namespace "%s" cannot be redeclared.'),
    (ID: XML_EMPTY_NOT_ALLOWED;
    Text: 'Element cannot be empty according to the DTD/Schema.'),
    (ID: XML_ELEMENT_NOT_COMPLETE;
    Text: 'Element content is incomplete according to the DTD/Schema.'),
    (ID: XML_ROOT_NAME_MISMATCH;
    Text: 'The name of the top-most element must match the name of the DOCTYPE declaration.'),
    (ID: XML_INVALID_CONTENT;
    Text: 'Element content is invalid according to the DTD/Schema.'),
    (ID: XML_ATTRIBUTE_NOT_DEFINED;
    Text: 'The attribute %s on this element is not defined in the DTD/Schema.'),
    (ID: XML_ATTRIBUTE_FIXED;
    Text: 'Attribute %s has a value which does not match the fixed value defined in the DTD/Schema.'),
    (ID: XML_ATTRIBUTE_VALUE;
    Text: 'Attribute %s has an invalid value according to the DTD/Schema.'),
    (ID: XML_ILLEGAL_TEXT;
    Text: 'Text is not allowed in this element according to the DTD/Schema.'),
    (ID: XML_MULTI_FIXED_VALUES;
    Text: 'An attribute declaration cannot contain multiple fixed values: "%s".'),
    ...
    (ID: XML_XMLNS_UNDEFINED;
    Text: 'Reference to undeclared namespace prefix: %s.'),
    (ID: XML_XMLNS_FIXED; Text: 'Attribute %s must be a #FIXED attribute.'),
    (ID: XML_E_UNKNOWNERROR; Text: 'Unknown error: %s.'),
    (ID: XML_REQUIRED_ATTRIBUTE_MISSING;
    Text: 'Required attribute %s is missing.'), (ID: XML_MISSING_NOTATION;
    Text: 'Declaration %s contains a reference to undefined notation %s.'),
    (ID: XML_ATTLIST_DUPLICATED_ID;
    Text: 'Cannot define multiple ID attributes on the same element.'),
    (ID: XML_ATTLIST_ID_PRESENCE;
    Text: 'An attribute of type ID must have a declared default of #IMPLIED or #REQUIRED.'),
    ...
    (ID: XML_NAMESPACE_URI_EMPTY;
    Text: 'Only a default namespace can have an empty URI.'),
    (ID: XML_LOAD_EXTERNALENTITY; Text: 'Could not load %s.'),
    (ID: XML_BAD_ENCODING; Text: 'Unable to save character to %s encoding.'));

var
  CharacterReferences: TCharacterReferences = ((Code: 34; Name: 'quot'),
    (Code: 38; Name: 'amp'), (Code: 39; Name: 'apos'), (Code: 60; Name: 'lt'),
    (Code: 62; Name: 'gt'), (Code: 160; Name: 'nbsp'), (Code: 161;
    Name: 'iexcl'), (Code: 162; Name: 'cent'), (Code: 163; Name: 'pound'),
    (Code: 164; Name: 'curren'), (Code: 165; Name: 'yen'), (Code: 166;
    Name: 'brvbar'), (Code: 167; Name: 'sect'), (Code: 168; Name: 'uml'),
    (Code: 169; Name: 'copy'), (Code: 170; Name: 'ordf'), (Code: 171;
    Name: 'laquo'), (Code: 172; Name: 'not'), (Code: 173; Name: 'shy'),
    (Code: 174; Name: 'reg'), (Code: 175; Name: 'macr'), (Code: 176;
    Name: 'deg'), (Code: 177; Name: 'plusm'), (Code: 178; Name: 'sup2'),
    (Code: 179; Name: 'sup3'), (Code: 180; Name: 'acute'), (Code: 181;
    Name: 'micro'), (Code: 182; Name: 'para'), (Code: 183; Name: 'middot'),
    (Code: 184; Name: 'cedil'), (Code: 185; Name: 'supl'), (Code: 186;
    Name: 'ordm'), (Code: 187; Name: 'raquo'), (Code: 188; Name: 'frac14'),
    (Code: 189; Name: 'frac12'), (Code: 190; Name: 'frac34'), (Code: 191;
    Name: 'iquest'), (Code: 192; Name: 'Agrave'), (Code: 193; Name: 'Aacute'),
    (Code: 194; Name: 'circ'), (Code: 195; Name: 'Atilde'), (Code: 196;
    Name: 'Auml'), (Code: 197; Name: 'ring'), (Code: 198; Name: 'AElig'),
    (Code: 199; Name: 'Ccedil'), (Code: 200; Name: 'Egrave'), (Code: 201;
    Name: 'Eacute'), (Code: 202; Name: 'Ecirc'), (Code: 203; Name: 'Euml'),
    (Code: 204; Name: 'Igrave'), (Code: 205; Name: 'Iacute'), (Code: 206;
    Name: 'Icirc'), (Code: 207; Name: 'Iuml'), (Code: 208; Name: 'ETH'),
    (Code: 209; Name: 'Ntilde'), (Code: 210; Name: 'Ograve'), (Code: 211;
    Name: 'Oacute'), (Code: 212; Name: 'Ocirc'), (Code: 213; Name: 'Otilde'),
    (Code: 214; Name: 'Ouml'), (Code: 215; Name: 'times'), (Code: 216;
    Name: 'Oslash'), (Code: 217; Name: 'Ugrave'), (Code: 218; Name: 'Uacute'),
    (Code: 219; Name: 'Ucirc'), (Code: 220; Name: 'Uuml'), (Code: 221;
    Name: 'Yacute'), (Code: 222; Name: 'THORN'), (Code: 223; Name: 'szlig'),
    (Code: 224; Name: 'agrave'), (Code: 225; Name: 'aacute'), (Code: 226;
    Name: 'acirc'), (Code: 227; Name: 'atilde'), (Code: 228; Name: 'auml'),
    (Code: 229; Name: 'aring'), (Code: 230; Name: 'aelig'), (Code: 231;
    Name: 'ccedil'), (Code: 232; Name: 'egrave'), (Code: 233; Name: 'eacute'),
    (Code: 234; Name: 'ecirc'), (Code: 235; Name: 'euml'), (Code: 236;
    Name: 'igrave'), (Code: 237; Name: 'iacute'), (Code: 238; Name: 'icirc'),
    (Code: 239; Name: 'iuml'), (Code: 240; Name: 'ieth'), (Code: 241;
    Name: 'ntilde'), (Code: 242; Name: 'ograve'), (Code: 243; Name: 'oacute'),
    (Code: 244; Name: 'ocirc'), (Code: 245; Name: 'otilde'), (Code: 246;
    Name: 'ouml'), (Code: 247; Name: 'divide'), (Code: 248; Name: 'oslash'),
    (Code: 249; Name: 'ugrave'), (Code: 250; Name: 'uacute'), (Code: 251;
    Name: 'ucirc'), (Code: 252; Name: 'uuml'), (Code: 253; Name: 'yacute'),
    (Code: 254; Name: 'thorn'), (Code: 255; Name: 'yuml'));
...
function CreateXMLDoc(const aRootNodeName: String;
  const aAddUTF8ProcessingInstruction: Boolean): IXMLDocument;
begin
  Result := CreateXMLDoc(aRootNodeName,
...
begin
  ...
  if aProcessingInstruction <> '' then
    Result.AppendChild(Result.CreateProcessingInstruction('xml',
...
function FindEncoding(const PI: IXMLProcessingInstruction;
  var CodePage: Word): Boolean;
var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Encoding := Copy(PI.Data, EncodingStartPos,
            EncodingEndPos - EncodingStartPos - 1);
...
function FindCharReference(const CharReferenceName: string;
  var Character: XmlChar): Boolean;
var
  i: Integer;
begin
  Result := False;
...
type
  TParserState = (psReference, psEntityRef, psCharRef, psCharDigitalRef,
    psCharHexRef);
var
  ReadChar: XmlChar;
  PState: TParserState;
    ...
    case PState of
      psReference:
        if CharIs_WhiteSpace(ReadChar) then
          raise EXMLException.CreateParseError(INVALID_CHARACTER_ERR,
      ...
      psCharDigitalRef:
        ...
        begin
          case ReadChar of
            '0' .. '9':
              CharRef := LongWord(CharRef * 10) + LongWord(Ord(ReadChar) - 48);
            ';':
              begin
      ...
      psCharHexRef:
        ...
        begin
          case ReadChar of
            '0' .. '9':
              CharRef := LongWord(CharRef shl 4) + LongWord(Ord(ReadChar) - 48);
            'A' .. 'F':
              CharRef := LongWord(CharRef shl 4) +
                LongWord(Ord(ReadChar) - 65 + 10);
            'a' .. 'f':
              CharRef := LongWord(CharRef shl 4) +
                LongWord(Ord(ReadChar) - 97 + 10);
            ';':
          ...
          ';':
            begin
              if FindCharReference(EntityName, Result) then
                ...
                raise EXMLException.CreateParseError(INVALID_CHARACTER_ERR,
                  XML_ENTITY_UNDEFINED, [EntityName]);
            end;
        else
...
begin
  for i := 1 to Length(Value) do
    if not CharIs_WhiteSpace(Value[i]) then
    begin
      Result := False;
      Exit;
    end;
...
constructor EXMLException.CreateParseError(const DOMCode, XMLCode: Integer;
  const Args: array of const);
begin
...
constructor TXMLTextStream.Create(const Stream: TStream;
  const Mode: TStreamMode; const Encoding: Word);
{$IFDEF MSWINDOWS}
const
...
function TXMLTextStream.GetNextString(var ReadString: XmlString;
  const Len: Integer): Boolean;
var
  i: Integer;
  ReadChar: XmlChar;
...
begin
  if FCanUndo then
    ...
    raise EXMLException.CreateParseError(INVALID_CHARACTER_ERR,
      MSG_E_INTERNALERROR, ['Only one undo level is supported']);
...
begin
  Result := Self.AppendChild(Self.OwnerDocument.CreateElement(aTagName))
    as IXMLElement;
end;
...
begin
  ...
  if (Self.OwnerDocument <> nil) and (NewChild.OwnerDocument <> nil) and
    (NewChild.OwnerDocument <> Self.OwnerDocument) then
    raise EXMLException.CreateParseError(WRONG_DOCUMENT_ERR,
      MSG_E_WRONG_DOCUMENT, []);
...
begin
  case NodeType of
    ELEMENT_NODE:
      Result := FOwnerDocument.CreateElement(Self.NodeName);
    ATTRIBUTE_NODE:
      Result := FOwnerDocument.CreateAttribute(Self.NodeName);
    TEXT_NODE:
      Result := FOwnerDocument.CreateTextNode(Self.NodeValue);
    CDATA_SECTION_NODE:
      Result := FOwnerDocument.CreateCDATASection(Self.NodeValue);
    ENTITY_REFERENCE_NODE, ENTITY_NODE, DOCUMENT_TYPE_NODE:
      Assert(False, 'NYI - CloneNode');
    PROCESSING_INSTRUCTION_NODE:
      Result := FOwnerDocument.CreateProcessingInstruction(Self.NodeName,
        Self.NodeValue);
    COMMENT_NODE:
      Result := FOwnerDocument.CreateComment(Self.NodeValue);
    DOCUMENT_NODE:
      Result := CreateXMLDoc;
    DOCUMENT_FRAGMENT_NODE:
      Result := FOwnerDocument.CreateDocumentFragment;
    NOTATION_NODE:
      raise EXMLException.Create
        ('Invalid operation: cannot clone Notation node');
  end;

  if NodeType in [ATTRIBUTE_NODE, CDATA_SECTION_NODE, COMMENT_NODE,
    PROCESSING_INSTRUCTION_NODE, TEXT_NODE] then
...
begin
  ...
  begin
    ...
    if RefChildIndex = -1 then
      raise EXMLException.CreateParseError(NOT_FOUND_ERR, MSG_E_NOTEXT,
  ...
  if (NewChild.ParentNode = (Self as IXMLNode)) and
    (FChildNodes.IndexOf(NewChild) = RefChildIndex) then
    Exit;
...
begin
  try
    for i := Self.ChildNodes.Count - 1 downto 0 do
      if Self.ChildNodes[i].NodeName = aTagName then
      begin
...
procedure TXMLNode.ReadFromStream(const Parent: TXMLNode;
...
var
  xNilDoc: IXMLDocument = nil;

function GetNilXMLElement: IXMLElement;
begin
  if not Assigned(xNilDoc) then
  begin
...
constructor TXMLElement.CreateElement(const OwnerDocument: TXMLDocument;
  const TagName: XmlString);
begin
...
function TXMLNode.GetAttributeDef(const Name, aDefValue: XmlString): XmlString;
var
  xA: IXMLAttr;
begin
    ...
    begin
      ...
      begin
        ...
        if (ChildNode.NodeType = ELEMENT_NODE) and
          ((ChildNode as IXMLElement).NodeName = Name) then
...
var
  i: Integer;
const
  PreserveWhiteSpaceToText: Array [False .. True] of String = ('default',
    'preserve');
begin
  // 2002-12-17 (mr): fixed indentation
  ...
  if (FOwnerDocument.WhiteSpaceHandling = wsAutoTag) and
    ((((Self as IXMLElement) = FOwnerDocument.DocumentElement) and
    (Self.PreserveWhiteSpace)) or (Assigned(FParentNode) and
    (Supports(FParentNode, IXMLElement)) and ((FParentNode as IXMLElement)
    .PreserveWhiteSpace <> Self.PreserveWhiteSpace))) then
    OutputStream.WriteString(' xml:space="' + PreserveWhiteSpaceToText
      [Self.PreserveWhiteSpace] + '"');
  // ONDREJ END
  ...
  if HasChildNodes and
    (not((ChildNodes.Length = 1) and (ChildNodes.Item[0].NodeType = TEXT_NODE)))
  then
    OutputStream.WriteIndent;
...
procedure TXMLElement.ReadFromStream(const Parent: TXMLNode;
  const InputStream: IUnicodeStream);
type
      ...
      psAttr:
        ...
        begin
          case ReadChar of
            ...
            // [41] Attribute
            // [5] Name
            if CharIs_Letter(ReadChar) or CharIs_Digit(ReadChar) or
              (ReadChar = '_') then
            // ONDREJ CharIs_Digit <- not XML standard, but sorry... for compatibility issues
            begin
              ...
              // BEGIN ONDREJ
              if (Self is TXMLElement) and (_nodeAttr.NodeName = 'xml:space')
              then
              begin
...
constructor TXMLProcessingInstruction.CreateProcessingInstruction
  (const OwnerDocument: TXMLDocument; const Target, Data: XmlString);
begin
  inherited Create(OwnerDocument);
...
procedure TXMLProcessingInstruction.InternalWriteToStream(const OutputStream
  : IUnicodeStream);
begin
  // 2002-12-17 (mr): fixed indentation
  OutputStream.WriteIndent;
...
procedure TXMLProcessingInstruction.ReadFromStream(const Parent: TXMLNode;
  const InputStream: IUnicodeStream);
type
      ...
      psInTarget:
        begin
          ...
          begin
            ...
            if SameText(FTarget, 'xml') and
              (not Supports(Parent as IXMLNode, IXMLDocument, _xmlDoc)) then
              raise EXMLException.CreateParseError(HIERARCHY_REQUEST_ERR,
                MSG_E_BADXMLDECL, []);
      ...
      psEndData:
        begin
          ...
          begin
            ...
            if (FOwnerDocument.FTextStreamForceEncoding = 0) and
              FindEncoding(Self, CodePage) then
              // ONLY IF WE DON'T FORCE ANY ENCODING
...
constructor TXMLAttr.CreateAttr(const OwnerDocument: TXMLDocument;
  const Name: XmlString);
begin
  inherited Create(OwnerDocument);
  SetName(Name);
...
procedure TXMLAttr.ReadFromStream(const Parent: TXMLNode;
  const InputStream: IUnicodeStream);
type
...
constructor TXMLCharacterData.CreateCharacterData(const OwnerDocument
  : TXMLDocument; const Data: XmlString);
begin
  inherited Create(OwnerDocument);
  NodeValue := Data;
end;
...
procedure TXMLCharacterData.InsertData(const Offset: Integer;
  const Arg: XmlString);
begin
  Assert(False, 'NYI - InsertData');
end;

procedure TXMLCharacterData.ReplaceData(const Offset, Count: Integer;
  const Arg: XmlString);
begin
...
function TXMLCharacterData.SubstringData(const Offset, Count: Integer)
  : XmlString;
begin
  Assert(False, 'NYI - SubstringData');
end;

procedure TXMLCharacterData.InternalWriteToStream(const OutputStream
  : IUnicodeStream);
begin
...
procedure TXMLCharacterData.ReadFromStream(const Parent: TXMLNode;
  const InputStream: IUnicodeStream);
begin
  // do nothing
end;
...
constructor TXMLText.Create(const OwnerDocument: TXMLDocument;
        ...
        case ReadChar of
          '<':
            begin
              ...
              begin
                // ONDREJ
                xPreserve := (OwnerDocument.WhiteSpaceHandling
                  in [wsPreserveAll, wsPreserveInTextOnly]) or
                // document default trim!!!
                  ((OwnerDocument.WhiteSpaceHandling = wsAutoTag) and
                  (Parent is TXMLElement) and
                  (TXMLElement(Parent).PreserveWhiteSpace));
                // parent tag has set preserve attribute to true

                xNewValue := NodeValue + InputStream.GetOutputBuffer;
                if xPreserve then
                begin
                  if (OwnerDocument.WhiteSpaceHandling = wsPreserveInTextOnly)
                    and StringIs_WhiteSpace(xNewValue) then
                    xNewValue := '';

                  if (Parent is TXMLElement) and (NodeValue <> '') and
                    (CharIs_WhiteSpace(NodeValue[1]) or
                    CharIs_WhiteSpace(NodeValue[System.Length(NodeValue)])) then
...
constructor TXMLCDATASection.CreateCDATASection(const OwnerDocument
  : TXMLDocument; const Data: XmlString);
begin
  CheckValue(Data);
  inherited CreateCharacterData(OwnerDocument, Data);
...
procedure TXMLCDATASection.InternalWriteToStream(const OutputStream
  : IUnicodeStream);
begin
  // 2002-12-17 (mr): fixed indentation
...
procedure TXMLCDATASection.ReadFromStream(const Parent: TXMLNode;
  const InputStream: IUnicodeStream);
type
...
constructor TXMLComment.CreateComment(const OwnerDocument: TXMLDocument;
  const Data: XmlString);
begin
  inherited CreateCharacterData(OwnerDocument, Data);
...
procedure TXMLComment.ReadFromStream(const Parent: TXMLNode;
  const InputStream: IUnicodeStream);
type
  TParserState = (psData, psInEnd, psEnd);
var
...
constructor TXMLDocumentType.CreateDocumentType(const OwnerDocument
  : TXMLDocument; const Data: XmlString);
begin
  inherited CreateCharacterData(OwnerDocument, Data);
...
procedure TXMLDocumentType.InternalWriteToStream(const OutputStream
  : IUnicodeStream);
begin
  OutputStream.WriteIndent;
...
procedure TXMLDocumentFragment.ReadFromStream(const Parent: TXMLNode;
  const InputStream: IUnicodeStream);
begin
...
procedure TXMLDocument.SetWhiteSpaceHandling(const Value
  : TXMLWhiteSpaceHandling);
begin
...
function TXMLDocument.InternalCreateCDATASection(const Data: XmlString)
  : TXMLCDATASection;
begin
...
function TXMLDocument.CreateCDATASection(const Data: XmlString)
  : IXMLCDATASection;
begin
...
function TXMLDocument.InternalCreateDocType(const Data: XmlString)
  : TXMLDocumentType;
begin
...
function TXMLDocument.InternalCreateElement(const TagName: XmlString)
  : TXMLElement;
begin
...
function TXMLDocument.InternalCreateEntityReference(const Name: XmlString)
  : TXMLEntityReference;
begin
...
function TXMLDocument.CreateEntityReference(const Name: XmlString)
  : IXMLEntityReference;
begin
  Result := InternalCreateEntityReference(Name);
end;

function TXMLDocument.InternalCreateProcessingInstruction(const Target,
  Data: XmlString): TXMLProcessingInstruction;
begin
  Result := FXMLProcessingInstructionClass.CreateProcessingInstruction(Self,
    Target, Data);
end;

function TXMLDocument.CreateProcessingInstruction(const Target, Data: XmlString)
  : IXMLProcessingInstruction;
begin
...
begin
  raise EXMLException.Create
    ('Don''t use IXMLDocument.Text property. Use IXMLDocument.XML instead.');
end;
...
function TXMLDocument.GetElementsByTagName(const TagName: XmlString)
  : IXMLNodeList;
var
  TempDocElement: IXMLElement;
begin
...
begin
  ...
  try
    try
      ...
      if UnclosedElementList.Count > 0 then
        raise EXMLException.CreateParseError(HIERARCHY_REQUEST_ERR,
          MSG_E_UNCLOSEDTAG, [GetUnclosedTags]);

      if FChildNodes = nil then
        // ONDREJ raise EXMLException.CreateParseError(0, MSG_E_MISSINGROOT, []) <- don't raise, just return FALSE
        Result := False;
    except
      on E: Exception do
      begin
...
procedure TXMLDocument.InternalWriteToStream(const OutputStream
  : IUnicodeStream);
var
  i: Integer;
begin
...
procedure TXMLDocument.SaveToStream(const OutputStream: TStream;
...
procedure TXMLDocument.Save(const FileName: string;
  const OutputFormat: TOutputFormat = ofNone);
var
...
procedure TXMLDocument.ReadFromStream(const Parent: TXMLNode;
  const InputStream: IUnicodeStream);
type
      ...
      psTagName: // one-time stop
        ...
        begin
          case ReadChar of
            ...
            '!':
              PState := psCommentOrCDATAOrDoctype;
            // not enough info, check also next char
            '/':
              PState := psEndTagName;
          else
            // [40] STag
          ...
          case ReadChar of
            '>':
              begin
                ...
                if (EndTagName = Parent.NodeName) and
                  (EndTagName = (UnclosedElementList[UnclosedElementList.Count -
                  1] as IXMLElement).NodeName) then
                begin
                  ...
                  raise EXMLException.CreateParseError(HIERARCHY_REQUEST_ERR,
                    MSG_E_ENDTAGMISMATCH, [EndTagName, Parent.NodeName]);
              end;
          else
            { 'A'..'Z', 'a'..'z', '0'..'9': }
            InputStream.WriteOutputChar(ReadChar);
            ...
            '-':
              begin
                ...
                begin
                  ...
                  raise Exception.CreateFmt('Invalid node %s',
