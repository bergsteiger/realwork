unit NOT_FINISHED_nsDocInfoHAFMacroReplacer;

// Модуль: "w:\garant6x\implementation\Garant\GbaNemesis\Business\Document\NOT_FINISHED_nsDocInfoHAFMacroReplacer.pas"
// Стереотип: "SimpleClass"
// Элемент модели: "TnsDocInfoHAFMacroReplacer" MUID: (4958968201A7)

{$Include w:\garant6x\implementation\Garant\nsDefine.inc}

interface

{$If NOT Defined(Admin) AND NOT Defined(Monitorings)}
uses
 l3IntfUses
;

type
 TnsDocInfoHAFMacroReplacer = class
 end;//TnsDocInfoHAFMacroReplacer
{$IfEnd} // NOT Defined(Admin) AND NOT Defined(Monitorings)

implementation

{$If NOT Defined(Admin) AND NOT Defined(Monitorings)}
uses
 l3ImplUses
 //#UC START# *4958968201A7impl_uses*
 //#UC END# *4958968201A7impl_uses*
;
{$IfEnd} // NOT Defined(Admin) AND NOT Defined(Monitorings)

end.
