...
// ћодуль: "w:\garant6x\implementation\Garant\GbaNemesis\Medic\deMedicFirmList.pas"
// —тереотип: "SimpleClass"
// Ёлемент модели: "TdeMedicFirmList" MUID: (561F6D380330)
 ...
 TdeMedicFirmList = class(Tl3ProtoObject, IdeMedicFirmList, IdeDocInfo)
  ...
  protected
   function As_IdeDocInfo: IdeDocInfo;
    {* ћетод приведени€ нашего интерфейса к IdeDocInfo }
   function IsSame(const aDocInfo: IdeDocInfo;
    aView: Boolean = True;
    aPosition: Boolean = True): Boolean;
    {* сравнить документы.
           - aView: возвращает равенство документов с учетом их состо€ни€ (в
                    отличии от is_same_entity, который возвращает равенство без
                    учета состо€ни€). —осто€нием документа €вл€етс€ суперпозици€
                    номера редакции, €зыка и списка извлечений.
...
//#UC START# *561F6E61019E_561F6D380330_impl*
 inherited Create;
 f_DocInfo := aDocInfo;
 // Ћучше заковыр€ть IdeDocInfo внутрь и перетранслировать запросы ему,
 // чем писать QueryInterface.
 // ’от€ всЄ это - признак кривизны архитектуры всей этой конструкции с sds/ds/de/d
 f_Current := aCurrent;
