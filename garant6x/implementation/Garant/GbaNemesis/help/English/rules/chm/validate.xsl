...
<xsl:template name="nodes">
 <xsl:variable name="nodes" select="/help/application/modules/module//node()[name()='usertype' or name()='entity' or name()='operation']"/>
  <xsl:for-each select="$nodes">
    <xsl:if test="not(count(//excludeddsc/com[@id=current()/@key]))">
    <xsl:if test="not(count(//description[@id=current()/@key]))">
    <xsl:if test="@key!=''">
