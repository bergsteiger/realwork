...
<tr class="blue">
 <td align="left"  nowrap="true">
  ...
  <span 
   ...
   id="versionInfo"
  >
   ГАРАНТ Платформа F1. <xsl:value-of select="/help/@title"/>
  </span>
 </td>
...
<!-- Получаем порядковый номер узла screen на множестве узлов от корня до контекстного и увеличиваем его на 2: стр.334 -->
   <b>Рис. <xsl:value-of select="count(preceding-sibling::screen)+3"/>
<!-- <xsl:number level="any" count="screen" value="position()+2"/> 
-->
            </b><br/>
  </xsl:if>
  <xsl:value-of select="@title"/>
