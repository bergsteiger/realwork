...
// Модуль: "w:/garant6x/implementation/Garant/tie/Garant/GblAdapterLib/BaseTypesUnit.pas"
...
type
 ...
 TPId = record
  {* Persistent ID, тип для идентификации хранимых объектов. }
   class_id : TClassId; // идентификатор класса
   object_id : TObjectId; // идентификатор объекта
 end;//TPId

 TDate = record
  {* Дата. }
   day : unsigned short; // Число
   month : unsigned short; // Месяц
   year : unsigned short; // Год.
...
//#UC END# *456EA6F00109publ*
 ...
 TDateInterval = record
  {* Интервал дат (включающий). }
   start : TDate; // начальная дата
   finish : TDate; // завершающая дата
 end;//TDateInterval

 TTime = record
  {* Время }
   hour : unsigned short; // часы
   min : unsigned short; // минуты
   sec : unsigned short; // секунды
 end;//TTime
...
Объеденяя с с ранее сохраненными данными }
 ...
 TSortParams = record
  {* Параметры сортировки. }
   sort_order : TSortOrder; // порядок сортировки
   sort_type : TSortType; // тип сортивровки
 end;//TSortParams
