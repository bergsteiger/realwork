...
if __name__ == '__main__':
    try:
        options = dict(getopt.getopt(sys.argv[1:], '', ['action=', 'port-corba=', 'port-zope='])[0])
        service_helper = ServiceHelper().set_corba_port(options.get('--port-corba')).set_zope_port(options.get('--port-zope')).set_force_mode(True)
