/*
 * $Id: hindex.h,v 1.5 2010/09/20 14:13:50 romank Exp $
 *
 *
...
public:
	...
	unsigned long & Parent ( BTKEY key ) const
	{
		return ((HIndexKey*)key)->parent;
	}

	unsigned long & Id ( BTKEY key ) const
	{
		return ((HIndexKey*)key)->id;
	}

	unsigned long & OwnChildrenCount ( BTKEY key ) const
	{
		return ((HIndexKey*)key)->chCount;
	}

	unsigned long & AllChildrenCount ( BTKEY key ) const 
	{
		return ((HIndexKey*)key)->achCount;
	}

	unsigned long OwnDataSize( BTKEY key, int sn=0) const
	{
