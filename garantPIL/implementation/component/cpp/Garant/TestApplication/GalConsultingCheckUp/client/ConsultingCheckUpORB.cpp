...
const std::string ConsultingCheckUpORB::s_default_consulting_server_host = "zzz.garant.ru";
const std::string ConsultingCheckUpORB::s_default_consulting_server_port = "80";

ConsultingCheckUpORB::ConsultingCheckUpORB (const char* name, size_t threads_count, std::string proxy_config_name)
	: ORB (name, threads_count)
	, m_proxy_config (proxy_config_name)
	, m_orb (0) 
	  , m_shutdowned (true) {
}

CORBA::ORB_ptr ConsultingCheckUpORB::get_cached_client_orb () {
	GDS_ASSERT (m_orb || !"Try to use uninitialized Consulting Checker ORB");
	return m_orb;
}

void ConsultingCheckUpORB::init (int argc, char* argv[]) {
	...
	try {
		...
		std::string server_address = Core::ParamManagerFactory::get ().get_string (
			...
			, ConsultingCheckUpORB::s_default_consulting_server_host.c_str ()
		);
		...
		std::string server_port = Core::ParamManagerFactory::get ().get_string (
			...
			, ConsultingCheckUpORB::s_default_consulting_server_port.c_str ()
		);
...
void ConsultingCheckUpORB::set_request_timeout (unsigned short seconds) {
	GDS_ASSERT (!m_shutdowned && "ConsultingClientORB must be enabled before setting timeout");

	CoreSrv::ORB::enable_request_timeout (this->get_cached_orb (), seconds);
}

void ConsultingCheckUpORB::spawn_run () {
	GDS_ASSERT (false || !"ConsultingClientORB::spawn_run not released!");
}

void ConsultingCheckUpORB::shutdown () {
	CoreSrv::ORB::shutdown ();
	m_shutdowned = true;
}
