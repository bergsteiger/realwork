...
#endif
{
  ...
  switch (pc)
	...
	if (   start_contents->superstate
       ...
       * 0 string, at an offset of STARTPOS.  STARTPOS is out
       * of range of the 0 string, so the first call to 
 ...
 test_match:
  ...
  return_from_test_match:
	...
	if (regs && (regs->start != search_state.best_lparen))
	  {
	    bcopy ((char *)search_state.best_lparen, (char *)regs->start,
		   regs->num_regs * sizeof (int));
	    bcopy ((char *)search_state.best_rparen, (char *)regs->end,
		   regs->num_regs * sizeof (int));
	  }
	if (regs && !rxb->no_sub)
	  {
