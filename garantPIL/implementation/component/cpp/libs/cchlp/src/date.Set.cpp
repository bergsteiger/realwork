...
Pix dateSet::seek(date& item)
{
  Pix i;
  for (i = first(); i != 0 && !(dateEQ((*this)(i), item)); next(i));
  return i;
}

int dateSet::owns(Pix idx)
{
  if (idx == 0) return 0;
...
int dateSet::contains (date& item)
{
  return seek(item) != 0;
}

int dateSet::operator <= (dateSet& b)
{
...
void dateSet::operator |= (dateSet& b)
{
  if (&b != this)
...
void dateSet::error(const char* msg)
{
