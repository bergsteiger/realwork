...
longVHSet::longVHSet(unsigned int sz)
{
...
longVHSet::longVHSet(longVHSet& a)
{
...
void longVHSet::clear()
{
  for (unsigned int i = 0; i < size; ++i) status[i] = EMPTYCELL;
  count = 0;
}

void longVHSet::resize(unsigned int newsize)
{
  if (newsize <= count)
  {
...
void longVHSet::next(Pix& i)
{
  if (i == 0) return;
  unsigned long pos = ((unsigned long)i - (unsigned long)tab) / sizeof(long) + 1;
  for (; pos < size; ++pos)
...
int longVHSet:: operator == (longVHSet& b)
{
  if (count != b.count)
    return 0;
  else
...
void longVHSet::operator |= (longVHSet& b)
{
  if (&b == this || b.count == 0)
    return;
