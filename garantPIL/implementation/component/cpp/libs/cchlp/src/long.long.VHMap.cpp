...
longlongVHMap::longlongVHMap(long  dflt, unsigned int sz)
     :longlongMap(dflt)
{
  tab = new long[size = sz];
  cont = new long[size];
  status = new char[size];
  for (unsigned int i = 0; i < size; ++i) status[i] = EMPTYCELL;
}

longlongVHMap::longlongVHMap(longlongVHMap& a) : longlongMap(a.def)
{
  tab = new long[size = a.size];
  cont = new long[size];
  status = new char[size];
  for (unsigned int i = 0; i < size; ++i) status[i] = EMPTYCELL;
  count = 0;
...
Pix longlongVHMap::seek(long  key)
{
  unsigned int hashval = longHASH(key);
  unsigned int h = hashval % size;
  for (unsigned int i = 0; i <= size; ++i)
  {
    if (status[h] == EMPTYCELL)
      return 0;
    else if (status[h] == VALIDCELL && longEQ(key, tab[h]))
      return Pix(&tab[h]);
    if (i == 0)
...
long& longlongVHMap::operator [](long  item)
{
  if (HASHTABLE_TOO_CROWDED(count, size))
    resize();

  unsigned int bestspot = size;
  unsigned int hashval = longHASH(item);
  unsigned int h = hashval % size;
  for (unsigned int i = 0; i <= size; ++i)
  {
    if (status[h] == EMPTYCELL)
...
void longlongVHMap::del(long  key)
{
  unsigned int hashval = longHASH(key);
  unsigned int h = hashval % size;
  for (unsigned int i = 0; i <= size; ++i)
...
void longlongVHMap::clear()
{
  for (unsigned int i = 0; i < size; ++i) status[i] = EMPTYCELL;
  count = 0;
}

void longlongVHMap::resize(unsigned int newsize)
{
  if (newsize <= count)
  {
...
void longlongVHMap::next(Pix& i)
{
  if (i == 0) return;
  unsigned int pos = ((unsigned)i - (unsigned)tab) / sizeof(long) + 1;
  for (; pos < size; ++pos)
