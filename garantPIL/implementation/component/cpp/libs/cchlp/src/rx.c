...
#endif
{
  bzero ((char *)b, rx_sizeof_bitset(size));
}
...
#endif
{
  ...
  
  {
	...
	if (!newtab)
	  goto add_to_bucket;
	bzero ((char *)newtab, sizeof (*newtab));
	newtab->parent = table;
	{
...
#endif
{
  ...
  rx->rx_id = rx_id++;
  bzero ((char *)&rx->se_list_memo, sizeof (rx->se_list_memo));
  bzero ((char *)&rx->set_list_memo, sizeof (rx->set_list_memo));
  while (n)
    {
      n->futures = 0;
...
#endif
{
  ...
  if (!car && !cdr)
      ...
      if (!cache->empty_superset)
	{
	  ...
	  if (!cache->empty_superset)
	    return 0;
	  bzero ((char *)cache->empty_superset, sizeof (struct rx_superset));
...
#endif
  {
	...
	if (!rx_eclose_nfa (&rxb->rx))
	...
	else
	  {
	    ...
	    bcopy ((char *)params, (char *)rxb->se_params, iterator_size);
