...
Pix u_int64_tSet::seek(u_int64_t  item)
{
  Pix i;
  for (i = first(); i != 0 && !(u_int64_tEQ((*this)(i), item)); next(i));
  return i;
}

int u_int64_tSet::owns(Pix idx)
{
  if (idx == 0) return 0;
...
int u_int64_tSet::contains (u_int64_t  item)
{
  return seek(item) != 0;
}

int u_int64_tSet::operator <= (u_int64_tSet& b)
{
...
void u_int64_tSet::operator |= (u_int64_tSet& b)
{
  if (&b != this)
...
void u_int64_tSet::error(const char* msg)
{
