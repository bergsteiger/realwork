...
u_int64_tVHSet::u_int64_tVHSet(unsigned int sz)
{
  tab = new u_int64_t[size = sz];
  status = new char[size];
  for (unsigned int i = 0; i < size; ++i) status[i] = EMPTYCELL;
  count = 0;
}

u_int64_tVHSet::u_int64_tVHSet(u_int64_tVHSet& a)
{
  tab = new u_int64_t[size = a.size];
  status = new char[size];
  for (unsigned int i = 0; i < size; ++i) status[i] = EMPTYCELL;
  count = 0;
...
Pix u_int64_tVHSet::seek(u_int64_t  key)
{
  unsigned int hashval = u_int64_tHASH(key);
  unsigned int h = hashval % size;
  for (unsigned int i = 0; i <= size; ++i)
  {
    if (status[h] == EMPTYCELL)
      return 0;
    else if (status[h] == VALIDCELL && u_int64_tEQ(key, tab[h]))
      return Pix(&tab[h]);
    if (i == 0)
...
Pix u_int64_tVHSet::add(u_int64_t  item)
{
  if (HASHTABLE_TOO_CROWDED(count, size))
    resize();

  unsigned int bestspot = size;
  unsigned int hashval = u_int64_tHASH(item);
  unsigned int h = hashval % size;
  for (unsigned int i = 0; i <= size; ++i)
  {
    if (status[h] == EMPTYCELL)
    ...
    else if (u_int64_tEQ(tab[h],item))
      return Pix(&tab[h]);
...
void u_int64_tVHSet::del(u_int64_t  key)
{
  unsigned int hashval = u_int64_tHASH(key);
  unsigned int h = hashval % size;
  for (unsigned int i = 0; i <= size; ++i)
...
void u_int64_tVHSet::clear()
{
  for (unsigned int i = 0; i < size; ++i) status[i] = EMPTYCELL;
  count = 0;
}

void u_int64_tVHSet::resize(unsigned int newsize)
{
  if (newsize <= count)
  {
  ...
  u_int64_t* oldtab = tab;
  char* oldstatus = status;
  unsigned int oldsize = size;
  tab = new u_int64_t[size = newsize];
  status = new char[size];
  unsigned int i;
...
void u_int64_tVHSet::next(Pix& i)
{
  if (i == 0) return;
  unsigned long pos = ((unsigned long)i - (unsigned long)tab) / sizeof(u_int64_t) + 1;
  for (; pos < size; ++pos)
...
int u_int64_tVHSet:: operator == (u_int64_tVHSet& b)
{
  if (count != b.count)
    return 0;
  else
...
void u_int64_tVHSet::operator |= (u_int64_tVHSet& b)
{
  if (&b == this || b.count == 0)
    return;
