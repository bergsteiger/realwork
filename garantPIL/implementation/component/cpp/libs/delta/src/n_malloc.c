/*
 * $Id: n_malloc.c,v 2.3 2000/05/03 12:56:14 max Exp $
 * $Log: n_malloc.c,v $
 ...
 *
 * Revision 1.3.4.3  1999/02/16 18:07:05  max
 * Fuck up cygwin
 *
 * Revision 1.3.4.2  1998/12/03 18:21:44  alik
 * make it compile with CygWin b20
 *
 * Revision 1.3.4.1  1998/05/18 15:47:50  max
 * Go away from RELENG_4_05
 *
 * Revision 1.3.2.2  1998/05/12 12:28:01  max
 * Some defenition for CYGWIN32
 *
 * Revision 1.3.2.1  1998/05/08 17:22:19  max
 * Disabele malinfo() structure return
 *
 * Revision 1.3  1998/04/02 08:47:49  max
...
zmh zmh_malloc_handler = NULL;

#if __STD_C
zmh Z_set_malloc_handler(zmh p) 
#else
zmh Z_set_malloc_handler(p) zmh p;
#endif
{
	zmh zmhRet = zmh_malloc_handler;
	zmh_malloc_handler = p;
	return zmhRet;
}
...
#if __STD_C
Void_t* mALLOc(size_t bytes_s)
#else
Void_t* mALLOc(bytes) size_t bytes_s;
#endif
{
...
TRY_AGANE :
  ...
  */
  ...
  if ( (remainder_size = chunksize(top) - nb) < (long)MINSIZE)
  {
    ...
    if ( (remainder_size = chunksize(top) - nb) < (long)MINSIZE) {
      /* propagate failure */
      if ( inLoop || !zmh_malloc_handler ) return 0;
      /* calls malloc_handler and try alloc agane */
      zmh_malloc_handler ();
      inLoop = 1;
      goto TRY_AGANE;
    }
...
#if __STD_C
#ifdef __CYGWIN32__
void cfree(Void_t *mem, Void_t *mem1)
#else
