...
typedef struct z_stream_s {
    ...
    char     *msg;      /* last error message, 0 if no error */
...
/*
  ...
  if next_in or next_out was 0), Z_BUF_ERROR if no progress is possible.
*/
...
/*
  ...
  next_in or next_out was 0), Z_MEM_ERROR if there was not enough memory,
...
/*
   ...
   parameter is invalid (such as 0 dictionary) or the stream state
...
/*
   ...
   (such as zalloc being 0). msg is left unchanged in both source and
   destination.
*/
...
/*
   ...
   stream state was inconsistent (such as zalloc or state being 0).
*/
...
/*
   ...
   parameter is invalid (such as 0 dictionary) or the stream state is
...
/*
   ...
   is as in mpcxc_fopen "rb" or "wb" but can also include a compression level
   ("wb9").  gzopen can be used to read a file which is not in gzip format;
   in this case gzread will directly read from the file without decompression.
     gzopen returns 0 if the file could not be opened or if there was
...
/*
   ...
   descriptor fd. If you want to keep fd open, use gzdopen(dup(fd), mode).
     gzdopen returns 0 if there was insufficient memory to allocate
...
/*
   ...
   return the updated checksum. If buf is 0, this function returns
...
extern uLong EXPORT z_crc32   OF((uLong crc, const Bytef *buf, uInt len));
/*
     Update a running crc with the bytes buf[0..len-1] and return the updated
   crc. If buf is 0, this function returns the required initial value
...
extern int EXPORT inflateInit2_ OF((z_stream *strm, int  windowBits,
				    const char *version, int stream_size));
#ifdef OPT_PACK
extern int EXPORT deflateInit__ OF((z_stream *strm, int level,
			           const char *version, int stream_size));
extern int EXPORT deflateInit2_Old OF((z_stream *strm, int  level, int  method,
					int windowBits, int memLevel, int strategy,
					const char *version, int stream_size));

	#define deflateInit_Old(strm, level) \
			deflateInit__((strm), (level),       ZLIB_VERSION, sizeof(z_stream))
#endif
