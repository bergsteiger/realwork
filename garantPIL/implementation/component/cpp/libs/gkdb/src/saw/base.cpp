...
const char Base::m_CorrNewName[12]	= "CorrNew.str" ;
...
static const int docTags [] =
{
	...
	IDD_DOCKIND,    IDD_SOURCEFILE
#ifdef	VERSION56
	, IDD_ACTIVE, IDD_PAGEBREAKS,
#ifndef VERSION61
	IDD_PARAIDS,
#endif
	...
	, IDD_KIND, IDD_CHDATE_EX, IDD_ADDCODE, IDD_CHDATE_EXEX, IDD_MARKEDTEXT, IDD_SAMES
};
...
{
	if(!pOther && (nameQ == m_SWordsName||nameQ == m_SubNamesName||nameQ == m_CorrNewName)){
...
#ifdef	_GCD_SERVER
	if (res == sizeof (info) && docId == PROTECTED_DOCID)
		info.ParaCount++;
#endif
	return res;
}
	...
	if(i < sizeof(indTable)/sizeof(*indTable)){
		if(pExt == 0){
			...
			*pExt= '.';
			...
			}else  if(!strcmp(name, "corrnew.str")){
				indTable[i]= new RealRefIndex	(pOtherNextIndex->pKey, nameQ, pOtherNextIndex->pStreamFile);
...
Index* Base::FindIndexFor (const char *name, BTKEY key)
{
	if (!strcmp (name, "Blob"))
		return FindBlobIndexFor (((ObjKey*)key)->id);
	else
		return FindIndex (name);
}

Index* Base::FindIndexFor (const char *name, const char* key)
{
	return FindIndex (name);
}

Index* Base::FindBlobIndexFor (long id)
{
	return FindIndex ("Blob");
}
...
StrList	*Base :: GetFileList()
{
	...
	if(!(fnd1= strrchr(strcpy(baseName, keyFile->FileName), '.')))
		return 0;
	*fnd1= 0;
	if(pl_GetFileList(baseName, &pList))
		return 0;
...
int Base::DelDoc (long DocId) {
	PageRun* runs;     // pointer to an array of PageRuns
	BlockRec* blocks;  // pointer to an array of BlockDescs
	SubDesc* subs;   // pointer to an array of SubDesc

	long RunCount = 0;
	long SubCount = 0;
	long BlockCount = 0;

	runs = (PageRun*) docInd->Load (DocId, IDD_RUNS, RunCount);
	subs = (SubDesc*) docInd->Load (DocId, IDD_SUBS, SubCount);
	blocks = (BlockRec*) docInd->Load (DocId, IDD_BLOCKS, BlockCount);
	...
	long i;
	for ( i = 0; i < sizeof ( docTags ) / sizeof ( int ); i++ )
		docInd -> Del ( DocId, docTags [i] );

	Ref ref = {DocId, 0};

	((AttrIndex*) FindIndex("Attribs"))->Del (DocId, IDD2_PARAIDS);
	subNames->Delete (&ref);
...
int Base :: RenameDoc ( long fromId, long toId )
{
	...
	for ( int i = 0; i < sizeof ( docTags ) / sizeof ( int ); i++ ){
...
DocCollection* Base::get_cache_all_docs () {
	if (!cacheAllDocs) {
		Index* ind = FindIndex ("Status");

		if (ind) { 
			if (!ind->streams->Fat)
				ind->streams->LoadFat ();

			cacheAllDocs = new DocCollection ();
			...
			for (size_t i = 0; i < 3; ++i) {
				...
				if (str) {
					if (cacheAllDocs->ItemCount) {
						DocCollection tmp;
						tmp.Get (str);
						cacheAllDocs->Merge (tmp);
					} else {
						cacheAllDocs->Get (str);
					}
					ind->Close (str);
				}
			}
	...
	cacheAllDocs->Deflate ();
	return cacheAllDocs;
}

DocCollection* Base::AllDocs () {
	GDS_ASSERT (this->get_cache_all_docs ());
	return new DocCollection (*cacheAllDocs);
}

static void load_inpharm_topics (DocCollection*& data, Base* base, short status_val) {
	if (!data) {
		data = new DocCollection();

		Index* index = base->FindIndex ("Status_ex");
		short status = status_val;
...
DocCollection* Base::inpharm_medtopics () {
	load_inpharm_topics (cacheAllMDocs, this, DS_MEDTOPIC);
	return new DocCollection (*cacheAllMDocs);
...
DocCollection* Base::inpharm_firmtopics () {
	load_inpharm_topics (cacheAllFDocs, this, DS_FIRMTOPIC);
	return new DocCollection (*cacheAllFDocs);
}
...
DocCollection* Base :: AllTopics (long borderId, int aIterator )
{
    DocCollection * col = new DocCollection;

	if ( !aIterator ) {
		Index* ind = FindIndex( "Aux" );
		if ( ind ) {
			char* key = "AllT";
...
int Base::FillSmallInfoDoc (long Id, char* &aName, char** &aSegNames, short& aSegs, int aRussian)
{
	aSegs = 0;
	...
	Document *pDoc = GetDoc (gl_doc);
	char	*pFndPara, *pPara, *pTmpTxt;
	long	lo = 0, mid, num = pDoc->Stat.ParaCount, hi = num - 1, lstId, half;
	do {
		if (half = num / 2) {
			mid = lo + (num & 1 ? half : (half - 1));
			lstId = atol ((pFndPara = ParaText ((pPara = pDoc->GetPara (mid)))));
			if (lstId == Id)
				break;
			...
			pFndPara = ParaText ((pPara = pDoc->GetPara (lo)));
			if (atol (pFndPara) != Id) {
				delete pDoc;
				return 0;
			}
	...
	if (baseInfos.empty ()) {
		Stream *biStr = FindIndex ("Aux")->Open ("Base");
		int biCount = biStr ? biStr->Length () / sizeof (BaseInfo) : 0;
		if (0 == biCount) {
			if (biStr)
				FindIndex ("Aux")->Close (biStr);
			delete pDoc;
			return 0;
		} else {
			while (biCount--) {
				...
				biStr->Read (&baseInfo, sizeof (BaseInfo));
				std::pair<std::string,std::string> names (baseInfo.Name, baseInfo.NameEng);
				baseInfos.insert (std::map<short, std::pair<std::string,std::string> >::value_type (baseInfo.BaseId, names));
			}
			FindIndex ("Aux")->Close (biStr);
		}
	}
	...
	*pTmpTxt = 0;
	aName = new char [DOC_NAME_SIZE];
	strcpy (aName, pPara + 1);

	std::vector<std::string> parts;
	boost::split (parts, ++pTmpTxt, boost::is_any_of ("і"));
	aSegNames = new char* [parts.size () - 1];
	for (std::vector<std::string>::const_iterator it = parts.begin (); it != parts.end (); it++) {
		if (it->size ()) {
			...
			if (map_it != baseInfos.end ())
				aSegNames [aSegs++] = (char*)(aRussian ? map_it->second.first.c_str () : map_it->second.second.c_str ());
		}
	}
