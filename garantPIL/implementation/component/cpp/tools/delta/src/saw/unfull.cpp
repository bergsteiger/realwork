...
UnFull::UnFull( UpdatedBase* oldB, UpdatedBase* newB, FILE* dF1, char* pSkipDir, bool english_face, bool b_load ) : m_mapBaseInfo ( oldB )
{
	b_cvarupdated = false;
	...
	//
	// cache for Class second stream changes
	//
	m_pClassCache = b_load ? new ClassCache ( (HIndex*)m_pNewBase->FindIndex( "Class" ), (HIndex*)m_pNewBase->FindIndex( "DocKind" ), b_english_face) : 0;

	//
	// cache for Adopted second stream changes
	//
	m_pAdoptedCache = b_load ? new ClassCache ( (HIndex*)m_pNewBase->FindIndex( "Adopted" ), (HIndex*)m_pNewBase->FindIndex( "Territory" ), b_english_face) : 0;
	
	//
	// cache for Context changes
	//
#if defined(QUICK_NEW_DOC) && defined(STORE_FORM_N) && defined(MIN_MEM_CONTEXT) && defined(INCLUDE_OLD_CONTXT)
#else
	m_pContext  = new ContextCollection ( m_pNewBase->FindIndex( "Context" ), 1024*200, b_english_face);
	assert ( m_pContext );
#endif
...
void	UnFull::delete_deleted_docs ()
{
	...
	for ( p = m_mapDelDocs.first(); p; m_mapDelDocs.next(p), count++) {
		...
		m_pNewBase = m_pOldBase = saved_m_pNewBase->find_base_for_doc (m_lDocId);
		docInd = (AttrIndex*) m_pNewBase->FindIndex ("Attrs");
		docInd2 = (AttrIndex*) m_pNewBase->FindIndex ("Attribs");
		...
		//
		docSegs -= m_mapDelDocs.contents(p);
	...
	m_pNewBase = m_pOldBase = saved_m_pNewBase;
...
void	UnFull::restore_corrs ()
{
	...
	//
	Index *ind = saved_m_pNewBase->FindIndex ("Correspondent"), *resp = saved_m_pNewBase->FindIndex ("Respondent");
	...
	ind = saved_m_pNewBase->FindIndex ("NewCorr"), acount = 0, oldPerc = 0;
	for (p = m_mapCorrs.first(); p; m_mapCorrs.next(p), acount++) {
		long docId = m_mapCorrs.key(p);
...
#if defined(QUICK_NEW_DOC) && defined(STORE_FORM_N) && defined(MIN_MEM_CONTEXT) && defined(INCLUDE_OLD_CONTXT)
#else
	logger.printf (lt_updateInfo, "delete m_pContext\n");
	delete m_pContext;
#endif

	flush_caches (false);
	if (m_pClassCache) {
		logger.printf (lt_updateInfo, "delete m_pClassCache\n");
		delete m_pClassCache;
	}
	if (m_pAdoptedCache) {
		logger.printf (lt_updateInfo, "delete m_pAdoptedCache\n");
		delete m_pAdoptedCache;
	}
	flush_caches (true);
...
int UnFull::MemFree ()
{
	...
	logger.printf ( lt_debug, "MemFree - called (used_mem=%lu)\n", getUsedPhsMem() );

	if ( m_pContext->InUse() )
	{
		logger.printf ( lt_debug, "MemFree - called while Context work (unable to flush)\n" );
		return 0;
	}

	m_pContext->Flush ();
	logger.printf ( lt_debug, "MemFree - returned (used_mem=%lu)\n", getUsedPhsMem() );
#endif
	return  1;
}
...
int UnFull::Work (bool b_first_delta)
{
	...
	long saved_delta_pos;
	mpcxc_fgetpos (m_DFile1 , &saved_delta_pos);
	char basechar = 0;
	for (std::vector<UpdatedBase*>::const_iterator base_it = saved_m_pNewBase->bases_ptrs.begin (); base_it != saved_m_pNewBase->bases_ptrs.end (); base_it++, basechar++) {
		fseek (m_DFile1, saved_delta_pos, ACE_OS_SEEK_SET);
		
		m_pOldBase = m_pNewBase = *base_it;
		docInd = (AttrIndex*) m_pNewBase->FindIndex ("Attrs");
		docInd2 = (AttrIndex*) m_pNewBase->FindIndex ("Attribs");
	...
	m_pOldBase = m_pNewBase = saved_m_pNewBase;
	...
	while ( lBlobCount-- ) {
		...
		Index *ind = saved_m_pNewBase->find_base_for_blob (objkey.id)->FindIndex("Blob");	
	...
	while ( m_lDocId != NEW_ID ) {		
		...
		m_mapDelDocs[m_lDocId] |= setDeltaSegs;
...
int UnFull :: PrepareDeltaRestoring ( )
{
	...
	if (saved_m_pNewBase->find_base_for_doc (m_lDocId) != m_pNewBase) {
		//документ должен быть обновлен в другой проход
		long ulDeltaSize, fptDeltaStart = 0;
		...
		if (ulDeltaSize < 0) {
			...
			fseek (m_DFile1, packed_size, ACE_OS_SEEK_CUR);		
		} else {
			fseek (m_DFile1, fptDeltaStart, ACE_OS_SEEK_SET);
	...
	m_pOldInfo = (DocInfo*) m_pNewBase->LoadAttr (m_lDocId, IDD_INFO, Size);
...
void UnFull::RestoreChgDoc ()
{
	...
	TempDoc *pTempDoc= new TmpUpdtDoc ( m_pNewBase, m_lDocId, &pTmpDoc, m_pOldInfo );
	assert ( pTempDoc );

	m_pNewBase->DelText (m_lDocId, pTmpDoc->runs, pTmpDoc->RunCount, pTmpDoc->blocks, pTmpDoc->BlockCount);
	if(!pNewDoc)
		pNewDoc= make_GU_Doc (m_pOldInfo->Title, PM_ZIP | PM_NOT_STORE_INFO | PM_NOT_STORE_PARAINFO, &s_UpdDocs);
	assert (pNewDoc);
...
void UnFull :: FillClassCache ( StringZ* classz, StringZ* prefixz, int c_or_p, int d_or_a, int tag )
{
	StringZ* from = c_or_p ? classz : prefixz;
	StringZ* what = c_or_p ? prefixz : classz;
	
	ClassCache * pCache = 0;
	int withSub = 0;
	long sub = -1;	// with out sub
	const char* cl_key;
	const char* pr_key;
	
	if ( tag == IDD_DOCKIND ) {
		withSub = 1;
		pCache = m_pClassCache;
	} else if ( tag == IDD_TERRITORY ) {
		withSub = 0;
		pCache = m_pAdoptedCache;
	} else {
		logger.printf ( lt_fatal, "Unknown pair attr (tag %d)\n", tag );
		_EXIT ( EXIT_FAILURE );
	}
	
	for ( long l=0; l < from->GetCount(); l++ ) {
		if ( !d_or_a || from->GetAttrib (l) & Z_ATTR_HIDE ) {
			if ( withSub ) {
				sscanf( from->Get (l), "%ld:", &sub );
			#ifdef	BLOCKSINHERIT
				if ( sub & INHERITED_BLOCK )
					continue;
			#endif
			}
			...
			if ( c_or_p ) {
				cl_key = from->GetKey(l,0);
				pr_key = what->IterateBySub ( sub, itPos );
			} else {
				cl_key = what->IterateBySub ( sub, itPos );
				pr_key = from->GetKey(l,0);
			}
			if ( !cl_key || !pr_key ) {
				logger.printf ( lt_fatal, "Unpaired keys on sub %ld (tag %d)\n", sub, tag ); /// at least one key must exist!
				_EXIT ( EXIT_FAILURE );
			}
						
			do { // remove and iterate other keys
				if ( d_or_a ) {
					pCache->DelPrefix ( cl_key, pr_key );
				} else {
					pCache->AddPrefix ( cl_key, pr_key );
				}
...
void UnFull :: CorrectPairIndex ( StringZ * pOldClass, StringZ * pNewClass, 
	StringZ * pOldPrefs, StringZ * pNewPrefs, int iTag )
{	
	/*
		In this we have five different situation:
		1) New doc with Class and Prefixs 
		2) Deleted doc with Class and Prefixs
		3) Changed doc, only Class changed
		4) Changed doc, only Prefix changed
		5) Changed doc, both Prefix and Class
		
		Args in:
		1) pOldClass, pOldPrefs - exist and empty
		   pNewClass, pNewPrefs - exist and must be add
		3) pOldClass, pOldPrefs - exist and must be del
		   pNewClass, pNewPrefs - 0, ignore
		3) pOldPrefs, pNewPrefs - 0, need to load from Base
		   pOldClass, pNewClass - exist and must be processed
		4) pOldPrefs, pNewPrefs - exist and must be processed
		   pOldClass, pNewClass - 0, need to load from Base
		5) pOldPrefs, pNewPrefs, 
		   pOldClass, pNewClass - exist and must be processed
		   
		1) and 5) is equal for processing		
	*/
	if (pOldClass) pOldClass->DisableHide();
	if (pNewClass) pNewClass->DisableHide();
	if (pOldPrefs) pOldPrefs->DisableHide();
	if (pNewPrefs) pNewPrefs->DisableHide();
	
	/*logger.printf ( lt_debug, cd_dos, "CorrectPairIndex: Tag=%d, OCc=%d, NCc=%d, OPc=%d, NPc=%d\n", iTag,
		pOldClass ? pOldClass->GetCount() : -1, pNewClass ? pNewClass->GetCount() : -1, 
		pOldPrefs ? pOldPrefs->GetCount() : -1, pNewPrefs ? pNewPrefs->GetCount() : -1 );
	*/
	...
	if ( iTag == IDD_DIV || iTag == IDD_DOCKIND ) {
		clTag = IDD_DIV;
		prTag = IDD_DOCKIND;
	} else if ( iTag == IDD_SOURCE || iTag == IDD_TERRITORY ) {
		clTag = IDD_SOURCE;
		prTag = IDD_TERRITORY;
	} else {
		logger.printf ( lt_fatal, "CorrectPairIndex:Unknown tag %d\n", iTag );
		_EXIT ( EXIT_FAILURE );
	}
	
	if ( !pOldClass && !pOldPrefs ) {
		logger.printf ( lt_fatal, "CorrectPairIndex:Old class and old prefix was not specifyed (tag %d)\n", iTag );
		_EXIT ( EXIT_FAILURE );
	}
			
	if ( !pOldClass ) { // Only prefix changed
		hasClass = 0;
		if ( pNewClass ) {
			logger.printf ( lt_fatal, "CorrectPairIndex:No old class, then new present (tag %d)\n", iTag );
			_EXIT ( EXIT_FAILURE );
		}
		pOldClass = pNewClass = LoadStrz ( m_pOldBase, m_lDocId, clTag ); // load exist class attrs
		if ( !pNewClass ) {
			logger.printf ( lt_fatal, "CorrectPairIndex:Can't load class attrs (tag %d)\n", iTag );
			_EXIT ( EXIT_FAILURE );
		}
	}
	if ( !pOldPrefs ) { // Only class changed
		hasPrefs = 0;
		if ( pNewPrefs ) {
			logger.printf ( lt_fatal, "CorrectPairIndex:No old prefix, then new present (tag %d)\n", iTag );
			_EXIT ( EXIT_FAILURE );
		}
		pOldPrefs = pNewPrefs = LoadStrz ( m_pOldBase, m_lDocId, prTag ); // load exist prefix attrs
		if ( !pNewPrefs ) {
			logger.printf ( lt_fatal, "CorrectPairIndex:Can't load prefix attrs (tag %d)\n", iTag );
			_EXIT ( EXIT_FAILURE );
		}
	}	
	
	if ( pNewClass && !pOldClass->GetCount() ) {
		// new doc - processing for added Class
		FillClassCache ( pNewClass, pNewPrefs, 1 /*class*/, 0 /*add*/, prTag );
	} else if ( !pNewClass ) {
		// deleted doc - processing for deleted Class
		FillClassCache ( pOldClass, pOldPrefs, 1 /*class*/, 1 /*del*/, prTag );
	} else {
		// changed doc
		if ( hasClass ) { // Class changed
			// processing for deleted Class
			FillClassCache ( pOldClass, pOldPrefs, 1 /*class*/, 1 /*del*/, prTag );
			// processing for added Class
			FillClassCache ( pNewClass, pNewPrefs, 1 /*class*/, 0 /*add*/, prTag );
		}
		if ( hasPrefs ) { // Prefix changed
			// processing for deleted Prefix
			FillClassCache ( pOldClass, pOldPrefs, 0 /*prefs*/, 1 /*del*/, prTag );
			// processing for added Prefix
			FillClassCache ( pNewClass, pNewPrefs, 0 /*prefs*/, 0 /*add*/, prTag );
		}		
	}
	
	if ( !hasClass ) delete pNewClass; // pNewClass was loaded here
	if ( !hasPrefs ) delete pNewPrefs; // pNewPrefs was loaded here
}
...
void UnFull :: TagAttrRestore ( long lDocId, int iTag, const char* log_str)
{
	...
	case IDD_CLASS6:
		{
		int withSub = iTag==IDD_KEY || iTag==IDD_DIV || iTag == IDD_DOCKIND || iTag == IDD_TYPE || iTag == IDD_SERVICEINFO || iTag == IDD_CLASS6 || iTag == IDD_KIND;
		int hKey = withSub || iTag == IDD_SOURCE || iTag == IDD_PUBLISHEDIN || iTag == IDD_TERRITORY || iTag == IDD_ANNOUSER || iTag == IDD_ANNOORGANIZATION || iTag == IDD_ANNOTAX || iTag == IDD_ANNOINTEREST || iTag == IDD_ANNOKIND || iTag == IDD_CLASS6;
		...
		if ( iTag == IDD_DIV || iTag == IDD_SOURCE ) { // first pair attr
			if ( m_pOldClass ) {
				logger.printf ( lt_fatal, "Not closed previos old pair Attr (tag %d)\n", iTag );
				_EXIT ( EXIT_FAILURE );
			}
			m_pOldClass = new StringZ ( vOldBuff, iOldSize + ( iTag == IDD_KEY ? 32768 : 0 ), withSub, 1 );
			pKeys = m_pOldClass;
		} else pKeys = new StringZ ( vOldBuff, iOldSize, withSub, 0 );	
		...
		// Full restoring second stream in pair indices
		if ( iTag == IDD_DOCKIND || iTag == IDD_TERRITORY ) { // second pair attr
			pKeys->Seek(0);
			StringZ aNewKeys ( vNewBuff, iNewSize, withSub, 0 );
			
			CorrectPairIndex ( m_pOldClass, m_pNewClass, pKeys, &aNewKeys, iTag );
		...
		// Save New pair keys and keep Old, or free Old (pKeys)
		if ( iTag == IDD_DIV || iTag == IDD_SOURCE ) { // first pair attr
			if ( m_pNewClass ) {
				logger.printf ( lt_fatal, "Not closed previos new pair Attr (tag %d)\n", iTag );
				_EXIT ( EXIT_FAILURE );
			}			
			m_pNewClass = new StringZ ( vNewBuff, iNewSize, withSub, 1 );
		} else {
			delete pKeys;
		}
	...
	case IDD_INFO :
	{	
		Index* statusex_index = saved_m_pNewBase->FindIndex ("Status_ex");
		Index* status_index = saved_m_pNewBase->FindIndex ("Status");
		...
		*/
		...
		if ( bNewDoc || m_pOldInfo->Status != m_pNewInfo->Status ) {
			short newDSDoc = m_pNewInfo->Status & (DS_DOC | DS_NODOC | DS_EDITION );
	...
	case IDD_BASES :
	{
		...
		while ( addCount-- ) {
			...
			if ( m_dsm == dsmStayDoc || m_dsm == dsmChgToDoc ) 
...
Index * UnFull :: SelectIndex ( int iTag )
{
	switch ( iTag ) {
	case IDD_INPHARMFIRM:
		return saved_m_pNewBase->FindIndex( "PhFirm" );
	case IDD_INPHARMCOUNTRY:
		return saved_m_pNewBase->FindIndex( "PhCountry" );
	case IDD_REGDATE:
		return saved_m_pNewBase->FindIndex( "RegDate" );
	case IDD_ANNULDATE:
		return saved_m_pNewBase->FindIndex( "AnnulDate" );
	case IDD_ACTIVEEX:
		return saved_m_pNewBase->FindIndex( "Active" );
	case IDD_TRADENAME:
		return saved_m_pNewBase->FindIndex( "TradeName" );
	case IDD_INTNAME:
		return saved_m_pNewBase->FindIndex( "IntName" );
	case IDD_PHARMGROUP:
		return saved_m_pNewBase->FindIndex( "PhGroup" );
	case IDD_PHARMEFFECT:
		return saved_m_pNewBase->FindIndex( "PhEffect" );
	case IDD_CHAPTER:
		return saved_m_pNewBase->FindIndex( "Chapter" );
	case IDD_ATC:
		return saved_m_pNewBase->FindIndex( "Atc" );
	case IDD_REGSTATUS:
		return saved_m_pNewBase->FindIndex( "RegStatus" );
	case IDD_LEKFORM:
		return saved_m_pNewBase->FindIndex( "LekForm" );
	case IDD_MKB:
		return saved_m_pNewBase->FindIndex( "Mkb" );
	case IDD_CLASS6 :
		return saved_m_pNewBase->FindIndex( "Class6" );
	case IDD_SERVICEINFO :
		return saved_m_pNewBase->FindIndex( "ServiceInfo" );
	case IDD_KEY :
		return saved_m_pNewBase->FindIndex( "KeyWord" );
	case IDD_DIV :
		return saved_m_pNewBase->FindIndex( "Class" );
	case IDD_DATE :
		return saved_m_pNewBase->FindIndex( "Date" );
	case IDD_ID :
		return saved_m_pNewBase->FindIndex( "Number" );
	case IDD_ADDCODE :
		return saved_m_pNewBase->FindIndex( "Number" );
	case IDD_SOURCE :
		return saved_m_pNewBase->FindIndex( "Adopted" );
	case IDD_TYPE :
		return saved_m_pNewBase->FindIndex( "Type" );
	case IDD_KIND :
		return saved_m_pNewBase->FindIndex( "Kind" );
	case IDD_RCODE :
		return saved_m_pNewBase->FindIndex( "RCode" );
	case IDD_RDATE :
		return saved_m_pNewBase->FindIndex( "RDate" );
	case IDD_CALIN :
		return saved_m_pNewBase->FindIndex( "CalIn" );
	case IDD_CALAB :
		return saved_m_pNewBase->FindIndex( "CalAb" );
	case IDD_CALCH :
		return saved_m_pNewBase->FindIndex( "CalCh" );
	case IDD_VINCLUDED :
		return saved_m_pNewBase->FindIndex( "VIncluded" );
	case IDD_VCHANGED :
		return saved_m_pNewBase->FindIndex( "VChanged" );
	case IDD_VABOLISHED :
		return saved_m_pNewBase->FindIndex( "VAbolished" );
	case IDD_SORTDATE :
		return saved_m_pNewBase->FindIndex( "SortDate" );
	case IDD_VLCONTROL :
		return saved_m_pNewBase->FindIndex( "VLControl" );
	case IDD_VANONCED :
		return saved_m_pNewBase->FindIndex( "VAnonced" );
	case IDD_CATEGORY :
		return saved_m_pNewBase->FindIndex( "Category" );
	case IDD_PUBLISHEDIN :
		return saved_m_pNewBase->FindIndex( "PublishedIn" );
	case IDD_BASES :
		return saved_m_pNewBase->FindIndex( "Segment" );
	case IDD_DOCKIND :
		return saved_m_pNewBase->FindIndex( "DocKind" );
	case IDD_TERRITORY :
		return saved_m_pNewBase->FindIndex( "Territory" );
	case IDD_ANNODATE:
		return saved_m_pNewBase->FindIndex( "AnnoDate" );
	case IDD_ANNOUSER:
		return saved_m_pNewBase->FindIndex( "AnnoUser" );
	case IDD_ANNOORGANIZATION:
		return saved_m_pNewBase->FindIndex( "AnnoOrganization" );
	case IDD_ANNOTAX:
		return saved_m_pNewBase->FindIndex( "AnnoTax" );
	case IDD_ANNOINTEREST:
		return saved_m_pNewBase->FindIndex( "AnnoInterest" );
	case IDD_ANNOKIND:
		return saved_m_pNewBase->FindIndex( "AnnoKind" );
	case IDD_WARNING :
	case IDD_INFO :
	case IDD_SUBS :
	case IDD_SOURCEFILE :
...
std::map<std::string,std::deque <Ref> > class_cache, type_cache, kw_cache, kind_cache, dockind_cache, class6_cache;
...
std::map<std::string,std::deque <long> > adopted_cache, territory_cache, publishedin_cache, category_cache;
...
std::map<long, std::deque<long> > date_cache, sortdate_cache, calin_cache, calab_cache, calch_cache, vanonced_cache, vincluded_cache, vchanged_cache, vabolished_cache, annodate_cache;
...
std::vector<std::string> delete_class_keys, delete_type_keys, delete_kw_keys, delete_kind_keys, delete_dockind_keys, delete_adopted_keys, delete_territory_keys, delete_class6_keys, delete_publishedin_keys, delete_category_keys, delete_number_keys, delete_rcode_keys;
...
std::vector<long> delete_date_keys, delete_sortdate_keys, delete_calin_keys, delete_calab_keys, delete_calch_keys, delete_vanonced_keys, delete_vincluded_keys, delete_vabolished_keys, delete_vchanged_keys, delete_annodate_keys;
...
void	UnFull::flush_caches (bool b_delete)
{
	if (b_delete) {
		...
		screen->SetProgress (0);
		int old_perc = 0, total_count = del_subnames.size () + add_subnames.size (), count = 0, new_perc;

		time (&long_time); pNewTime = localtime (&long_time);
		logger.printf (lt_updateInfo, "flush SubName at %s", asctime (pNewTime));

		for (std::vector<UpdatedBase*>::const_iterator base_it = saved_m_pNewBase->bases_ptrs.begin (); base_it != saved_m_pNewBase->bases_ptrs.end (); base_it++) {
			m_pNewBase = *base_it;
			Index *subnames = m_pNewBase->FindIndex ("SubName");

			std::deque<Ref>::const_iterator deq_it;
			for (deq_it = del_subnames.begin (); deq_it != del_subnames.end (); deq_it++) {
				Ref ref = *deq_it;

				if (saved_m_pNewBase->find_base_for_doc (ref.DocId) != m_pNewBase)
					continue;

				subnames->Delete (&ref);
				if ((new_perc = (++count * 100) / total_count) != old_perc)
					screen->SetProgress (old_perc = new_perc);
			}
			...
			for (int cycle = 0; cycle < 2; cycle++) {
				...
				for (deq_it = add_subnames.begin (); deq_it != add_subnames.end (); deq_it++, name_it++, size_it++) {
					Ref ref = *deq_it;
					if ((ref.Sub && !cycle) || (cycle && !ref.Sub))
						continue;
		...
		screen->SetProgress (0);
		float progress_step = 100.0 / 42.0, current_progress = 0;
		...
		time (&long_time); pNewTime = localtime (&long_time);
		logger.printf (lt_updateInfo, "flush InpharmFirm at %s", asctime (pNewTime));
		flush_class_type_dockind (SelectIndex (IDD_INPHARMFIRM), &phfirm_cache, false, &delete_phfirm_keys);
		current_progress += progress_step; screen->SetProgress ((int) current_progress);

		time (&long_time); pNewTime = localtime (&long_time);
		logger.printf (lt_updateInfo, "flush InpharmCountry at %s", asctime (pNewTime));
		...
		time (&long_time); pNewTime = localtime (&long_time);
		logger.printf (lt_updateInfo, "flush PharmGroup at %s", asctime (pNewTime));
		flush_adopted_territory (SelectIndex (IDD_PHARMGROUP), &phgroup_cache, &delete_phgroup_keys);
		current_progress += progress_step; screen->SetProgress ((int) current_progress);

		time (&long_time); pNewTime = localtime (&long_time);
		logger.printf (lt_updateInfo, "flush PharmEffect at %s", asctime (pNewTime));
...
int UnFull :: RestoreRefInd (long lDocId)
{
	...
	Index *pInd = saved_m_pNewBase->FindIndex ("Respondent");
...
{
	// delete all index
	DelIndByAttr ( IDD_DATE, m_lDocId );
	DelIndByAttr ( IDD_VCHANGED, m_lDocId );
	DelIndByAttr ( IDD_INFO, m_lDocId );
	DelIndByAttr ( IDD_KEY, m_lDocId );
	DelIndByAttr ( IDD_SUBS_EX, m_lDocId );
	DelIndByAttr ( IDD_TYPE, m_lDocId );
	DelIndByAttr ( IDD_KIND, m_lDocId );
	DelIndByAttr ( IDD_SOURCE, m_lDocId ); // first pair (class)
	DelIndByAttr ( IDD_TERRITORY, m_lDocId ); // first pair (prefix)
	DelIndByAttr ( IDD_BASES, m_lDocId );
	DelIndByAttr ( IDD_ID, m_lDocId );
	DelIndByAttr ( IDD_ADDCODE, m_lDocId );
	DelIndByAttr ( IDD_RCODE, m_lDocId );
	DelIndByAttr ( IDD_DIV, m_lDocId ); // second pair (class)
	DelIndByAttr ( IDD_DOCKIND, m_lDocId ); // second pair (prefix)
	DelIndByAttr ( IDD_SORTDATE, m_lDocId );
	DelIndByAttr ( IDD_ANNODATE, m_lDocId );
	DelIndByAttr ( IDD_ANNOUSER, m_lDocId );
	DelIndByAttr ( IDD_ANNOORGANIZATION, m_lDocId );
	DelIndByAttr ( IDD_ANNOTAX, m_lDocId );
	DelIndByAttr ( IDD_ANNOINTEREST, m_lDocId );
	DelIndByAttr ( IDD_ANNOKIND, m_lDocId );
	DelIndByAttr ( IDD_SERVICEINFO, m_lDocId );
	DelIndByAttr ( IDD_CLASS6, m_lDocId );
	DelIndByAttr ( IDD_RDATE, m_lDocId );
	DelIndByAttr ( IDD_WARNING, m_lDocId );
	DelIndByAttr ( IDD_VLCONTROL, m_lDocId );
	DelIndByAttr ( IDD_VANONCED, m_lDocId );
	DelIndByAttr ( IDD_CATEGORY, m_lDocId );
	DelIndByAttr ( IDD_PUBLISHEDIN, m_lDocId );
	DelIndByAttr ( IDD_VABOLISHED, m_lDocId );
	DelIndByAttr ( IDD_VINCLUDED, m_lDocId );
	DelIndByAttr ( IDD_SOURCEFILE, m_lDocId );
	DelIndByAttr ( IDD_CALIN, m_lDocId );
	DelIndByAttr ( IDD_CALAB, m_lDocId );
	DelIndByAttr ( IDD_CALCH, m_lDocId );

	DelIndByAttr (IDD_INPHARMFIRM, m_lDocId );
	DelIndByAttr (IDD_INPHARMCOUNTRY, m_lDocId );
	DelIndByAttr (IDD_REGDATE, m_lDocId );
	DelIndByAttr (IDD_ANNULDATE, m_lDocId );
	DelIndByAttr (IDD_ACTIVEEX, m_lDocId );
	DelIndByAttr (IDD_TRADENAME, m_lDocId );
	DelIndByAttr (IDD_INTNAME, m_lDocId );
	DelIndByAttr (IDD_PHARMGROUP, m_lDocId );
	DelIndByAttr (IDD_PHARMEFFECT, m_lDocId );
	DelIndByAttr (IDD_CHAPTER, m_lDocId );
	DelIndByAttr (IDD_ATC, m_lDocId );
	DelIndByAttr (IDD_REGSTATUS, m_lDocId );
	DelIndByAttr (IDD_LEKFORM, m_lDocId );
	DelIndByAttr (IDD_MKB, m_lDocId );

	DelIndByAttr (IDD_LIFE, m_lDocId );
	DelIndByAttr (IDD_NOTLIFE, m_lDocId );
	DelIndByAttr (IDD_RECIPE, m_lDocId );
	DelIndByAttr (IDD_UNRECIPE, m_lDocId );
	DelIndByAttr (IDD_ALLOWED, m_lDocId );
	DelIndByAttr (IDD_NOTALLOWED, m_lDocId );

	saved_m_pNewBase->FindIndex ("Respondent")->Delete (&m_lDocId);
...
int UnFull :: DelIndByAttr ( int iTag, long lDocId )
{
	...
	if ( iOldSize == -1 || !iOldSize ) return 0;
	...
	case IDD_TRADENAME:
	{
		...
		if ( iTag == IDD_DIV || iTag == IDD_SOURCE ) { // correct pair index
			StringZ * pPrefs = LoadStrz ( m_pOldBase, lDocId, iTag==IDD_DIV ? IDD_DOCKIND : IDD_TERRITORY );
			if ( !pPrefs ) {
				logger.printf ( lt_fatal, "Can't load prefix pair attrs for deleting (tag %d)\n", iTag );
				_EXIT ( EXIT_FAILURE );
			}
			CorrectPairIndex ( &aKeys, 0, pPrefs, 0, iTag );
			delete pPrefs;
		}		
	}
		break;
	...
	{
		Index* statusex_index = m_pNewBase->FindIndex ("Status_ex");
		short oldDSDoc = ((DocInfo*) vOldBuff)->Status & ( DS_DOC | DS_EDITION );
		...
		if ( ( oldDSDoc == DS_DOC ) || ( oldDSDoc == DS_EDITION )) {
			m_dsm = dsmChgToNodoc;
			if ( oldDSActive ) DelStatusDoc ( saved_m_pNewBase->FindIndex ("Status"), &oldDSActive, lDocId );
			if ( oldDSReg ) DelStatusDoc ( saved_m_pNewBase->FindIndex ("Status"), &oldDSReg, lDocId );
			if (oldDSWarning) DelStatusDoc (saved_m_pNewBase->FindIndex ("Status"), &oldDSWarning, lDocId);
	...
	case IDD_SUBS_EX :
	{
		SubDescEx* subs = (SubDescEx*) vOldBuff;
		int SubCount = iOldSize / sizeof ( SubDescEx );

		Ref ref;
		ref.DocId = lDocId;

		for ( int i = 0; i < SubCount; i++ ) {
			unsigned short flags = subs [i].Flags;
			if (flags == SUBDESCEX_SUB || flags == SUBDESCEX_BLOCK) {
				ref.Sub = subs [i].Sub;
				del_subname (ref);
			}
		}
	...
	case IDD_SUBS :
		/*
		не надо, так как делается в FullDelDoc
		{
