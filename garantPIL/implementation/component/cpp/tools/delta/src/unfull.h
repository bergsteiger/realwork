/*
 * $Id: unfull.h,v 2.99 2015/05/15 09:59:30 young Exp $
 *
 */
...
{
public:
	UnFull ( Base* oldB, UpdatedSearchBase* newB, FILE* dF1, char * pSkipDir, bool english_face, bool b_load);
	~UnFull();
	...
	AttrIndex   *docInd, *docInd2;
public :
	long m_lDocId;
	Base* m_pOldBase;
	UpdatedSearchBase* m_pNewBase;		
	DocInfo* m_pOldInfo;
	DocInfo* m_pNewInfo;
...
protected:
	...
	int AddStreamRef ( Index* pInd, const void* vpKey, RefwWeight aRef, short tag );
	int DelStreamRef ( Index* pInd, const void* vpKey, Ref aRef, short tag, int keepKey = 0, bool aAddRef = false );
	int DelStreamRef ( Index* pInd, const void* vpKey, RefwWeight aRef, short tag);
	...
	int AddTradeNameDoc ( Index* pInd, const void* vpKey, long lDocId );
	int AddIntNameDoc ( Index* pInd, const void* vpKey, long lDocId );
	...
	int AddServiceInfoRef ( Index* pInd, const void* vpKey, Ref aRef );
	...
	int DelIntNameDoc ( Index* pInd, const void* vpKey, long lDocId, int keepKey = 0 );
	int DelTradeNameDoc ( Index* pInd, const void* vpKey, long lDocId, int keepKey = 0 );
	...
	int DelServiceInfoRef ( Index* pInd, const void* vpKey, Ref aRef, int keepKey = 0, bool aAddRef = false );
	...
	int AddTagRef ( Index* pInd, const void* vpKey, RefwWeight aRef );
	int DelTagRef ( Index* pInd, const void* vpKey, RefwWeight aRef );
	...
	void	flush_tag (Index*, std::map<std::string, std::deque<RefwWeight> >*, bool, std::vector<std::string>*);
