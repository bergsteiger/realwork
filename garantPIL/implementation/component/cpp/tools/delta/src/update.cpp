...
public:
	UpdatedBase(const char * name
				, int mode= ACE_OS_O_RDWR
				, int key_cpages=-1
				, int str_cpages =-1
				, int ndt_cache = -1
				, FILE *stat_file =0 ) : UpdatedSearchBase(name)
	{
...
int main ( int argc, char *argv[] ) {
	...
	exit (res);
#else
	int res =	mpcxc_run_main_stack_size (main_logic, argc, argv, (
#ifdef WIN64
																		132
#else
																		32
...
int main_update ( int argc, char** argv, ProgressScreen* ascreen )
{
	...
	UpdatedSearchBase *pNewBase = new UpdatedBase ( pBaseName );
	...
	if (!b_updateonlyturbo) {
		...
		pNewBase->check_version ();
		delete pNewBase->AllDocs ();
	}
	...
	UpdatedSearchBase *pOldBase = pNewBase;
	...
	if (lDeltaCount && iRetVal == ERR_NONE)
	...
	if (iRetVal == ERR_NONE) {
		...
		if ((FullUnDiff->s_UpdDocs.size () > 1 || FullUnDiff->s_DelDocs.size () > 1) && !b_skipturbo) {
			...
			if(pContextIndex && (pContextIndex->m_pNIdList || FullUnDiff->s_UpdDocs.size() >= 1 || pOut->size() >= 1)){
				...
				AttrIndex *attrI= FullUnDiff->docInd;
...
void ReRead_Stream(NContxtIndex *pNIndx, stream_st *pstr, stref_st *pref, u_intPointerV_tCtxLongSetSplayMap *ptmpA, char *flushName= 0)
{
	...
	if((*(size_t*)&(pstr->ref.size)) <= pstr->info.immds)
				...
				for( ii = ptmpA->first(); ii; ptmpA->next(ii) ){
					if(ptmpA->contents(ii).lastI ==  -1){
					  if(pstr->ArrayIOffsCount){
						...
						if(--pstr->ArrayIOffsCount){
							memcpy(pstr->ArrayIOffs, pstr->ArrayIOffs+1, sizeof(*pstr->ArrayIOffs)*pstr->ArrayIOffsCount);
						}
					  }else
						ptmpA->contents(ii).lastI= NewVal++;
					}
...
#ifndef WIN64
  if(pdstr->ref.size > max_size00 && (pdstr+1)->ref.size == 32 && (*pPos) && (*pPos) != 1 && add_tmpA && isLast < 33337){
	{
