...
//	#include <io.h>
//	#include <fcntl.h>
	#define ypipe(xxx,yyy,zzz) ace_os_open(xxx,yyy)
//#else
//	#include <fcntl.h>
//	#include <unistd.h>
//#ifdef __FreeBSD__ // GARANT_FREEBSD
//	#include <pthread.h>
//	#define ypipe(xxx,yyy,zzz) ace_os_open(xxx,yyy,zzz)
//#else
//	#include <thread.h>
...
int write_my_pipe(LPTHREAD_CNTX pCntxt, char *pBuff, int wr)
{
	...
	if(!pCntxt->h_currPipe){
		...
		pCntxt->h_currPipe= (void*)ACE_OS::thr_self();
	}
	if(pCntxt->bStepByStep){
		cwr= ace_os_write(pCntxt->bStepByStep, pBuff, wr);
		if(pCntxt->LastWrite){
			ace_os_close(pCntxt->bStepByStep);
			pCntxt->bStepByStep= 0;
			*pCntxt->p_Wr= BUFSIZE;
		}else{
...
int read_my_pipe(LPTHREAD_CNTX pCntxt, char *bff, int rd)
{
  ...
  if(!pCntxt->h_currPipe){
   ...
   while(!pCntxt->h_currPipe){
	...
	if(!pCntxt->h_currPipe) {
		((LPTHREAD_CNTX)pCntxt->pWriterCtxt)->PThreadCompileInfo= pCntxt->PThreadCompileInfo;
		((LPTHREAD_CNTX)pCntxt->pWriterCtxt)->PGCParameters= pCntxt->PGCParameters;
		if(pCntxt->PrepWriterProc){
			(*(ACE_THR_FUNC)pCntxt->PrepWriterProc)(pCntxt->pWriterCtxt);
		}
		pCntxt->h_currPipe= (void*)ACE_OS::thr_self();
		ACE_thread_t working_thread_id = 0;
		ACE_hthread_t working_thread_handle = 0;
		if(pCntxt->bStepByStep){
                //*pCntxt->p_Wr= 41943040;
                //*pCntxt->p_all_= 109487940;
					(*(ACE_THR_FUNC)pCntxt->pWriterProc)(pCntxt->pWriterCtxt);
                }else{
			int res = ACE_OS::thr_create (
				(ACE_THR_FUNC)pCntxt->pWriterProc
				, pCntxt->pWriterCtxt
	...
	do{
		if(!*pCntxt->p_Rd){
			...
			if(pCntxt->iSw){
				if(pCntxt->bStepByStep){
					crd= ace_os_read(pCntxt->iSw, bff, rd);
					if(!memcmp(bff, END_METAFILE_SIGN, sizeof(long))){
						ace_os_close(pCntxt->iSw);
						...
						*pCntxt->p_all_R += (crd + pCntxt->_all_z)/8;
						pCntxt->_all_z= crd%8;
						pre= (int)((((unsigned long long)(*pCntxt->p_all_R))*100) / *pCntxt->p_all_);
...
extern void* local_main_PRE_ptr;
					...
					void *pWriter)
{
	...
	*pp_Reader= p_cntxt1;
	...
	p_cntxt0->PrepWriterProc= 0;

	p_cntxt1->pWriterCtxt= p_cntxt0;
	p_cntxt1->pWriterProc= pWriter;
	p_cntxt1->PrepWriterProc= local_main_PRE_ptr;
