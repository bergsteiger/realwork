...
<xsl:template match="p[@style]">
 <xsl:choose>
  ...
  <xsl:when test="@style='pict'">
   <img src="help/uarr.gif" style="float:left;cursor:help;" title="Пояснение к иллюстрации"/>
  </xsl:when>
 ...
 <p class="{@style}">
  ...
  <xsl:if test="@style='debug'">
    Прим. разработчика справочной системы:<br/><br/>
  </xsl:if>
  <xsl:apply-templates/>
 </p>
</xsl:template>
...
<xsl:template match="kbd">
 <span class="kbd" title="Название кнопки диалогового окна">
  <xsl:choose>
