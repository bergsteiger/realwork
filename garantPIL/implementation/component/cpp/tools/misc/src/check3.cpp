...
void	check_doc (long doc, long prev, bool prev_edition)
{
	...
	for (std::vector<long>::const_iterator it = blocks_to_proceed [0].begin (); it != blocks_to_proceed [0].end (); it++) {
		long block = *it;
		if (block_beginpara [1].find (block) == block_beginpara [1].end ()) {
			printf ("%ld and %ld, block %ld\n", prev - ID_BORDER, doc - ID_BORDER, block);
		}
	}
