...
void	calculate_same (int* thread_id)
{
	...
	for (long count = 0;;count++) {
		...
		*/
		...
		if (ado_it->second.size () > 1) {
			for (std::vector<long>::const_iterator it = ado_it->second.begin (); it != ado_it->second.end (); it++) {
				if (it == ado_it->second.end ())
					break;
				long size1 = map_doc_size.find (*it)->second;

				std::vector<long>::const_iterator it2 = it;
				for (it2++; it2 != ado_it->second.end (); it2++) {
					long size2 = map_doc_size.find (*it2)->second;
...
int main_logic ( int argc, char *argv[] )
{
	...
	for (int i = 0; i < all_docs->ItemCount; i++) {
		...
		map_doc_size.insert (std::map<long,long>::value_type (id, docinfo.Size));
