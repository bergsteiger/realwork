...
class iProfiles:
    ...
    def _put (self, profile_name):        
        ...
        data = urllib.urlopen(url = delete_url, proxies = {})
	data.close()
        ...
        # загрузим профиль
        data = urllib.urlopen(url = import_url, proxies = {})
	data.close()

        # загрузим новый профиль, чтобы внести возможные изменения из эталонного профиля
        update_url = 'http://%s:%s@%s/profiles/%s/edit' % (self.login, self.password, self.server_url, profile_name)
        data = urllib.urlopen(url = update_url, proxies = {})
	data.close()
