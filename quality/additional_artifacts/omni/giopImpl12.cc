...
/*
  $Log: giopImpl12.cc,v $
  Revision 1.1  2002/01/23 13:33:33  dimab
  added
...
giopImpl12::unmarshalReplyHeader(giopStream* g) {
  ...
  omni_thread::ensure_self my_thread;
  char* hdr = (char*)g->pd_currentInputBuffer + 
                     g->pd_currentInputBuffer->start;
  ...
  svcccount <<= s;
  fprintf(stderr, "SvcContext count: %d\n", svcccount);
  while (svcccount-- > 0) {
    svcctag <<= s;
    svcctxtsize <<= s;
    fprintf(stderr, "SvcContext: %d, size: %d\n", svcctag, svcctxtsize);
    s.skipInput(svcctxtsize);
    }*/
  IOP::ServiceContextList service_contexts;
  service_contexts <<= s;
  for(int i = 0; i < (signed)service_contexts.length(); i++) {
    CORBA::ULong id = service_contexts[i].context_id;
  }
  omniInterceptors::clientReceiveReply_T::info_T info(calldesc.op(), service_contexts);
  omniORB::getInterceptors()->clientReceiveReply.visit(info);
  if (inputRemaining(g))
...
giopImpl12::marshalRequestHeader(giopStream* g) {
  ...
  //fprintf(stderr, "GIOP 1.2: marshal request header: %s %d\n", calldesc.op(), omni_thread::self()->id());
