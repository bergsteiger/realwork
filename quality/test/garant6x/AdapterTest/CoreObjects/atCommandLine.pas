...
//#UC START# *4808975D00A9_4808971F0196_impl*
    ...
    begin // перебираем все параметры
      param := f_ParamList.ParamByIndex[i];
      if FindCmdLineSwitch(param.Name, [f_Delimiter], true) then
      begin // параметр найден
        case param.ParamType of
          clptKey : param.AsBool := true;
          clptString, clptNumber:
            for j := 1 to ParamCount do // ищем сам параметр
              if ( UpperCase(ParamStr(j)) = UpperCase(f_Delimiter + param.Name) ) then // нашли
                if (ParamStr(j+1) <> '') then // делаем его содержимым то что за ним
