...
//#UC START# *48077F3F01F9_4807838102C9_impl*
    f_CS.Enter;
    try
      // проверяем, может мы его уже слушаем
      if IsListen(notifier) then Exit; // слушаем, ничего делать не надо
      // не слушаем
      // добавляем в список
      f_Notifiers.Add(notifier);
      // дергаем нотификатор, чтобы он начал уведомлять
...
//#UC START# *48077F4B02E8_4807838102C9_impl*
    ...
    try
      while (true) do begin
        if (f_Notifiers.Count = 0) then Exit; // и так никого не слушаем
...
//#UC START# *48077F540156_4807838102C9_impl*
    f_CS.Enter;
    try
      // проверяем, может мы не слушаем
      if NOT IsListen(notifier) then Exit; // не слушаем, ничего делать не надо
      // слушаем
      // удаляем из списка
      f_Notifiers.Remove(notifier);
      // дергаем нотификатор, чтобы перестал уведомлять
