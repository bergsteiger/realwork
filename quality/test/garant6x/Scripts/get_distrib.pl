...
qx ($blat_dir\\blat.exe - -body "Пожалуйста не запускайте пересборку, пока не забирется версия" -charset "windows-1251" -f "$mail_form" -priority "1" -replyto "$reply_to" -server "$smtp_server" -subject "Забираем сборку" -to "$mail_to");    
