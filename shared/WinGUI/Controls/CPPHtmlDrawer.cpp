////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Модуль: "w:/shared/WinGUI/Controls/CPPHtmlDrawer.cpp"
// генератор файлов реализации C++ (.cpp)
// Generated from UML model, root element: <<SimpleClass::Class>> shared::WinGUI::Controls::CPPHtmlDrawer
//
// класс для отрисовки html'я
//
//
...
//#UC END# *4700B81F006D_CUSTOM_INCLUDES*

namespace WinGUI {

//#UC START# *4700B81F006D*
enum {
	MODE_DRAW = 0,
	MODE_FIRSTPASS,
	MODE_SECONDPASS
			...
			for (int y = 0; y < szTable.cy; y++)
			{
				...
				for (int x = 0; x < szTable.cx; x++)
				{
					...
					if (sc.nColSpan == i)
					{
						if (i == 1)
						{
							cur_table.width [x] = std::max ((long)cur_table.width [x], sc.szCell.cx);
							if (sc.bFixedWidth)
		...
		for (i = 1; i <= szTable.cy; i++)
		{
			for (int y = 0; y < szTable.cy; y++)
			{
				...
				for (int x = 0; x < szTable.cx; x++)
				{
					...
					{
						if (i == 1)
							cur_table.height [y] = std::max ((long)cur_table.height [y], sc.szCell.cy);
						else
						{
							int span_height = 0;
							for (int z = 0; z < i; z++)
...
//	pt.y = lpRect->top;
	...
	for (i = nFirstLine; i < m_nCurLine; i++)
	{
		...
		if (0 != m_hline.nAddPercentWidth)
		{
			...
			szTextArea.cx = std::max((long)szTextArea.cx, (long)m_hline.nWidthLine);
		} //if
	} //for
//
//	if (NULL != lpSize)
...
int CPPHtmlDrawer::VerticalAlignText(int y, int nHeight)
{
	...
	if (MODE_FIRSTPASS == m_nNumPass)
	{
		...
		m_hline.nDescentLine = std::max((long)m_hline.nDescentLine, (long)nHeight - m_tm.tmAscent);
		m_hline.nHeightLine = std::max((long)m_hline.nHeightLine, m_tm.tmAscent);
	}
	else if (MODE_DRAW == m_nNumPass)
	{
		switch (m_defStyle.nVertAlign)
...
CPPHtmlDrawer::~CPPHtmlDrawer () {
	//#UC START# *4700B81F006D_DESTR_BODY*
	SetResourceDll(NULL);
	...
	if (NULL != m_hImageList)
		::DeleteObject(m_hImageList);
	//#UC END# *4700B81F006D_DESTR_BODY*
}
