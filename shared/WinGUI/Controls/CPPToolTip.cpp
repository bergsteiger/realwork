////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Модуль: "w:/shared/WinGUI/Controls/CPPToolTip.cpp"
// генератор файлов реализации C++ (.cpp)
// Generated from UML model, root element: <<SimpleClass::Class>> shared::WinGUI::Controls::CPPToolTip
//
// расширенный контрол тул-тип
//
//
...
//#UC START# *4700B6AC00CB_CUSTOM_INCLUDES*
...
//#UC END# *4700B6AC00CB_CUSTOM_INCLUDES*

namespace WinGUI {

//#UC START# *4700B6AC00CB*
//////////////////
...
CPPToolTip::CPPToolTip()
{
	...
	m_last_dynamic_tool = 0;
	m_next_dynamic_tool = 0;
...
LRESULT CPPToolTip::OnRepaintWindow(WPARAM wParam, LPARAM lParam)
{
	...
	if (m_bHyperlinkEnabled)
	{
		...
		CDC * pDC = GetDC();
		OnRedrawTooltip(pDC->GetSafeHdc(), m_dwCurTransparency);
		ReleaseDC(pDC);
	}
    return TRUE;
...
void CPPToolTip::OnPaint() 
{
	...
	m_last_dynamic_tool = m_next_dynamic_tool;
	...
	case WM_MOUSEMOVE:
		...
		if ((m_ptOriginal.x != pMsg->pt.x) || (m_ptOriginal.y != pMsg->pt.y))
		{
			...
			if (m_bDebugMode)
			{
				...
				ti.nMask = 0;
				m_hwndDisplayedTool = NULL;
				SetNewTooltip(this->GetSafeHwnd(), 0, ti);
			}
			...
			else
			{
				...
				if (NULL == hWnd)
				{
					//ищём среди динамических элементов
					CallBackMap::iterator it = m_dynamic_tools.begin ();
					CallBackMap::iterator it_end = m_dynamic_tools.end ();
					DynamicToolID finded_tool = 0;

					for (; it != it_end && finded_tool == 0; ++it) {
						if (it->second (ti)) {
							finded_tool = it->first;
						}
					}

					if (finded_tool != 0) {
						if (
							(finded_tool != m_last_dynamic_tool) 
							|| (ti.rectBounds != m_tiDisplayed.rectBounds/* m_rcDisplayedTool*/)
						) {
							//если новый элемент
							SetNewTooltip(hWnd, finded_tool, ti);
						} else {
							//элемент не поменялся
...
void CPPToolTip::SetNewTooltip(HWND hWnd, DynamicToolID toolID, const PPTOOLTIP_INFO & ti, BOOL bDisplayWithDelay /* = TRUE */, TooltipType type /* = PPTOOLTIP_NORMAL */)
{
...
void CPPToolTip::AddDynamicTool (DynamicToolID tool, const ExternalFindCallback& callback) {
	m_dynamic_tools[tool] = callback;
}
...
void CPPToolTip::RemoveDynamicTool(DynamicToolID tool) {
	CallBackMap::iterator it = m_dynamic_tools.find (tool);
	if (it != m_dynamic_tools.end ()) {
		m_dynamic_tools.erase (it);
	}
}
////////////////////////////////////////////////////////////////////
...
void CPPToolTip::ShowHelpTooltip(LPPOINT pt, PPTOOLTIP_INFO & ti)
{
	...
	SetNewTooltip(NULL, 0, ti, FALSE, PPTOOLTIP_HELP);
} //End ShowHelpTooltip
		...
		if (NULL != hwndMenu)
		{
			...
			for(int nItem = 0; nItem < count; nItem++) 
			{
				...
				if(nItemID == ::GetMenuItemID(hSubMenu, nItem)) 
				{
					...
					m_ptOriginal = pt;
					SetNewTooltip(hwndMenu, 0, ti, TRUE, PPTOOLTIP_MENU);
...
CPPToolTip::~CPPToolTip () {
	//#UC START# *4700B6AC00CB_DESTR_BODY*
	FreeResources();
	RemoveAllTools();
	HideBorder();
	//#UC END# *4700B6AC00CB_DESTR_BODY*
}
