////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Модуль: "w:/shared/WinGUI/Controls/ETSLayout.cpp"
// генератор файлов реализации C++ (.cpp)
// Generated from UML model, root element: <<UtilityPack::Class>> shared::WinGUI::Controls::ETSLayout
// Файл реализации утилитного набора ETSLayout
//
// набор классов и контролов для поддержки ресайза
//
//
...
//#UC START# *46A88BDA035B_CUSTOM_INCLUDES*
//#UC END# *46A88BDA035B_CUSTOM_INCLUDES*

namespace WinGUI {

//#UC START# *46A88BDA035B*
////////////////////////////////////////////
...
// DISCLAIMER:
//
// This Sourcecode and all accompaning material is й1998-1999 Erwin Tratar. 
// All rights reserved.
//
...
// 2000/05/31   Fix for PropertySheets with PSH_WIZARDHASFINISH [ThЎmmi]
...
using namespace WinGUI::ETSLayout;
...
int ETSLayoutMgr::PaneItem::getMinConstrainHorz() 
{
	...
	return std::max(nMinConstrain,m_sizeXMin);
}
...
int ETSLayoutMgr::PaneItem::getMinConstrainVert() 
{
	...
	return std::max(nMinConstrain,m_sizeYMin);
}
	...
	if(m_Orientation == HORIZONTAL) {
		...
		for(int i=0; i<m_paneItems.GetSize(); ++i) {
			...
			if( nConstrain == -1)
				...
				nMaxConstr = std::max(nMaxConstr, nConstrain);
...
int ETSLayoutMgr::Pane::getMinConstrainHorz() 
{
	if(m_Orientation == HORIZONTAL) {
		...
		for(int i=0; i<m_paneItems.GetSize(); ++i) {
			CPaneBase pItem = m_paneItems[i];
			nMaxConstr += std::max(nMinConstrain, pItem->getMinConstrainHorz());
		}
...
int ETSLayoutMgr::Pane::getMinConstrainVert() 
{
	if(m_Orientation == VERTICAL) {
		...
		for(int i=0; i<m_paneItems.GetSize(); ++i) {
			CPaneBase pItem = m_paneItems[i];
			nMaxConstr += std::max(nMinConstrain, pItem->getMinConstrainVert());
		}
		return nMaxConstr + (m_paneItems.GetUpperBound()*m_sizeBorder) + 2*m_sizeExtraBorder;
	}
	else if( m_modeResize & ABSOLUTE_VERT && m_sizeSecondary!=0) {
										 ...
										 CArray<int,int>& sizeMin, CArray<int,int>& sizeMax)
{
	...
	availSpace = std::max(availSpace, 0);
			...
			)
			{
				if( sizePrimary[i] == -sizeMin[i] ) {
					...
					if( sizeMax[i] == -1) {
						...
						sizePrimary[i] += -std::min( -greedyDiff, sizeMax[i]-sizeMin[i]);
						greedyDiff     -= -std::min( -greedyDiff, sizeMax[i]-sizeMin[i]);
					}
				}
			}
		}
...
//			m_RootPane->getMinConstrainVert() + sizeDiff.cy + 2*m_sizeRootBorders.cy);
	...
	{
		sizeRet.cx = std::min( sizeRet.cx, sizeMax.cx );
	}
	if( IsFloating() || (dwMode&LM_HORZ))
	{
		sizeRet.cy = std::min( sizeRet.cy, sizeMax.cy );
	}

	sizeRet.cx = std::max( sizeRet.cx, sizeMin.cx );
	sizeRet.cy = std::max( sizeRet.cy, sizeMin.cy );
...
void ETSLayoutPropertySheet::Resize(int cx, int cy)
{
	if( abs(cx) + abs(cy) > 0 && m_RootPane.IsValid() ) 
	{
		...
		// Fix for PSH_WIZARDHASFINISH [ThЎmmi]
...
void ETSLayoutPropertySheet::AddButtons(CPane paneBottom)
{
	...
	{
		// Prevent movement of finish button, if it is not shown explicitly [ThЎmmi]
		if( IsWizard() 
...
void ETSLayoutPropertySheet::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI) 
{
	if(m_RootPane.IsValid() && GetTabControl() != 0 ) 
	{
		...
		for( int nPage=0; nPage<GetPageCount(); ++nPage)
		{
			...
			if( pPage )
			{
				...
				if( IsWindow(pPage->GetSafeHwnd()) )
				{
					...
					{
						sizePageMax.cx = std::min(sizePageMax.cx, mmi.ptMaxTrackSize.x);
					}
					if(mmi.ptMaxTrackSize.y != 0)
					{
						sizePageMax.cy = std::min(sizePageMax.cy, mmi.ptMaxTrackSize.y);
					}
					if(mmi.ptMinTrackSize.x != 0)
					{
						sizePageMin.cx = std::max(sizePageMin.cx, mmi.ptMinTrackSize.x);
					}
					if(mmi.ptMinTrackSize.y != 0)
					{
						sizePageMin.cy = std::max(sizePageMin.cy, mmi.ptMinTrackSize.y);
					}
				}
			}
		}
		...
		lpMMI->ptMinTrackSize.x = std::max((long)lpMMI->ptMinTrackSize.x, (long)m_rcStart.Width() );
		lpMMI->ptMinTrackSize.y = std::max((long)lpMMI->ptMinTrackSize.y, (long)m_rcStart.Height() );
